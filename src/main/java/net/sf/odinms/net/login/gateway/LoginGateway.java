...
public class LoginGateway {
    ...
    public void Init(Node _tmp, LoginServerConfig config) {
        ...
        version = Double.parseDouble(config.getSingleNodeValue(element, "VerSion", ""));
            ...
            for (int i = 0; i < list.getLength(); i++) {
                if (list.item(i) instanceof Element) {
                    ...
                    if (_te.getNodeName().equals("Ip")) {
                        serverpoint.put(_te.getAttribute("name"), _te.getTextContent());
                    }
                }
            }
        }
            ...
            for (int i = 0; i < list.getLength(); i++) {
                if (list.item(i) instanceof Element) {
                    ...
                    if (_te.getNodeName().equals("Checkd")) {
                        vcode.add(new Pair<String, String>(_te.getAttribute("name"), _te.getTextContent()));
                    }
                }
            }
        }
    ...
    public void run() throws Exception {
        if (start) {
            ...
            acceptor.getFilterChain().addLast("executor", new ExecutorFilter(LoginServer.ExecutorService));
