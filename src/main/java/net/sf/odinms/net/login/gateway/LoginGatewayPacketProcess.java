...
public class LoginGatewayPacketProcess {
	...
	public static void HandlerPacket(SeekableLittleEndianAccessor slea,
			IoSession session) {
		LoginGatewayClient client = (LoginGatewayClient) session
				.getAttribute(LoginGatewayClient.CLIENTKEY);
		try {
			...
			case 1:
				...
				PreparedStatement ps = con
						.prepareStatement("select COUNT(*) = 0  from accounts WHERE `name` = ?");
				ps.setString(1, user);
			...
			case 2:
				...
				client.setPort(port);
				client.setClientId(LoginServer.getInstance()
						.getWorldInterface().checkClientIvKey(key, port));
				session.write(LoginGatewayPacket.getSessionIDResult(client
						.getClientId()));
				break;
			}
		} catch (Exception e) {
