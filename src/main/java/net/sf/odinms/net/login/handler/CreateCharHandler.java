...
public class CreateCharHandler extends AbstractMaplePacketHandler {
	...
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(CreateCharHandler.class);

	@Override
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		// 11 00 [08 00 B9 FE B9 FE D9 E2 D2 B6] [03 00 00 00] [00 00] [84 4E 00
		// 00] [30 75 00 00] [F7 E6 0F 00] [E3 34 10 00] [FF 5C 10 00] [1F 01 16
		// 00] //088的创建职业封包,貌似是send的
		// 11 00 [08 00 CA B2 C3 B4 CA B2 B4 F3] [00 00 00 00] [00 00] [B1 4F 00
		// 00] [30 75 00 00] [3E 06 10 00] [00 00 00 00] [A6 5B 10 00] [15 2C 14
		// 00]//089
		// 25 00 [08 00 73 64 68 68 79 79 79 79] [01 00 00 00] [00 00] [00 06]
		// [B1 4F 00 00] [30 75 00 00] [82 DE 0F 00] [A2 2C 10 00] [85 5B 10 00]
		// [04 05 14 00]//97
		/*
		...
		if (job == 0) { // 反抗者 089建立没问题的，什么装备都没啊
			...
			newchar.getInventory(MapleInventoryType.ETC).addItem(
					new Item(4161054, (byte) 0, (short) 1));
			...
			// 送
			newchar.getInventory(MapleInventoryType.ETC).addItem(
					new Item(4160005, (byte) 0, (short) 1));
			...
			// 送
			newchar.getInventory(MapleInventoryType.ETC).addItem(
					new Item(4161079, (byte) 0, (short) 1));
			...
			Equip d = (Equip) MapleItemInformationProvider.getInstance()
					.getEquipById(1352504);
		...
		if (bottom != 0) {
			Equip eq_bottom = new Equip(bottom, job != 14 ? (byte) -6
					: (byte) -5);// 生成裙裤 部分职业生成连体衣
		...
		if (MapleCharacterUtil.canCreateChar(name, c.getWorld())) { // 检测角色名
			...
			newchar.setAttribute(MapleCharAttribute.createCharAttribute(newchar
					.getId()));
			c.getSession().write(
					MaplePacketCreator.addNewCharEntry(newchar, true));
			// c.getSession().write(MaplePacketCreator.serverNotice(1,
			// "成功创建一个角色!"));
		} else {
			log.warn(MapleClient.getLogMessage(c,
