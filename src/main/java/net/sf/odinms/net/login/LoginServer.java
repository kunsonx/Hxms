...
public class LoginServer extends GeneralServer implements Runnable,
		LoginServerMBean {
		...
		try {
			mBeanServer
					.registerMBean(
							instance,
							new ObjectName(
									"net.sf.odinms.net.login:type=LoginServer,name=LoginServer"));
		} catch (Exception e) {
	...
	public String getIP(ChannelDescriptor channel) {
		...
		if (ip == null) {
			log.error("无法获得 世界:" + channel.getWorld() + "-频道:"
					+ channel.getId() + " 的IP地址。");
		}
		return ip;
	}
				...
				synchronized (wli) {
					...
					try {
						...
						Registry registry = LocateRegistry.getRegistry(
								getConfig().getWorldHost(),
								Registry.REGISTRY_PORT,
								new SslRMIClientSocketFactory());
						worldRegistry = (WorldRegistry) registry
								.lookup("WorldRegistry");
						lwi = new LoginWorldInterfaceImpl();
						wli = worldRegistry.registerLoginServer(getConfig()
								.getKey(), lwi);
					} catch (Exception e) {
	...
	public void run() {
		try {
			Registry registry = LocateRegistry.getRegistry(getConfig()
					.getWorldHost(), Registry.REGISTRY_PORT,
					new SslRMIClientSocketFactory());
		...
		try {
			acceptor.setHandler(new MapleServerHandler(PacketProcessor
					.getProcessor(PacketProcessor.Mode.LOGINSERVER)));
	...
	public static void main(String args[]) {
		try {
			...
			ExecutorService = new ThreadPoolExecutor(0, LoginServer
					.getInstance().getMaxiNumPoolSize(), 2, TimeUnit.MINUTES,
	...
	public LoginServerConfig getConfig() {
		return (LoginServerConfig) _Config;// To change body of generated
