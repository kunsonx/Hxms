...
public class ChannelLoadInfo implements java.io.Serializable {
    ...
    public ChannelLoadInfo(ChannelServerStorage channelServerStorage) throws RemoteException {
        for (ChannelDescriptor channelDescriptor : channelServerStorage.getChannelDescriptors()) {
            loads.put(channelDescriptor, channelServerStorage.getChannelWorldInterface(channelDescriptor).getConnected());
        }
        updateInfo();
    }
    ...
    public void setChannelValue(int world, int channel, int value) {
        for (ChannelDescriptor channelDescriptor : loads.keySet()) {
            if (channelDescriptor.getWorld() == world && channelDescriptor.getId() == channel) {
