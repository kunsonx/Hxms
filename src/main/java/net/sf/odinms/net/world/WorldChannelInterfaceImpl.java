 ...
 * @author Matze
 */
public class WorldChannelInterfaceImpl extends UnicastRemoteObject implements
		WorldChannelInterface {

	private static final long serialVersionUID = -5568606556435590482L;
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(WorldChannelInterfaceImpl.class);
	/**
	 * 成员变量
	 */
	...
	public WorldChannelInterfaceImpl(ChannelWorldInterface cb)
	...
	public int getGameServerPort() throws RemoteException {
		return ChannelServerStorage.getPort()
				- 1
				+ (channelDescriptor.getWorld() * 10 + channelDescriptor
						.getId());
	}
	...
	public void serverReady() throws RemoteException {
		ready = true;
		for (LoginWorldInterface wli : WorldRegistryImpl.getInstance()
				.getLoginServer()) {
			try {
		...
		log.info(String.format("世界 %d 频道服务器 %d 已上线.",
	...
	public void shutdown(int time) throws RemoteException {
		for (LoginWorldInterface lwi : WorldRegistryImpl.getInstance()
				.getLoginServer()) {
			try {
				lwi.shutdown();
			} catch (RemoteException e) {
		...
		for (ChannelWorldInterface cwi : WorldRegistryImpl.getInstance()
				.getChannelServerStorage().getServers()) {
			try {
				cwi.shutdown(time);
			} catch (RemoteException e) {
				ServerExceptionHandler.HandlerRemoteException(e);
	...
	public Map<Integer, Integer> getConnected() throws RemoteException {
		...
		for (ChannelDescriptor descriptor : list.getChannelDescriptors()) {
			try {
				int curConnected = list.getChannel(descriptor.getId())
						.getConnected();
	...
	public WorldLocation getLocation(String charName) throws RemoteException {
		for (ChannelWorldInterface cwi : all()) {
			try {
				if (cwi.isConnected(charName)) {
					return new WorldLocation(cwi.getLocation(charName),
							cwi.getChannelId());
				}
			} catch (RemoteException e) {
				ServerExceptionHandler.HandlerRemoteException(e);
	...
	public CharacterIdChannelPair[] multiBuddyFind(int charIdFrom,
			int[] characterIds) throws RemoteException {
		...
		for (ChannelDescriptor descriptor : list.getChannelDescriptors()) {
			...
			for (int charid : cwi.multiBuddyFind(charIdFrom, characterIds)) {
				foundsChars.add(new CharacterIdChannelPair(charid, descriptor
						.getId()));
			}
		}
	...
	public void setGuildMemberOnline(MapleGuildCharacter mgc, boolean bOnline,
		...
		WorldRegistryImpl.getInstance().setGuildMemberOnline(mgc, bOnline,
				channel);
	}
	...
	public void setGuildEmblem(int gid, short bg, byte bgcolor, short logo,
			byte logocolor) throws RemoteException {
		WorldRegistryImpl.getInstance().setGuildEmblem(gid, bg, bgcolor, logo,
				logocolor);
	}
	...
	public void leaveMessenger(int messengerid, MapleMessengerCharacter target)
			throws RemoteException {
		MapleMessenger messenger = WorldRegistryImpl.getInstance()
	...
	public void joinMessenger(int messengerid, MapleMessengerCharacter target,
			String from, int fromchannel) throws RemoteException {
		MapleMessenger messenger = WorldRegistryImpl.getInstance()
				.getMessenger(messengerid);
		if (messenger == null) {
			throw new IllegalArgumentException(
		...
		for (ChannelWorldInterface cwi : all()) {
			try {
				cwi.addMessengerPlayer(messenger, from, fromchannel,
						target.getPosition());
			} catch (RemoteException e) {
				ServerExceptionHandler.HandlerRemoteException(e);
			}
	...
	@Override
	public void messengerChat(int messengerid, String chattext,
	...
	public void silentLeaveMessenger(int messengerid,
			MapleMessengerCharacter target) throws RemoteException {
		MapleMessenger messenger = WorldRegistryImpl.getInstance()
				.getMessenger(messengerid);
		if (messenger == null) {
			throw new IllegalArgumentException(
	...
	public void addBuffsToStorage(int chrid, List<PlayerBuffValueHolder> toStore)
			throws RemoteException {
		PlayerBuffStorage buffStorage = WorldRegistryImpl.getInstance()
	...
	@Override
	public void allianceMessage(int id, MaplePacket packet, int exception,
			int guildex) throws RemoteException {
		WorldRegistryImpl.getInstance().allianceMessage(id, packet, exception,
				guildex);
	}
	...
	public boolean removeGuildFromAlliance(int aId, int guildId)
			throws RemoteException {
		return WorldRegistryImpl.getInstance().removeGuildFromAlliance(aId,
				guildId);
	}
	...
	public boolean increaseAllianceCapacity(int aId, int inc)
			throws RemoteException {
		return WorldRegistryImpl.getInstance().increaseAllianceCapacity(aId,
				inc);
	}
	...
	@Override
	public void setGuildLeader(int id, int labe, int fromchrid)
			throws RemoteException {
	...
	private ChannelList all() {
		return WorldRegistryImpl.getInstance()
				.getChannelList(channelDescriptor);
	}

	@Override
	public void registryOfflinePlayer(int cid) throws RemoteException {
		WorldRegistryImpl.getInstance().getPlayerStorage()
				.registryOfficePlayer(cid, channelDescriptor);
	}

	@Override
	public void deregisterOfflinePlayer(int cid) throws RemoteException {
		ChannelDescriptor cd = WorldRegistryImpl.getInstance()
				.getPlayerStorage().deregisterOfficePlayer(cid);
		if (cd != null) {
			WorldRegistryImpl.getInstance().getChannel(cd)
					.deregisterOfflinePlayer(cid);
		}
	}
}
