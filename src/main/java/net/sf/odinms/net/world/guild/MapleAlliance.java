...
public class MapleAlliance implements java.io.Serializable {
	...
	public static void disbandAlliance(MapleClient c, int allianceId) {
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("DELETE FROM `alliance` WHERE id = ?");
		...
		try {
			c.getChannelServer()
					.getWorldInterface()
					.allianceMessage(c.getPlayer().getGuild().getAllianceId(),
							ALLIANCE_msg.解散联盟(allianceId), -1, -1);
	...
	public static boolean canBeUsedAllianceName(String name) {
		if (name.contains(" ") || name.length() > 12) { // im using starswith
														// because the
														// 'contains' method
														// fails.
			return false;
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT * FROM alliance WHERE name = ?");
			ps.setString(1, name);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				ret = false;
	...
	public static MapleAlliance createAlliance(MapleCharacter chr1,
		...
		try {
			WorldChannelInterface wci = chr1.getClient().getChannelServer()
					.getWorldInterface();
			...
			wci.allianceMessage(id,
					ALLIANCE_msg.makeNewAlliance(alliance, chr1.getClient()),
					-1, -1);
		} catch (RemoteException e) {
			ServerExceptionHandler.HandlerRemoteException(e);
	...
	public void saveToDB() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("UPDATE `alliance` SET " + sb.toString()
							+ " WHERE id = ?");
			...
			if (rs.next()) {
				...
				if (avail != -1) { // empty slot
					PreparedStatement ps1 = con
							.prepareStatement("UPDATE alliance SET guild"
									+ avail + " = ? WHERE id = ?");
					if (add) {
						ps1.setInt(1, gid);
					} else {
	...
	public void setRankTitle(String[] ranks) {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("UPDATE `alliance` SET `rank_title1` =  ?, `rank_title2` =  ?, `rank_title3` =  ?, `rank_title4` =  ?, `rank_title5` =  ?, `capacity` = ? where `id` = ?");
	...
	public void setNotice(String notice) {
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("UPDATE `alliance` SET `notice` = ? where id = ?");
	...
	public String getNotice() {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("SELECT notice from alliance where id=?");
			ps.setInt(1, getId());
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
