...
public class MapleGuildCharacter implements java.io.Serializable // alias for a
																	// character
{
	...
	public MapleGuildCharacter(int id, int lv, String name,
			ChannelDescriptor channel, int job, int rank, int gid, boolean on,
			int allianceRank) {
		this.level = lv;
		this.id = id;
		this.name = name;
	...
	public int getAllianceRank() {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("SELECT alliancerank from characters where id = ?");
			ps.setInt(1, getId());
