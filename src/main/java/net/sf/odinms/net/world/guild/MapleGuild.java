...
public final class MapleGuild implements java.io.Serializable {
    ...
    public MapleGuild(int guildid, MapleGuildCharacter initiator) {
        ...
        try {
            ...
            do {
                members.add(new MapleGuildCharacter(rs.getInt("id"), rs.getInt("level"), rs.getString("name"), new ChannelDescriptor(0, world), rs.getInt("job"), rs.getInt("guildrank"), guildid, false, rs.getInt("allianceRank")));
            } while (rs.next());
    ...
    public void setLeader(int l, MapleClient c, int fromchrid) {
        //setLeader(l);
        try {
            c.getChannelServer().getWorldInterface().setGuildLeader(id, l, fromchrid);
    ...
    public void gainGP(int amount) {
        ...
        this.broadcast(MaplePacketCreator.serverNotice(5, "[注意]家族获得 " + amount + " GP点"));
    }
    ...
    public void buildNotifications() {
        List<Integer> chs = WorldRegistryImpl.getInstance().getChannelServerStorage().getChannelNumber(world);
        ...
        synchronized (members) {
            for (MapleGuildCharacter mgc : members) {
                ...
                List<Integer> ch = notifications.get(Integer.valueOf(mgc.getChannel()));
                if (ch == null) {
    ...
    public void writeToDB(boolean bDisband) {
        ...
        try {
            if (!bDisband) {
                ...
                sql += "capacity = ?, " + "notice = ? , leader = ? " + "" + "WHERE guildid = ?";
    ...
    public void broadcast(MaplePacket packet, int exceptionId, BCOp bcop) {
        synchronized (notifications) {
            ...
            try {
                ChannelList list = WorldRegistryImpl.getInstance().getChannelList(world);
                ...
                for (; it.hasNext();) {
                    ...
                    if (next instanceof ChannelDescriptor) {
                        ...
                        cwi = ChannelManager.getChannelServer(descriptor).getWorldInterface().getChannelInterface(descriptor.getId());
                    }
                    if (notifications.size() > 0 && notifications.get(descriptor.getId()).size() > 0) {
                        if (bcop == BCOp.DISBAND) {
                            cwi.setGuildAndRank(notifications.get(descriptor.getId()), 0, 5, exceptionId);
                        } else if (bcop == BCOp.EMBELMCHANGE) {
                            cwi.changeEmblem(this.id, notifications.get(descriptor.getId()), new MapleGuildSummary(this));
                        } else {
                            cwi.sendPacket(notifications.get(descriptor.getId()), packet, exceptionId);
                        }
                    }
                }
        ...
        synchronized (members) {
            ...
            while (itr.hasNext()) {
                ...
                if (mgc.getId() == cid && initiator.getGuildRank() < mgc.getGuildRank()) {
                    ...
                    try {
                        if (mgc.isOnline()) {
                            WorldRegistryImpl.getInstance().getChannel(mgc.getChannelDescriptor()).setGuildAndRank(cid, 0, 5);
                        } else {
                            try {
                                initiator.getName();
                                MaplePacketCreator.sendUnkwnNote(mgc.getName(), "你已被踢出家族", initiator.getName());
                            } catch (SQLException e) {
                                log.error("SAVING NOTE", e);
                            }
                            WorldRegistryImpl.getInstance().getChannel(new ChannelDescriptor(1, world)).setOfflineGuildStatus((short) 0, (byte) 5, cid);
                        }
                    } catch (RemoteException e) {
                        ServerExceptionHandler.HandlerRemoteException(e);
                        return;
    ...
    public void changeRank(int cid, int newRank, int fid) {
        for (MapleGuildCharacter mgc : members) {
            if (cid == mgc.getId()) {
                try {
                    if (mgc.isOnline()) {
                        WorldRegistryImpl.getInstance().getChannel(mgc.getChannelDescriptor()).setGuildAndRank(cid, this.id, newRank);
                    } else {
                        WorldRegistryImpl.getInstance().getChannel(new ChannelDescriptor(1, world)).setOfflineGuildStatus((short) this.id, (byte) newRank, cid);
                    }
                } catch (RemoteException e) {
                    ServerExceptionHandler.HandlerRemoteException(e);
                    return;
                ...
                if (fid != -2) {
                    if (fid == -1) {
                        ...
                        this.broadcast(MapleGuild_Msg.ChangeLeader(id, fid, cid));
                    }
                }
                return;
            }
        ...
        log.info("unable to find the correct id for changeRank(" + cid + ", " + newRank + ")");
    }
...
//            log.error("返回值:" + MapleGuildResponse.NOT_IN_CHANNEL);
 ...
 //           log.error("返回值:" + MapleGuildResponse.ALREADY_IN_GUILD);
        ...
        mc.getClient().getSession().write(MapleGuild_Msg.家族邀请(c.getPlayer().getGuildid(), c.getPlayer().getName(), c.getPlayer().getLevel(), c.getPlayer().getJob().getId()));
    ...
    public int getAllianceId() {
        ...
        try {
            ...
            PreparedStatement ps = con.prepareStatement("SELECT allianceid from guilds where guildid = ?");
            ps.setInt(1, getId());
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
