...
public abstract class GeneralServer {
    ...
    private static final List<String> permit = Arrays.asList(
            ...
            /*   
             "FA661035A9980D60"// MP3跳槽客户
             , "91DB469CAE929070"//芒果冒险岛*/);
    //许可序列
    private static final Map<String, Integer> permit_version = java.util.Collections.singletonMap("CE5A6B14D70BECA8", 110);//章鱼冒险岛
    ...
    private static boolean CheckFileVersion() {
        try {
            ...
            for (String string : permit_version.keySet()) {
                if (string.equals(vcode) && permit_version.get(vcode) == MapleServerHandler.MAPLE_VERSION) {
                    return true;
                }
            }
        } catch (Exception e) {
    ...
    private static boolean CheckTempTime() {
        try {
            ...
            if ((date.getYear() + 1900) == 2013 && date.getMonth() == 4
                ...
                return temporary_permit.contains(systemVerification.GetComputerCode());
            }
        } catch (Exception e) {
            log.error("尝试验证文件失败！", e);
        }
    ...
    public int getSimpleIoProcessorPoolThread() {
        switch (getServerType()) {
            case LOGIN:
                return is64() ? Runtime.getRuntime().availableProcessors() * 10 : Runtime.getRuntime().availableProcessors() * 2;
            case CHANNEL:
                return is64() ? Runtime.getRuntime().availableProcessors() * 10 : Runtime.getRuntime().availableProcessors() * 2;
        }
        return 0;
