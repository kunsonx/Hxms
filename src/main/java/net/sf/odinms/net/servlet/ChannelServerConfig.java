...
public class ChannelServerConfig extends GeneralServerConfig {
    ...
    private int worlds, channels, expRate, mesoRate, dropRate, bossdropRate, petExpRate, port;
    private boolean dropUndroppables, moreThanOne, cashshop, mts, useAddMaxAttack, gateway;
        ...
        if (_Server != null) {
            ...
            for (int i = 0; i < list.getLength(); i++) {
                ...
                if (n instanceof Element) {
                    el = (Element) n;
                    if (el.getNodeName().equals("Key") && el.getAttribute("name").equals(id)) {
            ...
            bossdropRate = Integer.parseInt(getSingleNodeValue(element, "BossDropRate", ""));
            petExpRate = Integer.parseInt(getSingleNodeValue(element, "PetRate", ""));
            serverMessage = getSingleNodeValue(element, "ServerMessage", "");
            dropUndroppables = Boolean.parseBoolean(getSingleNodeValue(element, "AllDrop", "false"));
            moreThanOne = Boolean.parseBoolean(getSingleNodeValue(element, "MoreThanOne", "false"));
            ...
            cashshop = Boolean.parseBoolean(getSingleNodeValue(element, "UseCashShop", "false"));
            mts = Boolean.parseBoolean(getSingleNodeValue(element, "UseMTS", "false"));
            servername = getSingleNodeValue(element, "ServerName", "");
            useAddMaxAttack = Boolean.parseBoolean(getSingleNodeValue(element, "UseAddMaxAttack", "false"));
            _t = getSingleNode(document.getDocumentElement(), "LoginGateWay");
            if (_t instanceof Element) {
                gateway = Boolean.parseBoolean(((Element) _t).getAttribute("enable"));
            }
            _t = getSingleNode(element, "ServerWeb");
            if (_t instanceof Element) {
                WebServer.getWebServer().setUse(Boolean.parseBoolean(((Element) _t).getAttribute("use")));
                WebServer.getWebServer().setPort(Integer.parseInt(((Element) _t).getAttribute("port")));
                WebServer.getWebServer().setServername(servername);
            }
            _t = getSingleNode(element, "Pvp");
            if (_t instanceof Element) {
                MaplePvp.PVP_CHANNEL = Integer.parseInt(((Element) _t).getAttribute("channel"));
                MaplePvp.PVP_MAP = Integer.parseInt(((Element) _t).getAttribute("map"));
            }
            ip = getSingleNodeValue(element, "InterfaceIpAddrees", "");
        }
        worldHost = getSingleNodeValue(element, "WorldHost", "");
        worlds = Integer.parseInt(getSingleNodeValue(element, "WorldCount", "1"));
        channels = Integer.parseInt(getSingleNodeValue(element, "ChannelCount", "1"));
    }
