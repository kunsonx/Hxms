...
public class MapleServerHandler extends IoHandlerAdapter {
    ...
    private final static Logger log = Logger.getLogger(MapleServerHandler.class);
    ...
    public MapleServerHandler(PacketProcessor processor, ChannelDescriptor channel) {
    ...
    public void Reset() {
        processor.reset(this.channel == null ? PacketProcessor.Mode.LOGINSERVER : PacketProcessor.Mode.CHANNELSERVER);
    }
    ...
    public void exceptionCaught(IoSession session, Throwable cause) throws Exception {
        ...
        if (client != null) {
            //        log.error(String.format("角色名：%s", client.getPlayer().getName()), cause);
        }
    }
    ...
    public void sessionOpened(IoSession session) throws Exception {
        ...
        byte ivRecv[] = {(byte) (Math.random() * 255), (byte) (Math.random() * 255), (byte) (Math.random() * 255), (byte) (Math.random() * 255)};
        byte ivSend[] = {(byte) (Math.random() * 255), (byte) (Math.random() * 255), (byte) (Math.random() * 255), (byte) (Math.random() * 255)};
        ...
        session.write(MaplePacketCreator.getHello(MAPLE_VERSION, MAPLE_SECONDARY_VERSION, ivSend, ivRecv, false));
    ...
    public void messageReceived(IoSession session, Object message) throws Exception {
        ...
        if (log.isTraceEnabled() || log.isInfoEnabled()) {
            ...
            if (packetHandler == null) {
                //log.info("未解密的封包1{} 长度：({}) \n包：{}", new Object[]{from, content.length, HexTool.toString(content)});
                //log.info("需解密的封包{} \n长度：({}) \n包：{}\nAscii码：{}", new Object[]{from, content.length, HexTool.toString(content), HexTool.toStringFromAscii(content)});
            }
        }
        ...
        if (packetHandler != null && packetHandler.validateState(client)) {
            try {
                ...
                t.printStackTrace();
                //log.debug(MapleClient.getLogMessage(client, "发生错误\n处理类: " + packetHandler.getClass().getName() + "\n原因: " + t.getMessage()));
                //StringWriter sw = new StringWriter();
                //t.printStackTrace(new PrintWriter(sw, true));
                //WriteToFile re = new WriteToFile("D:\\ErrorPacket\\处理错误.txt");
