...
public class CashShopHandler extends AbstractMaplePacketHandler {
	...
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(CashShopHandler.class);
		...
		if (action == 3) { // 购买物品089ok
			...
			if (item.getItemId() == 0) { // Gendri == 2 是全性别适用
				...
				if (item.getGender() < 2) // Gendri == 2 是全性别适用
				{
					 ...
					 */if (item.getCount() <= 0 || item.getItemId() <= 0
						...
						c.getSession().write(
								MaplePacketCreator.showNXMapleTokens(c
										.getPlayer()));
						return;
					}
				}
				...
				if (GameConstants.isPet(item.getItemId())) {
					MapleCSInventoryItem citem = new MapleCSInventoryItem(-1,
							item.getItemId(), snCS, (short) item.getCount(),
							false);
					int period = 90;
					citem.setBaseIItem(MaplePet.createPet(item.getItemId(),
							period));
					c.getPlayer().getCSInventory().addItem(citem);
					c.getSession().write(
							MaplePacketCreator.showBoughtCSItem(c, citem));
				} else {
					MapleCSInventoryItem citem = new MapleCSInventoryItem(
							MapleCharacter.getNextUniqueId(), item.getItemId(),
							snCS, (short) item.getCount(), false);
					long period = item.getPeriod();
					Timestamp ExpirationDate;
					ExpirationDate = new Timestamp(
							((System.currentTimeMillis() / 1000) + (period * 24 * 60 * 60)) * 1000);
					if (period == 0) {
						ExpirationDate = null;
					}
					if (item.getItemId() == 5211047
			...
			try {
				...
				PreparedStatement ps = con
						.prepareStatement("DELETE FROM wishlist WHERE charid = ?");
			...
			c.getSession().write(
					MaplePacketCreator.updateWishList(c.getPlayer().getId()));
			...
			if (add == 0) {
				...
				if (c.getPlayer().getCSPoints(useNX) < 600) {
					c.getSession()
							.write(MaplePacketCreator.showNXMapleTokens(c
									.getPlayer()));
					c.getSession().write(MaplePacketCreator.enableActions());
					return;
				}
				if (slots <= 92) {
					...
					c.getPlayer().getInventory(invtype)
							.setSlotLimit((byte) (slots + 4));
					c.getSession().write(
							MaplePacketCreator.serverNotice(1, "扩充成功."));
				} else {
					c.getSession().write(
							MaplePacketCreator.serverNotice(1, "您无法继续进行扩充."));
				}
			} else if (add == 1) {
				int sn = slea.readInt();
				byte type = 1;
			...
			c.getSession().write(
					MaplePacketCreator.serverNotice(1, "现在无法扩充仓库."));
			...
			if (c.getPlayer().getCSInventory().getItem(uniqueid) == null) {
				c.getSession().write(
						MaplePacketCreator.serverNotice(1, "请您重新进入商城！"));
				c.getSession().write(
						MaplePacketCreator.showNXMapleTokens(c.getPlayer()));
				c.getSession().write(MaplePacketCreator.enableActions());
				return;
			}
			IItem item = c.getPlayer().getCSInventory().getItem(uniqueid)
					.toItem();
			if (item != null) {
				short slot = c
						.getPlayer()
				...
				if (slot == -1) {
					c.getSession().write(
							MaplePacketCreator.serverNotice(1, "您的背包已满."));
				} else {
					c.getPlayer()
							.getInventory(
									MapleItemInformationProvider.getInstance()
					...
					c.getSession().write(
							MaplePacketCreator.transferFromCSToInv(item, slot));
				}
			}
			...
			for (MapleInventory inventory : c.getPlayer().getAllInventories()) {
				...
				if (item != null
						&& item instanceof Equip
						&& (((Equip) item).getStr() > 0
								|| ((Equip) item).getDex() > 0
								|| ((Equip) item).getInt() > 0 || ((Equip) item)
								.getLuk() > 0)) {
					c.getPlayer().dropMessage("此物品已经被管理员封了!!");
					c.getSession().write(MaplePacketCreator.enableActions());
					c.getSession()
				...
				if (item != null) {
					MapleCSInventoryItem citem = new MapleCSInventoryItem(
							item.getUniqueid(), item.getItemId(),
							CashItemFactory.getSnFromId(item.getItemId()),
					...
					inventory.removeItem(item.getPosition(),
							item.getQuantity(), false);
					c.getSession().write(
							MaplePacketCreator.transferFromInvToCS(
									c.getPlayer(), citem));
					break;
				}
			}
			...
			// MapleInventoryManipulator.addById(c, item.getItemId(), (short)
			// item.getCount(), "购买了任务物品");
			// MapleInventory etcInventory =
			// c.getPlayer().getInventory(MapleInventoryType.ETC);
			// short slot =
			// etcInventory.findById(item.getItemId()).getPosition();
			// c.getSession().write(MaplePacketCreator.showBoughtCSQuestItem(slot,
			// item.getItemId()));
			// } else {
			// c.getSession().write(MaplePacketCreator.enableActions());
			// return;
			...
			if (c.getPlayer().getCSPoints(type) >= cashPackage.getPrice()) {
				if ((cashPackage.getItemId() >= 5070000 && cashPackage
						.getItemId() <= 5077000)
						|| (cashPackage.getItemId() >= 5390000 && cashPackage
								.getItemId() <= 5390006)) {
					c.getPlayer().dropMessage("此物品已经被管理员封了!!");
					c.getSession().write(MaplePacketCreator.enableActions());
					return;
			...
			if (!c.getPlayer()
					.getCSInventory()
					.CheckSpace(
							CashItemFactory.getPackageItemsCount(cashPackage
									.getItemId()))) {
				c.getPlayer().dropMessage("商城空间不足。");
				c.getSession().write(
						MaplePacketCreator.showNXMapleTokens(c.getPlayer()));
			...
			for (CashItemInfo item : CashItemFactory
					.getPackageItems(cashPackage.getItemId())) {
				if (cashSetting.IsBanedItemId(item.getItemId())) {
					continue;
				}
				if (GameConstants.isPet(item.getItemId())) {
					...
					if (item.getItemId() == 5211047
							|| item.getItemId() == 5360014) {// 双倍经验值卡三小时权
						ExpirationDate = new Timestamp(
								((System.currentTimeMillis() / 1000) + (3 * 60 * 60)) * 1000);
					}
					if (item.getItemId() == 1112906
							&& item.getItemId() == 1112905) {// 心心祝福戒指
						ExpirationDate = new Timestamp(
								((System.currentTimeMillis() / 1000) + (period * 24 * 60 * 60)) * 1000);
					}
					citem.setExpire(ExpirationDate);
			...
			if (c.getPlayer().getCSPoints(0) >= item.getPrice()) {
				MapleCharacter pchr = c.getChannelServer().getPlayerStorage()
				...
				if (pchr != null) {
					...
					int pid = pchr.getId();
					MapleRing ring = new MapleRing(item.getItemId(), 0, pid,
							recipient);
					if (ring.恋人戒指()
							&& (c.getPlayer().getGender() == pchr.getGender())) {// 同性别
						c.getSession().write(
								MaplePacketCreator.showCheckPartner());
						return;
					}
					c.getPlayer().modifyCSPoints(0, -item.getPrice());
					MapleCSInventoryItem gift = new MapleCSInventoryItem(
							MapleCharacter.getNextUniqueId(), item.getItemId(),
							snCS, (short) item.getCount(), true);
					...
					gift.setBaseIItem(new MapleRing(item.getItemId(), citem
							.getUniqueId(), c.getPlayer().getId(), c
							.getPlayer().getName()).setRingId(gift
							.getUniqueId()));
					gift.setSender(c.getPlayer().getName());
					gift.setMessage(message);
					c.getPlayer().getCSInventory().addItem(citem);
					...
					pchr.getCSInventory().getCSGifts()
							.put(gift.getUniqueId(), gift);
					c.getSession().write(
							MaplePacketCreator.showBoughtCSItem(c, citem));
				} else {
			...
			if (c.getMaxCharSlot() >= 15) {
				c.getSession().write(
						MaplePacketCreator.serverNotice(1, "您的人物栏无法继续增加."));
				c.getSession().write(MaplePacketCreator.enableActions());
				return;
			}
			if (c.getPlayer().getCSPoints(useNX) > 5000) {
				...
				c.getSession().write(
						MaplePacketCreator.serverNotice(1, "成功扩充人物栏."));
				c.getSession().write(MaplePacketCreator.enableActions());
			} else {
				c.getSession().write(
						MaplePacketCreator.serverNotice(1, "您的点卷不足."));
				c.getSession().write(MaplePacketCreator.enableActions());
			}
			...
			if (c.getPlayer().getCSPoints(useNX) >= item.getPrice()) {
				...
				if (item.getItemId() == 5550001) {// 项链扩充（7天权）
					long time = c.getPlayer().getNecklace_Expansion() == null ? System
							.currentTimeMillis() : c.getPlayer()
							.getNecklace_Expansion().getTime();
					time += 1000 * 60 * 60 * 24 * 7;
					c.getPlayer().setNecklace_Expansion(new Timestamp(time));
					c.getSession().write(
							MaplePacketCreator.showNecklace_Expansion(7));
					...
					c.getPlayer().setNecklace_Expansion(new Timestamp(time));
					c.getSession().write(
							MaplePacketCreator.showNecklace_Expansion(30));
				}
			} else {
	 ...
	 * @return
	 */
	public static int GiftItem(MapleCSInventoryItem gift, String recipient,
			String playername) {
		int autokey = -1;
		try {
			int chrid = MapleCharacter.getIdByName(recipient);
			MapleInventoryType type = MapleItemInformationProvider
					.getInstance().getInventoryType(gift.getItemId());
			Connection con = DatabaseConnection.getConnection();
			...
			PreparedStatement ps = con
					.prepareStatement("INSERT INTO items (characterid, itemid, type, quantity, expiredate, uniqueid, gift) VALUES (?, ?, ?, ?, ?, ?, ?)");
			ps.setInt(1, chrid);
	 ...
	 *            对方角色名
	 */
	public static void CreateRingInfo(int item_id, long pu, int pchrid,
			String pn) {
		try {
			Connection con = DatabaseConnection.getConnection();
			...
			PreparedStatement ps = con
					.prepareStatement("INSERT INTO items_ring (items_id, partnerUniqueid, partnerChrId, partnerName) VALUES (?, ?, ?, ?)");
			ps.setInt(1, item_id);
			ps.setLong(2, pu);
			ps.setInt(3, pchrid);
			ps.setString(4, pn);
