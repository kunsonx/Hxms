...
public class UseCashItemHandler extends AbstractMaplePacketHandler {
    ...
    private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(UseCashItemHandler.class);
            ...
            switch (itemType) {
                    ...
                    if (itemId > 5050000) {
                        ...
                        switch (APFrom) {
                            ...
                            case 2048: // HP
                                ...
                                statupdate.add(new Pair<MapleStat, Number>(MapleStat.HP, c.getPlayer().getMaxhp()));
                            case 8192: // MP
                                if (c.getPlayer().getHpApUsed() <= 0) {
                                    return;
                                }
                                ...
                                c.getPlayer().setHpApUsed(c.getPlayer().getHpApUsed() - 1);
                                statupdate.add(new Pair<MapleStat, Number>(MapleStat.MP, c.getPlayer().getMaxmp()));
                            default:
                                c.getSession().write(MaplePacketCreator.enableActions());
                                return;
                        }
                ...
                case 506:
                    ...
                    if (itemId == 5060000) { //道具取名
                        ...
                        //IItem scroll = c.getPlayer().getInventory(MapleInventoryType.CASH).findById(itemId);
                        // c.getPlayer().getInventory(MapleInventoryType.CASH).removeItem(scroll.getPosition(), (short) 1, false);
                        // c.getSession().write(MaplePacketCreator.scrolledItem(scroll, Identify, false));//更新装备栏
                        c.getSession().write(MaplePacketCreator.updateEquipSlot(Identify));
                        MapleInventoryManipulator.removeById(c, MapleInventoryType.CASH, itemId, (short) 1, true, false);
                        c.getSession().write(MaplePacketCreator.魔方光效(player.getId(), itemId));
                        c.SendPacket(MaplePacketCreator.getMagicResult());
                        MapleInventoryManipulator.addById(c, 2430481, (short) 1, "", "");//用了魔方之后给一个魔方碎片
                        ...
                        c.getSession().write(MaplePacketCreator.魔方光效(player.getId(), itemId));
                        c.SendPacket(MaplePacketCreator.getMagicResult());
                        MapleInventoryManipulator.addById(c, 2430112, (short) 1, "", "");//用了魔方之后给一个魔方碎片
                        ...
                        if (a >= 35) {
                            ...
                            //c.getSession().write(MaplePacketCreator.updateEquipSlot(Identify));//更新装备栏
                            //MapleInventoryManipulator.removeById(c, MapleInventoryType.CASH, itemId, (short) 1, false, false);
                            //IItem scroll = c.getPlayer().getInventory(MapleInventoryType.CASH).findById(itemId);
                            // c.getPlayer().getInventory(MapleInventoryType.CASH).removeItem(scroll.getPosition(), (short) 1, false);
                            // c.getSession().write(MaplePacketCreator.scrolledItem(scroll, Identify, false));//更新装备栏

                            //  c.getSession().write(MaplePacketCreator.魔方光效(player.getId(), itemId));
                            // MapleInventoryManipulator.addById(c, 2430112, (short) 1, "", "", -1);//用了魔方之后给一个魔方碎片
                            //  c.getSession().write(MaplePacketCreator.enableActions());
                            //  return;
                        } else {
                            Identify.setIdentified(Identify.getIdentify());//先设置记录 免得没写记录系统之前的装备鉴定时重新鉴定
                        ...
                        c.getSession().write(MaplePacketCreator.updateEquipSlot(eq, true));
                        c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.getScrollEffect(c.getPlayer().getId(), IEquip.ScrollResult.SUCCESS, false, itemId, eq.getItemId()));
                        MapleInventoryManipulator.removeById(c, MapleInventoryType.CASH, itemId, 1, true, false);
                    }
                    c.getSession().write(MaplePacketCreator.enableActions());
                    break;
                case 507: { //使用商场喇叭
                    ...
                    switch (itemId / 1000 % 10) {
                        ...
                        case 2: // Super megaphone
                            ...
                            break;
                        case 3: // 心脏3
                            c.getChannelServer().getWorldInterface().broadcastMessage(null, MaplePacketCreator.serverNotice(PhoneType.心脏高级喇叭, c.getChannel(), prefix + player.getName() + " : " + slea.readMapleAsciiString(), (slea.readByte() != 0)).getBytes());
                            break;
                        case 4: // 白骨
                            c.getChannelServer().getWorldInterface().broadcastMessage(null, MaplePacketCreator.serverNotice(PhoneType.白骨高级喇叭, c.getChannel(), prefix + player.getName() + " : " + slea.readMapleAsciiString(), (slea.readByte() != 0)).getBytes());
                            break;
                        case 5: // Maple TV
                        ...
                        case 6: //道具喇叭
                            ...
                            c.getChannelServer().getWorldInterface().broadcastMessage(null, MaplePacketCreator.getMegaphone(PhoneType.道具喇叭, c.getChannel(), prefix + player.getName() + " : " + msg, megaitem, showEar).getBytes());
                            break;
                        case 7: //(缤纷喇叭7)
                        ...
                        case 9: {
                            switch (itemId) {
                                case 5079001://蛋糕高级喇叭
                                {
                                    c.getChannelServer().getWorldInterface().broadcastMessage(null, MaplePacketCreator.serverNotice(PhoneType.蛋糕喇叭, c.getChannel(), prefix + player.getName() + " : " + slea.readMapleAsciiString(), (slea.readByte() != 0)).getBytes());
                                    break;
                                }
                                case 5079002://馅饼高级喇叭
                                {
                                    c.getChannelServer().getWorldInterface().broadcastMessage(null, MaplePacketCreator.serverNotice(PhoneType.馅饼喇叭, c.getChannel(), prefix + player.getName() + " : " + slea.readMapleAsciiString(), (slea.readByte() != 0)).getBytes());
                                    break;
                                }
                                default:
                                    break;
                ...
                case 510: //音乐盒
                    ...
                    c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.getMusicBox(itemId, c.getPlayer().getName()));
                    MapleInventoryManipulator.removeById(c, MapleInventoryType.CASH, itemId, 1, true, false);
                    break;
                case 512://商场buff
                ...
                case 528:
                    ...
                    Rectangle bounds = mse.calculateBoundingBox(c.getPlayer().getPosition(), c.getPlayer().isFacingLeft());
                ...
                case 552: //剪刀
                    ...
                    if (item == null || item.getQuantity() <= 0 || item.HasFlag(InventoryConstants.Items.Flags.KARMA) && ii.isKarmaAble(item.getItemId())) {
                        c.getSession().write(MaplePacketCreator.enableActions());
                        return;
                    }
