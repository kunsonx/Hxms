...
public class SpecialMoveHandler extends AbstractMaplePacketHandler {
	...
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(SpecialMoveHandler.class);
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (c.getPlayer().getMapId() == 910000000 && !c.getPlayer().isGM()
				&& c.getChannel() != 4) {
		...
		if (skillLevel == 0) {
			...
			if (effect.getCooldown() > 0) { // 如果有冷却时间
				if (chr.skillisCooling(effect.getColldownId())) {
					...
					if (skillid != 5221006
							|| (skillid == 机械师.磁场 && chr.getSummons()
									.get(机械师.磁场).size() == 2)) { // 武装不设置冷却时间
						c.getSession().write(
								MaplePacketCreator.skillCooldown(
										effect.getColldownId(),
										effect.getCooldown()));
						ScheduledFuture<?> timer = TimerManager.getInstance()
								.schedule(
										new CancelCooldownAction(chr,
												effect.getColldownId()),
										effect.getCooldown() * 1000);
						chr.addCooldown(effect.getColldownId(),
			...
			if (skillid == 4341003 || skillid == 4121004 || skillid == 4221004) { // 怪物炸弹
																					// 忍者伏击
				...
				if (hasMonster > 0) {
					for (int i = 0; i < hasMonster; i++) {
						...
						MapleMonster monster = c.getPlayer().getMap()
								.getMonsterByOid(mobid);
						MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(
								effect.getMonsterStati(), skill, false);
						monster.applyStatus(chr, monsterStatusEffect, isPoison,
								effect.getDuration());
					}
				}
				...
				// log.info("吞噬技能");
				MapleMonster monster = c.getPlayer().getMap()
						.getMonsterByOid(c.getPlayer().获得吞噬的怪的id());
				if (monster != null) {
					monster.switchController(chr,
							monster.isControllerHasAggro());
				}
				...
				if (skillid == 机械师.磁场) {
					磁场数量 = slea.readByte(); // 0 - 第一个磁场 1 - 第二个磁场 2 -
											// 第三个磁场[召唤磁场效果]
				}
				/*
				 * if (磁场数量 == 2) { pos = new
				 * Point(c.getPlayer().getPosition()); } else { pos = new
				 * Point(slea.readShort(), slea.readShort()); }
				 */
			...
			if (skillid == 机械师.加速器) {
				...
				for (MapleMapObject monstermo : monsters) {
					...
					MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(
							effect.getMonsterStati(), skill, false);
					monster.applyStatus(chr, monsterStatusEffect, false,
							effect.getDuration());
				}
			}
			if (chr.isAlive()) {
				...
				} else if (skillid == 战法.进阶黑暗灵气 || skillid == 战法.进阶蓝色灵气
						|| skillid == 战法.进阶黄色灵气) {
				...
				if (skill.getId() != 2311002 || chr.canDoor()) {
					...
					if (skillid == 机械师.磁场 && 磁场数量 == 2) {
						// 产生磁场
						chr.getMap().broadcastMessage(
								MaplePacketCreator.召唤磁场(chr.getId(), chr
										.getSummons().get(机械师.磁场).get(0)
										.getObjectId(),
										chr.getSummons().get(机械师.磁场).get(1)
												.getObjectId(), chr
												.getSummons().get(机械师.磁场)
												.get(2).getObjectId()));
						...
						SkillFactory.getSkill(21000000)
								.getEffect(combo > 10 ? 10 : combo)
								.applyTo(chr);
						chr.setCombo(comboCount);
						chr.getClient()
								.getSession()
								.write(MaplePacketCreator
										.Combo_Effect(comboCount));
						...
						if (!monsters.isEmpty()) {
							chr.getMap().broadcastMessage(
									MaplePacketCreator.getAttackEffect(c
											.getPlayer().getId(), skillid,
											monsters, 6));
						}
					}
					if (skill.getId() != 2301002 && effect != null
							&& effect.getMpCon() != 0) { // 群体治愈
						if (chr.getMp() - beforeMp < skill
								.getEffect(skillLevel).getMpCon()) {
							int remainingMp = beforeMp
									- skill.getEffect(skillLevel).getMpCon();
	...
	public int changeSkillId(int skillid, MapleCharacter chr) {
		int a = skillid;
		if (skillid == 机械师.金属机甲_重机枪
				&& chr.getBuffedValue(MapleBuffStat.机械师) != null) {
			a = 35121013;
