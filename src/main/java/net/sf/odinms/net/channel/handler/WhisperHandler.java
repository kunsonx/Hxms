        ...
        if (mode == 6) { // Whisper
            ...
            if (!CommandProcessor.getInstance().processCommand(c, text)) {
                ...
                if (player != null) {
                    ...
                    int result = 0;
                    try {
                        result = c.getChannelServer().getWorldInterface().whisper(c.getPlayer().getName(), recipient, c.getChannel(), text, c.getPlayer().isGM());
                        c.getSession().write(MaplePacketCreator.getWhisperReply(recipient, (byte) result));
            ...
            if (player != null) {
                if (player.inCS()) {
                    c.getSession().write(MaplePacketCreator.getFindReplyWithCS(player.getName(), mode));
                } else {
                    c.getSession().write(MaplePacketCreator.getFindReplyWithMap(player.getName(), player.getMap().getId(), mode));
                }
            } else { // Not found
                ...
                if (player != null && (player.isGM() ? c.getPlayer().isGM() : true)) {
                    c.getSession().write(MaplePacketCreator.getFindReply(player.getName(), (byte) player.getClient().getChannel(), mode));
                } else {
                    c.getSession().write(MaplePacketCreator.getWhisperReply(recipient, (byte) 0));
                }
            }
