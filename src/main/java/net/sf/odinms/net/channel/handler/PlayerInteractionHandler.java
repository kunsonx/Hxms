...
public class PlayerInteractionHandler extends AbstractMaplePacketHandler {
	...
	public enum Action {
		...
		CREATE(0x07), // 创建0-6
		ENTER_RESPOSE(0X10), INVITE(0x0C), // 邀请2-11
		DECLINE(0x0D), VISIT(0x0A), // 0x04-09
		CHAT(0x0F), EXIT(0x13), // 0A
		OPEN(0x11), // 0B
		MAINTAIN(0x15), SET_ITEMS(0x00), // 770e
		SET_MESO(0x01), // 0f
		CONFIRM(0x02), // 10
		ADD_ITEM(0x16), // 13
		BUY(0x17), // 14
		REMOVE_ITEM(0x1D), // 18
		BAN_PLAYER(0x24), // 1B
		ADDTOBANLIST(0X25), REMOVEBANLIST(0X26), PUT_ITEM(0xFF), // 1E
		MERCHANT_BUY(0x21), // 1F
		TAKE_ITEM_BACK(0xFF), // 23

		MAINTENANCE_OFF(0x1E), // 24
		MERCHANT_ORGANIZE(0x1F), // 25
		CLOSE_MERCHANT(0x20), RPSGameStart(0x45), RPSGameSet(0x47), RPSGame(
				0x55), RPSGameResult(0x56);
		final byte code;
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (mode == Action.CREATE.getCode()) {
			...
			if (createType == 3) {
				...
				if (createType == 1 || createType == 2) {
					...
					c.getPlayer()
							.getMap()
							.broadcastMessage(
									MaplePacketCreator.sendInteractionBox(c
											.getPlayer()));
					...
					if (c.getPlayer().haveItem(itemId, 1, false, true)) {
						if (createType == 5) {
							shop = new MaplePlayerShop(c.getPlayer(), itemId,
									desc);
						} else {
							shop = new HiredMerchant(c.getPlayer(), itemId,
									desc);
						}
						c.getPlayer().setInteraction(shop);
						c.getSession().write(
								MaplePacketCreator.getInteraction(
										c.getPlayer(), true));
					} else {
						AutobanManager
								.getInstance()
			...
			MapleCharacter otherChar = c.getPlayer().getMap()
					.getCharacterById(otherPlayer);
			MapleTrade.inviteTrade(c.getPlayer(), otherChar);
		} else if (mode == Action.DECLINE.getCode()) {
			MapleCharacter src = c.getPlayer().getMap()
					.getCharacterById(slea.readInt());
			...
			if (dst != null && dst.getRPSGame() != null) {
				...
				if (ob instanceof IPlayerInteractionManager
					...
					if (ips.getShopType() == 1) {
						...
						if (merchant.isOwner(c.getPlayer())) {
							...
							// merchant.broadcast(MaplePacketCreator.shopErrorMessage(0x0D,
							// 1), false);
							...
							c.getSession().write(
									MaplePacketCreator.getMiniBoxfailenter());
							return;
						}
					} else if (ips.getShopType() == 2) {
						if (((MaplePlayerShop) ips).isBanned(c.getPlayer()
								.getName())) {
					...
					if (ips.getFreeSlot() == -1) {
						c.getSession().write(
								MaplePacketCreator.getMiniBoxFull());
						return;
					}
					c.getPlayer().setInteraction(ips);
					ips.addVisitor(c.getPlayer());
					c.getSession().write(
							MaplePacketCreator.getInteraction(c.getPlayer(),
									false));
				}
			}
			...
			if (c.getPlayer().getTrade() != null) {
				...
				CommandProcessor.getInstance().processCommand(c, message); // debug
																			// purposes
				ips.broadcast(MaplePacketCreator.shopChat(
						c.getPlayer().getName() + " : " + message,
						ips.isOwner(c.getPlayer()) ? 0 : ips.getVisitorSlot(c
								.getPlayer()) + 1), true);
			}
			...
			if (shop != null && shop.isOwner(c.getPlayer())) {
				c.getPlayer().getMap()
						.addMapObject((PlayerInteractionManager) shop);
				if (shop.getShopType() == 1) {
					HiredMerchant merchant = (HiredMerchant) shop;
					merchant.setOpen(true);
					c.getChannelServer().getCim()
							.add(merchant.getOwnerId(), merchant);
					c.getPlayer()
							.getMap()
							.broadcastMessage(
									MaplePacketCreator
											.spawnHiredMerchant(merchant));
					c.getPlayer().setInteraction(null);
				} else if (shop.getShopType() == 2) {
					c.getChannelServer()
							.getCim()
							.add(((PlayerInteractionManager) shop).getOwnerId(),
									(PlayerInteractionManager) shop);
					c.getPlayer()
							.getMap()
							.broadcastMessage(
									MaplePacketCreator.sendInteractionBox(c
			...
			MapleItemInformationProvider ii = MapleItemInformationProvider
					.getInstance();
			MapleInventoryType ivType = MapleInventoryType.getByType(slea
					.readByte());
			IItem item = c.getPlayer().getInventory(ivType)
			...
			if (c.getPlayer().getTrade() != null && item != null) {
				...
				if ((quantity <= item.getQuantity() && quantity >= 0)
					...
					if (!c.getChannelServer().allowUndroppablesDrop()
							&& ii.isDropRestricted(item.getItemId())) { // ensure
																		// that
																		// undroppable
																		// items
																		// do
			...
			MapleInventoryType type = MapleInventoryType.getByType(slea
					.readByte());
				...
				if (ivItem != null
					...
					if (ivItem.HasFlag(InventoryConstants.Items.Flags.锁定)
							|| MapleItemInformationProvider.getInstance()
									.isCash(ivItem.getItemId())) {
						c.getPlayer().gainWarning(true);
						c.getSession()
								.write(MaplePacketCreator.enableActions());
						return;
					}
					if (bundles <= 0 || perBundle <= 0 || checkquantity > 20000
							|| checksmquantity < 0 || checkiquantity < 0
							|| checkiquantity > 20000) {
						AutobanManager.getInstance().autoban(c,
								"异常物品销售: " + sellItem.getItemId());
						return;
					}
					if (bundles > MapleItemInformationProvider.getInstance()
							.getSlotMax(c, sellItem.getItemId())
							|| perBundle > 4000) {
					...
					MapleItemInformationProvider ii = MapleItemInformationProvider
							.getInstance();
					if (ii.isThrowingStar(ivItem.getItemId())
					...
					shop.addItem(item);
					c.getSession().write(
							MaplePacketCreator.shopItemUpdate(shop));
				}
			}
			...
			if (shop != null) {
				...
				try {
					if (shop.isOwner(c.getPlayer())
						...
						t.setQuantity((short) (item.getBundles() * item
								.getItem().getQuantity()));
						if (item.getBundles() > 0
								&& MapleInventoryManipulator.checkSpace(c,
										t.getItemId(), item.getBundles(),
										t.getOwner())
					...
					if (merchant.getShopType() == 1
						...
						for (MaplePlayerShopItem items : merchant.getItems()) {
							if (items.getBundles() > 0) {
								IItem item = items.getItem();
								item.setQuantity(items.getBundles());
								if (MapleInventoryManipulator.checkSpace(c,
						...
						c.getSession().write(
								MaplePacketCreator.shopErrorMessage(3, 0));
			...
			HiredMerchant merchant = (HiredMerchant) c.getPlayer()
					.getInteraction();
			...
			PlayerInteractionManager imps = (PlayerInteractionManager) c
					.getPlayer().getInteraction();
			if (slea.available() == 0) {
				c.getSession().write(MaplePacketCreator.getBannedList(imps));
			} else {
				if (imps != null && imps.isOwner(c.getPlayer())) {
					((MaplePlayerShop) imps).banPlayer(slea
