...
public class UserItemServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet UserItemServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        } finally {
            out.close();
        ...
        if (user != null && name != null) {
            ...
            for (WebUserPlayer up : user.getPlayers()) {
                if (up.getName().equals(name)) {
                    result = up.Items().HandlerAction(request.getParameter("action"), Long.parseLong(request.getParameter("id")));
                    break;
                }
            }
            response.setContentType("text/html;charset=UTF-8");
