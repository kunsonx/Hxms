...
public class DistributeSPHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        int type = 0;
        /*  if (player.isEvan()) {
         type = player.getDragonSkill(skillid);
         remainingSp = player.getDragonSp(type);
         //log.debug("龙神技能");
         } else if (player.isResistance()) {
         type = player.getBigBang(skillid);
         remainingSp = player.getBigBangSP(type);
         //log.debug("反抗者技能");
         } else {
        ...
        if ((level <= remainingSp
                && curLevel + 1 <= (skill.hasMastery() ? player.getMasterLevel(skill) : skill.getMaxLevel()))
                || isBeginnerSkill) {
            if (!isBeginnerSkill) {
                /*   if (player.isEvan()) {
                 player.givedragonSP(type, -1);
                 } else if (player.isResistance()) {
                 player.gainBigBangSP(type, -1);
                 } else {*/
                player.setRemainingSp(remainingSp - level, MapleJob.GetSkillAtSlot(skillid));
                player.updateSingleStat(MapleStat.AVAILABLESP, player.getRemainingSp());
//                }
            }
            //player.changeSkillLevel(skill, curLevel + 1, player.getMasterLevel(skill));
            teachLinkSkills(player, skill, curLevel + level, player.getMasterLevel(skill));
        }
