...
public class MTSHandler extends AbstractMaplePacketHandler {
	...
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(DistributeSPHandler.class);
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (slea.available() > 0) {
			...
			if (op == 2) { // put item up for sale
				...
				if (itemtype == 1) {
					slea.skip(32); // item stats, but we don't need them (can be
									// exploited!)
				} else {
				...
				if (i != null && c.getPlayer().getMeso() >= 5000) {
					...
					try {
						...
						if (rs.next()) {
							if (rs.getInt(1) > 10) { // They have more than 10
								...
								c.getSession()
										.write(MaplePacketCreator
												.TransferInventory(getTransfer(c
														.getPlayer().getId())));
								c.getSession().write(
										MaplePacketCreator
												.NotYetSoldInv(getNotYetSold(c
														.getPlayer().getId())));
								return;
							}
						}
					...
					c.getSession().write(getMTS(1, 0, 0));
					c.getSession().write(
							MaplePacketCreator.TransferInventory(getTransfer(c
									.getPlayer().getId())));
					c.getSession().write(
							MaplePacketCreator.NotYetSoldInv(getNotYetSold(c
									.getPlayer().getId())));
				}
				...
				c.getSession().write(
						getMTSSearch(tab, type, ci, search, c.getPlayer()
								.getCurrentPage()));
				c.getSession().write(
						MaplePacketCreator.showMTSCash(c.getPlayer()));
				c.getSession().write(
						MaplePacketCreator.TransferInventory(getTransfer(c
								.getPlayer().getId())));
				c.getSession().write(
				...
				c.getSession().write(
						getMTS(c.getPlayer().getCurrentTab(), c.getPlayer()
								.getCurrentType(), c.getPlayer()
								.getCurrentPage()));
				c.getSession().write(
						MaplePacketCreator.NotYetSoldInv(getNotYetSold(c
								.getPlayer().getId())));
				...
				try {
					...
					if (rs.next()) {
						IItem i;
						if (rs.getInt("type") != 1) {
							Item ii = new Item(rs.getInt("itemid"), (byte) 0,
						...
						MapleInventoryManipulator.addFromDrop(c, i,
								"MTS transfer", false);

						c.getSession().write(getCart(c.getPlayer().getId()));
						c.getSession().write(
								getMTS(c.getPlayer().getCurrentTab(), c
					...
					if (rs.next()) {
						...
						if (c.getPlayer().getCSPoints(4) >= price) {
							...
							if (alwaysnull) {
								...
								if (rse.next()) {
									PreparedStatement psee = con
											.prepareStatement("UPDATE accounts SET cardNX = cardNX + ? WHERE id = ?");
							...
							c.getSession().write(
									getMTS(c.getPlayer().getCurrentTab(), c
											...
											.getPlayer().getCurrentPage()));
							c.getSession().write(
									MaplePacketCreator.MTSConfirmBuy());
							c.getSession().write(
									MaplePacketCreator.showMTSCash(c
											.getPlayer()));
							c.getSession().write(
									MaplePacketCreator
											.TransferInventory(getTransfer(c
				...
				try {
					...
					if (rs.next()) {
						...
						if (c.getPlayer().getCSPoints(4) >= price) {
							...
							c.getPlayer().modifyCSPoints(4, -price);
							c.getSession()
									.write(getCart(c.getPlayer().getId()));
	...
	public List<MTSItemInfo> getNotYetSold(int cid) {
		...
		try {
			...
			while (rs.next()) {
				if (rs.getInt("type") != 1) {
					Item i = new Item(rs.getInt("itemid"), (byte) 0,
							(short) rs.getInt("quantity"));
					i.setOwner(rs.getString("owner"));
					items.add(new MTSItemInfo((IItem) i, rs.getInt("price"), rs
			...
			while (rs.next()) {
				...
				if (rse.next()) {
					if (rse.getInt("type") != 1) {
						Item i = new Item(rse.getInt("itemid"), (byte) 0,
								(short) rse.getInt("quantity"));
						i.setOwner(rse.getString("owner"));
						items.add(new MTSItemInfo((IItem) i, rse
								.getInt("price"), rse.getInt("id"), rse
								.getInt("seller"), rse.getString("sellername"),
								rse.getString("sell_ends")));
					} else {
						Equip equip = new Equip(rse.getInt("itemid"),
								(byte) rse.getInt("position"), false);
						...
						items.add(new MTSItemInfo((IItem) equip, rse
								.getInt("price"), rse.getInt("id"), rse
								.getInt("seller"), rse.getString("sellername"),
								rse.getString("sell_ends")));
					}
	...
	public MaplePacket getMTSSearch(int tab, int type, int cOi, String search,
			int page) {
		List<MTSItemInfo> items = new ArrayList<MTSItemInfo>();
		...
		try {
			...
			if (type != 0) {
				ps = con.prepareStatement("SELECT COUNT(*) FROM mts_items WHERE tab = ? "
						+ listaitems + " AND type = ? AND transfer = 0");
			} else {
				ps = con.prepareStatement("SELECT COUNT(*) FROM mts_items WHERE tab = ? "
