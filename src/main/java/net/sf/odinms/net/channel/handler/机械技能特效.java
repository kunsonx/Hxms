...
public class 机械技能特效 extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if ((skillId == 机械师.强化火焰喷射器 || skillId == 机械师.火焰喷射器
				|| skillId == 机械师.金属机甲_导弹战车 || skillId == 机械师.金属机甲_重机枪 || skillId == 机械师.金属机甲_重机枪_4转 // ||
																										// skillId
																										// ==
																										// 机械师.火箭推进器
		// 机械的特效技能
			...
			c.getPlayer()
					.getClient()
					.getSession()
					.write(MaplePacketCreator.机械技能特效(1, 0, skillId - 1000,
							skillLevel, unknow, -1));
			// c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.机械技能特效(c.getPlayer().getId(),
			// 1, 0, skillId - 1000, skillLevel, unknow, -1));
