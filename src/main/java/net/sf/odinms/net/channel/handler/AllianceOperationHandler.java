...
public class AllianceOperationHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        try {
            ...
            switch (fengb) {
                case 0x01://联盟上线
                    c.getChannelServer().getWorldInterface().allianceMessage(alliance.getId(), ALLIANCE_msg.在线联盟成员(c.getPlayer(), true), c.getPlayer().getId(), -1);
                    break;
                case 0x02://自己退出联盟
                    c.getPlayer().getGuild().setAllianceId(0);
                    alliance.addRemGuildFromDB(c.getPlayer().getGuild().getId(), false);
                    c.getSession().write(ALLIANCE_msg.removeGuildFromAlliance(alliance, c.getPlayer().getGuild().getId(), c, false));//自己的提示
                    c.getChannelServer().getWorldInterface().allianceMessage(alliance.getId(), ALLIANCE_msg.removeGuildFromAlliance(alliance, c.getPlayer().getGuild().getId(), c, false), -1, -1);
                ...
                case 0x03://发送邀请联盟
                    ...
                    if (victim_guildId != -1) {
                        MapleGuild guild = c.getChannelServer().getWorldInterface().getGuild(victim_guildId, null);
                        victim_player = c.getChannelServer().getPlayerStorage().getCharacterById(guild.getLeaderId());
                        if (guild != null && victim_player != null) {
                            victim_player.getClient().getSession().write(ALLIANCE_msg.发送邀请(alliance, c.getPlayer().getId(), c, c.getPlayer().getName()));
                        }
                    } else {
                        log.debug("查询不到此家族");
                    }
                ...
                case 0x04://玩家接受邀请
                    ...
                    //提示
                    c.getSession().write(ALLIANCE_msg.addGuildToAlliance(alliance, c.getPlayer().getGuild().getId(), c));//自己的提示
                    c.getChannelServer().getWorldInterface().allianceMessage(alliance.getId(), ALLIANCE_msg.addGuildToAlliance(alliance, c.getPlayer().getGuild().getId(), c), -1, -1);//联盟的提示
                    //联盟信息
                    c.getSession().write(ALLIANCE_msg.getAllianceInfo(alliance));//自己的提示
                    c.getChannelServer().getWorldInterface().allianceMessage(alliance.getId(), ALLIANCE_msg.getAllianceInfo(alliance), -1, -1);//联盟的提示
                    //综合信息
                    c.getSession().write(ALLIANCE_msg.getGuildAlliances(alliance, c));//自己的提示
                    c.getChannelServer().getWorldInterface().allianceMessage(alliance.getId(), ALLIANCE_msg.getGuildAlliances(alliance, c), -1, -1);//联盟的提示
                    break;
                case 0x0A://联盟公告
                    String notice = slea.readMapleAsciiString();
                    alliance.setNotice(notice);
                    //c.getChannelServer().getWorldInterface().setAllianceNotice(alliance.getId(), notice);
                    c.getSession().write(ALLIANCE_msg.联盟公告(alliance.getId(), notice));
                ...
                case 0x06://T出联盟 被踢出者的家族ID 被踢出的联盟ID
                    ...
                    MapleGuild guild = c.getChannelServer().getWorldInterface().getGuild(cid, null);//从家族中找到玩家Id
                    victim_player = c.getChannelServer().getPlayerStorage().getCharacterById(guild.getLeaderId());//获取玩家的实例
                    victim_player.getGuild().setAllianceId(0);
                    alliance.removeGuild(cid);
                    victim_player.getClient().getSession().write(ALLIANCE_msg.removeGuildFromAlliance(alliance, cid, c, true));
                    c.getChannelServer().getWorldInterface().allianceMessage(alliance.getId(), ALLIANCE_msg.removeGuildFromAlliance(alliance, c.getPlayer().getGuild().getId(), c, true), -1, -1);
                    //respawnPlayer(c.getPlayer());
                    break;
                case 0x07://设置联盟主
                    cid = slea.readInt();
                    //设置权限
                    victim_player = c.getChannelServer().getPlayerStorage().getCharacterById(cid);
                    if (victim_player == null) {
                        return;
                    }
                    victim_player.setAllianceRank(1);//新任的联盟主权限
                    c.getPlayer().setAllianceRank(2);//以前的联盟主权限
                    c.getSession().write(ALLIANCE_msg.联盟主(alliance.getId(), c.getPlayer().getId(), cid));
                    c.getChannelServer().getWorldInterface().allianceMessage(alliance.getId(), ALLIANCE_msg.联盟主(alliance.getId(), c.getPlayer().getId(), cid), -1, -1);
                    break;
                case 0x08://联盟职位改变
                    ...
                    c.getSession().write(ALLIANCE_msg.联盟地位改变(alliance.getId(), ranks));
