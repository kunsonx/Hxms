...
public class CloseRangeDamageHandler extends DamageParseHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) { //参数slea就是接到的数据
        ...
        //MaplePacket packet = MaplePacketCreator.closeRangeAttack(player.getId(), attack.skill, attack.stance, attack.numAttackedAndDamage, attack.allDamage, attack.speed, attack.pos, player.getSkillLevel(attack.skill));
        player.getMap().broadcastMessage(player, MaplePacketCreator.closeRangeAttack(attack, player), false, true);

        if (player.getBuffManager().hasBuff(4221013)
                && attack.skill == 4221007
        ...
        if (isFinisher(attack.skill)) {
            ...
            if (attack.skill != 1111008 && comboBuff != null && (player.getSkillLevel(1111002) > 0 || player.getSkillLevel(11111001) > 0)) { // shout should not give orbs虎咆哮
        ...
        if (attack.numAttacked > 0 && attack.skill == 1311005) { //比如龙之献祭技能
            ...
            player.setHp(player.getHp() - totDamageToOneMonster * attack.getAttackEffect(player, theSkill).getX() / 100);
        ...
        if (attack.skill != 0) {
            ...
            theSkill = SkillFactory.getSkill(attack.skill);
            MapleStatEffect effect = attack.getAttackEffect(c.getPlayer(), theSkill);
            if (theSkill != null && effect != null) {
                ...
                if (effect.getCooldown() > 0) {
                    ...
                    ScheduledFuture<?> timer = TimerManager.getInstance().schedule(new CancelCooldownAction(c.getPlayer(), attack.skill), effect.getCooldown() * 1000);
                    c.getPlayer().addCooldown(attack.skill, System.currentTimeMillis(), effect.getCooldown() * 1000, timer);
                }
            }
        }
        ...
        if (attack.skill == 21120002 || attack.skill == 21110002 || attack.skill == 21110006) {
            ...
            if (effect_.getCooldown() > 0) {
                ...
                c.getPlayer().addCooldown(attack.skill, System.currentTimeMillis(), effect_.getCooldown() * 1000, timer);//冷却时间
            }
        }
