...
public class PlayerLoggedinHandler extends AbstractMaplePacketHandler {
    ...
    private static final org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(PlayerLoggedinHandler.class);
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        try {
            List<PlayerBuffValueHolder> buffs = c.getChannelServer().getWorldInterface().getBuffsFromStorage(cid);
            if (buffs != null) {
                c.getPlayer().silentGiveBuffs(buffs);
            }
            c.getChannelServer().getWorldInterface().deregisterOfflinePlayer(player.getAccountid());
            if (player.getGuildid() > 0) {
                MapleGuild g = player.getClient().getChannelServer().getGuild(player.getMGC());
        ...
        try {
            ...
            if (player.getGuildid() > 0) {
                ...
                if (allianceId > 0) {
                    ...
                    c.getSession().write(ALLIANCE_msg.getAllianceInfo(newAlliance));
                    c.getSession().write(ALLIANCE_msg.getGuildAlliances(newAlliance, c));
                }
            }
        } catch (RemoteException e) {
            ServerExceptionHandler.HandlerRemoteException(e);
        ...
        if (player.getMapId() == 555000000) {
            //c.getSession().write(MaplePacketCreator.sendHint("欢迎光临我的朋友,请点击NPC：#r[冒险岛管理员]#k 与它谈话.", 450, 5));
            player.startMapEffect("欢迎.请点击NPC：[黑鼻白毛] 与它谈话！", 5121030);
        }
        int vip = c.getPlayer().getVip();
        try {
            if (c.getPlayer().isGM()) {
                c.getPlayer().dropMessage("伟大的管理员大神上线巡逻了！    所有在线人数为:" + c.getPlayer().Lianjie() + "人");
                player.getClient().getSession().write(MaplePacketCreator.sendHint(String.format("欢迎来到%s\r\n#b@帮助 #k可查看游戏命令\r\n#b@npc#k 打开万能NPC\r\n#r祝大家游戏愉快天天开心...", channelServer.getServerName()), 300, 5));

            } else {
                String name = "[Game Master]";
                if (vip > 1) {
                    int value = GameConstants.getLoginTipType(vip);
                    c.getChannelServer().getWorldInterface().broadcastMessage(null, MaplePacketCreator.serverMessage(
                            value,
                            c.getChannel(),
                            name + (value == PhoneType.绿色抽奖公告.getValue() ? " :     " : " : ") + GameConstants.getLoginTip(vip, player.getName()),
                            false,
                            false,
                            GameConstants.getDisplay_item()).getBytes());
                }
                player.getClient().getSession().write(MaplePacketCreator.sendHint(String.format("欢迎来到%s\r\n#b@帮助 #k可查看游戏命令\r\n#b@npc#k 打开万能NPC\r\n#r祝大家游戏愉快天天开心...", channelServer.getServerName()), 300, 5));
            }
        } catch (RemoteException e) {
        ...
        c.getPlayer().getAttribute().setDataValue(ConstantTable._PLAYER_DATA_LOGINTIME, System.currentTimeMillis());
        c.getPlayer().getAttribute().getAttribute().put("p_lastLoginTime", GameConstants.getFormatter().format(new Date()));
        c.getSession().write(MaplePacketCreator.sendNecklace_Expansion(
                player.getNecklace_Expansion() != null
                || player.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -37) != null));//项链扩充
        player.showMapleTips();//服务器公告提示
        player.sendKeymap();//键盘表。
        player.showNote();//小纸条
        c.getSession().write(MaplePacketCreator.sendAutoHpPot(c.getPlayer().getAutoHpPot()));//自动吃药？
        ...
        if (player.getAndroid() != null) {
            c.getSession().write(MaplePacketCreator.spawnAndroid(player, player.getAndroid()));
            c.getSession().write(MaplePacketCreator.showAndroidEmotion(player.getId(), Randomizer.getInstance().nextInt(17) + 1));
        }
