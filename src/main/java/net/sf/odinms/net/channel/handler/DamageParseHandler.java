...
public abstract class DamageParseHandler extends AbstractMaplePacketHandler {
	...
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(DamageParseHandler.class);
	public boolean savePacket = false; // 是否记录攻击错误包

	public enum AttackMode {

		CLOSE, MAGIC, RANGE, Passive
	}
	...
	public class AttackInfo {
		...
		public int skill, stance, direction, charge, pos, aranCombo, csstar,
				slot, unk1;
		...
		public MapleStatEffect getAttackEffect(MapleCharacter chr,
			...
			if (skillLevel == 0) {// 特殊处理。。
				return null;
				// return
				// SkillFactory.getSkill(skill).getEffect(SkillFactory.getSkill(skill).getMaxLevel());
			}
	...
	// 应用攻击
	protected void applyAttack(AttackInfo attack, MapleCharacter player,
			int maxDamagePerMonster, int attackCount) { // 应用攻击
		...
		if (attack.skill != 0) { // 如果攻击的技能不为0
			...
			if (attackEffect != null && attackEffect.getSourceId() == 35001001
					&& player.getSkillLevel(35100008) > 0) {
				attackEffect = SkillFactory.getSkill(35101009).getEffect(
						player.getSkillLevel(35100008));
			}
			if (attackEffect == null) { // 如果没有攻击效果
				log.debug(player.getName() + "使用了没有的技能 - 技能ID: ("
						+ attack.skill + ")");
			}
			if (attack.skill != 牧师.群体治愈 && attack.skill != 战法.飓风
					&& attack.skill != 机械师.金属机甲_重机枪
					&& attack.skill != 机械师.金属机甲_导弹战车
					&& attack.skill != 机械师.金属机甲_重机枪_4转
					&& attack.skill != 机械师.火箭推进器 && attack.skill != 31121005 // 黑暗变形
			) {
				if (player.isAlive() && attackEffect != null
						&& attackEffect.getSourceId() != 61120007) {
		...
		if (!player.isAlive()) { // 如果玩家死了
			player.getCheatTracker().registerOffense(
					CheatingOffense.ATTACKING_WHILE_DEAD);
			return;
		}
		int totDamage = 0;
			...
			for (Pair<Integer, List<Integer>> oned : attack.allDamage) {
				...
				if (mapobject != null
					...
					if (mapitem.getMeso() > 0) {
						...
						player.getCheatTracker().registerOffense(
								CheatingOffense.ETC_EXPLOSION);
						return;
					}
				} else if (mapobject != null
						&& mapobject.getType() != MapleMapObjectType.MONSTER) {
					player.getCheatTracker().registerOffense(
							CheatingOffense.EXPLODING_NONEXISTANT);
		...
		for (Pair<Integer, List<Integer>> oned : attack.allDamage) {
			...
			if (monster != null) {
				...
				int totDamageToOneMonster = MapleDamageHandler
						.CalcAllDamage(oned.getRight());
				MapleDamageHandler.GainKuangLongNuQiHandler(oned.getRight()
						.size(), player);
				...
				if (player.hasBufferStat(MapleBuffStat.吸血鬼之触_伤害转化)) {
					player.addHP((int) Math.floor((totDamageToOneMonster * (player
							.getBuffedValue(MapleBuffStat.吸血鬼之触_伤害转化) / 100.0))));
				}
				totDamage += totDamageToOneMonster;
				player.checkMonsterAggro(monster);
				if (totDamageToOneMonster > attack.numDamage + 1) {
					int dmgCheck = player.getCheatTracker().checkDamage(
				...
				if (attackEffect != null) {
					checkHighDamage(player, monster, attack, theSkill,
							attackEffect, totDamageToOneMonster,
							maxDamagePerMonster);
				}
				double distance = player.getPosition().distanceSq(
						monster.getPosition());
				...
				if (attack.skill == 拳手.能量耗转 || attack.skill == 弩骑.利爪狂风
						|| attack.skill == 标飞.生命吸收 || attack.skill == 夜行者.吸血
					...
					int gainhp = (int) ((double) totDamage
							* (double) skill.getEffect(
									player.getSkillLevel(skillid)).getX() / 100.0);
					if (attack.skill == 弩骑.利爪狂风) // 利爪狂风
					{
						gainhp = Math.min(monster.getMaxHp(),
								Math.min(gainhp, player.getMaxhp() / 6)); // 15%
					} else if (player.getBuffedValue(MapleBuffStat.连环吸血) != null) {
						gainhp = Math.min(monster.getMaxHp(),
								Math.min(gainhp, player.getMaxhp() / 10)); // 10%
					} else {
						gainhp = Math.min(monster.getMaxHp(),
								Math.min(gainhp, player.getMaxhp() / 2)); // 50%
					}
					player.addHP(gainhp);
				} else if ((attack.skill == 4001334 || attack.skill == 4201005
						|| attack.skill == 0 || attack.skill == 4211002 || attack.skill == 4211004)
						&& player.getBuffedValue(MapleBuffStat.PICKPOCKET) != null) {
					handlePickPocket(player, monster, oned);
				} else if (attack.skill == 2301002 && !monster.getUndead()) { // 群体治愈
					player.getCheatTracker().registerOffense(
							CheatingOffense.HEAL_ATTACKING_UNDEAD);
					return;
				}
				...
				if (player.getJob().isA(MapleJob.WHITEKNIGHT)
						|| (player.isAran() && player
								.getBuffedValue(MapleBuffStat.WK_CHARGE) != null)) {
					// 寒冰冲击 冰雪矛
					...
					for (int charge : charges) {
						...
						MapleStatEffect chargeEffect = chargeSkill
						...
						if (player.isBuffFrom(MapleBuffStat.WK_CHARGE,
							...
							if (totDamageToOneMonster > 0) {
								...
								if (charge == 1211006) {
									// 获取怪物对冰属性的抗性
									ElementalEffectiveness iceEffectiveness = monster
											.getEffectiveness(Element.ICE);
									// 属性不是 抗冰 或者 免疫冰
									if (iceEffectiveness != ElementalEffectiveness.IMMUNE
											&& iceEffectiveness != ElementalEffectiveness.STRONG) {
										monsterStatusEffect = new MonsterStatusEffect(
												Collections.singletonMap(
														MapleMonsterStat.FREEZE,
														1), chargeSkill, false);
									} else {
										break;
									}
				...
				if (player.getBuffedValue(MapleBuffStat.重生) != null
						&& totDamage > monster.getHp()) { // 幻灵 重生
					ISkill chongsheng = SkillFactory.getSkill(32111006);
					if (chongsheng.getEffect(player.getSkillLevel(chongsheng))
							.makeChanceResult()) {
						...
						MapleSummon tosummon = new MapleSummon(player,
								32111006, player.getPosition(),
								SummonMovementType.UNKNOWN);
				...
				if (attack.skill == 3111008 && attack.allDamage.size() == 1) {// 枯竭箭
					...
					int damage = (int) (attack.allDamage.get(0).getRight()
							.get(0) * (attackEffect.getX() / 100.0));
				...
				if (player.getSkillLevel(venomNL) > 0) {
					MapleStatEffect venomEffect = venomNL.getEffect(player
							.getSkillLevel(venomNL));
		...
		if (totDamage > 1) {
			player.getCheatTracker().setAttacksWithoutHit(
			...
			if (player.getCheatTracker().getAttacksWithoutHit() > offenseLimit) {
				player.getCheatTracker().registerOffense(
						...
						Integer.toString(player.getCheatTracker()
	...
	// 偷盗技能
	private void handlePickPocket(MapleCharacter player, MapleMonster monster,
			Pair<Integer, List<Integer>> oned) { // 敛财术
		...
		for (Integer eachd : oned.getRight()) {
			if (pickpocket.getEffect(player.getSkillLevel(pickpocket))
					.makeChanceResult()) {
				...
				final Point tdpos = new Point((int) (monsterPosition.getX()
						+ (Math.random() * 100) - 50),
	...
	// 检查高伤害
	private void checkHighDamage(MapleCharacter player, MapleMonster monster,
			AttackInfo attack, ISkill theSkill, MapleStatEffect attackEffect,
			int damageToMonster, int maximumDamageToMonster) { // 检查高攻击伤害
		...
		if (player.getBuffedValue(MapleBuffStat.WK_CHARGE) != null) {
			...
			maximumDamageToMonster *= chargeSkill.getEffect(
					player.getSkillLevel(chargeSkill)).getDamage() / 100.0;
		}
		if (element != Element.PHYSICAL) {
			double elementalEffect;
			if (attack.skill == 3211003 || attack.skill == 3111003) { // inferno
																		// and
																		// blizzard
	...
	// 反击(抗压 飓风等)
	public AttackInfo parsePassiveEnergy(MapleCharacter chr,
			LittleEndianAccessor lea) { // 解析伤害数值
	 ...
	 * @param attack
	 */
	public void ParseMovement(MapleCharacter chr, LittleEndianAccessor lea,
			AttackInfo attack) {
	...
	public void ParseDamage(LittleEndianAccessor lea, AttackInfo attack,
			ISkill skill, MapleCharacter chr) {
		for (int i = 0; i < attack.numAttacked; i++) {
			...
			lea.skip(12);
			attack.allDamage.add(new Pair<Integer, List<Integer>>(Integer
					.valueOf(mobid), allDamageNumbers));
		}
	}
	...
	public AttackInfo parseClose(MapleCharacter chr, LittleEndianAccessor lea) { // 解析伤害数值
		...
		try {
			...
			if (sourceid == 战神.斗气爆裂 || sourceid == 战神.连环吸血
					|| sourceid == 战神.终极投掷 || sourceid == 战神.幻影狼牙
					|| sourceid == 战神.钻石星辰 || sourceid == 战神.战神之盾) {
				chr.setCombo(1);
			}
			...
			ret.speed = lea.readByte();// 速度
			/*
			 * if (sourceid == 4211006) { //金钱炸弹 return parseMesoExplosion(lea,
			 * ret, chr); }
			 */
	...
	public AttackInfo parseMesoExplosion(LittleEndianAccessor lea,
			AttackInfo ret, MapleCharacter chr) {
		if (ret.numAttackedAndDamage == 0) { // 没打怪
			...
			for (int j = 0; j < bullets; j++) {
				...
				lea.skip(2);
				ret.allDamage.add(new Pair<Integer, List<Integer>>(Integer
						.valueOf(mesoid), null));
			}
			return ret;
		} else {
	...
	public int maxDamage(MapleCharacter chr, AttackInfo ret, int damage) {
		boolean addmaxAttack = chr.getClient().getChannelServer()
				.isUseAddMaxAttack();
		ISkill skill = SkillFactory.getSkill(ret.skill);
		int Dm = 999999;
		/*
		 * if (skill != null) { Dm = skill.getEffect(1).getMDamageOver(); } else
		 * {
		 */
		// Dm = 999999;
		if (addmaxAttack) {
		...
		int ak = 0;
		int str = chr.getStr(), dex = chr.getDex(), luk = chr.getLuk(), _int = chr
				.getInt();
		for (IItem iItem : chr.getInventory(MapleInventoryType.EQUIPPED)
				.AllItems()) {
			if (iItem != null && iItem instanceof IEquip) {
				IEquip eqp1 = (IEquip) iItem;
				ak += (eqp1.getDex() + eqp1.getInt() + eqp1.getLuk() + eqp1
						.getStr()) * 1.5;
		...
		if (chr.getVip() <= 4
				&& (str < 32767 || dex < 32767 || luk < 32767 || _int < 32767)
				&& damage > 1000000000) {
		...
		if (ret.skill != 14101006) {
			...
			if (chr.getJobid() >= 0 && chr.getJobid() <= 132
					|| chr.getJobid() >= 1100 && chr.getJobid() <= 1111
					|| chr.getJobid() >= 2000 && chr.getJobid() <= 2112
					|| chr.getJobid() >= 3100 && chr.getJobid() <= 3112
					|| chr.getJobid() >= 5000 && chr.getJobid() <= 5112) {
				damage = (int) ((double) chr.getStr() * randomNum + (double) (chr
						.getDex() / 2)) * 25 + admge * (chr.getVip() + 1);
			}
			// 普通魔法师 骑士团魔法师 龙神 反抗者幻灵斗师
			if (chr.getJobid() >= 200 && chr.getJobid() <= 232
					|| chr.getJobid() >= 1200 && chr.getJobid() <= 1211
					|| chr.getJobid() >= 2001 && chr.getJobid() <= 2218
					|| chr.getJobid() >= 3200 && chr.getJobid() <= 3212) {
				damage = (int) ((double) chr.getLuk() * randomNum + (double) (chr
						.getDex() / 4)) * 25 + admge * (chr.getVip() + 1);
			}
			// 普通弓箭手 骑士团弓箭手 反抗者弩豹游侠 双弩精灵
			if (chr.getJobid() >= 300 && chr.getJobid() <= 322
					|| chr.getJobid() >= 1300 && chr.getJobid() <= 1311
					|| chr.getJobid() >= 3300 && chr.getJobid() <= 3312
					|| chr.getJobid() >= 2300 && chr.getJobid() <= 2312) {
				damage = (int) ((double) chr.getDex() * randomNum + (double) (chr
						.getStr() / 4)) * 25 + admge * (chr.getVip() + 1);
			}
			// 普通飞侠 骑士团飞侠 暗影双刀
			if (chr.getJobid() >= 400 && chr.getJobid() <= 422
					|| chr.getJobid() >= 1400 && chr.getJobid() <= 1412
					|| chr.getJobid() >= 430 && chr.getJobid() <= 434
					|| chr.getJobid() >= 2400 && chr.getJobid() <= 2412) {
				damage = (int) ((double) chr.getLuk() * randomNum + (double) (chr
						.getDex() / 4)) * 25 + admge * (chr.getVip() + 1);
			}
			// 普通海盗 反抗者机械师 火炮手 尖兵
			if (chr.getJobid() >= 500 && chr.getJobid() <= 522
					|| chr.getJobid() >= 1500 && chr.getJobid() <= 1511
					|| chr.getJobid() >= 3500 && chr.getJobid() <= 3512
					|| chr.getJobid() >= 501 && chr.getJobid() <= 532
					|| chr.getJobid() >= 3002 && chr.getJobid() <= 3612) {
				damage = (int) ((double) chr.getLuk() * randomNum + (double) (chr
						.getDex() / 4)) * 25 + admge * (chr.getVip() + 1);
			}
			...
			if (damage > 999999) {
				// chr.公告("攻击力:" + damage);
				// chr.getClient().getSession().write(MaplePacketCreator.serverNotice(5,
				// "破攻实际伤害:" + damage));
				chr.getClient()
						.getSession()
						.write(MaplePacketCreator.sendHint(
								String.format("#e破攻伤害:#r%d", damage), 148, 5));
			}
		}
