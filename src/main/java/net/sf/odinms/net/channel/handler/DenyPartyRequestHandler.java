...
public class DenyPartyRequestHandler extends AbstractMaplePacketHandler {
    ...
    private final static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(DenyPartyRequestHandler.class);
        ...
        if (mode == 0x1e) { // 拒绝邀请
            ...
            try {
                ...
                if (party != null && party.getLeader().isOnline()) {
                    ...
                    if (cfrom != null) {
                        cfrom.dropMessage(5, "“" + player.getName() + "”拒绝了组队招待！");
                        //cfrom.getClient().getSession().write(MaplePacketCreator.partyStatusMessage(23, c.getPlayer().getName()));
                    }
                } else {
                    c.getSession().write(MaplePacketCreator.serverNotice(5, "该组队已经不存在!"));
                }
            ...
            if (c.getPlayer().getParty() == null) {
                ...
                c.getSession().write(MaplePacketCreator.serverNotice(5, "您已经有一个组队，无法加入其他组队!"));
            }
        } else {
