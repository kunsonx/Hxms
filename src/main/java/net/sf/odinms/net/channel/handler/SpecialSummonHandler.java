...
public class SpecialSummonHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if (summon != null) {
			...
			final MapleStatEffect effect = skill.getEffect(c.getPlayer()
					.getSkillLevel(skill));
			if (skillId == 1320009) {
				/*
				 * D1 00 14 91 23 00 49 24 14 00 [08 00] 物防 8 D1 00 14 91 23 00
				 * 49 24 14 00 [0C 04] 攻击 1036 D1 00 14 91 23 00 49 24 14 00 [09
				 * 01] 魔防 265 D1 00 14 91 23 00 49 24 14 00 [0A 02] 回避 522 D1 00
				 * 14 91 23 00 49 24 14 00 [0B 03] 命中 779
				 * 
				 * 2022125 物理防御力上升 2022126 魔法防御力上升 2022127 命中率上升 2022128 回避率上升
				 * 2022129 攻击力上升
				 */
				...
				if (Type == 8 || Type == 136) { // 物防
					...
					buffid = -2022125;
					stat = new Pair<MapleBuffStat, Integer>(
							MapleBuffStat.EWdef, effect.getEWdef());
				} else if (Type == 265 || Type == 393) { // 魔防
					// System.out.println("魔防");
					buffid = -2022126;
					stat = new Pair<MapleBuffStat, Integer>(
							MapleBuffStat.EMdef, effect.getEMdef());
				} else if (Type == 522 || Type == 650) { // 回避
					// System.out.println("回避");
					buffid = -2022128;
					stat = new Pair<MapleBuffStat, Integer>(
							MapleBuffStat.AVOID, (int) effect.getAvoid());
				} else if (Type == 779 || Type == 907) { // 命中
					// System.out.println("命中");
					buffid = -2022127;
					stat = new Pair<MapleBuffStat, Integer>(MapleBuffStat.ACC,
							(int) effect.getAcc());
				} else if (Type == 1036 || Type == 1164) { // 攻击
					// System.out.println("攻击");
					buffid = -2022129;
					stat = new Pair<MapleBuffStat, Integer>(
							MapleBuffStat.EWatk, effect.getEWatk());
				} else {
				...
				for (int i = 0; i < 3; i++) {
					// SkillFactory.getSkill(35121011).getEffect(c.getPlayer().getSkillLevel(35121009)).applyTo(c.getPlayer(),
					// summon.getPosition());
					SkillFactory.getSkill(35121011)
							.getEffect(c.getPlayer().getSkillLevel(35121011))
							.applyTo(c.getPlayer(), summon.getPosition());
				}
			} else if (skillId == 35111011) { // 治疗机器人
				double multiplier = (double) SkillFactory.getSkill(skillId)
						.getEffect(c.getPlayer().getSkillLevel(skillId))
						.getHp() / 100;
				c.getPlayer().addHP(
						(int) (c.getPlayer().getMaxhp() * multiplier));
				c.getSession().write(
						MaplePacketCreator.showOwnBuffEffect(skillId, 2));
			}
		}
