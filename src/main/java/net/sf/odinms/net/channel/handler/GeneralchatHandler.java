...
public class GeneralchatHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (!CommandProcessor.getInstance().processCommand(c, text)) {
            ...
            if (c.getPlayer().getGMText() == 0) {
                if ((c.getPlayer().isHidden()) && (c.getPlayer().isGM())) { //If a GM is hidden, then it'll have cool blue text with a blue background!
                    c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.serverNotice(2, "[" + c.getPlayer().getId() + "][超级管理员]" + c.getPlayer().getName() + " : " + text));
                } else {
                    String formatstring = "";
                    switch (c.getPlayer().getVip()) {
                        /*  case 2:
                         c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.multiChat(c.getPlayer().getName(), text, 2));
                         break;*/
                       /* case 7:
                            c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.multiChat(c.getPlayer().getName(), text, 3));
                            break;*/
                        case 3:
                            formatstring = "[章魚墨汁]%s";
                            c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.multiChat(String.format(formatstring, c.getPlayer().getName()), text, 0));
                            break;
                        case 4://
                            formatstring = "[章魚脑袋]%s";
                            c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.multiChat(String.format(formatstring, c.getPlayer().getName()), text, 1));
                            break;
                        case 5:
                            formatstring = "[章魚心脏]%s";
                            c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.multiChat(String.format(formatstring, c.getPlayer().getName()), text, 0));
                        ...
                        case 6: {
                            formatstring = "[章魚大使]%s";
                            c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.multiChat(String.format(String.format(
                                    formatstring, c.getPlayer().getName()), c.getPlayer().getVip(), c.getPlayer().getName()), text, 4));
                    ...
                    c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.getChatText(c.getPlayer().getId(), text, c.getPlayer().isGM(), 1));
                }
            }
        } else {
            c.getPlayer().dropMessage("您的命令已执行.");
