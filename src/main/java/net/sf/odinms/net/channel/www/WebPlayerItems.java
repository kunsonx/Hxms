...
public class WebPlayerItems implements java.io.Serializable {
	...
	public static WebPlayerItems get(int chrId, int aId, WebUserPlayer player) {
		...
		try {
			...
			while (rs.next()) {
				item.storages.add(new WebPlayerItem(rs.getLong("items_id"), rs
						.getInt("itemid"), rs.getInt("position"), rs
						.getInt("space"), rs.getInt("quantity")));
			}
			rs.close();
			call.getMoreResults();
			rs = call.getResultSet();
			while (rs.next()) {
				item.items.add(new WebPlayerItem(rs.getLong("items_id"), rs
						.getInt("itemid"), rs.getInt("position"), rs
						.getInt("space"), rs.getInt("quantity")));
			}
			rs.close();
	...
	public int deleteFromDbId(long id) {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("delete from items where items_id = ?");
			ps.setLong(1, id);
	...
	public boolean checkOnlineStats(boolean isStorages) {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			// PreparedStatement ps = con.prepareStatement(isStorages ?
			// "SELECT loggedin <> 0 FROM accounts where id = ?" :
			// "SELECT loggedin <> 0 from characters WHERE id = ?");
			// ps.setInt(1, isStorages ? aId : cId);
			PreparedStatement ps = con
					.prepareStatement("SELECT loggedin <> 0 FROM accounts where id = ?");
			ps.setInt(1, aId);
