...
public class MultiChatHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (!CommandProcessor.getInstance().processCommand(c, chattext)) {
			...
			if (player.isMuted()) {
				player.dropMessage(5, player.isMuted() ? "You are "
						: "The map is "
			...
			try {
				if (type == 0) {
					c.getChannelServer()
							.getWorldInterface()
							.buddyChat(recipients, player.getId(),
									player.getName(), chattext);
				} else if (type == 1 && player.getParty() != null) {
					c.getChannelServer()
							.getWorldInterface()
							.partyChat(player.getParty().getId(), chattext,
									player.getName());
				} else if (type == 2 && player.getGuildid() > 0) {
					c.getChannelServer()
							.getWorldInterface()
							.guildChat(c.getPlayer().getGuildid(),
									c.getPlayer().getName(),
									c.getPlayer().getId(), chattext);
