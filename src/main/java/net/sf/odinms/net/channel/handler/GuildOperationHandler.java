...
public class GuildOperationHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		case 0x02: // 创建家族
			...
			try {
				guildId = c.getChannelServer().getWorldInterface()
						.createGuild(c.getPlayer().getId(), guildName);
			...
			if (guildId == 0) {
				c.getSession().write(
						MapleGuild_Msg.genericGuildMessage((byte) 0x1c));// 提示无法处理。发生未知错误无法处理有关家族的邀请
				return;
			}
		...
		case 0x05: // 家族邀请
			if (c.getPlayer().getGuildid() <= 0
					|| c.getPlayer().getGuildrank() > 2) { // 1 == guild master,
															// 2 == jr
				return;
			}
		...
		case 0x06: // 接受家族邀请
			...
			while (itr.hasNext()) {
				...
				if (guildId == inv.gid && name.equals(inv.name)) {
					...
					try {
						s = c.getChannelServer().getWorldInterface()
								.addGuildMember(c.getPlayer().getMGC());
					} catch (RemoteException e) {
						System.err
								.println("RemoteException occurred while attempting to add character to guild"
										+ e);
					...
					c.getSession().write(
							MapleGuild_Msg.showGuildInfo(c.getPlayer()));
					c.getPlayer().saveGuildStatus();
					respawnPlayer(c.getPlayer());
					break;
				}
		...
		case 0x07: // 离开家族
			...
			try {
				c.getChannelServer().getWorldInterface()
						.leaveGuild(c.getPlayer().getMGC());
			} catch (RemoteException re) {
				System.err
						.println("RemoteException occurred while attempting to leave guild"
								+ re);
				ServerExceptionHandler.HandlerRemoteException(re);
				c.getPlayer().dropMessage(5, "系统错误，请稍后再试。");
				return;
		...
		case 0x08: // 家族驱逐
			...
			if (c.getPlayer().getGuildrank() > 2
					|| c.getPlayer().getGuildid() <= 0) {
				return;
			}
			try {
				c.getChannelServer().getWorldInterface()
						.expelMember(c.getPlayer().getMGC(), name, cid);
			} catch (RemoteException re) {
				System.err
						.println("RemoteException occurred while attempting to change rank"
								+ re);
				ServerExceptionHandler.HandlerRemoteException(re);
				c.getPlayer().dropMessage(5, "系统错误，请稍后再试。");
				return;
		...
		case 0x0e: // 家族等级职称修改
			...
			try {
				c.getChannelServer().getWorldInterface()
						.changeRankTitle(c.getPlayer().getGuildid(), ranks);
			} catch (RemoteException re) {
		...
		case 0x0f: // 排名变化
			...
			try {
				c.getChannelServer().getWorldInterface()
						.changeRank(c.getPlayer().getGuildid(), cid, newRank);
			} catch (RemoteException re) {
				System.err
						.println("RemoteException occurred while attempting to change rank"
								+ re);
		...
		case 0x10: // 家族徽章修改
			...
			try {
				c.getChannelServer()
						.getWorldInterface()
						.setGuildEmblem(c.getPlayer().getGuildid(), bg,
								bgcolor, logo, logocolor);
			} catch (RemoteException re) {
				System.err.println("RemoteException occurred" + re);
				ServerExceptionHandler.HandlerRemoteException(re);
				c.getPlayer().dropMessage(5, "系统错误，请稍后再试。");
			...
			c.getPlayer()
					.getGuild()
					.guildMessage(
							MapleGuild_Msg.家族徽章变更(c.getPlayer().getGuildid(),
									bg, bgcolor, logo, logocolor));
			break;
		case 0x1D:
		...
		case 0x11: // 家族公告修改
			...
			try {
				c.getChannelServer().getWorldInterface()
						.setGuildNotice(c.getPlayer().getGuildid(), notice);
			} catch (RemoteException re) {
				System.err.println("RemoteException occurred" + re);
				ServerExceptionHandler.HandlerRemoteException(re);
				c.getPlayer().dropMessage(5, "系统错误，请稍后再试。");
		...
		case 0x1f:// 转移族长。
			...
			// c.getPlayer().getGuild().guildMessage(MapleGuild_Msg.ChangeLeader(c.getPlayer().getGuildId(),
			// c.getPlayer().getId(), toid));
			break;
		}
	}
}
