...
public class RangedAttackHandler extends DamageParseHandler {
	...
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(RangedAttackHandler.class);
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (type == MapleWeaponType.NOT_A_WEAPON) { // 如果是空手
			throw new RuntimeException("玩家 " + player.getName()
					+ " 没有装备武器，不能进行攻击!");
		}
		...
		if (attack.skill != 0) {
			...
			if ((attack.skill != 弩骑.吞噬_攻击 || attack.skill != 35101010 || attack.skill != 35101009)
					&& effect != null) {
		...
		useNoProjectile = type.equals(MapleWeaponType.CANNON)
				|| type.equals(MapleWeaponType.能量剑) || useNoProjectile
				|| type.equals(MapleWeaponType.双弩枪) || attack.slot == 0;

		boolean hasShadowPartner = player
		...
		if (!useNoProjectile) {
			...
			if (effect != null && effect.getBulletConsume() != 0) {
				bulletConsume = effect.getBulletConsume()
						* (hasShadowPartner ? 2 : 1);
			}
		...
		if (projectile != 0 || useNoProjectile) {
			int visProjectile;
			if (attack.csstar > 0
					&& player.getInventory(MapleInventoryType.CASH).getItem(
							(short) attack.csstar) != null) {
				visProjectile = player.getInventory(MapleInventoryType.CASH)
						.getItem((short) attack.csstar).getItemId();
			} else {
			...
			try {
				 ...
				 * player.getSkillLevel(attack.skill), attack.charge); break; }
				 */
				packet = MaplePacketCreator.rangedAttack(attack, player,
						visProjectile);
			...
			if (attack.skill != 4001344 && attack.skill != 14001004) { // not
																		// lucky
																		// 7
				if (projectileWatk != 0) {
					basedamage = c.getPlayer().calculateMaxBaseDamage(
							c.getPlayer().getTotalWatk() + projectileWatk);
				} else {
					basedamage = c.getPlayer().getCurrentMaxBaseDamage();
				}
			} else { // l7 has a different formula :>
				basedamage = (int) (((c.getPlayer().getTotalLuk() * 5.0) / 100.0) * (c
						.getPlayer().getTotalWatk() + projectileWatk));
			}
			...
			if (player.getJob().isA(MapleJob.ASSASSIN)) {
				...
				if (critlevel > 0) {
					critdamagerate = (criticalthrow.getEffect(
							player.getSkillLevel(criticalthrow)).getDamage() / 100.0);
				}
				...
				if (critlevel > 0) {
					critdamagerate = (criticalshot.getEffect(critlevel)
							.getDamage() / 100.0) - 1.0;
				}
			}
			int critdamage = (int) (basedamage * critdamagerate);
			if (effect != null) {
				maxdamage *= attack.skill == 14101006 ? effect.getDamage()
						: effect.getDamage() / 100.0;
			}
			...
			if (hasShadowPartner) {
				...
				MapleStatEffect shadowPartnerEffect = shadowPartner
						.getEffect(shadowPartnerLevel);
				if (attack.skill != 0) {
			...
			if (attack.skill != 0) {
				...
				if (effect_.getCooldown() > 0) {
					if (player.skillisCooling(attack.skill)) {
						...
						player.addCooldown(attack.skill,
