...
public class MoveLifeHandler extends MovementParse {
    ...
    private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(MoveLifeHandler.class);
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (skillByte && monster.getNoSkills() > 0) {
            ...
            toUse = MobSkillFactory.getMobSkill(skillToUse.getSkill(), skillToUse.getLevel());
            int percHpLeft = (monster.getHp() / monster.getMaxHp()) * 100;
            if (toUse != null
                    && (toUse.getHP() < percHpLeft || !monster.canUseSkill(toUse))) {
                toUse = null;
            }
        }
        if (//skill_id >= 100 && skill_id <= 200 &&
                monster.hasSkill(skill_id, skill_level)) {
            MobSkill skillData = MobSkillFactory.getMobSkill(skill_id, skill_level);
   ...
   //     slea.readInt();
        ...
        if (res != null) {
            if (monster.isAlive()) {
                MaplePacket packet = MaplePacketCreator.moveMonster(skillByte, skill, skill_id, skill_level, skill_3, skill_4, objectid, startPos, nextPos, res);
