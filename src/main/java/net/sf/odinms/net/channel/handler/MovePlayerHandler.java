...
public class MovePlayerHandler extends MovementParse {
    ...
    private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(MovePlayerHandler.class);
    ...
    public static void MovePlayerHandler(List<LifeMovementFragment> res, MapleCharacter player, boolean fromdamage) {
        try {
            ...
            if (!player.isHidden()) { //如果玩家不是处于GM隐身状态
                ...
                for (MapleCharacter toplayer : player.getMap().getCharacters()) {
                    if ((toplayer.isHidden() || (toplayer.getGm() > 0)) && toplayer != player) {
