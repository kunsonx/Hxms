	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (mode == 4) { // 取出
			...
			if (item != null) {
				if (MapleInventoryManipulator.checkSpace(c, item.getItemId(),
					...
					MapleInventoryManipulator.addFromDrop(c, item,
							"Taken out from storage by "
									+ c.getPlayer().getName(), false);
				} else {
					storage.store(item);
					c.getSession().write(
							MaplePacketCreator.serverNotice(1, "背包已满"));
				}
			...
			if (c.getPlayer().getMeso() < 100) {
				c.getSession().write(
						MaplePacketCreator.serverNotice(1, "你没有足够的金币"));
