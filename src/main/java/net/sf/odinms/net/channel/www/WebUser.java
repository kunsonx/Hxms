	...
	public String Login(Map session) {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("SELECT lastlogin,createdat,salt,`password`,banned,gm,id,loggedin FROM accounts WHERE `name` = ?");
			ps.setString(1, name);
			...
			if (rs.next()) {
				...
				if (LoginCryptoLegacy.isLegacyPassword(passhash)
						&& LoginCryptoLegacy.checkPassword(pass, passhash)
						|| salt == null
						&& LoginCrypto.checkSha1Hash(passhash, pass)
						|| LoginCrypto.checkSaltedSha512Hash(passhash, pass,
								salt)) {
	...
	public List<WebUserPlayer> getPlayers() {
		if (players == null) {
			...
			try {
				Connection con = DatabaseConnection.getConnection();
				PreparedStatement ps = con
						.prepareStatement("SELECT id,`name`, vip, str, dex, luk, `int`, `level`, meso, gender, fame,createdate,reborns FROM characters WHERE accountid = ?");
				ps.setInt(1, id);
				ResultSet rs = ps.executeQuery();
				while (rs.next()) {
