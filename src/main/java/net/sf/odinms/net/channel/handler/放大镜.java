...
public final class 放大镜 extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        IEquip toUse = (IEquip) c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem(dst);
        if (dst >= 0) {
            toUse = (IEquip) c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem(dst);
        }
        if (toUse == null) {
        ...
        c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.showMagniflerEffect(c.getPlayer().getId(), scrolled.getPosition()));
        if (dst < 0) {
