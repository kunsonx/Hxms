...
public class ChannelServer extends GeneralServer implements Runnable,
		ChannelServerMBean {
	...
	public void run() {
		...
		SimpleIoProcessorPool<NioSession> simpleIoProcessorPool = new SimpleIoProcessorPool(
				NioProcessor.class, ExecutorService);
		...
		tMan.start(Runtime.getRuntime().availableProcessors() * 2
				+ eventSM.getEventCount() / 10);
		...
		try {
			...
			log.info(String.format("世界 %d 频道服务 %d 已上线,监听于:%s.", getDescriptor()
					.getWorld(), getDescriptor().getId(), getConfig().getIp()));
			log.info("频道 " + getChannel() + " 正常开启.监听于:" + getConfig().getIp()
					+ ".");
			wci.serverReady();
			eventSM.init();
		} catch (IOException e) {
			log.error("端口 " + getConfig().getPort() + " 已被使用 (频道: "
					+ getChannel() + ")", e);
		}
		CheckConfig();
	}
	...
	public void shutdown() { // dc all clients by hand so we get
								// sessionClosed...
		try {
			eventSM.cancel();
		} catch (Exception e) {
		...
		MapleCharacter chrs[] = allchars.toArray(new MapleCharacter[allchars
				.size()]);
	...
	public static ChannelServer newInstance(String id)
		...
		mBeanServer.registerMBean(instance, new ObjectName(
				"net.sf.odinms.net.channel:type=ChannelServer,name=ChannelServer"
						+ uniqueID++));
		return instance;
	}
	...
	public static ChannelServer getInstance(ChannelDescriptor channel) {
		...
		return channel == null ? null : ChannelManager
	...
	public void addPlayer(MapleCharacter chr) {
		...
		chr.getClient()
				.getSession()
				.write(MaplePacketCreator.serverMessage(getConfig()
						.getServerMessage()));
	}
	...
	public ChannelServer getChannelServer(int channel) {
		return ChannelManager
				.getChannelServerFromThisWorld(descriptor, channel);
	}
	...
	public void shutdown(int time) {
		broadcastPacket(MaplePacketCreator.serverNotice(0, "服务器将会在 "
				+ (time / 60000) + " 分钟后关闭维护,请各位玩家提前安全下线."));
		TimerManager.getInstance().schedule(
				new ShutdownServer(getDescriptor()), time);
	}
	...
	public MapleGuildSummary getGuildSummary(int gid) {
		if (gsStore.containsKey(gid)) {
			...
			try {
				...
				return gsStore.get(gid); // if g is null, we will end up
											// returning null
			} catch (RemoteException re) {
	...
	public static void main(String args[]) throws FileNotFoundException,
			...
			MalformedObjectNameException, SQLException, InterruptedException {
		if (CheckFilePermit()) {
			log.error("机器码验证失败......");
			System.exit(0);
		}
		...
		ExecutorService = new ThreadPoolExecutor(0,
				getMaxiNumPoolSize(GeneralServerType.CHANNEL), 2,
				...
				new ThreadFactory() {
					...
					public Thread newThread(Runnable r) {
						Thread thread = new Thread(null, r, "ChannelServer["
		...
		Registry registry = LocateRegistry.getRegistry(config.getWorldHost(),
	...
	public List<MapleCharacter> getPartyMembers(MapleParty party) {
		...
		for (net.sf.odinms.net.world.MaplePartyCharacter partychar : party
				.getMembers()) {
			if (partychar.getChannel() == getChannel()) { // Make sure the thing
															// doesn't get
															// duplicate plays
															// due to ccing bug.
				MapleCharacter chr = getPlayerStorage().getCharacterByName(
						partychar.getName());
				if (chr != null) {
					partym.add(chr);
				}
	...
	public MapleCharacter getCharacterFromAllServers(String name) {
		for (ChannelServer cserv_ : getChannelServers()) {
			MapleCharacter ret = cserv_.getPlayerStorage().getCharacterByName(
					name);
			if (ret != null) {
				return ret;
			}
	...
	public String toString() {
		return String.format("世界：【%d】 频道：【%d】端口：【%d】IP：【%s】", descriptor
				.getWorld(), descriptor.getId(), getConfig().getPort(),
				getConfig().getIp());
	}
