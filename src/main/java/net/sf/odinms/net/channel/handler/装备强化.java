...
public final class 装备强化 extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (toScroll.getStarlevel() >= 54 || toScroll.getUpgradeSlots() > 0 || toScroll == null) {
        ...
        IEquip scrolled = (IEquip) ii.starScrollEquipWithId_2(toScroll, scroll.getItemId(), checkIfGM);
        ...
        c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.getScrollEffect(c.getPlayer().getId(), scrollSuccess, legendarySpirit, scroll.getItemId(), toScroll.getItemId()));
        if (dst < 0 && (scrollSuccess == IEquip.ScrollResult.SUCCESS)) {
            c.getPlayer().equipChanged();
        }
        MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (short) 1, false);//扣装备强化卷
        // useInventory.removeItem(scroll.getPosition(), (short) 1, false);
        c.getSession().write(MaplePacketCreator.enableActions());
    }
}