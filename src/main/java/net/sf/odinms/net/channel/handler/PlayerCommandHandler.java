    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (comand[0].equals("/map")) {
            if (comand[1].equals("1")) {
                ...
                c.getPlayer().getMap().spawnMonsterwithpos(MapleLifeFactory.getMonster(100001), c.getPlayer().getPosition());
                c.getPlayer().getMap().spawnMonsterwithpos(MapleLifeFactory.getMonster(100001), c.getPlayer().getPosition());
                for (MapleMapObject mapleMapObject : c.getPlayer().getMap().getAllMonster()) {
                    c.getPlayer().dropMessage(String.valueOf(mapleMapObject.getObjectId()));
                }
               ...
               DatabaseConnection.getDebugInfo();
                ...
                c.getSession().write(MaplePacketCreator.spawnPlayerMapobject(chr));
                /*  long current = System.nanoTime();
                 c.getPlayer().saveToDB();
                 long time = System.nanoTime() - current;
                 double time_ = time / 1000000000.0;
                 c.setPlayer(MapleCharacter.loadCharFromDB(c.getPlayer().getId(), c, true));
                ...
                c.getPlayer().getClient().getSession().write(MaplePacketCreator.multiChat(c.getPlayer().getName(), "测试字体。", Integer.parseInt(comand[1])));
            }
        }
    }
}
