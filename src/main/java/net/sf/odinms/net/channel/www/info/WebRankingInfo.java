	...
	public void load() {
		try {
			...
			PreparedStatement ps = c
					.prepareStatement("SELECT `name`,reborns from characters ORDER BY reborns desc LIMIT 10");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				reborns.add(new WebRanking(rs.getRow(), rs.getString(1), rs
						.getInt(2)));
			}
			rs.close();
			ps.close();
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				fames.add(new WebRanking(rs.getRow(), rs.getString(1), rs
						.getInt(2)));
			}
			rs.close();
			ps.close();
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				guilds.add(new WebRanking(rs.getRow(), rs.getString(1), rs
						.getInt(2)));
			}
			rs.close();
			ps.close();
