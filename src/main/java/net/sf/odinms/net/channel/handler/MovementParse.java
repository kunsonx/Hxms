...
public abstract class MovementParse extends AbstractMaplePacketHandler {
    ...
    private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(MovementParse.class);

    public static List<LifeMovementFragment> parseMovement(final LittleEndianAccessor lea) {
        ...
        for (int i = 0; i < numCommands; i++) {
            ...
            switch (command) {
                ...
                { // Jump Down
                    ...
                    res.add(jdm);
                    break;
                }/* { //?... 00 00 7A 03 0F 02 64 01 00 00 0F 00 04 5A 00
                ...
                case 38:
                case 39: {
                    res.add(new RefreshLifeMovement(command, lea.readByte(), lea.readShort()));
                    break;
                }
                case -111: // rush ?
                {
                    final byte unk = lea.readByte();
                    UnknownMovementAction um = new UnknownMovementAction(new UnknownMovementSerialize() {
                        @Override
                ...
                default:
                    ...
                    re.WriteFile(GameConstants.getFormatter().format(new Date()) + " Remaining : " + (numCommands - res.size()) + " New type of movement ID : " + command + ", packet : " + lea.toString());
                    re.CloseFile();
                    return res;
            }
        }
