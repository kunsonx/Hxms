...
public class UserServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            response.setContentType("text/html;charset=UTF-8");
            WebUser user = (WebUser) request.getSession().getAttribute(WebUser.SESSION_KEY);
            if (user != null) {
    ...
    public void handlerAction(String op, String name, WebUser user, HttpServletResponse response) throws IOException {
        if (op != null && op.equals("playerlist")) {
            ...
            if (id == -1) {
                ...
                MapleCharacter chr = ChannelServer.getCharacterFromAllServersAndWorld(id);
                if (chr != null) {
                    chr.ClearAllSkills();
                } else {
                    deleteSkillInfo(id);
            ...
            for (WebUserPlayer up : user.getPlayers()) {
                MapleCharacter chr = ChannelServer.getCharacterFromAllServersAndWorld(up.getId());
                if (chr != null) {
                    chr.Dci();
                }
            ...
            if (id == -1) {
                ...
                if (chr != null) {
                    if (chr.getEventInstance() != null) {
                        ...
                        chr.changeMap(chr.getClient().getChannelServer().getMapFactory().getMap(910000018));
                    }
                } else {
                    updateMap(id);
                }
    ...
    public void deleteSkillInfo(int chrId) {
        try {
            ...
            java.sql.PreparedStatement ps = con.prepareStatement("SELECT skillid FROM skills WHERE characterid = ?");
            ps.setInt(1, chrId);
    ...
    public void updateofflineStats(int aid, int chrId) {
        try {
            ...
            java.sql.PreparedStatement ps = con.prepareStatement("update accounts set loggedin = 0 where id = ?");
            ps.setInt(1, aid);
            ps.executeUpdate();
            ps.close();
    ...
    public void updateofflineStats(int chrId) {
        try {
            Connection con = DatabaseConnection.getConnection();
            java.sql.PreparedStatement ps = con.prepareStatement("SELECT accountid FROM characters WHERE id = ?");
            ps.setInt(1, chrId);
            ResultSet rs = ps.executeQuery();
            rs.next();
            int aid = rs.getInt(1);
    ...
    public void updateFaceAndHair(int chrId) {
        try {
            Connection con = DatabaseConnection.getConnection();
            java.sql.PreparedStatement ps = con.prepareStatement("update characters set face = (case WHEN gender = 0 then 20000 else 21000 end), hair = (case WHEN gender = 0 then 30000 else 37290 end) WHERE id = ?");
            ps.setInt(1, chrId);
            ps.executeUpdate();
            ps.close();
            con.close();
    ...
    public void updateMap(int chrId) {
        try {
            Connection con = DatabaseConnection.getConnection();
            java.sql.PreparedStatement ps = con.prepareStatement("update characters set map = 910000020 WHERE id = ?");
            ps.setInt(1, chrId);
            ps.executeUpdate();
            ps.close();
            con.close();
    ...
    public void updateJob(int chrId) {
        try {
            Connection con = DatabaseConnection.getConnection();
            java.sql.PreparedStatement ps = con.prepareStatement("update characters set job = 0 WHERE id = ?");
            ps.setInt(1, chrId);
            ps.executeUpdate();
            ps.close();
            con.close();
    ...
    private void writeData(HttpServletResponse response, String ret) throws IOException {
