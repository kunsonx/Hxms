...
public class BBSOperationHandler extends AbstractMaplePacketHandler {
	...
	private static final org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(BBSOperationHandler.class);
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		case 0:
			...
			if (icon >= 0x64 && icon <= 0x6a) {
				if (!c.getPlayer().haveItem(5290000 + icon - 0x64, 1, false,
						false)) {
	...
	public static void newBBSReply(MapleClient client, int localthreadid,
			String text) {
	...
	public static void editBBSThread(MapleClient client, String title,
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("UPDATE bbs_threads SET `name` = ?, `timestamp` = ?, "
							+ "`icon` = ?, "
	...
	public static void newBBSThread(MapleClient client, String title,
		...
		try {
			...
			if (!bNotice) // notice's local id is always 0, so we don't need to
							// fetch it
			{
	...
	public static void displayThread(MapleClient client, int threadid,
		...
		try {
			...
			client.getSession().write(
					MaplePacketCreator.showThread(bIsThreadIdLocal ? threadid
							: threadRS.getInt("localthreadid"), threadRS,
							repliesRS));
			repliesRS.close();
			threadRS.close();
			ps.close();
			...
			if (re.getMessage() != null) {
				...
				try {
					PreparedStatement ps = con
							.prepareStatement("DELETE FROM bbs_threads WHERE threadid = ?");
