	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		case 1: { // 创建
			if (c.getPlayer().getParty() == null) {
				...
				c.getSession().write(
						MaplePacketCreator.serverNotice(1, "你已经存在一个队伍中，无法创建！"));
			}
			break;
		}
		case 2: { // 离开组队
			if (party != null) {
				try {
					if (partyplayer.equals(party.getLeader())) { // disband
						for (MapleMapObject mapleMapObject : player.getMap()
								.getAllMonster()) {
		...
		case 4: { // 邀请
			...
			MapleCharacter invited = c.getChannelServer().getPlayerStorage()
			...
			if (invited != null) {
						...
						if (party != null && party.getMembers().size() < 6) {
							...
							invited.getClient()
									...
									.write(MaplePacketCreator
											.partyInvite(player));
							c.getSession().write(
									MaplePacketCreator.serverNotice(1, "向“"
											+ name + "”发送了组队邀请！"));
						} else {
							c.getSession().write(
									MaplePacketCreator.serverNotice(1,
											"您的组队成员已达到6人，无法再邀请其他人员！"));
						}
					} else {
						c.getSession().write(
								MaplePacketCreator.serverNotice(5, "“" + name
										+ "”正在做其他的事情！"));
					}
				} else {
					c.getSession().write(
				...
				// c.getSession().write(MaplePacketCreator.partyStatusMessage(13));//显示未找到玩家的消息
				// 13
		...
		case 5: { // 驱逐成员
			...
			if (partyplayer.equals(party.getLeader())) {
				...
				if (expelled != null && !expelled.equals(party.getLeader())) {
					try {
						wci.updateParty(party.getId(), PartyOperation.EXPEL,
								expelled);
						if (player.getEventInstance() != null) {
							/*
							...
							if (expelled.isOnline()) {
								MapleCharacter expellee = c
										.getChannelServer(expelled.getChannel())
										.getPlayerStorage()
				...
				c.getSession().write(
						MaplePacketCreator.serverNotice(1, "不是队长，无法进行该操作！"));
			}
			break;
		}
		case 6: { // 更改队长
			...
			if (partyplayer.equals(party.getLeader()) && newleader.isOnline()) {
				try {
					...
					wci.updateParty(party.getId(),
							PartyOperation.CHANGE_LEADER, newleader);
