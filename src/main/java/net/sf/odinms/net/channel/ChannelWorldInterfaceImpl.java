 ...
 * @author Matze
 */
public class ChannelWorldInterfaceImpl extends UnicastRemoteObject implements
	...
	public int whisper(String sender, String target, int channel,
		...
		if (server.getPlayerStorage().hasCharacter(target)
				&& (!gm || server.getPlayerStorage().getCharacterByName(target)
						.isGM())) {
			server.getPlayerStorage()
					.getCharacterByName(target)
					.getClient()
					.getSession()
					.write(MaplePacketCreator.getWhisper(sender, channel,
							message));
			return 1;
		}
		return 0;
	...
	public void shutdown(int time) throws RemoteException {
		if (time / 60000 != 0) {
			server.broadcastPacket(MaplePacketCreator.serverNotice(0, "服务器将在 "
					+ (time / 60000) + " 分钟后关闭维护.为避免数据丢失,请各玩家及时下线！"));
		}
		TimerManager.getInstance().schedule(
				new ShutdownServer(server.getDescriptor()), time);
	}
		...
		for (int buddy : buddies) {
			...
			if (chr != null) {
				...
				if (ble != null && ble.isVisible()) {
					...
					chr.getClient()
							.getSession()
							.write(MaplePacketCreator.updateBuddyChannel(
	...
	public void updateParty(MapleParty party, PartyOperation operation,
		...
		for (MaplePartyCharacter partychar : party.getMembers()) {
			if (partychar.getChannel() == server.getChannel()) {
				MapleCharacter chr = server.getPlayerStorage()
				...
				if (chr != null) {
					...
					chr.getClient()
							.getSession()
							.write(MaplePacketCreator.updateParty(chr
									.getClient().getChannel(), party,
									operation, target));
				}
			}
		}
		...
		for (MaplePartyCharacter partychar : party.getMembers()) {
			if (partychar.getChannel() == server.getChannel()
				...
				if (chr != null) {
					chr.getClient()
							.getSession()
							.write(MaplePacketCreator.multiChat(namefrom,
									chattext, 1));
				}
			}
		}
	...
	public int getLocation(String name) throws RemoteException {
		...
		if (chr != null) {
			return server.getPlayerStorage().getCharacterByName(name)
					.getMapId();
		}
		return -1;
	}
	...
	public List<CheaterData> getCheaters() throws RemoteException {
		...
		List<MapleCharacter> allplayers = new ArrayList<MapleCharacter>(server
		...
		for (int x = allplayers.size() - 1; x >= 0; x--) {
			...
			if (cheater.getCheatTracker().getPoints() > 0) {
				cheaters.add(new CheaterData(cheater.getCheatTracker()
						.getPoints(), MapleCharacterUtil
						.makeMapleReadable("ID:" + cheater.getId() + " Name: "
								+ cheater.getName())
						+ " ("
	...
	public BuddyAddResult requestBuddyAdd(String addName, int Fromid,
		...
		MapleCharacter addChar = server.getPlayerStorage().getCharacterByName(
				addName);
		if (addChar != null) {
			...
			if (!buddylist.contains(Fromid)) {
				// addChar是被添加的人
				buddylist.addBuddyRequest(addChar.getClient(), Fromid,
						Fromname, FromChannel, FromLevel, FromJobid);
			} else {
	...
	public void buddyChanged(int cid, int cidFrom, String name, int channel,
		...
		MapleCharacter addChar = server.getPlayerStorage()
		...
		if (addChar != null) {
			...
			case ADDED:
				if (buddylist.contains(cidFrom)) {
					buddylist.put(new BuddylistEntry(name, cidFrom, channel,
							true));
					addChar.getClient()
							.getSession()
							.write(MaplePacketCreator.updateBuddyChannel(
			...
			case DELETED:
				if (buddylist.contains(cidFrom)) {
					buddylist.put(new BuddylistEntry(name, cidFrom, -1,
	...
	public void reloadGuildCharacters() throws RemoteException {
		for (MapleCharacter mc : server.getPlayerStorage().getAllCharacters()) {
			if (mc.getGuildid() > 0) {
				...
				server.getWorldInterface().setGuildMemberOnline(mc.getMGC(),
						true, server.getChannel());
	...
	public void changeEmblem(int gid, List<Integer> affectedPlayers,
		...
		this.sendPacket(affectedPlayers, MapleGuild_Msg.家族徽章变更(gid,
				mgs.getLogoBG(), mgs.getLogoBGColor(), mgs.getLogo(),
				mgs.getLogoColor()), -1);
	...
	public void messengerInvite(String sender, int messengerid, String target,
		...
		if (isConnected(target)) {
			MapleMessenger messenger = server.getPlayerStorage()
			...
			if (messenger == null) {
				server.getPlayerStorage()
						...
						.getClient()
						.getSession()
						.write(MaplePacketCreator.messengerInvite(sender,
								messengerid));
				MapleCharacter from = server.getChannelServer(fromchannel)
						.getPlayerStorage().getCharacterByName(sender);
				from.getClient().getSession()
				...
				from.getClient()
						.getSession()
						.write(MaplePacketCreator.messengerChat(sender + " : "
								+ target + " 已经使用小纸条", "[系统]"));
			}
		}
	}
				...
				if (chr != null) {
					chr.getClient()
							...
							.write(MaplePacketCreator
									.joinMessenger(messengerchar.getPosition()));
				}
			}
		}
	}
	...
	public void messengerChat(MapleMessenger messenger, String chattext,
			String namefrom, String user) throws RemoteException {
		for (MapleMessengerCharacter messengerchar : messenger.getMembers()) {
			if (messengerchar.getChannel() == server.getChannel()
					&& !(messengerchar.getName().equals(namefrom))) {
				MapleCharacter chr = server.getPlayerStorage()
				...
				if (chr != null) {
					chr.getClient()
							.getSession()
							.write(MaplePacketCreator.messengerChat(user,
									chattext));
				}
			}
		}
	...
	@Override
	public void spouseChat(String from, String target, String message)
			throws RemoteException {
		if (isConnected(target)) {
			server.getPlayerStorage().getCharacterByName(target).getClient()
					.getSession()
					.write(MaplePacketCreator.spouseChat(from, message, 5));
		}
	}
	...
	public long checkClientIvKey(byte[] ivCheck) throws RemoteException {
		for (Map.Entry<Long, IoSession> entry : server.getAcceptor()
				.getManagedSessions().entrySet()) {
			MapleClient client = (MapleClient) entry.getValue().getAttribute(
	...
	public void disconnectClient(long clientId) throws RemoteException {
		IoSession session = server.getAcceptor().getManagedSessions()
				.get(clientId);
		if (session != null) {
			session.close();
		}
