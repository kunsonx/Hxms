...
public class CashShopGiftHandler extends AbstractMaplePacketHandler {
    ...
    private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(CashShopGiftHandler.class);
        ...
        if (c.getPlayer().getCSPoints(type) >= item.getPrice()) {
            ...
            if (MapleCharacter.getAccountIdByName(recipient) != -1) {
                if (MapleCharacter.getAccountIdByName(recipient) == c.getPlayer().getAccountid()) {
                    ...
                    CashShopHandler.GiftItem(gift, recipient, c.getPlayer().getName());
