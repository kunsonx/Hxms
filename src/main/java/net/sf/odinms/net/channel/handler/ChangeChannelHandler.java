...
public class ChangeChannelHandler extends AbstractMaplePacketHandler {
	...
	private static final org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(ChangeChannelHandler.class);
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		 ...
		 * (c.getPlayer().getSkillLevel(1320009) > 0) { for (int i = 0; i < 5;
		 * i++) { c.getSession().write(MaplePacketCreator.cancelBuff(null,
		 * 2022125 + i)); } } }
		 */
		...
		try {
			c.getChannelServer()
					.getWorldInterface()
					.addBuffsToStorage(c.getPlayer().getId(),
