...
public class QuestActionHandler extends AbstractMaplePacketHandler {
	...
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(QuestActionHandler.class);
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (action == 1) { // 接受任务
			...
			try {
				if (slea.available() >= 4L) {
					...
					c.getPlayer().getClient().getSession()
							.write(MaplePacketCreator.showOwnBuffEffect(0, 10));
					c.getPlayer()
							.getMap()
							.broadcastMessage(
									c.getPlayer(),
									MaplePacketCreator.showBuffeffect(c
											.getPlayer().getId(), 0, 10,
											(byte) 3, 1), false);
				}
			} catch (Exception e) {
			...
			c.getPlayer()
					...
					.broadcastMessage(
							c.getPlayer(),
							MaplePacketCreator.showBuffeffect(c.getPlayer()
									.getId(), 0, 10, (byte) 3, 1), false);
		}
	}
}