...
public class DistributeAPHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		// 8E 00 [71 38 7B 00] [02 00 00 00] [00 02 00 00 00 00 00 00] [0A 00 00
		...
		if (c.getPlayer().getRemainingAp() > 0) {
			...
			case 2048: // HP ok 92..8192
				...
				if (c.getPlayer().getJob().isA(MapleJob.BEGINNER)) {
					...
					improvingMaxHPLevel = c.getPlayer().getSkillLevel(
							improvingMaxHP);
					if (improvingMaxHPLevel >= 1) {
						MaxHP += rand(20, 24)
								+ improvingMaxHP.getEffect(improvingMaxHPLevel)
										.getY();
					} else {
						MaxHP += rand(20, 24);
					}
				...
				int HPSkillAdd = 0;
				c.getPlayer().setMaxhp(
						c.getPlayer().getMaxhp() + 1 + HPSkillAdd);
				statupdate.add(new Pair<MapleStat, Number>(MapleStat.MAXHP, c
						.getPlayer().getMaxhp()));
				break;
			case 8192: // MP ok 92..32768
				int MaxMP = c.getPlayer().getMaxmp();
				if (c.getPlayer().getMpApUsed() == 10000
				...
				if (c.getPlayer().getJob().isA(MapleJob.BEGINNER)) {
					...
					int improvingMaxMPLevel = c.getPlayer().getSkillLevel(
							improvingMaxMP);
					if (improvingMaxMPLevel >= 1) {
						MaxMP += rand(18, 20)
								+ improvingMaxMP.getEffect(improvingMaxMPLevel)
										.getY();
					} else {
						MaxMP += rand(18, 20);
					}
				...
				int MPSkillAdd = 0;
				c.getPlayer().setMaxmp(
						c.getPlayer().getMaxmp() + 1 + MPSkillAdd);
				statupdate.add(new Pair<MapleStat, Number>(MapleStat.MAXMP, c
						.getPlayer().getMaxmp()));
				break;
			default:
				c.getSession().write(
