...
public class ItemPickupHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        synchronized (ob) {
            if (ob instanceof MapleMapItem) {
                ...
                if (log.isDebugEnabled() && c.getPlayer().isGM() && mapitem.getItem() != null) {
                    c.getPlayer().dropMessage("捡取物品：" + mapitem.getItem().getItemId());
                }
                ...
                if (mapitem.getMeso() > 0) {
                    ...
                    if (GameConstants.isPet(mapitem.getItem().getItemId())) {
                        MapleInventoryManipulator.addFromDrop(c, MaplePet.createPet(mapitem.getItem().getItemId()));
                        c.getPlayer().getMap().broadcastMessage(
                                MaplePacketCreator.removeItemFromMap(mapitem.getObjectId(), 2, c.getPlayer().getId()),
                                mapitem.getPosition());
                        c.getPlayer().getCheatTracker().pickupComplete();
    ...
    static boolean useItem(final MapleClient c, final int id) { //怪物书拣取效果操作
        if (id / 1000000 == 2) {
            ...
            if (id == 2430513) {
                Integer LittleIndian = c.getPlayer().getAttribute().getDataValue("LittleIndian");
                if (LittleIndian == null) {
                    LittleIndian = 0;
                }
                LittleIndian++;
                c.getPlayer().getAttribute().setDataValue("LittleIndian", LittleIndian);
                c.SendPacket(MaplePacketCreator.getGamePropertyChange("LittleIndian", LittleIndian.toString()));
                return true;
            }
        }
        return false;
