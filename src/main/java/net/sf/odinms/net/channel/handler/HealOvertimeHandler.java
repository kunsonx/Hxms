...
public class HealOvertimeHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		// 8A 00 [AA] [EF 9A] [00] [00 14 00 00] [00 00] [00 00] 0A 00 00 00 00
		// 87 00 9D 00
		// 8F 00 [AB] [65 8C] [00] [00 14 00 00] [00 00] [00 00] [00 00] 04 00
		// 02 91 FD 8D 00
		// 8F 00 [7F] [6C 3B] [00] [00 14 00 00] [00 00] [00 00] [0A 00] 00 00
		// 00 3A 36 3C 00
		// 8F 00
		...
		if (healHP != 0) {
			if (healHP > 140) {
				c.getPlayer()
						.getCheatTracker()
						.registerOffense(CheatingOffense.REGEN_HIGH_HP,
		...
		if (healMP != 0) {
			...
			theoreticalRecovery += Math.floor((((float) c.getPlayer()
					.getSkillLevel(SkillFactory.getSkill(2000000)) / 10) * c
					.getPlayer().getLevel()) + 3);
			if (healMP > theoreticalRecovery) {
				if (healMP > 300) { // seems almost impossible
					c.getPlayer()
							.getCheatTracker()
							.registerOffense(CheatingOffense.REGEN_HIGH_MP,
									String.valueOf(healMP));
				}
			}
