...
public class ExternalCodeTableGetter {
	...
	public static <T extends Enum<? extends WritableIntValueHolder> & WritableIntValueHolder> void 客户端包头获取(
		...
		org.apache.log4j.Logger log = org.apache.log4j.Logger
				.getLogger(ExternalCodeTableGetter.class);
		if (log.isTraceEnabled()) { // generics - copy pasted between send and
									// recv current?
			log.trace(getOpcodeTable(values));
		}
	}

	public static <T extends Enum<? extends WritableIntValueHolder> & WritableIntValueHolder> void 服务端包头获取(
			Properties properties, T[] values) {
		ExternalCodeTableGetter exc = new ExternalCodeTableGetter(properties);
		for (T code : values) {
			int a = exc.getValue(code.name(), values, -2); // 要给包头赋的值
