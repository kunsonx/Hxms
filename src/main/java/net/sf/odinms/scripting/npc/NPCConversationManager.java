...
public class NPCConversationManager extends AbstractPlayerInteraction {
	...
	public NPCConversationManager(MapleClient c, int npc,
			List<MaplePartyCharacter> otherParty, int b) { // CPQ
		super(c);
		this.c = c;
		this.npc = npc;
	}
	...
	public void sendNext(String text) {
		getClient().getSession().write(
				MaplePacketCreator.getNPCTalk(npc, (byte) 0, text, "00 01",
						(byte) 0));
	}
	...
	public void sendPrev(String text) {
		getClient().getSession().write(
				MaplePacketCreator.getNPCTalk(npc, (byte) 0, text, "01 00",
						(byte) 0));
	}
	...
	public void sendNextPrev(String text) {
		getClient().getSession().write(
				MaplePacketCreator.getNPCTalk(npc, (byte) 0, text, "01 01",
						(byte) 0));
	}
	...
	public void sendOk(String text) {
		getClient().getSession().write(
				MaplePacketCreator.getNPCTalk(npc, (byte) 0, text, "00 00",
						(byte) 0));
	}
	...
	public void sendYesNo(String text) {
		getClient().getSession().write(
				MaplePacketCreator
						.getNPCTalk(npc, (byte) 2, text, "", (byte) 0));
	}

	public void sendYesNo(String text, int speaker) {
		getClient().getSession().write(
				MaplePacketCreator.getNPCTalk(npc, (byte) 2, text, "",
						(byte) speaker));
	}

	public void sendAcceptDecline(String text) {
		getClient().getSession().write(
				MaplePacketCreator.getNPCTalk(npc, (byte) 0x0E, text, "",
						(byte) 0));
	}
	...
	public void sendSimple(String text) {
		getClient().getSession().write(
				MaplePacketCreator
						.getNPCTalk(npc, (byte) 5, text, "", (byte) 0));
	}

	public void sendSimple(String text, int speaker) {
		getClient().getSession().write(
				MaplePacketCreator.getNPCTalk(npc, (byte) 5, text, "",
						(byte) speaker));
	}

	public void sendStyle(String text, int styles[], int card) {
		getClient().getSession().write(
				MaplePacketCreator.getNPCTalkStyle(npc, text, styles, card));
	}

	public void sendGetNumber(String text, int def, int min, int max) {
		getClient().getSession().write(
				MaplePacketCreator.getNPCTalkNum(npc, text, def, min, max));
	}

	public void sendGetText(String text) {
		getClient().getSession().write(
				MaplePacketCreator.getNPCTalkText(npc, text));
	}

	public void sendGetText(String text, int min, int max) {
		getClient().getSession().write(
				MaplePacketCreator.getNPCTalkText(npc, text, min, max));
	}

	public void sendCY1(String text, byte type) {
		getClient().getSession().write(
				MaplePacketCreator.getCY1(npc, text, type));
	}

	public void sendCY2(String text, byte type) {
		getClient().getSession().write(
				MaplePacketCreator.getCY2(npc, text, type));
	}
	...
	public void openBeans(int type) {
		c.getSession().write(
				MaplePacketCreator.openBeans(getPlayer().getBeans(), type));
	}
	...
	public void unequipEverything() {
		MapleInventory equipped = getPlayer().getInventory(
				MapleInventoryType.EQUIPPED);
		MapleInventory equip = getPlayer().getInventory(
		...
		for (short id : ids) {
			MapleInventoryManipulator.unequip(getC(), id,
	...
	public EventManager getEventManager(String event) {
		return getClient().getChannelServer().getEventSM()
				.getEventManager(event);
	}

	public void showEffect(String effect) {
		getPlayer().getMap().broadcastMessage(
				MaplePacketCreator.showEffect(effect));
	}

	public void playSound(String sound) {
		getClient().getPlayer().getMap()
	...
	public void warpParty(int mapId) {
		...
		for (MaplePartyCharacter chrs : getPlayer().getParty().getMembers()) {
			MapleCharacter curChar = c.getChannelServer().getPlayerStorage()
					.getCharacterByName(chrs.getName());
			if ((curChar.getEventInstance() == null && c.getPlayer()
	...
	public void warpRandom(int mapid) {
		...
		MaplePortal portal = target.getPortal((int) (Math.random() * (target
				.getPortals().size()))); // generate random portal
	...
	public void SpawnZakumF() {
		...
		getPlayer().getMap().spawnFakeMonsterOnGroundBelow(monster,
		...
		for (int i = 9400903; i <= 9400910; i++) {
			...
			c.getPlayer()
					.getMap()
					.spawnMonsterOnGroundBelow(monster,
	...
	public void addRandomItem(int id) {
		MapleItemInformationProvider i = MapleItemInformationProvider
				.getInstance();
		MapleInventoryManipulator.addFromDrop(getClient(),
				i.randomizeStats((Equip) i.getEquipById(id)), true);
	}

	public void removeSquadMember(MapleSquadType type, MapleCharacter chr,
			boolean ban) {
		MapleSquad squad = c.getChannelServer().getMapleSquad(type);
		if (squad != null) {
	...
	public MapleCharacter getCharByName(String name) {
		try {
			return c.getChannelServer().getPlayerStorage()
	...
	public void changeSex() {
		...
		getClient().getSession().write(
				MaplePacketCreator.updateCharLook(getPlayer()));
	}
	...
	public void warpAllInMap(int mapid, int portal) {
		...
		for (MapleCharacter aaa : outMap.getCharacters()) {
			...
			aaa.getClient().getPlayer()
					.changeMap(outMap, outMap.getPortal(portal));
			outMap = mapFactory.getMap(mapid);
			aaa.getClient().getPlayer().getEventInstance()
					.unregisterPlayer(aaa.getClient().getPlayer()); // Unregister
																	// them all
		}
	}
	...
	public int countMonster() {
		...
		List<MapleMapObject> monsters = map.getMapObjectsInRange(c.getPlayer()
	...
	public int countReactor() {
		MapleMap map = c.getPlayer().getMap();
		double range = Double.POSITIVE_INFINITY;
		List<MapleMapObject> reactors = map.getMapObjectsInRange(c.getPlayer()
				.getPosition(), range, Arrays
	...
	public void spawnMonster(int mobid, int HP, int MP, int level, int EXP,
			int boss, int undead, int amount, int x, int y) {
		MapleMonsterStats newStats = new MapleMonsterStats();
	...
	public boolean removePlayerFromInstance() {
		if (getClient().getPlayer().getEventInstance() != null) {
			getClient().getPlayer().getEventInstance()
					.removePlayer(getClient().getPlayer());
			return true;
		}
		return false;
	}
	...
	public void openDuey() {
		c.getSession().write(
				MaplePacketCreator.sendDuey((byte) 9,
	...
	public void removeHiredMerchantItem(int id) {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("DELETE FROM hiredmerchant WHERE id = ?");
			ps.setInt(1, id);
			ps.executeUpdate();
			ps.close();
			con.close();
	...
	public long getHiredMerchantMesos() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT MerchantMesos FROM characters WHERE id = ?");
	...
	public List<Pair<Integer, IItem>> getStoredMerchantItems() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT * FROM hiredmerchant WHERE ownerid = ? AND onSale = false");
			ps.setInt(1, getPlayer().getId());
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				if (rs.getInt("type") == 1) {
					...
					Item newItem = new Item(rs.getInt("itemid"), (byte) 0,
	...
	public void sendCPQMapLists() {
		...
		for (int i = 0; i < 6; i++) {
			if (fieldTaken(i)) {
				if (fieldLobbied(i)) {
					msg += "#b#L" + i + "#Monster Carnival Field " + (i + 1)
							+ " Avg Lvl: "
							+ getAverageLevel(980000100 + i * 100) + "#l\\r\\n";
				} else {
					continue;
				}
			} else {
	...
	public boolean fieldLobbied(int field) {
		if (c.getChannelServer().getMapFactory()
				.getMap(980000100 + field * 100).getAllPlayers().size() >= 2) {
			return true;
		} else {
			return false;
		}
	...
	public boolean fieldTaken(int field) {
		...
		if ((!mf.getMap(980000100 + field * 100).getAllPlayers().isEmpty())
				|| (!mf.getMap(980000101 + field * 100).getAllPlayers()
						.isEmpty())
				|| (!mf.getMap(980000102 + field * 100).getAllPlayers()
						.isEmpty())) {
			return true;
		} else {
			return false;
			...
			for (MaplePartyCharacter mpc : c.getPlayer().getParty()
				...
				if (mc != null) {
					...
					mc.getClient().getSession()
							.write(MaplePacketCreator.serverNotice(5, msg));
					mc.getClient().getSession()
	...
	public void challengeParty(int field) {
		...
		if (leader != null) {
			if (!leader.isCPQChallenged()) {
				...
				for (MaplePartyCharacter member : c.getPlayer().getParty()
						.getMembers()) {
					challengers.add(member);
				}
				NPCScriptManager.getInstance().start("cpqchallenge",
						leader.getClient(), npc, challengers);
			} else {
		...
		try {
			if (challenger != null) {
				if (challenger.getParty() == null) {
					throw new RuntimeException(
							"ERROR: CPQ Challenger's party was null!");
				}
				for (MaplePartyCharacter mpc : challenger.getParty()
						.getMembers()) {
					MapleCharacter mc;
					mc = c.getChannelServer().getPlayerStorage()
							.getCharacterByName(mpc.getName());
					if (mc != null) {
						mc.changeMap(c.getPlayer().getMap(), c.getPlayer()
								.getMap().getPortal(0));
						mc.getClient().getSession()
								.write(MaplePacketCreator.getClock(10));
					}
				}
			}
			...
			TimerManager.getInstance().schedule(new Runnable() {
				...
				public void run() {
					...
					new MapleMonsterCarnival(getPlayer().getParty(),
							challenger.getParty(), mapid);
					map.broadcastMessage(MaplePacketCreator.serverNotice(5,
							"The Monster Carnival has begun!"));
				}
			}, 10000);
	...
	public boolean startPopMessage(int charid, String msg) {
		if (getPlayerOnline(charid)) {
			c.getChannelServer().getCharacterFromAllServers(charid)
					.dropMessage(1, msg);
			return true;
		}
		return false;
	...
	public void serverNotice(int s, String Text) {
		getClient().getChannelServer().broadcastPacket(
				MaplePacketCreator.serverNotice(s, Text));
	}

	public void serverNotice(String Text) {
		getClient().getChannelServer().broadcastPacket(
				MaplePacketCreator.serverNotice(Text));
	}
	...
	public boolean getHiredMerchantItems(boolean tempTable) {
		...
		try {
			...
			while (rs.next()) {
				if (rs.getInt("type") == 1) {
					Equip spItem = new Equip(rs.getInt("itemid"), (byte) 0,
							false);
					...
					if (!getPlayer().getInventory(MapleInventoryType.EQUIP)
							.isFull()) {
					...
					Item spItem = new Item(rs.getInt("itemid"), (byte) 0,
							(short) rs.getInt("quantity"));
					MapleItemInformationProvider ii = MapleItemInformationProvider
							.getInstance();
					MapleInventoryType type = ii.getInventoryType(spItem
							.getItemId());
	...
	public void gainItem(int id, short quantity) {
		...
		c.getSession().write(
				MaplePacketCreator.getShowItemGain(id, quantity, true));
	}
	...
	public void gainItem(int id, short quantity, long expDate) {
		if (quantity >= 0) {
			...
			IItem nItem = type.equals(MapleInventoryType.EQUIP) ? ii
					.getEquipById(id) : new Item(id, (byte) 0, quantity);
			nItem.setExpiration(new Timestamp(System.currentTimeMillis()
					+ expDate));
	...
	public void deleteItem(int inventorytype) {
		MapleInventoryManipulator.CleanInventory(c,
				MapleInventoryType.getByType((byte) inventorytype));
	}
	...
	// 高级任务系统 - 任务创建
	public void MissionMake(int charid, int missionid, int repeat,
			int repeattime, int lockmap, int mobid) {
		getPlayer().MissionMake(charid, missionid, repeat, repeattime, lockmap,
				mobid);
	}

	// 高级任务系统 - 重新做同一个任务
	public void MissionReMake(int charid, int missionid, int repeat,
			int repeattime, int lockmap) {
		getPlayer().MissionReMake(charid, missionid, repeat, repeattime,
				lockmap);
	}
	...
	// 高级任务系统 - 获得任务是否可以做
	public boolean MissionStatus(int charid, int missionid, int maxtimes,
			int checktype) {
		return getPlayer()
				.MissionStatus(charid, missionid, maxtimes, checktype);
	}
	...
	public void gainCashItem(int Sn, long Time, String Name) {
		...
		Timestamp ExpirationDate = new Timestamp(System.currentTimeMillis()
				+ Time);
		MapleCSInventoryItem citem = new MapleCSInventoryItem(
				MapleCharacter.getNextUniqueId(), item.getItemId(), Sn,
				(short) item.getCount(), false);
	...
	public void modifyNX(int amount, int type) {
		...
		if (amount > 0) {
			getClient().getSession().write(
					MaplePacketCreator.serverNotice(5, "你获得抵用卷 (+" + amount
							+ ") 点."));
		} else {
			getClient().getSession().write(
					MaplePacketCreator.serverNotice(5, "你被扣除抵用卷 (" + (amount)
							+ ") 点."));
		}
	}
	...
	public void dropMessage(String message) {
		getClient().getSession().write(
				MaplePacketCreator.serverNotice(6, message));
	}
	...
	public String guildMing() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("select * from guilds order by GP desc limit 0, 20");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				result = ""
						+ result
	...
	public String paiMing() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("select * from characters order by reborns desc limit 0, 20");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				result = ""
						+ result
	...
	public String jkMing() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("select * from characters order by juank desc limit 0, 10");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				result = ""
						+ result
	...
	public String dhMing() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("select * from characters order by Remains desc limit 0, 20");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				result = ""
						+ result
	...
	public String ckMing() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("select * from accounts order by money desc limit 0, 10");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				result = ""
						+ result
	...
	public String rqMing() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("select * from characters order by fame desc limit 0, 10");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				result = ""
						+ result
	...
	public String EquipList(MapleClient c) {// 读取背包装备栏道具
		...
		MapleInventory equip = c.getPlayer().getInventory(
				MapleInventoryType.EQUIP);
		List<String> stra = new LinkedList<String>();
		for (IItem item : equip.list()) {
			stra.add("#L" + item.getPosition() + "##v" + item.getItemId()
					+ "##l");
		}
	...
	public String CashList(MapleClient c) {// 读取背包现金栏道具
		StringBuilder str = new StringBuilder();
		MapleInventory cash = c.getPlayer().getInventory(
