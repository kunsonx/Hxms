...
public class NPCScriptManager extends AbstractScriptManager {
	...
	public void start(String filename, MapleClient c, int npc,
			List<MaplePartyCharacter> chars) { // CPQ start
		try {
			NPCConversationManager cm = new NPCConversationManager(c, npc,
					chars, 0);
			cm.dispose();
	...
	public void action(MapleClient c, byte mode, byte type, int selection) {
		...
		if (ns != null) {
			try {
				...
				if (c != null && e != null) {
					log.error("Error executing NPC script : "
							+ (c.getCM() == null ? "Unknown" : c.getCM()
									.getNpc()), e);
					dispose(c);
				}
				// WriteToFile re = new
