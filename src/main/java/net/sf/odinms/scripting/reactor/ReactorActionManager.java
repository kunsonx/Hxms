...
public class ReactorActionManager extends AbstractPlayerInteraction {
    ...
    public void dropItems(boolean meso, int mesoChance, int minMeso, int maxMeso, int minItems) {
        ...
        for (DropEntry d : items) {
            if (d.itemid == 0) {
                ...
                int mesoDrop = (int) (((int) (Math.random() * range) + minMeso) * getClient().getChannelServer().getMesoRate());
    ...
    public void createMapMonitor(int mapId) {
        c.getChannelServer().getMapFactory().getMap(mapId).setTask(new MapleMapTask() {
