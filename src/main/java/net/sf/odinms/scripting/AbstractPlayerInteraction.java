...
public class AbstractPlayerInteraction {
    ...
    public void gainItem(int id, short quantity, boolean randomStats) {
        if (quantity >= 0) {
            ...
            if (type.equals(MapleInventoryType.EQUIP) && !ii.isThrowingStar(item.getItemId()) && !ii.isBullet(item.getItemId())) {
                ...
                MapleInventoryManipulator.addById(c, id, quantity, logInfo.toString(), null);
            }
        } else {
    ...
    public void givePartyItems(int id, short quantity, List<MapleCharacter> party) {
        for (MapleCharacter chr : party) {
            ...
            if (quantity >= 0) {
                ...
                MapleInventoryManipulator.addById(cl, id, quantity, logInfo.toString(), null);
            } else {
    ...
    public void worldMessage(int type, String message) {
        ...
        try {
            c.getChannelServer().getWorldInterface().broadcastMessage(chr.getName(), packet.getBytes());
        } catch (RemoteException e) {
            ServerExceptionHandler.HandlerRemoteException(e);
