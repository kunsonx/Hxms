...
public class AbstractPlayerInteraction {
	...
	public void warp(int map, int portal) {
		getPlayer().changeMap(getWarpMap(map),
				getWarpMap(map).getPortal(portal));
	}
	...
	public void setTimeOut(long time, final int mapId) {
		TimerManager.getInstance().schedule(new Runnable() {
			public void run() {
				...
				MapleMap outMap = c.getChannelServer().getMapFactory()
						.getMap(mapId);
				for (MapleCharacter player : map.getCharacters()) {
					player.getClient().getPlayer()
							.changeMap(outMap, outMap.getPortal(0));
				}
			}
		}, time);
	}
	...
	public void gainItem(int id, short quantity, boolean randomStats) {
		if (quantity >= 0) {
			...
			if (!MapleInventoryManipulator.checkSpace(c, id, quantity, "")) {
				...
				c.getSession().write(
						MaplePacketCreator.serverNotice(1, "你的背包已满"));
				return;
			}
			if (type.equals(MapleInventoryType.EQUIP)
				...
				if (randomStats) {
					...
					MapleInventoryManipulator.addFromDrop(c, (Equip) item,
							logInfo.toString(), false);
				}
			} else {
				MapleInventoryManipulator.addById(c, id, quantity,
						logInfo.toString(), null);
			}
		} else {
	...
	public void changeMusic(String songName) {
		getPlayer().getMap().broadcastMessage(
				MaplePacketCreator.musicChange(songName));
	}
	...
	public MapleGuild getGuild() {
		try {
			return c.getChannelServer()
					.getWorldInterface()
					.getGuild(getPlayer().getGuildid(),
							new MapleGuildCharacter(getPlayer()));
	...
	public void gainGP(int amount) {
		try {
			c.getChannelServer().getWorldInterface()
					.gainGP(getPlayer().getGuildid(), amount);
		} catch (RemoteException e) {
			ServerExceptionHandler.HandlerRemoteException(e);
	...
	public boolean isLeader() {
		return (getParty().getLeader().equals(new MaplePartyCharacter(c
				.getPlayer())));
	}
	...
	public void givePartyItems(int id, short quantity,
		...
		for (MapleCharacter chr : party) {
			...
			if (quantity >= 0) {
				StringBuilder logInfo = new StringBuilder(cl.getPlayer()
						.getName());
				logInfo.append(" received ");
				logInfo.append(quantity);
	...
	/**
	 * PQ gain EXP: Multiplied by channel rate here to allow global values to be
	 * input direct into NPCs
	 */
		...
		for (MapleCharacter chr : party) {
			...
			if (possesed > 0) {
				MapleInventoryManipulator.removeById(c,
						...
						false);
				cl.getSession().write(
						MaplePacketCreator.getShowItemGain(id,
								(short) -possesed, true));
			}
		}
	}
	...
	public void removeAll(int id, MapleClient cl) {
		...
		if (possessed > 0) {
			...
			cl.getSession().write(
					MaplePacketCreator.getShowItemGain(id, (short) -possessed,
							true));
		}
	}
		...
		for (MaplePet pet : getPlayer().getPets()) {
			if (pet != null) {
				...
				getClient().getSession().write(
						MaplePacketCreator.updatePet(pet));
			}
		}
	}
	...
	public int getPlayerCount(int mapid) {
		return c.getChannelServer().getMapFactory().getMap(mapid)
				.getCharacters().size();
	}
	...
	public void worldMessage(int type, String message) {
		net.sf.odinms.net.MaplePacket packet = MaplePacketCreator.serverNotice(
				type, message);
	...
	public void resetMap(int mapid) {
		...
		for (MapleMapObject i : getMap(mapid).getMapObjectsInRange(
				c.getPlayer().getPosition(), Double.POSITIVE_INFINITY,
				Arrays.asList(MapleMapObjectType.ITEM))) {
			getMap(mapid).removeMapObject(i);
			getMap(mapid).broadcastMessage(
					MaplePacketCreator.removeItemFromMap(i.getObjectId(), 0, c
							.getPlayer().getId()));
		}
	}

	public int getMapCharCount(int map) {
		return getMap(map).getCharacters().size();
	}

	public void sendClock(MapleClient d, int time) {
		d.getSession().write(
				MaplePacketCreator.getClock((int) (time - System
	...
	public void showMapDH(String path, int info) { // 播放剧情
		lockUI();
		c.getSession().write(MaplePacketCreator.removeTutorialStats());
		c.getSession().write(MaplePacketCreator.showWZEffect(path, info));
	}

	public void showMapEFFECTXG(String path) {
		c.getSession().write(MaplePacketCreator.showMapEFFECTXG(path));
	}
