...
public class EventManager {
	...
	public void startInstance(MapleParty party, MapleMap map) {
		try {
			EventInstanceManager eim = (EventInstanceManager) (iv
	...
	public void startInstance(MapleParty party, MapleMap map, boolean partyid) {
		try {
			...
			if (partyid) {
				eim = (EventInstanceManager) (iv.invokeFunction("setup",
						party.getId()));
			} else {
				eim = (EventInstanceManager) (iv.invokeFunction("setup",
						(Object) null));
	...
	public EventInstanceManager startEventInstance(MapleParty party,
			MapleMap map, boolean partyid) {
		try {
			EventInstanceManager eim;
			if (partyid) {
				eim = (EventInstanceManager) (iv.invokeFunction("setup",
	...
	public void autoLianjie() {
		Collection<MapleCharacter> chrs = this.cserv.getPlayerStorage()
				.getAllCharacters();
	...
	public void autoNx() {
		...
		for (MapleCharacter chr : chrs) {
			...
			if ((chr.getMapId() == 910000000)
					&& (chr.getClient().getChannel() == 1)) {
				chr.gainNX(giveNX);
				chr.getClient()
						.getSession()
						.write(MaplePacketCreator.serverNotice(5,
								"[泡点系统]：恭喜你获得 --泡点奖励--[" + giveNX + "]--点卷!"));
			}
		}
	}
}
