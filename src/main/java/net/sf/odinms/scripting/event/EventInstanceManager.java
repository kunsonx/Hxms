...
public class EventInstanceManager {
	...
	// private ArrayList<ScheduledFuture> eventDSQ = new
	...
	public String getEventTimeString() {
		...
		return t.get(Calendar.HOUR) + "时" + t.get(Calendar.MINUTE) + "分"
				+ t.get(Calendar.SECOND) + "秒";
	}
	...
	public void monsterKilled(MapleCharacter chr, MapleMonster mob) {
		try {
			kclLock.lock();
			int inc = ((Double) em.getIv().invokeFunction("monsterValue", this,
					mob.getId())).intValue();
			killCount.put(chr, killCount.containsKey(chr) ? killCount.get(chr)
					+ inc : inc);
		} catch (ScriptException ex) {
			log.error("事件脚本解析错误：", ex);
		} catch (NoSuchMethodException ex) {
			...
			public void run() {
				try {
					if (em == null && mapFactory == null) {
						...
						em.getIv().invokeFunction(methodName,
