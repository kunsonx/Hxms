...
public class QuestScriptManager extends AbstractScriptManager {
    ...
    public void start(MapleClient c, int npc, int quest) {
        try {
            ...
            if (iv == null) {
                if (c.getPlayer().isGM() && !(c.getPlayer().getJob().getId() >= 3000)) //qm.sendOk("这项任务尚未创建成功：\r\n NpcID: " + npc + "  QuestID: " + quest);
                {
                    if (log.isDebugEnabled()) {
                        c.getPlayer().dropMessage(PhoneType.系统公告_红字.getValue(), "这项任务尚未创建成功：\r\n NpcID: " + npc + "  QuestID: " + quest);
                    }
                }
                qm.dispose();
                return;
