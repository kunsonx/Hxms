...
public class MaplePlayerBuffManager {
	...
	public static class MapleBuff {
		...
		public String toString() {
			return "ID :" + getId() + ";StartTime:" + getStartTime()
					+ ";Schedule:" + getScheduledFuture(); // To change body of
															// generated
															// methods, choose
															// Tools |
															// Templates.
	...
	public void registerEffect(MapleStatEffect effect, long starttime,
		...
		try {
			...
			for (Pair<MapleBuffStat, Integer> pair : stat) {
				MapleBuffEffect buffEffect = new MapleBuffEffect(buff,
	...
	public boolean isBuffFrom(MapleBuffStat stat, int skill) {
		...
		try {
			...
			return effects.containsKey(stat) ? (effects.get(stat).getBuff()
					.getEffect().isSkill() && effects.get(stat).getBuff()
					.getEffect().getSourceId() == skill) : false;
		} finally {
			lock.readLock().unlock();
		}
	}
		...
		try {
			...
			for (MapleBuffEffect buffEffect : effects.values()) {
				if (buffEffect != null && buffEffect.getEffect() != null) {
					Integer nid = Integer.valueOf(buffEffect.getEffect()
							.isSkill() ? buffEffect.getEffect().getSourceId()
							: -buffEffect.getEffect().getSourceId());
					if (!hs.contains(nid)) {
	...
	public Long getBuffedStarttime(MapleBuffStat effect) {
		return effects.containsKey(effect) ? effects.get(effect).getBuff().startTime
				: null;
	}

	public MapleStatEffect getStatForBuff(MapleBuffStat effect) {
		return effects.containsKey(effect) ? effects.get(effect).getEffect()
				: null;
	}
	...
	public List<PlayerBuffValueHolder> getAllBuffs() {
		try {
			...
			for (MapleBuff buff : buffs) {
				...
				for (MapleBuffEffect buffEffect : buff.buffEffects) {
					stat.add(Pair.Create(buffEffect.getStat(),
							buffEffect.getValue()));
				}
				ret.add(new PlayerBuffValueHolder(buff.getStartTime(), buff
						.getEffect(), stat));
			}
			return ret;
		} finally {
		...
		for (MapleBuff buff : buffs) {
			if (skillid == 0) { // 人物死亡时
				if (buff.effect.isSkill()
						&& (buff.effect.getSourceId() % 20000000 == 1004 || MapleCharacter
								.dispelSkills(buff.effect.getSourceId()))) {
					cancelEffect(buff);
				}
	...
	public void cancelEffect(MapleBuff buff, boolean overwrite) {
		if (target.get() != null) {
			target.get().cancelEffect(buff.getEffect(), overwrite,
					buff.getStartTime());
		}
	}
	...
	public List<MapleBuffStat> getSpawnList(List<MapleBuffStat> list) {
		...
		for (MapleBuffStat buffStat : list) {
			if (effects.containsKey(buffStat)
					&& effects.get(buffStat).getEffect().isRefreshstyle()) {
				_buffs.add(buffStat);
			}
		}
		return _buffs;
	...
	public String toString() {
		...
		try {
			...
			for (MapleBuff buff : buffs) {
				...
				sb.append("StartTime:").append(buff.getStartTime())
						.append("\r\n");
				sb.append("ScheduledFuture:").append(buff.getScheduledFuture())
						.append("\r\n");
				for (MapleBuffEffect buffEffect : buff.buffEffects) {
					sb.append("Stat :").append(buffEffect.stat.toString())
							.append(" - Value:").append(buffEffect.value)
							.append("\r\n");
				}
			}
		} finally {
		...
		try {
			...
			for (MapleBuffEffect buffStat : effects.values()) {
				sb.append("From Buff:").append(buffStat.getBuff())
						.append("\r\n");
		...
		return sb.toString(); // To change body of generated methods, choose
								// Tools | Templates.
	}
	...
	public void cancelSkill(int skill) {
		for (MapleBuff buff : buffs) {
			if (buff.getId() == skill) {
				if (target.get() != null) {
					target.get().cancelEffect(buff.getEffect(), false,
							buff.getStartTime());
				}
				break;
			}
