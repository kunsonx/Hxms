...
public class MapleCharAttribute {
	...
	public static MapleCharAttribute loadFromDatabase(int id) {
		...
		try {
			...
			ret = (MapleCharAttribute) session
					.get(MapleCharAttribute.class, id);
			session.clear();
			session.close();
		} catch (Exception e) {
		...
		if (ret == null) {
			...
			ret.getAttribute().put("p_createAtt",
					GameConstants.getFormatter().format(new java.util.Date()));
		}
		return ret;
	}
	...
	public static MapleCharAttribute createCharAttribute(int id) {
		...
		ret.setId(id);
		ret.setAttribute(new HashMap<String, String>());
		ret.getAttribute().put("p_createAttAndChar",
				GameConstants.getFormatter().format(new java.util.Date()));
		try {
			Session session = DatabaseConnection.getSession();
	...
	public void writeAreaData(MaplePacketLittleEndianWriter mplew) {
		for (String s : playerdata.keySet()) {
			...
			if (s.equals(KUANGLONGKEYSET)) {
				...
				mplew.writeMapleAsciiString(playerdata.get(KUANGLONGKEYSET)
						.toString());
			}
	...
	public void clearAttributes() {
		for (Iterator<Map.Entry<String, String>> it = attribute.entrySet()
				.iterator(); it.hasNext();) {
			Map.Entry<String, String> entry = it.next();
			if (Character.isDigit(entry.getKey().charAt(0))
					&& !entry.getKey()
							.startsWith(ConstantTable.getCurrentDay())) {
				it.remove();
			}
		}
	}
