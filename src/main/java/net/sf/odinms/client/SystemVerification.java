...
public class SystemVerification {
	...
	private static final Logger log = Logger
			.getLogger(SystemVerification.class);
	...
	public void init() {
		try {
			...
			Variant conRet = WmiServer.invoke("ConnectServer",
					variantParameters);
			WmiServer = new ActiveXComponent(conRet.toDispatch());
		} catch (Exception e) {
			log.error(
					"初始化 WMI 服务失败，请在 服务中 开始 [Windows Management Instrumentation] 服务",
					e);
		}
	}
	...
	public void configMysql() {
		...
		try {
			...
			if (enumVariant.hasMoreElements()) {
				...
				Matcher matcher = pattern.matcher(Dispatch.call(mysqlDispatch,
						"PathName").getString());
				if (matcher.find()) {
					...
					if (checkIniFileValue(section)) {
						...
						log.info("MYSQL StopService :"
								+ Dispatch.call(mysqlDispatch, "StopService")
										.toString());
						String state = Dispatch.call(mysqlDispatch, "State")
								.toString();
						while (!state.equals("Stopped")) {
							log.info("MYSQL :" + state);
							Thread.sleep(100);
							enumVariant = Query("SELECT * FROM Win32_Service WHERE DisplayName = 'MySql'");
							mysqlDispatch = enumVariant.nextElement()
									.toDispatch();
							state = Dispatch.call(mysqlDispatch, "State")
									.toString();
						}
						log.info("MYSQL StartService :"
								+ Dispatch.call(mysqlDispatch, "StartService")
										.toString());
					}
				}
			}
	...
	public static void main(String[] args) throws IOException {
		...
		wmi.init();
		/*
		 * Date date = wmi.getCurrentDate(); System.out.println("年:" +
		 * date.getYear()); System.out.println("月:" + date.getMonth());
		 * System.out.println("日:" + date.getDate()); System.out.println("时:" +
		 * date.getHours()); System.out.println("分:" + date.getMinutes());
