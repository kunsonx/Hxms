...
public class MonsterSpawningCommands implements Command {
	...
	private static Logger log = Logger.getLogger(MonsterSpawningCommands.class);
	...
	public void execute(MapleClient c, MessageCallback mc, String[] splitted)
		...
		if (splitted[0].equals("!spawn")) {
			...
			if (newExpRatio < oldExpRatio && newexp > 0) {
				mc.dropMessage("You cannot spawn this monster! The new hp/exp ratio is better than the old one. ("
						+ newExpRatio + " < " + oldExpRatio + ")");
				return;
			}
			...
			for (int i = 0; i < num; i++) {
				...
				if (moveLock) {
					c.getPlayer()
							.getMap()
							.spawnFakeMonsterOnGroundBelow(mob,
			...
			c.getPlayer()
					.getMap()
					.spawnMonsterOnGroundBelow(mob0,
							c.getPlayer().getPosition());
			...
			for (int i = 9400110; i <= 9400113; i++) {
				...
				c.getPlayer()
						.getMap()
						.spawnMonsterOnGroundBelow(mob2,
								c.getPlayer().getPosition());
			}
			...
			c.getPlayer()
					.getMap()
					.spawnMonsterOnGroundBelow(mob3,
							c.getPlayer().getPosition());
			...
			for (int i = 8800003; i <= 8800010; i++) {
				c.getPlayer()
						...
						.spawnMonsterOnGroundBelow(
								MapleLifeFactory.getMonster(i),
								c.getPlayer().getPosition());
			}
		} else if (splitted[0].equals("!zakumf")) {
			c.getPlayer()
					.getMap()
					.spawnFakeMonsterOnGroundBelow(
							MapleLifeFactory.getMonster(9400900),
							c.getPlayer().getPosition());
			...
			c.getPlayer()
					.getMap()
					.broadcastMessage(
							MaplePacketCreator.musicChange("Bgm14/HonTale"));
			c.getPlayer()
					.getMap()
					.spawnMonsterOnGroundBelow(
							MapleLifeFactory.getMonster(8810026),
							c.getPlayer().getPosition());
			...
			if (mmo != null) {
				...
				mc.dropMessage("Monster with oID " + oid + " is "
						+ (mmo.isHpLocked() ? "" : "no longer ") + "HP Locked.");
			}
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] {
				new CommandDefinition("spawn", "",
						"Spawns the monster with the given id", 4),
				...
				new CommandDefinition("zakumf", "", "", 4),
