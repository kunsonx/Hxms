    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception {
        ...
        if (player.getMapId() != 980000404) {
            if (splitted[0].equals("@力量") || splitted[0].equals("@智力") || splitted[0].equals("@运气") || splitted[0].equals("@敏捷")) {
                ...
                if (c.getPlayer().getMapId() >= 910000018 && c.getPlayer().getMapId() <= 910000022) {
                ...
                try {
                    ...
                    PreparedStatement ps = (PreparedStatement) dcon.prepareStatement("DELETE FROM spawns WHERE idd = ? and mid = ?");
                    ps.setInt(1, mobId);
                    ps.setInt(2, MapId);
                    ps.executeUpdate();
                    ps.close();
                 ...
                 * player.getClient().getChannelServer().getWorldInterface().broadcastMessage(null,
                 ...
                 * player.getClient().getChannelServer().getWorldInterface().broadcastMessage(null,
                 * MaplePacketCreator.serverNotice(6, "Congratulations to " +
                 * player.getName() + " for rebirthing " + player.getReborns() +
                 * " times.").getBytes()); } } catch (RemoteException e) {
                 * c.getChannelServer().reconnectWorld(); }
                 * player.unequipEverything(); } else { mc.dropMessage("You must
                 * be at least level 200.");
                }
                 */
                ...
                if ((Integer.parseInt(splitted[1]) >= 5000000) && (Integer.parseInt(splitted[1]) <= 5000100)) {
                    ...
                    MapleInventoryManipulator.addFromDrop(c, MaplePet.createPet(Integer.parseInt(splitted[1])));
                    return;
                }
                if (ii.isRechargable(Integer.parseInt(splitted[1]))) {
                    quantity = ii.getSlotMax(c, Integer.parseInt(splitted[1]));
                    MapleInventoryManipulator.addById(c, Integer.parseInt(splitted[1]), quantity, "Rechargable item created.", player.getName());
                    return;
                }
                MapleInventoryManipulator.addById(c, Integer.parseInt(splitted[1]), quantity, player.getName() + "used !item with quantity " + quantity, player.getName());
