...
public class ReportCommands implements Command {
	...
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(ReportCommands.class);
		...
		if (splitted[0].equals("!listreports")) {
			...
			try {
				...
				PreparedStatement ps = con
						.prepareStatement("SELECT * FROM reports ORDER BY id DESC LIMIT ?, 15");
				ps.setInt(1, page * 15);
				...
				while (rs.next()) {
					mc.dropMessage(rs.getInt("id")
							...
							+ " | "
							+ ReportPlayerHandler.getNameById(rs
									.getInt("reporterid"))
							+ " | "
							+ ReportPlayerHandler.getNameById(rs
									.getInt("victimid")) + " | "
			...
			try {
				...
				if (rs.next()) {
					mc.dropMessage(ReportPlayerHandler.reasons[rs
							.getInt("reason")]
							+ " | "
							+ ReportPlayerHandler.getNameById(rs
									.getInt("reporterid"))
			...
			try {
				Connection con = DatabaseConnection.getConnection();
				PreparedStatement ps = con
						.prepareStatement("UPDATE reports SET status = ? WHERE id = ?");
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] {
				new CommandDefinition("listreports", "<page>", "Lists reports",
						50),
				new CommandDefinition("getreport", "id",
						"Gets the report from the specified id", 50),
				new CommandDefinition("editreportstatus", "id status",
						"Edits the status of a report", 50) };
	}
}