	...
	public void execute(MapleClient c, MessageCallback mc, String[] splitted)
		...
		if (splitted[0].equals("!rate")) {
			if (splitted.length > 2) {
				...
				boolean bOk = true;
				for (final ChannelServer cservs : c.getChannelServers()) {
					if (splitted[1].equals("exp")) {
						if (arg <= 50) {
							cservs.setExpRate(arg);
							cservs.broadcastPacket(MaplePacketCreator
									.serverNotice(6, "经验已经成功修改为 " + arg
											+ "倍。祝大家游戏开心！"));
						} else {
						...
						if (arg <= 5) {
							...
							cservs.broadcastPacket(MaplePacketCreator
									.serverNotice(6, "BOSS掉宝已经成功修改为 " + arg
											+ "倍。祝大家游戏开心！"));
						} else {
							mc.dropMessage("操作已被系统限制。");
						}
						...
						if (arg <= 5) {
							...
							cservs.broadcastPacket(MaplePacketCreator
									.serverNotice(6, "宠物经验已经成该修改为 " + arg
											+ "倍。祝大家游戏开心！"));
						} else {
							mc.dropMessage("操作已被系统限制。");
						}
			...
			mc.dropMessage("经验: " + cserv.getExpRate() + "倍 | 宠物: "
					+ cserv.getPetExpRate() + "倍");
			mc.dropMessage("爆率: " + cserv.getDropRate() + "倍 | BOSS爆率: "
					+ cserv.getBossDropRate() + "倍");
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] {
				new CommandDefinition(
						"rate",
						"<exp|drop|meso|bossdrop|petexp> <amount> <seconds> <minutes> <hours>",
						"Changes the specified rate", 50),
