...
public class WarpCommands implements Command {
	...
	public void execute(MapleClient c, MessageCallback mc, String[] splitted)
		...
		if (splitted[0].equals("!跟踪")) {
			...
			if (victim != null) {
				...
				try {
					...
					WorldLocation loc = c.getChannelServer()
							.getWorldInterface().getLocation(splitted[1]);
					if (loc != null) {
						...
						try {
							WorldChannelInterface wci = c.getChannelServer()
									.getWorldInterface();
							wci.addBuffsToStorage(c.getPlayer().getId(), c
									.getPlayer().getAllBuffs());
		...
		} else if (splitted[0].equals("!跟踪1")) {
			MapleCharacter victim = cserv.getPlayerStorage().getCharacterById(
			...
			if (victim != null) {
				...
				try {
					...
					WorldLocation loc = c
							...
							.getWorldInterface()
							.getLocation(
									getNameById(Integer.parseInt(splitted[1])));
					if (loc != null) {
						mc.dropMessage("你将更换频道,这可能需要几秒钟.");
						MapleMap target = c.getChannelServer().getMapFactory()
								.getMap(loc.map);
			...
			if (splitted.length == 1) {
				...
				try {
					target = cserv.getMapFactory().getMap(
							Integer.parseInt(splitted[1]));
				} catch (Exception e) {
				}
				if (target == null) {
					...
					if (splitted.length > 2) {
						try {
							targetPortal = target.getPortal(Integer
									.parseInt(splitted[2]));
						} catch (Exception e) {
						}
					}
		...
		} else if (splitted[0].equals("!身边")) {
			int mapid = Integer.parseInt(splitted[1]);
			MapleMap map = c.getChannelServer().getMapFactory().getMap(mapid);
			MapleMap warpFrom = c.getPlayer().getMap();
			Collection<MapleCharacter> cmcwf = new LinkedHashSet<MapleCharacter>(
					warpFrom.getCharacters());
			...
			MapleCharacter victim = cserv.getPlayerStorage()
					.getCharacterByName(splitted[1]);
			if (victim != null) {
				mc.dropMessage(splitted[1]
						+ " is online. Please use the normal functions!");
				return;
			} else {
				WorldLocation loc = c.getChannelServer().getWorldInterface()
				...
				if (loc != null) {
					...
					MapleMap toMap = c.getChannelServer().getMapFactory()
							.getMap(toMapId);
					if (toMap == null) {
					...
					if (c.getPlayer().changeMapOffline(splitted[1], toMapId)) {
						...
						mc.dropMessage(splitted[1]
								+ " was not warped. Please ensure you typed their name correctly.");
					}
				}
			}
		} else if (splitted[0].equalsIgnoreCase("!warpalive")
				|| splitted[0].equalsIgnoreCase("!warpdead")) {
			int mapid = Integer.parseInt(splitted[1]);
			MapleMap map = c.getChannelServer().getMapFactory().getMap(mapid);
			boolean alive = splitted[0].equalsIgnoreCase("!warpalive") ? true
					: false;
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] {
				new CommandDefinition("跟踪", "playername",
						"Warps yourself to the player", 3),
				new CommandDefinition("跟踪1", "playerid",
						"Warps yourself to the player", 3),
				new CommandDefinition("单人", "playername",
						"Warps the player with the given name to yourself", 4),
				new CommandDefinition("offlinewarp", "playername mapid",
						"Warps a player whilst they are offline", 50),
				new CommandDefinition("map", "mapid",
						"Warps you to the given mapid (use /m instead)", 50),
				new CommandDefinition(
						"身边",
						"mapid",
						"Warps you and everyone in your map to the mapid specified",
						5),
				new CommandDefinition(
						"warpalive",
						"mapid",
						"Warps all alive people on your map to the designated map",
						1),
				new CommandDefinition(
						"warpdead",
						"mapid",
						"Warps all dead people on your map to the designated map",
						1) };
	}
}