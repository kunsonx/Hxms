...
public class DebugCommands implements Command {
	...
	public void execute(MapleClient c, MessageCallback mc, String[] splitted)
		...
		if (splitted[0].equals("!resetquest")) {
			MapleQuest.getInstance(Integer.parseInt(splitted[1])).forfeit(
					c.getPlayer());
		} else if (splitted[0].equals("!nearestportal")) {
			final MaplePortal portal = player.getMap().findClosestSpawnpoint(
					player.getPosition());
			mc.dropMessage(portal.getName() + " id: " + portal.getId()
			...
			if (splitted.length > 1) {
				player.getMap().toggleDrops();
				final ChannelServer cservv = player.getClient()
						.getChannelServer();
				final int mapid = player.getMapId();
				c.getSession().write(
						MaplePacketCreator.getClock(Integer
								.parseInt(splitted[1])));
			...
			MapleReactorStats reactorSt = MapleReactorFactory
					.getReactor(Integer.parseInt(splitted[1]));
			MapleReactor reactor = new MapleReactor(reactorSt,
			...
			List<MapleMapObject> reactors = map.getMapObjectsInRange(c
					.getPlayer().getPosition(), Double.POSITIVE_INFINITY,
					Arrays.asList(MapleMapObjectType.REACTOR));
			for (MapleMapObject reactorL : reactors) {
				MapleReactor reactor2l = (MapleReactor) reactorL;
				mc.dropMessage("Reactor: oID: " + reactor2l.getObjectId()
						+ " reactorID: " + reactor2l.getId() + " Position: "
			...
			if (splitted[1].equals("all")) {
				for (MapleMapObject reactorL : reactors) {
					MapleReactor reactor2l = (MapleReactor) reactorL;
					c.getPlayer().getMap()
							.destroyReactor(reactor2l.getObjectId());
				}
			} else {
				c.getPlayer().getMap()
						.destroyReactor(Integer.parseInt(splitted[1]));
			}
		} else if (splitted[0].equals("!rreactor")) {
			c.getPlayer().getMap().resetReactors();
		} else if (splitted[0].equals("!cleardrops")) {
			MapleMap map = c.getPlayer().getMap();
			List<MapleMapObject> items = map.getMapObjectsInRange(c.getPlayer()
					.getPosition(), Double.POSITIVE_INFINITY, Arrays
					.asList(MapleMapObjectType.ITEM));
			for (MapleMapObject itemmo : items) {
				map.removeMapObject(itemmo);
				map.broadcastMessage(MaplePacketCreator.removeItemFromMap(
						itemmo.getObjectId(), 0, c.getPlayer().getId()));
			}
			...
			map.broadcastMessage(MaplePacketCreator.serverNotice(5,
					map.getMapName()
							+ " is now "
							+ (map.isLootable() ? "lootable" : "unlootable"
									+ ".")));
			...
			c.getPlayer().handleOrbgain();
		} else if (splitted[0].equals("!命令列表")) {
			CommandProcessor.getInstance().dropHelp(player, mc);
		}
	}
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] {
				new CommandDefinition("resetquest", "<questid>",
						"Resets the specified quest", 50),
				new CommandDefinition("nearestportal", "",
						"Gives you the nearest portal", 50),
				...
				new CommandDefinition("hreactor", "[object ID]", "Hit reactor",
						50),
				...
				new CommandDefinition("dropd", "",
						"toggle drops on a map for a set time", 50),
				new CommandDefinition("cleardrops", "",
						"Clear all drops on the current map", 50),
				new CommandDefinition("toggleloot", "",
						"Makes item pickup un/available to the map", 50),
				new CommandDefinition("questdebug", "", "", 50),
				new CommandDefinition("energycharge", "", "", 50),
				new CommandDefinition("命令列表", "", "", 1), };
	}
}