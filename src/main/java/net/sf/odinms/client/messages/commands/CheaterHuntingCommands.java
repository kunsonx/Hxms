...
public class CheaterHuntingCommands implements Command {
	...
	public void execute(MapleClient c, MessageCallback mc, String[] splitted)
		...
		if (splitted[0].equals("!whosthere")) {
			MessageCallback callback = new ServerNoticeMapleClientMessageCallback(
					c);
			...
			for (MapleCharacter chr : c.getPlayer().getMap().getCharacters()) {
				...
				builder.append(MapleCharacterUtil.makeMapleReadable(chr
						.getName()));
				builder.append(", ");
			}
			builder.setLength(builder.length() - 2);
			c.getSession().write(
					MaplePacketCreator.serverNotice(6, builder.toString()));
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] {
				new CommandDefinition("whosthere", "",
						"Gives you a list of players on your map", 50),
				new CommandDefinition("扫描", "", "Gives you a list of cheaters",
						3) };
	}
}