...
public class 玩家命令 implements Command {
		...
		if ((splitted[0].equals("@力量") || splitted[0].equals("@智力")
			...
			if (amount > 0 && amount <= player.getRemainingAp()
				...
				if (str && (amount + player.getStr()) <= Short.MAX_VALUE) {
					...
					player.updateSingleStat(Arrays.asList(
							new Pair<MapleStat, Number>(MapleStat.STR, player
									.getStr()),
							new Pair<MapleStat, Number>(MapleStat.AVAILABLEAP,
									player.getRemainingAp())));
				} else if (Int && amount + player.getInt() <= Short.MAX_VALUE) {
					player.setInt(player.getInt() + amount);
					player.setRemainingAp(player.getRemainingAp() - amount);
					player.updateSingleStat(Arrays.asList(
							new Pair<MapleStat, Number>(MapleStat.INT, player
									.getInt()),
							new Pair<MapleStat, Number>(MapleStat.AVAILABLEAP,
									player.getRemainingAp())));
					...
					player.setRemainingAp(player.getRemainingAp() - amount);
					player.updateSingleStat(Arrays.asList(
							new Pair<MapleStat, Number>(MapleStat.LUK, player
									.getLuk()),
							new Pair<MapleStat, Number>(MapleStat.AVAILABLEAP,
									player.getRemainingAp())));
					...
					player.setRemainingAp(player.getRemainingAp() - amount);
					player.updateSingleStat(Arrays.asList(
							new Pair<MapleStat, Number>(MapleStat.DEX, player
									.getDex()),
							new Pair<MapleStat, Number>(MapleStat.AVAILABLEAP,
									player.getRemainingAp())));
				} else {
			...
			mc.dropMessage("============================================================");
			mc.dropMessage("    " + c.getChannelServer().getServerName()
					+ "    玩家命令.");
			mc.dropMessage("============================================================");
			...
			while (rs.next()) {
				...
				String xb1;
				mc.dropMessage(i + ". " + rs.getString("name") + "  ||  职业: "
						+ job + "  ||  转生次数: " + rs.getInt("reborns")
						+ "  ||  等级: " + rs.getInt("level") + "  ||  性别: ");
				i++;
			}
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] { new CommandDefinition("力量", 0),
				...
				new CommandDefinition("假死", 0), new CommandDefinition("排名", 0),
				new CommandDefinition("npc", 0) };
	}
}
