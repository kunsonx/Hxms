...
public final class CommandProcessor implements CommandProcessorMBean {
    ...
    public boolean processCommand(MapleClient c, String line) {
        return instance.processCommandInternal(c, new ServerNoticeMapleClientMessageCallback(c), c.getPlayer().getGm(), line);
    }
            ...
            if (file.isFile() && file.canRead()) {
                ...
                try {
                    /*  ScriptEngine command = sef.getScriptEngine();
                     ...
                     registerCommand(c);*/
                    ScriptCommand scriptCommand = ScriptCommand.getScriptCommand(file);
                    if (scriptCommand != null) {
                        registerCommand(scriptCommand);
                    }
                } catch (ScriptException e) {
                    log.error(String.format("脚本：%s 运行异常：%s", file.getName(), e.getMessage()));
                } catch (IOException e) {
                    log.error(String.format("脚本：%s 文件处理异常：%s", file.getName(), e.getMessage()));
                } finally {
                    if (fr != null) {
                        try {
                            fr.close();
    ...
    public void dropHelp(MapleCharacter chr, MessageCallback mc) {
        ...
        for (DefinitionCommandPair definitionCommandPair : allCommands) {
            if (chr.hasGMLevel(definitionCommandPair.getDefinition().getRequiredLevel())) {
    ...
    private boolean processCommandInternal(MapleClient c, MessageCallback mc, int gmLevel, String line) {
        ...
        if (line.charAt(0) == '!' || line.charAt(0) == '@') {
            byte type;
            if (line.charAt(0) == '!' && c.getPlayer().isGM() || line.charAt(0) == '@') {
                type = 0;
            } else {
                type = 1;
            }
                ...
                if (definitionCommandPair != null && gmLevel >= definitionCommandPair.getDefinition().getRequiredLevel()) {
                    if (type == 0) {
                        if (definitionCommandPair.getDefinition().getRequiredLevel() > 0) {
                            synchronized (gmlog) {
                                gmlog.add(new Pair<Integer, String>(player.getId(), line));
                            }
                        }
                    }
                    try {
                    ...
                    if (definitionCommandPair == null && c.getPlayer().getGm() > 0) {
                        if (type == 0) {
