...
public class NoticeCommand implements Command {
	...
	public void execute(MapleClient c, MessageCallback mc, String[] splitted)
		...
		if (splitted[0].equals("!notice") || splitted[0].equals("!公告")) {
			int joinmod = 1;
			int range = -1;
			...
			MaplePacket packet = MaplePacketCreator.serverNotice(type,
					notice.toString());
			if (range == 0) {
			...
			MaplePacket msgpacket = MaplePacketCreator.serverNotice(6,
					msg.toString());
			c.getChannelServer()
					.getWorldInterface()
					.broadcastMessage(c.getPlayer().getName(),
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] {
				new CommandDefinition("notice", "[mcw] [n/p/l/nv/v/b] message",
						"", 3),
				new CommandDefinition("公告", "[mcw] [n/p/l/nv/v/b] message", "",
						3),
				new CommandDefinition("me", "message",
						"send a message with your name as the prefix", 50),
				new CommandDefinition("gmtalk", "message",
						"send a message to all GMs in the server", 50),
