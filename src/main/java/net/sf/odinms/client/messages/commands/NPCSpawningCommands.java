...
public class NPCSpawningCommands implements Command {
	...
	public void execute(MapleClient c, MessageCallback mc, String[] splitted)
		...
		if (splitted[0].equals("!tnpc")) {
			...
			int fh = player.getMap().getFootholds()
			...
			MapleCharacter player = c.getPlayer();
			List<MapleMapObject> npcs = player.getMap().getMapObjectsInRange(
			...
			mc.dropMessage("CY: "
					...
					+ (pos.x - 50)
					+ " | FH: "
					+ c.getPlayer().getMap().getFootholds().findBelow(pos)
							.getId());
			...
			MaplePacket msgpacket = MaplePacketCreator
					.serverNotice(
							...
							"The NPC "
									...
									+ " for "
									+ minutes
									+ "minute(s). Please talk to it to be warped to the JQ Event");
			c.getChannelServer()
					.getWorldInterface()
			...
			TimerManager.getInstance().schedule(new Runnable() {
				...
				@Override
				public void run() {
					List<MapleMapObject> npcs = player.getMap()
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] {
				...
				new CommandDefinition(
						...
						50),
				new CommandDefinition("mynpcpos", "",
						"Gets the info for making an npc", 50),
				new CommandDefinition("jqevent", "<chimney, fitness>",
						"Spawns an NPC for a JQ Event", 50) };
	}
}