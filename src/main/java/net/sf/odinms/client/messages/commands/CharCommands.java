    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception, IllegalCommandSyntaxException {
        ...
        if (splitted[0].equals("!maxstats")) {
            ...
            if (splitted.length == 2) {
                ...
                int slot = Math.min(Math.max(Integer.parseInt(splitted[2]), 0), player.getJob().GetMaxSpSlots());
                int sp = Integer.parseInt(splitted[1]);
                if (sp + player.getRemainingSp() > Short.MAX_VALUE) {
                    sp = Short.MAX_VALUE;
                }
            ...
            if (GameConstants.MAX_MESO - (player.getMeso() + Long.parseLong(splitted[1])) >= 0) {
            ...
            if (GameConstants.isPet(Integer.parseInt(splitted[1]))) {
                ...
                MaplePet pet = MaplePet.createPet(Integer.parseInt(splitted[1]));
            ...
            cserv.getPlayerStorage().getCharacterByName(splitted[1]).gainWarning(true);
            ...
            mc.dropMessage("本级经验：" + ExpTable.getExpNeededForLevel(c.getPlayer().getLevel()));
