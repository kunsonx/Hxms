    ...
    public static IEquip MagniflerEquipWithId(IItem equip) {
        ...
        for (int i = 1; i <= 3; i++) {
            if (nEquip.getPotential(i) == 1) {
                nEquip.setPotential(i, randomPotentiaStat(nEquip, nEquip.getIdentify(), nEquip.getItemId(), nEquip.getLevel()));
            }
        }

        if (log.isDebugEnabled()) {
            log.debug("潜能属性 - 1：" + getOption(nEquip.getPotential_1()).getString());
            log.debug("潜能属性 - 2：" + getOption(nEquip.getPotential_2()).getString());
            //log.debug("潜能属性 - 3：" + getOption(nEquip.getPotential_3()).getString());
        }

        return nEquip;
    }

    public static int randomPotentiaStat(Equip equip, int oldPotentiaLevel, int itemId, byte itemLevel) {
        int rndOne;
        int base = oldPotentiaLevel - 16;
        if (1 > base) {
            base = 1;
        }
        rndOne = allid.get(Integer.valueOf(base)).get((int) Math.floor(Math.random() * allid.get(Integer.valueOf(base)).size()));
