...
public class CheatingOffensePersister {
	...
	public class PersistingTask implements Runnable {
		...
		public void run() {
			...
			synchronized (toPersist) {
				offenses = toPersist.toArray(new CheatingOffenseEntry[toPersist
						.size()]);
				toPersist.clear();
			}
			...
			// con.prepareStatement("INSERT INTO cheatlog (cid, offense, count, lastoffensetime, param) VALUES (?, ?, ?, ?, ?)");
			// PreparedStatement updateps =
			// con.prepareStatement("UPDATE cheatlog SET count = ?, lastoffensetime = ?, param = ? WHERE id = ?");
			// for (CheatingOffenseEntry offense : offenses) {
			// String parm = offense.getParam() == null ? "" :
			// offense.getParam();
			...
			// insertps.setTimestamp(4, new
			// Timestamp(offense.getLastOffenseTime()));
