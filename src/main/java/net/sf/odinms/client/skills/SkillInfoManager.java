...
public class SkillInfoManager {
	...
	private static MapleSkillInfo getSkillInfoFromData(
			MapleSkillDatabase database) {
		if (database == null) {
			return null;
		}
		MapleSkillInfo info = new MapleSkillInfo(database.getId(),
				database.getName(), database.getJobid());
		for (MapleSkillDatabaseInfo dinfoDatabaseInfo : database.getValues()) {
			info.doCreate(dinfoDatabaseInfo.getPath().split("/"),
					dinfoDatabaseInfo.getValue(), 0, info);
		}
	...
	public static void readNodes(MapleSQLData info, MapleSkillDatabase base,
			String path) {
		if (info.hasNodes()) {
			for (MapleSQLData mapleSkillData : info.getNodes()) {
				readNodes(mapleSkillData, base,
						path + "/" + mapleSkillData.getName());
			}
		}
		if (info.hasValues()) {
			for (MapleSQLDataValue value : info.getValues()) {
	...
	public static MapleSkillInfo getSkill(int id) {
		...
		if (cache.containsKey(id)) {
			...
			info = getSkillInfoFromData((MapleSkillDatabase) session.get(
					MapleSkillDatabase.class, id));
			session.close();
			if (info != null) {
				cache.put(id, info);
			}
