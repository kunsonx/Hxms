...
public class MapleCharacter extends AbstractAnimatedMapleMapObject implements
	...
	// private Map<MapleBuffStat, MapleBuffStatValueHolder> effects = new
	// ConcurrentSkipListMap<MapleBuffStat, MapleBuffStatValueHolder>();
	...
	static {
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("SELECT MAX(uniqueid)+1 FROM items");
	...
	public synchronized static MapleCharacter loadCharFromDB(int charid,
		...
		try {
			...
			if (channelserver) {
				...
				if (partyid >= 0) {
					try {
						MapleParty party = client.getChannelServer()
								.getWorldInterface().getParty(partyid);
						if (party != null
			...
			while (rs.next()) {
				IItem to = ret.LoadItem(rs, MapleItemsNameSpaceType.Storages,
						currenttime, channelserver);
				if (to == null) {
					continue;
				}
			...
			if (call.getMoreResults()) {
				...
				while (rs.next()) {
					...
					if (nameSpaceType.equals(MapleItemsNameSpaceType.Inventory)) {
						ret.getInventory(
								MapleInventoryType.getByType(rs.getByte("type")))
								.addFromDB(to);
					} else if (nameSpaceType
							.equals(MapleItemsNameSpaceType.CsInventory)) {
						ret.csinventory.addFromDb(to, rs,
								rs.getInt("space") == -3);
					}
				}
				rs.close();
			...
			/*
			 * for (int i = 0; i < ITEMS_SQL.size(); i++) { String sql_ =
			 * ITEMS_SQL.get(i); MapleItemsNameSpaceType nameSpaceType =
			 * MapleItemsNameSpaceType.Unknown; ps = con.prepareStatement(sql_);
			 * switch (i) { case 0: nameSpaceType =
			 * MapleItemsNameSpaceType.Inventory; ps.setInt(1, charid); break;
			 * case 1: nameSpaceType = MapleItemsNameSpaceType.Storages;
			 * ps.setInt(1, ret.accountid); break; case 2: case 3: nameSpaceType
			 * = MapleItemsNameSpaceType.CsInventory; ps.setInt(1, charid);
			 * break; } rs = ps.executeQuery(); while (rs.next()) { IItem to =
			 * ret.LoadItem(rs, nameSpaceType, currenttime, channelserver); if
			 * (to == null) { continue; } if
			 * (nameSpaceType.equals(MapleItemsNameSpaceType.Inventory)) {
			 * ret.getInventory(
			 * MapleInventoryType.getByType(rs.getByte("type"))).addFromDB(to);
			 * } else if
			 * (nameSpaceType.equals(MapleItemsNameSpaceType.CsInventory)) {
			 * ret.csinventory.addFromDb(to, rs, i == 3); } else if
			 * (nameSpaceType.equals(MapleItemsNameSpaceType.Storages)) {
			 * ret.storage.AddItem(to); } else if
			 * (nameSpaceType.equals(MapleItemsNameSpaceType.HiredMerchant)) {
			 * ret.hiredmerchantinventory.addItem(to); } else {
			 * log.error("未知空间名：" + rs.getByte("type")); } } rs.close();
			 * ps.close(); }
			 */
			if (channelserver) {
			...
			if (ret.getInventory(MapleInventoryType.EQUIPPED).getItem(
				...
				ret.maplemount = new MapleMount(ret, ret
						.getInventory(MapleInventoryType.EQUIPPED)
						.getItem((byte) -18).getItemId(), skillid);
	...
	private IItem LoadItem(ResultSet rs, MapleItemsNameSpaceType nameSpaceType,
		...
		if (!nameSpaceType.equals(MapleItemsNameSpaceType.Inventory)) {
			...
			type = MapleItemsNameSpaceType.GetInventoryTypeByDbId(rs
					.getInt("type"));
		}
		Timestamp expiration = rs.getTimestamp("expiredate");
		IItem to;
		if ((type.equals(MapleInventoryType.EQUIP) || type
				.equals(MapleInventoryType.EQUIPPED))) {
			...
			if (rs.getLong("partnerUniqueid") > 0) {
				...
				if (equip.getUniqueid() > 0) {
					if (equip.getItemId() / 10000 == 166) {
						MapleAndroid ring = MapleAndroid.loadFromDb(
								equip.getItemId(), equip.getUniqueid());
						if (ring != null) {
			...
			if (GameConstants.isPet(rs.getInt("itemid"))) {
				...
				if (index > -1 && channelserver) {
					...
					pos.y -= 12;
					pet.setPos(pos);
					pet.setFh(getMap().getFootholds().findBelow(pet.getPos())
							.getId());
					pet.setStance(0);
					pets[index] = pet;
					pet.setSlot(index);
	...
	public synchronized void saveToDB(boolean update) {
		...
		try {
			...
			ps.setInt(
					20,
					(map != null) ? (map.getForcedReturnId() != 999999999) ? map
							.getForcedReturnId() : map.getId()
							: mapid);
			...
			if (update) {
				/*
				 * ps = con.prepareStatement(
				 * "DELETE FROM items_inventoryinfo WHERE characterid = ?");
				 * ps.setInt(1, id); ps.executeUpdate(); ps.close(); ps =
				 * con.prepareStatement
				 * ("DELETE FROM items_storageinfo WHERE accountid = ?");
				 * ps.setInt(1, accountid); ps.executeUpdate(); ps.close(); ps =
				 * con.prepareStatement(
				 * "DELETE FROM items_csinventoryinfo WHERE characterid = ?");
				 * ps.setInt(1, id); ps.executeUpdate(); ps.close();
				 */
				CallableStatement call = con
						.prepareCall("call deleteItems(?,?)");
				call.setInt(1, id);
			...
			CallableStatement create = con
					.prepareCall("call CreateItem(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			create.setInt(1, id);
			for (MapleItemsNameSpace mapleItemsNameSpace : GetSaveNameSpaces()) {
				...
				for (IItem iItem : mapleItemsNameSpace.AllItems()) {
					...
					create.setString(5,
							iItem.getOwner() != null ? iItem.getOwner() : "");
					...
					case Inventory:
						create.setInt(51, -1);
						create.setInt(52,
								((MapleInventory) mapleItemsNameSpace)
										.getType().type);
						break;
					case Storages:
			...
			/*
			 * ps = con.prepareStatement(
			 * "INSERT INTO items (itemid, position, quantity, owner, expiredate, uniqueid, flag, log) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
			 * ); PreparedStatement pse =
			 * con.prepareStatement("INSERT INTO items_equip VALUES " +
			 * "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, " +
			 * "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, " +
			 * "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, " + "?)"); PreparedStatement ps_cs
			 * =
			 * con.prepareStatement("INSERT INTO items_cs VALUES (?, ?, ?, ?)");
			 * PreparedStatement ps_pet = con.prepareStatement(
			 * "INSERT INTO items_pet VALUES (?, ?, ?, ?, ?, ?)");
			 * PreparedStatement ps_ring =
			 * con.prepareStatement("INSERT INTO items_ring VALUES (?, ?, ?, ?)"
			 * ); PreparedStatement ps_in_info = con.prepareStatement(
			 * "INSERT INTO `items_inventoryinfo` VALUES (?, ?, ?)"),
			 * ps_storage_info =
			 * con.prepareStatement("INSERT INTO `items_storageinfo` VALUES (?, ?)"
			 * ), ps_csin_info = con.prepareStatement(
			 * "INSERT INTO `items_csinventoryinfo` VALUES (?, ?)");
			 * ps_in_info.setInt(1, id); ps_storage_info.setInt(1, accountid);
			 * ps_csin_info.setInt(1, id); for (MapleItemsNameSpace
			 * mapleItemsNameSpace : GetSaveNameSpaces()) { if
			 * (mapleItemsNameSpace == null) { continue; } for (IItem iItem :
			 * mapleItemsNameSpace.AllItems()) { ps.setInt(1,
			 * iItem.getItemId()); ps.setInt(2, iItem.getPosition());
			 * ps.setInt(3, iItem.getQuantity()); ps.setString(4,
			 * iItem.getOwner()); ps.setTimestamp(5, iItem.getExpiration()); if
			 * (MapleItemInformationProvider.getInstance().isCash(
			 * iItem.getItemId())) { if (iItem.getUniqueid() == 0) {
			 * iItem.setUniqueId(getNextUniqueId()); } } else { if
			 * (iItem.getItemId() / 10000 != 166) { iItem.setUniqueId(0); } }
			 * ps.setLong(6, iItem.getUniqueid()); ps.setInt(7,
			 * iItem.getFlag()); ps.setString(8, iItem.getLog().toString()); int
			 * rows = ps.executeUpdate(); ResultSet rs = ps.getGeneratedKeys();
			 * long itemsI_id = -1; if (rs.next()) { itemsI_id = rs.getLong(1);
			 * } else { throw new DatabaseException("Inserting char failed."); }
			 * if (iItem instanceof Equip) { Equip equip = (Equip) iItem;
			 * pse.setLong(1, itemsI_id); pse.setInt(2,
			 * equip.getUpgradeSlots()); pse.setInt(3, equip.getLevel());
			 * pse.setInt(4, equip.getStr()); pse.setInt(5, equip.getDex());
			 * pse.setInt(6, equip.getInt()); pse.setInt(7, equip.getLuk());
			 * pse.setInt(8, equip.getHp()); pse.setInt(9, equip.getMp());
			 * pse.setInt(10, equip.getWatk()); pse.setInt(11, equip.getMatk());
			 * pse.setInt(12, equip.getWdef()); pse.setInt(13, equip.getMdef());
			 * pse.setInt(14, equip.getAcc()); pse.setInt(15, equip.getAvoid());
			 * pse.setInt(16, equip.getHands()); pse.setInt(17,
			 * equip.getSpeed()); pse.setInt(18, equip.getJump());
			 * pse.setInt(19, equip.getVicious()); pse.setInt(20,
			 * equip.getPotential_1()); pse.setInt(21, equip.getPotential_2());
			 * pse.setInt(22, equip.getPotential_3()); pse.setInt(23,
			 * equip.getIdentify()); pse.setInt(24, equip.getIdentified());
			 * pse.setInt(25, equip.getStarlevel()); pse.setInt(26,
			 * equip.getItemLevel()); pse.setInt(27, equip.getItemExp());
			 * pse.setInt(28, equip.getItemSkill()); pse.setInt(29,
			 * equip.getDurability()); pse.setInt(30, equip.getPvpWatk());
			 * pse.setTimestamp(31, equip.getUnlockTime()); pse.addBatch(); } if
			 * (mapleItemsNameSpace.GetSpaceType().equals(
			 * MapleItemsNameSpaceType.CsInventory)) { Item tItem = (Item)
			 * iItem; ps_cs.setLong(1, itemsI_id); ps_cs.setInt(2,
			 * iItem.getSN()); ps_cs.setString(3, tItem.getSender());
			 * ps_cs.setString(4, tItem.getMessage()); ps_cs.addBatch(); } if
			 * (iItem instanceof MaplePet) { MaplePet maplePet = (MaplePet)
			 * iItem; ps_pet.setLong(1, itemsI_id); ps_pet.setString(2,
			 * maplePet.getName()); ps_pet.setInt(3, maplePet.getLevel());
			 * ps_pet.setInt(4, maplePet.getCloseness()); ps_pet.setInt(5,
			 * maplePet.getFullness()); ps_pet.setInt(6, maplePet.getSlot() +
			 * 1); ps_pet.addBatch(); } if (iItem instanceof MapleRing) {
			 * MapleRing mapleRing = (MapleRing) iItem; ps_ring.setLong(1,
			 * itemsI_id); ps_ring.setLong(2, mapleRing.getPartnerUniqueId());
			 * ps_ring.setInt(3, mapleRing.getPartnerChrId());
			 * ps_ring.setString(4, mapleRing.getPartnerName());
			 * ps_ring.addBatch(); } switch (mapleItemsNameSpace.GetSpaceType())
			 * { case Inventory: ps_in_info.setLong(2, itemsI_id);
			 * ps_in_info.setInt(3, ((MapleInventory)
			 * mapleItemsNameSpace).getType().type); ps_in_info.addBatch();
			 * break; case Storages: ps_storage_info.setLong(2, itemsI_id);
			 * ps_storage_info.addBatch(); break; case CsInventory:
			 * ps_csin_info.setLong(2, itemsI_id); ps_csin_info.addBatch();
			 * break; } } } ps.close(); pse.executeBatch(); pse.close();
			 * ps_cs.executeBatch(); ps_cs.close(); ps_pet.executeBatch();
			 * ps_pet.close(); ps_ring.executeBatch(); ps_ring.close();
			 * 
			 * ps_in_info.executeBatch(); ps_storage_info.executeBatch();
			 * ps_csin_info.executeBatch(); ps_in_info.close();
			 * ps_storage_info.close(); ps_csin_info.close();
			 */
			...
			PreparedStatement pse_ = con
					.prepareStatement("INSERT INTO queststatusmobs VALUES (?, ?, ?)");
			ps.setInt(1, id);
	...
	public void updateQuest(MapleQuestStatus quest) {
		...
		if (quest.getQuest().getId() == 4760) {
			client.getSession().write(
					MaplePacketCreator.completeQuest(this, (short) quest
							.getQuest().getId()));
			client.getSession().write(
					MaplePacketCreator.updateQuestInfo(this, (short) quest
							.getQuest().getId(), quest.getNpc(), (byte) 10));
			...
			if (quest.getStatus().equals(MapleQuestStatus.Status.STARTED)) {// 入门...
				client.getSession().write(
						MaplePacketCreator.startQuest(this, (short) quest
								.getQuest().getId()));
				client.getSession()
						.write(MaplePacketCreator.updateQuestInfo(this,
				...
				client.getSession().write(
						MaplePacketCreator.forfeitQuest(this, (short) quest
								.getQuest().getId()));
			}
		}
	}
	...
	public static int getIdByName(String name, int world) {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT id FROM characters WHERE name = ? AND world = ?");
			ps.setString(1, name);
			ps.setInt(2, world);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
	...
	public boolean isActiveBuffedValue(int skillid) {
		/*
		 * LinkedList<MapleBuffStatValueHolder> allBuffs = new
		 * LinkedList<MapleBuffStatValueHolder>( effects.values()); for
		 * (MapleBuffStatValueHolder mbsvh : allBuffs) { if
		 * (mbsvh.effect.isSkill() && mbsvh.effect.getSourceId() == skillid &&
		 * !isGM()) { return true; } } return false;
		 */
	...
	public Integer getBuffedValue(MapleBuffStat effect) {
		/*
		 * MapleBuffStatValueHolder mbsvh = effects.get(effect); if (mbsvh ==
		 * null) { return null; } return Integer.valueOf(mbsvh.value);
		 */
	...
	public boolean isBuffFrom(MapleBuffStat stat, ISkill skill) {
		/*
		 * MapleBuffStatValueHolder mbsvh = effects.get(stat); if (mbsvh ==
		 * null) { return false; } return mbsvh.effect.isSkill() &&
	...
	public List<MapleStatEffect> getBuffEffects() {
		/*
		 * ArrayList<MapleStatEffect> almseret = new
		 * ArrayList<MapleStatEffect>(); HashSet<Integer> hs = new
		 * HashSet<Integer>(); for (MapleBuffStatValueHolder mbsvh :
		 * effects.values()) { if (mbsvh != null && mbsvh.effect != null) {
		 * Integer nid = Integer.valueOf(mbsvh.effect.isSkill() ?
		 * mbsvh.effect.getSourceId() : -mbsvh.effect.getSourceId()); if
		 * (!hs.contains(nid)) { almseret.add(mbsvh.effect); hs.add(nid); } } }
		 * return almseret;
		 */
	...
	public Long getBuffedStarttime(MapleBuffStat effect) {
		/*
		 * MapleBuffStatValueHolder mbsvh = effects.get(effect); if (mbsvh ==
		 * null) { return null; } return Long.valueOf(mbsvh.startTime);
		 */
	...
	public MapleStatEffect getStatForBuff(MapleBuffStat effect) {
		/*
		 * MapleBuffStatValueHolder mbsvh = effects.get(effect); if (mbsvh ==
		 * null) { return null; } return mbsvh.effect;
		 */
	...
	private List<MapleBuffStat> getBuffStats(MapleStatEffect effect,
			long startTime) {
		/*
		 * List<MapleBuffStat> stats = new ArrayList<MapleBuffStat>(); try { for
		 * (Entry<MapleBuffStat, MapleBuffStatValueHolder> stateffect :
		 * effects.entrySet()) { MapleBuffStatValueHolder mbsvh =
		 * stateffect.getValue(); if (mbsvh != null &&
		 * (mbsvh.effect.sameSource(effect) && (startTime == -1 || startTime ==
		 * mbsvh.startTime))) { stats.add(stateffect.getKey()); } } } catch
	 ...
	 * List<MapleBuffStatValueHolder> effectsToCancel = new
	 * ArrayList<MapleBuffStatValueHolder>( stats.size()); for (MapleBuffStat
	 * stat : stats) { MapleBuffStatValueHolder mbsvh = effects.get(stat); if
	 * (mbsvh != null) { effects.remove(stat); boolean addMbsvh = true; for
	 * (MapleBuffStatValueHolder contained : effectsToCancel) { if
	 * (mbsvh.startTime == contained.startTime && contained.effect ==
	 * mbsvh.effect) { addMbsvh = false; } } if (addMbsvh) {
	 * effectsToCancel.add(mbsvh); }
	 * 
	 * } } for (MapleBuffStatValueHolder cancelEffectCancelTasks :
	 * effectsToCancel) { if (getBuffStats(cancelEffectCancelTasks.effect,
	 * cancelEffectCancelTasks.startTime).isEmpty()) {
	 * cancelEffectCancelTasks.schedule.cancel(false); } } }
	 */
	public void checkCancelBuffStat(
			List<MaplePlayerBuffManager.MapleBuffEffect> list) {
	...
	public void cancelBuffStats(MapleBuffStat stat) {
		...
		// cancelPlayerBuffs(buffStatList, 0,
		// effects.get(stat).effect.isRefreshstyle());
		// }
		ArrayList<MapleBuffStat> list = new ArrayList<MapleBuffStat>();
		MapleStatEffect effect = buffManager.deregisterBuffStats(
				buffManager.getBuff(stat), list);
		// cancelPlayerBuffs(list, effect.getSourceId(),
		// effect.isRefreshstyle());
	...
	private void cancelPlayerBuffs(List<MapleBuffStat> buffstats,
		...
		if (getClient().getChannelServer().getPlayerStorage()
				.getCharacterById(getId()) != null) { // are we still connected
			// ?
			recalcLocalStats();
			enforceMaxHpMp();
			getClient().getSession().write(
					MaplePacketCreator.cancelBuff(buffstats,
							effect.getSourceId()));
			if (effect.isRefreshstyle()) {
				getMap().broadcastMessage(
						this,
						MaplePacketCreator.cancelForeignBuff(getId(),
								buffstats, this, effect), false);
			}
		}
	}
	...
	public void dispel() {
		 ...
		 * LinkedList<MapleBuffStatValueHolder>( effects.values()); for
		 * (MapleBuffStatValueHolder mbsvh : allBuffs) { if
		 * (mbsvh.effect.isSkill()) { cancelEffect(mbsvh.effect, false,
		 * mbsvh.startTime); } }
		 */
		buffManager.dispel();
	}
	...
	public void cancelMorphs() {
		 ...
		 * LinkedList<MapleBuffStatValueHolder>( effects.values()); for
		 * (MapleBuffStatValueHolder mbsvh : allBuffs) { if
		 * (mbsvh.effect.isMorph() && mbsvh.effect.getSourceId() != 5111005 &&
		 * mbsvh.effect.getSourceId() != 5121003) { cancelEffect(mbsvh.effect,
		 * false, mbsvh.startTime); } }
		 */
	...
	public void silentGiveBuffs(List<PlayerBuffValueHolder> buffs) {
		for (PlayerBuffValueHolder mbsvh : buffs) {
			mbsvh.effect
					.silentApplyBuff(this, mbsvh.startTime, mbsvh.getStat());
		}
	}
	...
	public List<PlayerBuffValueHolder> getAllBuffs() {
		/*
		 * List<PlayerBuffValueHolder> ret = new
		 * ArrayList<PlayerBuffValueHolder>(); for (MapleBuffStatValueHolder
		 * mbsvh : effects.values()) { ret.add(new
		 * PlayerBuffValueHolder(mbsvh.startTime, mbsvh.effect)); } return ret;
		 */
	...
	public void cancelMagicDoor() {
		 ...
		 * LinkedList<MapleBuffStatValueHolder>( effects.values()); for
		 * (MapleBuffStatValueHolder mbsvh : allBuffs) { if
		 * (mbsvh.effect.isMagicDoor()) { cancelEffect(mbsvh.effect, false,
		 * mbsvh.startTime); } }
		 */
	...
	public void handleOrbgain() {
		...
		if (getBuffedValue(MapleBuffStat.COMBO) < ceffect.getX() + 1) {
			...
			duration += (int) ((getBuffedStarttime(MapleBuffStat.COMBO) - System
					.currentTimeMillis())); // 时间递减
			int effectlevel = getSkillLevel(1111002);
			if (effectlevel > 0) {
				getClient().getSession().write(
						MaplePacketCreator.giveBuff(1111002, duration, stat,
								this));
			} else {
	...
	public void handleOrbconsume() {
		...
		int duration = ceffect.getDuration();
		duration += (int) (getBuffedStarttime(MapleBuffStat.COMBO) - System
				.currentTimeMillis()); // 对时间进行递减
		if (Cskillid == 11111001) // 修改 原来是根据skilllevel判断
		{
			getClient().getSession()
					.write(MaplePacketCreator.giveBuff(11111001, duration,
							stat, this));
		} else {
			getClient().getSession().write(
	...
	private void enforceMaxHpMp() {
		...
		if (getMp() > getCurrentMaxMp()) {
			setMp(getMp());
			stats.add(new Pair<MapleStat, Number>(MapleStat.MP, Integer
					.valueOf(getMp())));
		}
		if (getHp() > getCurrentMaxHp()) {
			setHp(getHp());
			stats.add(new Pair<MapleStat, Number>(MapleStat.HP, Integer
					.valueOf(getHp())));
		}
		if (stats.size() > 0) {
			getClient().getSession().write(
	...
	public int getRemainingSp() {
		return this.remainingsp[0 > this.job.GetMaxSpSlots() ? 0 : this.job
				.GetMaxSpSlots()];
	}
	...
	public void setExp(long exp) {
		this.exp.set(Math.max(0, Math.min(Math.max(0, exp),
	...
	public void setRemainingSp(int remainingSp) {
		this.remainingsp[0 > this.job.GetMaxSpSlots() ? 0 : this.job
				.GetMaxSpSlots()] = remainingSp;
	}
	...
	public boolean changeMapOffline(String victim, int mapId) {
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("UPDATE characters SET map = ?, spawnpoint = ? WHERE name = ?");
			ps.setInt(1, mapId);
			ps.setInt(2, 0);
	...
	private void changeMapInternal(final MapleMap to, final Point pos,
		...
		warpPacket.setOnSend(new Runnable() {
			@Override
			public void run() {
				IPlayerInteractionManager interaction = MapleCharacter.this
						.getInteraction();
				if (interaction != null) {
					if (interaction.isOwner(MapleCharacter.this)) {
						if (interaction.getShopType() == 2) {
							...
							interaction
									.closeShop(((MaplePlayerShop) interaction)
											.returnItems(getClient()));
							...
							if (interaction.getItems().isEmpty()) {
								interaction.removeAllVisitors(3, 1);
								interaction
										.closeShop(((HiredMerchant) interaction)
												.returnItems(getClient()));
							}
				...
				if (getClient().getChannelServer().getPlayerStorage()
					...
					if (party != null) {
						silentPartyUpdate();
						getClient().getSession().write(
								MaplePacketCreator.updateParty(getClient()
										.getChannel(), party,
	...
	public void changeJob(MapleJob newJob) {
		...
		for (Iterator<Entry<ISkill, SkillEntry>> it = skills.entrySet()
				.iterator(); it.hasNext();) {
		...
		statup.add(new Pair<MapleStat, Number>(MapleStat.MAXHP, Integer
				.valueOf(maxhp)));
		statup.add(new Pair<MapleStat, Number>(MapleStat.MAXMP, Integer
				.valueOf(maxmp)));
		recalcLocalStats();
		/*
	...
	public void ClearAllSkills() {
		...
		synchronized (skills) {
			...
			this.getClient()
					.getSession()
					.write(MaplePacketCreator.updateSkill(skill,
	...
	public void changeSkillLevel(ISkill skill, int newLevel,
		...
		this.getClient()
				.getSession()
				.write(MaplePacketCreator.updateSkill(skill.getId(), newLevel,
	...
	public void setHp(int newhp, boolean silent) {
		/*
		 * if (GameConstants.CheckMapAndServer(getMapId())) { return; }
		 */
	...
	private void playerDead() {
		...
		/*
		 * if (effects.containsKey(MapleBuffStat.神秘的运气)) { addHP(maxhp); }
		 */
		dispelSkill(0);
		cancelAllDebuffs();
		cancelMorphs();
		...
		if (possesed > 0 && !getMap().hasEvent()) {
			...
			if (player.getJob() != MapleJob.BEGINNER
				...
				// Lose XP
				long XPdummy = ExpTable
						.getExpNeededForLevel(player.getLevel() + 1);
		...
		if (party != null) {
			...
			for (MaplePartyCharacter partychar : party.getMembers()) {
				if (partychar.getMapid() == getMapId()
					...
					MapleCharacter other = client.getChannelServer()
							.getPlayerStorage()
							.getCharacterByName(partychar.getName());
					if (other != null) {
						other.getClient()
								.getSession()
								.write(MaplePacketCreator.updatePartyMemberHP(
	...
	public void gainExp(long gain, boolean show, boolean inChat, boolean white,
		...
		if (level < getMaxLevel()) {
			/*
			 * if (this.exp.get() + gain > Integer.MAX_VALUE) { long gainFirst =
			 * ExpTable.getExpNeededForLevel(level) - this.exp.get(); gain -=
			 * gainFirst + 1; gainExp(gainFirst + 1, false, inChat, white); }
			 */
			updateSingleStat(MapleStat.EXP, this.exp.addAndGet(gain));
			if (show && gain != 0 && !fish) {
				client.getSession().write(
						MaplePacketCreator.getShowExpGain(
								gain > Integer.MAX_VALUE ? Integer.MAX_VALUE
										: (int) gain, inChat, white));
			}
			if (fish) {
				client.getSession().write(
						MaplePacketCreator.getShowExpGain(
	...
	public void mobKilled(int id) {
		for (MapleQuestStatus q : quests.values()) {
			if (MapleQuest.getInstance(q.getQuest().getId())
					.nullCompleteQuestData()) {
			...
			if (q.mobKilled(id) && !(q.getQuest() instanceof MapleCustomQuest)) {
				...
				if (q.getQuest().canComplete(this, null)) {
					client.getSession().write(
							MaplePacketCreator.getShowQuestCompletion(q
									.getQuest().getId()));
				}
			}
		}
	...
	public void dispelSkill(int skillid) {
		 ...
		 * LinkedList<MapleBuffStatValueHolder> allBuffs = new
		 * LinkedList<MapleBuffStatValueHolder>( effects.values()); for
		 * (MapleBuffStatValueHolder mbsvh : allBuffs) { if (skillid == 0) { //
		 * 人物死亡时 if (mbsvh.effect.isSkill() && (mbsvh.effect.getSourceId() %
		 * 20000000 == 1004 || dispelSkills(mbsvh.effect.getSourceId()))) {
		 * cancelEffect(mbsvh.effect, false, mbsvh.startTime); } } else if
		 * (mbsvh.effect.isSkill() && mbsvh.effect.getSourceId() == skillid) {
		 * cancelEffect(mbsvh.effect, false, mbsvh.startTime); } }
		 */
	...
	public void levelUp() {
		...
		if (level == 200) {
			...
			try {
				getClient().getChannelServer().getWorldInterface()
		...
		setMp(maxmp);
		List<Pair<MapleStat, Number>> statup = new ArrayList<Pair<MapleStat, Number>>();
		statup.add(new Pair<MapleStat, Number>(MapleStat.AVAILABLEAP, Integer
				.valueOf(remainingAp)));
		statup.add(new Pair<MapleStat, Number>(MapleStat.MAXHP, Integer
				.valueOf(maxhp)));
		statup.add(new Pair<MapleStat, Number>(MapleStat.MAXMP, Integer
				.valueOf(maxmp)));
		statup.add(new Pair<MapleStat, Number>(MapleStat.HP, Integer
				.valueOf(maxhp)));
		statup.add(new Pair<MapleStat, Number>(MapleStat.MP, Integer
				.valueOf(maxmp)));
		statup.add(new Pair<MapleStat, Number>(MapleStat.EXP, exp.get()));
		statup.add(new Pair<MapleStat, Number>(MapleStat.LEVEL, Integer
				.valueOf(level)));
	...
	public static boolean ban(String id, String reason, boolean account) {
		try {
			...
			if (rs.next()) {
				...
				psb = con
						.prepareStatement("SELECT ip FROM iplog WHERE accountid = ? ORDER by login DESC LIMIT 1");
				...
				psb = con
						.prepareStatement("SELECT ip FROM ipbans WHERE ip = ?");
				psb.setString(1, to);
				...
				if (!macAddress.matches("")) {
					...
					for (int i = 0; i < macs.length; i++) {
						PreparedStatement psc = con
								.prepareStatement("SELECT mac FROM macbans WHERE mac = ?");
	...
	public boolean hasBattleShip() {
		/*
		 * try { LinkedList<MapleBuffStatValueHolder> allBuffs = new
		 * LinkedList<MapleBuffStatValueHolder>( effects.values()); for
		 * (MapleBuffStatValueHolder mbsvh : allBuffs) { if
		 * (mbsvh.effect.getSourceId() == 5221006) { return true; } } } catch
		 * (Exception ex) { return false; } return false;
		 */
		...
		if ((this.isHidden() && client.getPlayer().getGm() > 0)
			...
			if (getMap().getId() != 910000000) {
				for (MaplePet pett : getPets()) {
					if (pett != null) {
						client.getSession().write(
								MaplePacketCreator.showPet(this, pett, false,
										false));
					}
				}
			}
			...
			for (List<MapleSummon> integer : summons.values()) {
				for (MapleSummon mapleSummon : integer) {
					...
					client.getSession()
							.write(MaplePacketCreator.spawnSpecialMapObject(
									mapleSummon, getSkillLevel(SkillFactory
											.getSkill(mapleSummon.getSkill()))));
					client.getPlayer()
							.getMap()
							.updateMapObjectVisibility(client.getPlayer(),
									mapleSummon);
				}
			}
			if (android != null
					&& getInventory(MapleInventoryType.EQUIPPED).getItem(
							(byte) -35) != null) {
				client.getSession().write(
						MaplePacketCreator.spawnAndroid(this, android));
			}
			/*
	...
	public void equipChanged() {
		...
		if (getClient().getPlayer().getMessenger() != null) {
			WorldChannelInterface wci = client.getChannelServer()
					.getWorldInterface();
			try {
				wci.updateMessenger(getClient().getPlayer().getMessenger()
						.getId(), getClient().getPlayer().getName(),
	...
	public void setPartyId(int id) {
		...
		if (id >= 0 && cs != null) {
			try {
				MapleParty party_MapleParty = cs.getWorldInterface().getParty(
						id);
	...
	public MapleGuild getGuild() {
		try {
			return getClient().getChannelServer().getWorldInterface()
					.getGuild(getGuildid(), mgc);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void guildUpdate() {
		...
		try {
			this.client.getChannelServer().getWorldInterface()
	...
	public void disbandGuild() {
		...
		try {
			client.getChannelServer().getWorldInterface()
					.disbandGuild(this.guildid);
		} catch (Exception e) {
	 ...
	 *            : 0 = NX, 1 = MP, 2 = GT
	 * @param quantity
	 *            : how much to modify it by. Negatives subtract points,
	...
	public boolean haveItem(int itemid, int quantity, boolean checkEquipped,
		...
		if (checkEquipped) {
			possessed += inventory[MapleInventoryType.EQUIPPED.ordinal()]
					.countById(itemid);
		}
		if (exact) {
	...
	/**
	 * @param canSetBeansNum
	 *            the canSetBeansNum to set
	 */
			...
			for (IItem item : inv.list()) {
				...
				if (expiration != null) {
					if (!currenttime2.after(expiration)) {
						client.getSession()
								.write(MaplePacketCreator.itemExpired(item
										.getItemId()));
						toberemove.add(item);
					}
				}
	...
	public void startCygnusIntro() {
		client.getSession()
				.write(MaplePacketCreator.CygnusIntroDisableUI(true));
		client.getSession().write(MaplePacketCreator.CygnusIntroLock(true));
		saveLocation(SavedLocationType.CYGNUSINTRO);

		MapleMap introMap = client.getChannelServer().getMapFactory()
				.getMap(913040000);
	...
	public boolean getNXCodeValid(String code, boolean validcode)
		...
		Connection con = DatabaseConnection.getConnection();
		PreparedStatement ps = con
				.prepareStatement("SELECT `valid` FROM nxcode WHERE code = ?");
		ps.setString(1, code);
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
	...
	public int getNXCodeType(String code) throws SQLException {
		...
		Connection con = DatabaseConnection.getConnection();
		PreparedStatement ps = con
				.prepareStatement("SELECT `type` FROM nxcode WHERE code = ?");
		ps.setString(1, code);
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			type = rs.getInt("type");
	...
	public int getNXCodeItem(String code) throws SQLException {
		...
		Connection con = DatabaseConnection.getConnection();
		PreparedStatement ps = con
				.prepareStatement("SELECT `item` FROM nxcode WHERE code = ?");
		ps.setString(1, code);
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			item = rs.getInt("item");
		...
		synchronized (diseases) {
			if (isAlive() && !isActiveBuffedValue(2321005)
				...
				List<Pair<MapleDisease, Integer>> debuff = Collections
						.singletonList(new Pair<MapleDisease, Integer>(disease,
	...
	public void showNote() {
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT * FROM notes WHERE `to` = ?",
	...
	public MapleCharacter getPartner() {
		MapleCharacter test = this.getClient().getChannelServer()
				.getPlayerStorage().getCharacterById(partnerid);
		if (test != null) {
			return test;
		}
		return null;
	...
	public void unstick() {
		try {
			...
			PreparedStatement ps2 = con
					.prepareStatement("UPDATE characters SET loggedin = 0 WHERE accountid = ?");
			ps2.setInt(1, id);
			ps2.executeUpdate();
			ps2.close();
			con.close();
	...
	public boolean tempHasItems() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT ownerid FROM hiredmerchanttemp WHERE ownerid = ?");
			ps.setInt(1, getId());
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				value = true;
	...
	public int getBossLogCount(String boss) {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT COUNT(*) FROM bosslog WHERE characterid = ? AND bossid = ?");
			ps.setInt(1, this.id);
			ps.setString(2, boss);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
	...
	public void checkDuey() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT * FROM dueypackages WHERE receiverid = ? AND alerted = 0");
			ps.setInt(1, getId());
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				PreparedStatement ps2 = con
						.prepareStatement("UPDATE dueypackages SET alerted = 1 WHERE receiverid = ?");
				ps2.setInt(1, getId());
				ps2.executeUpdate();
				ps2.close();
				getClient().getSession().write(
						MaplePacketCreator
								.sendDueyMessage(Actions.TOCLIENT_PACKAGE_MSG
										.getCode()));
			}
			rs.close();
			ps.close();
	...
	public void gainCP(int gain) {
		...
		this.getClient()
				.getSession()
				.write(MaplePacketCreator.CPUpdate(false, this.getCP(),
						this.getTotalCP(), getTeam()));
		if (this.getParty() != null && getTeam() != -1) {
			this.getMap().broadcastMessage(
					MaplePacketCreator.CPUpdate(true, this.getMonsterCarnival()
							.getCP(team),
							this.getMonsterCarnival().getTotalCP(team),
							getTeam()));
		} else {
	...
	public void setEnergyPoint(int i) {
		energyPoint = i;
		List<Pair<MapleBuffStat, Integer>> statups = Collections
				.singletonList(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.能量获得, energyPoint));
		/*
		 * getClient().getSession().write( MaplePacketCreator.givePirateBuff(0,
		 * 200, statups));
		 */
	}
	...
	public void addAreaData(int quest, String data) {
		if (!this.ares_data.contains(data)) {
			...
			try {
				Connection con = DatabaseConnection.getConnection();
				PreparedStatement ps = con
						.prepareStatement("INSERT INTO char_ares_info VALUES (DEFAULT, ?, ?, ?)");
				ps.setInt(1, getId());
				ps.setInt(2, quest);
				ps.setString(3, data);
				ps.executeUpdate();
	...
	public void removeAreaData() {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("DELETE FROM char_ares_info WHERE charid = ?");
			ps.setInt(1, getId());
			ps.executeUpdate();
			ps.close();
			con.close();
	...
	public void handle_JiShaDian_Gain() {
		if (combo < 5 && (getJobid() == 422)) {
			combo += 1;
			getClient().getSession().write(
					MaplePacketCreator.giveBuff(combo, 0, MapleStatEffect.侠盗本能,
							this));
		}
	}
	...
	public void setMainsumons(MapleSummon mainsumons) {
		if (mainsumons == null
				&& buffManager.hasBufferStat(MapleBuffStat.射手_精神连接)) {
		...
		if (mainsumons != null) {
			...
			case 3111005: {// 火凤凰
				...
				if (skilllevel > 0) {
					SkillFactory.getSkill(3120006).getEffect(skilllevel)
							.applyTo(this);// 激活技能 精神连接。
				}
				break;
			}
			case 3211005: {// 冰凤凰
				...
				if (skilllevel > 0) {
					// log.debug("应用了冰凤凰。");
					SkillFactory.getSkill(3220005).getEffect(skilllevel)
							.applyTo(this);// 激活技能 精神连接。
				}
				break;
			}
	...
	public void skillAddMaxHp() {
		int[] 提升百分比 = { 4100006, 4200006, 4310004, 3120011, 3220009, 5100009,
				11000005, 15100007, 1000006, 31000003, 4210013, 4110008,
				61100007, 61110007 }; // mhpR
		...
		for (int skillid1 : 提升百分比) {
			...
			if (skilllevel > 0) {
				addHpr += SkillFactory.getSkill(skillid1).getEffect(skilllevel)
						.getmHpR();
			}
		}

		if (buffManager.hasBufferStat(MapleBuffStat.射手_精神连接)) {// 计算 射手_精神连接。
			addHpr += buffManager.getBuff(MapleBuffStat.射手_精神连接).getEffect()
					.getmHpR();
		}
		...
		for (int skillid2 : 提升数值) {
			...
			if (skillid2 == 机械师.终极机甲 && skilllevel > 0
				...
				addHp += SkillFactory.getSkill(skillid2).getEffect(skilllevel)
						.geteMHp();
			}
		}
		...
		if (getBuffedValue(MapleBuffStat.骑宠1) != null
			...
			addHpr += SkillFactory.getSkill(33001001)
					.getEffect(getSkillLevel(33001001)).getZR();
		}
	...
	public void skillAddMaxMp() {
		...
		for (int skillid1 : 提升百分比) {
			skilllevel = getSkillLevel(skillid1);
			if (skilllevel > 0) {
				addMpr += SkillFactory.getSkill(skillid1).getEffect(skilllevel)
						.getmMpR();
			}
		}
		...
		for (int skillid2 : 提升数值) {
			...
			if (skillid2 == 机械师.终极机甲 && skilllevel > 0
					|| getUsingMount() != null
					&& getUsingMount().getUsingSkillId() == skillid2) {
				addMp += SkillFactory.getSkill(skillid2).getEffect(skilllevel)
						.geteMMp();
			}
		}
	...
	public void setAndroid(MapleAndroid a) {
		if (checkHearts()) {
			...
			if (map != null && a != null) {
				...
				map.broadcastMessage(MaplePacketCreator.showAndroidEmotion(
						this.getId(), Randomizer.getInstance().nextInt(17) + 1));
			}
		}
	}
	...
	public void updateAndroid(int size, int itemId) {
		if (map != null) {
			map.broadcastMessage(MaplePacketCreator.updateAndroidLook(
					this.getId(), size, itemId));
		}
	}
	...
	public void sendAddAttackLimit() {
		if (client.getChannelServer().isUseAddMaxAttack()) {
			SendPacket(MaplePacketCreator.giveBuff(-2022118, -1498229862,
					Collections.singletonList(Pair.Create(MapleBuffStat.攻击上限,
							1000000000)), this));
		}
	}
	...
	public void sendDemonAvengerPacket() {
		if (job.IsDemonAvenger()) {
			SendPacket(MaplePacketCreator.giveBuff(0, 1643685997,
					MapleStatEffect.恶魔复仇者, this));
		}
	}
	...
	public synchronized void reloadOnlieTime() {
		try {
			String _str = getAttribute().getAttribute().get(
					ConstantTable.getCurrentDay()
							+ ConstantTable._S_ONLINE_MINUTE);
			if (_str == null) {
				_str = "0";
			}
			Long logintime = getAttribute().getDataValue(
					ConstantTable._PLAYER_DATA_LOGINTIME);
			if (logintime == null) {
				logintime = System.currentTimeMillis();
			}
			long time = System.currentTimeMillis() - logintime;
			time = time / 1000 / 60;
			_str = new Integer((Integer.parseInt(_str) + (int) time))
					.toString();
			getAttribute().getAttribute().put(
					ConstantTable.getCurrentDay()
							+ ConstantTable._S_ONLINE_MINUTE, _str);
			getAttribute().setDataValue(ConstantTable._PLAYER_DATA_LOGINTIME,
	...
	public void setPower(int power) {
		...
		SendPacket(MaplePacketCreator.giveBuff(30020232, -436000866,
				Arrays.asList(Pair.Create(MapleBuffStat.尖兵电力, power)), this));
	}
	...
	public void setPowerOpen() {
		...
		SendPacket(MaplePacketCreator.giveBuff(0, 36000,
				Arrays.asList(Pair.Create(MapleBuffStat.尖兵电池时间, 0)), this));
		SendPacket(MaplePacketCreator.getShowSkillEffectCode(powerOpen ? 0x36
				: 0x37));
	}
	...
	public void initPower() {
		if (getJob().IsTrailblazer()) {
			setPower(0);
			SendPacket(MaplePacketCreator.giveBuff(0, 36000,
					Arrays.asList(Pair.Create(MapleBuffStat.尖兵电池时间, 0)), this));// 给你10个小时电池够不够用
		}
	}
