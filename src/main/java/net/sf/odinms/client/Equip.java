...
public class Equip extends Item implements IEquip {
	...
	public Equip(int itemid, short position, boolean ring, int partnerUniqueId,
	...
	public void gainItemExp2(MapleClient c, int gain, boolean timeless) {
		...
		if (timeless) {
			expNeeded = ExpTable
					.getTimelessItemExpNeededForLevel(itemLevel + 1);
		} else {
	...
	public void gainItemLevel(MapleClient c, boolean timeless) {
		List<Pair<String, Integer>> stats = MapleItemInformationProvider
				.getInstance().getItemLevelupStats(getItemId(), itemLevel,
						timeless);
		...
		c.getPlayer().getClient().getSession()
				.write(MaplePacketCreator.showEquipmentLevelUp());
		// c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
		// MaplePacketCreator.showForeignEffect(c.getPlayer().getId(), 17));
		// c.getPlayer().forceUpdateItem(MapleInventoryType.EQUIPPED, this);
	}
	...
	public void gainItemExp(MapleClient c, int gain, boolean timeless) {
		/*
		 * int expneeded = timeless ? (10 * itemLevel + 70) : (5 * itemLevel +
		 * 65); float modifier = 364 / expneeded; float exp = (expneeded /
		 * (1000000 * modifier * modifier)) * gain;
		 */
	...
	public void writePacket(MaplePacketLittleEndianWriter mplew) {
		int mask = 0;
		int[] values = { getUpgradeSlots(),// 可升级次数
