...
public class GenericLittleEndianAccessor implements LittleEndianAccessor {
	...
	public int readInt() {
		 ...
		 * 
		 * byte1 = bs.readByte(); byte2 = bs.readByte(); byte3 = bs.readByte();
		 * byte4 = bs.readByte();
		 */
		cache[0] = bs.readByte();
		cache[1] = bs.readByte();
	...
	public short readShort() {
		/*
		 * int byte1, byte2; byte1 = bs.readByte(); byte2 = bs.readByte();
	...
	public long readLong() {
		/*
		 * long byte1 = bs.readByte(); long byte2 = bs.readByte(); long byte3 =
		 * bs.readByte(); long byte4 = bs.readByte(); long byte5 =
		 * bs.readByte(); long byte6 = bs.readByte(); long byte7 =
		 * bs.readByte(); long byte8 = bs.readByte(); return (byte8 << 56) +
		 * (byte7 << 48) + (byte6 << 40) + (byte5 << 32) + (byte4 << 24) +
		...
		return (cache[7] << 56) + (cache[6] << 48) + (cache[5] << 40)
				+ (cache[4] << 32) + (cache[3] << 24) + (cache[2] << 16)
				+ (cache[1] << 8) + cache[0];
	}
	...
	public final String readAsciiString(int n) {
		...
		try {
			String str = new String(ret, GenericLittleEndianWriter.ASCII);
			return str;
		} catch (Exception e) {
			e.printStackTrace();
		}
