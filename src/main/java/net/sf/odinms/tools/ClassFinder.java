...
public class ClassFinder {
	...
	private void addClassesInFolder(List<String> classes, File folder,
		...
		for (File f : folder.listFiles()) {
			if (!f.isDirectory()) {
				...
				addClassesInFolder(classes, f, packageName + "." + f.getName(),
						recurse);
			}
		}
	}
			...
			for (Enumeration<JarEntry> entries = jar.entries(); entries
				...
				if (entryName.endsWith(".class")
					...
					if (lastSlash <= jarPackagePath.length() || recurse) {
						...
						String className = entryName.substring(lastSlash + 1,
								entryName.length() - 6);
