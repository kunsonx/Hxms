...
public class MaplePacketCreator {
	...
	private final static byte[] CHAR_INFO_MAGIC = new byte[] { (byte) 0xff,
			(byte) 0xc9, (byte) 0x9a, 0x3b };
	public static final List<Pair<MapleStat, Number>> EMPTY_STATUPDATE = Collections
			.emptyList();
	/**
	...
	private static final MapleItemInformationProvider ii = MapleItemInformationProvider
			.getInstance();
	private static final String[] HELLO_STRINGS = { "MapLogin", "MapLogin1",
			"MapLogin2", "MapLogin3" };

	// 登录界面

	public static MaplePacket getHello(short mapleVersion,
			String secondaryVersion, byte[] sendIv, byte[] recvIv,
			boolean testServer) {
	...
	public static int 获取当前时间() {
		String 当前时间 = new SimpleDateFormat("yyyyMMddHH")
	...
	// 服务器列表
	public static MaplePacket getServerList(int serverId, String serverName,
			ChannelLoadInfo channelLoad) {
	...
	// 得到一个包告诉客户通道服务器的IP位址。
	public static MaplePacket getServerIP(InetAddress inetAddr, int port,
			int clientId) {
	...
	// 分配新职业的SP技能点1
	public static void addRemainingSp(MaplePacketLittleEndianWriter mplew,
			MapleCharacter chr) {
	...
	// 建立角色---
	private static void addCharStats(MaplePacketLittleEndianWriter mplew,
			MapleCharacter chr) {
	...
	private static void addCharLook(MaplePacketLittleEndianWriter mplew,
	...
	// 背包的方法
	private static void addInventoryInfo(MaplePacketLittleEndianWriter mplew,
			MapleCharacter chr) {
		...
		mplew.writeInt(0);// ??98
		mplew.write(chr.getInventory(MapleInventoryType.EQUIP).getSlots()); // 装备栏容量
																			// 游戏内为18个则封包数值为18
		...
		mplew.write(chr.getInventory(MapleInventoryType.CASH).getSlots()); // 特殊栏容量
																			// 游戏内为18个则封包数值为30
		mplew.writeLong(DateUtil
				.getFileTimestamp(chr.getNecklace_Expansion() == null ? FINAL_TIME
						: chr.getNecklace_Expansion().getTime())); // 扩充时间
		...
		if (chr.isEvan()) { // 是龙神才加载
			...
			synchronized (iv) {
				for (IItem item : iv.list()) {
					if (((Item) item).getPosition() > -1100
							&& ((Item) item).getPosition() <= -1000) {
						addItemInfo(mplew, item);
		...
		if (chr.isMechinic()) { // 是机械师才加载
			...
			synchronized (iv) {
				for (IItem item : iv.list()) {
					if (((Item) item).getPosition() >= -1200
							&& ((Item) item).getPosition() <= -1100) {
						addItemInfo(mplew, item);
		...
		synchronized (iv) {
			for (IItem item : iv.list()) {
				...
				if (((Item) item).getPosition() > -1300
						&& ((Item) item).getPosition() <= -1200) {
					addItemInfo(mplew, item);
	...
	// 技能生成
	private static void addSkillRecord(MaplePacketLittleEndianWriter mplew,
			MapleCharacter chr) {
		...
		for (Entry<ISkill, MapleCharacter.SkillEntry> skill : skills.entrySet()) { // 遍历输出技能
			...
			if (id == 32001008 || id == 32001009 || id == 32001010
					|| id == 32001011) {
				x += 1;
			}
		}
		...
		for (Entry<ISkill, MapleCharacter.SkillEntry> skill : skills.entrySet()) { // 遍历输出技能
			...
			if (skillid == 32001008 || skillid == 32001009
					|| skillid == 32001010 || skillid == 32001011) {
				continue;
			} else {
				mplew.writeInt(skillid);// 技能ID
				mplew.writeInt(skill.getValue().skillevel);// 技能等级
				DateUtil.addSkillExpirationTime(mplew,
						skill.getValue().expiredate);
		...
		for (PlayerCoolDownValueHolder cooling : chr.getAllCooldowns()) {
			...
			int timeLeft = (int) (cooling.length + cooling.startTime - System
					.currentTimeMillis());
	...
	private static void addCharEntry(MaplePacketLittleEndianWriter mplew,
			MapleCharacter chr) {
	...
	// 任务信息
	private static void addQuestRecord(MaplePacketLittleEndianWriter mplew,
			MapleCharacter chr) {
		...
		for (MapleQuestStatus q : started) { // 检测是否接过任务
			...
			for (int kills : q.getMobKills().values()) { // 检测是否使用需要打怪的
				killStr += StringUtil.getLeftPaddedStr(String.valueOf(kills),
						'0', 3);
			}
		...
		for (MapleQuestStatus q : completed) { // 检测是否完成了任务
			...
			// //8位 即取封包内40 D5 C2 43 95 90 CB 01
		}

	}

	// 戒指信息?
	private static void addRingInfo(MaplePacketLittleEndianWriter mplew,
			MapleCharacter chr) {
		MapleInventory iv = chr.getInventory(MapleInventoryType.EQUIPPED);
		Collection<IItem> equippedC = iv.list();
		...
		for (IEquip ring : 恋人戒指) {
			...
			mplew.writeMapleNameString(ring.getPartnerName()); // 对方的名字
																// 且自动填充到13位 C2
																// B7 B7 C9 D7
																// A5 B0 FC CA
																// AE BA C5 00
		...
		if (结婚戒指 // && chr.isMarried()
			 ...
			 * mplew.writeMapleNameString(chr.getPartnerName()); //对方的名字
			 * 且自动填充到13位 C2 B7 B7 C9 D7 A5 B0 FC CA AE BA C5 00
			...
			mplew.writeMapleNameString(chr.getPartnerName()); // 对方的名字 且自动填充到13位
																// C2 B7 B7 C9
																// D7 A5 B0 FC
																// CA AE BA C5
																// 00
	...
	private static void addTeleportRockRecord(
		...
		mplew.writeShort(chr.area_data.size()
				+ chr.getAttribute().getToAreaData());
	...
	public static MaplePacket getCharInfo(MapleCharacter chr) {
		...
		mplew.write(HexTool
				.getByteArrayFromHexString("01 00 01 00 00 00 00 00 00 00"));
		mplew.writeInt(chr.getClient().getChannel() - 1);
		mplew.write(HexTool
				.getByteArrayFromHexString("00 00 00 00 00 00 01 00 00 00 00 01 00 00")); // 新增Int
		for (int i = 0; i < 3; i++) {
	...
	public static void AddSkillTree(MapleCharacter chr,
	...
	public static MaplePacket updatePlayerStats(
			List<Pair<MapleStat, Number>> stats, MapleCharacter chr) {

		return updatePlayerStats(stats, false, chr);
	}

	public static MaplePacket updatePlayerStats(
			List<Pair<MapleStat, Number>> stats, boolean itemReaction,
			MapleCharacter chr) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		...
		if (mystats.size() > 1) {
			Collections.sort(mystats,
					new Comparator<Pair<MapleStat, Number>>() {
						@Override
						public int compare(Pair<MapleStat, Number> o1,
								Pair<MapleStat, Number> o2) {
		...
		for (Pair<MapleStat, Number> statupdate : mystats) {
			if (statupdate.getLeft().getValue() >= 1) {
				if (statupdate.getLeft().getValue() == MapleStat.SKIN
						.getValue()) {
					mplew.write(statupdate.getRight().intValue());
				} else if (statupdate.getLeft().getValue() == MapleStat.LEVEL
						.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.Pk等级
								.getValue()) {
					mplew.write(statupdate.getRight().shortValue());
				} else if (statupdate.getLeft().getValue() == MapleStat.JOB
						.getValue()) {
					mplew.writeShort(statupdate.getRight().shortValue());
				} else if (statupdate.getLeft().getValue() == MapleStat.FACE
						.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.HAIR
								.getValue()) {
					mplew.writeInt(statupdate.getRight().intValue());
				} else if (statupdate.getLeft().getValue() == MapleStat.STR
						.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.DEX
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.INT
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.LUK
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.AVAILABLEAP
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.AVAILABLESP
								.getValue()) {
					if (statupdate.getLeft().getValue() == MapleStat.AVAILABLESP
							.getValue()) {
						addRemainingSp(mplew, chr);
					} else {
						mplew.writeShort(statupdate.getRight().shortValue());
					}
				} else if (statupdate.getLeft().getValue() == MapleStat.HP
						.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.MAXHP
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.MP
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.MAXMP
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.FAME
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.领袖气质
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.洞察力
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.意志
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.手技
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.感性
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.魅力
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.战斗经验值
								.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.保有BP
								.getValue()) {
					mplew.writeInt(statupdate.getRight().intValue());
				} else if (statupdate.getLeft().getValue() == MapleStat.MESO
						.getValue()
						|| statupdate.getLeft().getValue() == MapleStat.EXP
								.getValue()) {
	...
	// 传送到地图即切换地图
	public static MaplePacket getWarpToMap(int to, int spawnPoint,
			MapleCharacter chr) {

		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.WARP_TO_MAP.getValue());
		mplew.write(HexTool
				.getByteArrayFromHexString("01 00 01 00 00 00 01 00 00 00"));
		mplew.writeInt(chr.getClient().getChannel() - 1);
		mplew.writeInt(0);
	...
	// 召唤召唤兽
	public static MaplePacket spawnSpecialMapObject(MapleSummon summon,
			int skillLevel) {
	...
	public static void addCharLook2(MaplePacketLittleEndianWriter mplew,
			MapleCharacter chr, boolean mega) {
	...
	public static void addCharEquips(MaplePacketLittleEndianWriter mplew,
			MapleInventory equip, MapleCharacter chr) {
	 ...
	 */
	public static MaplePacket removeSpecialMapObject(MapleSummon summon,
			boolean animated) {
	...
	// 生成身上的装备方法
	protected static void addItemInfo(MaplePacketLittleEndianWriter mplew,
			IItem item) {
		addItemInfo(mplew, item, false, false, false);
	}

	protected static void addItemInfo(MaplePacketLittleEndianWriter mplew,
			IItem item, boolean zeroPosition, boolean leaveOut,
			boolean AddFormDorp) {
	...
	protected static void addItemInfo(MaplePacketLittleEndianWriter mplew,
			IItem item, boolean AddFormDorp) {
		addItemInfo(mplew, item, false, false, false, AddFormDorp, AddFormDorp);
	}

	protected static void addItemInfoNoPos(MaplePacketLittleEndianWriter mplew,
			IItem item) {
		addItemInfo(mplew, item, false, false, false, true, true);
	}

	private static void addItemInfo(MaplePacketLittleEndianWriter mplew,
			IItem item, boolean zeroPosition, boolean leaveOut, boolean cs,
			boolean AddFormDorp, boolean nopos) {// , boolean AddFormDorp
		/*
		 ...
		 */
		addNormalItemInfo(mplew, item, zeroPosition, leaveOut, AddFormDorp,
				nopos);// 普通装备
	}

	// 普通物品的生成
	private static void addNormalItemInfo(MaplePacketLittleEndianWriter mplew,
			IItem item, boolean zeroPosition, boolean leaveOut,
			boolean AddFormDorp, boolean nopos) {
		...
		mplew.writeLong(DateUtil.getFileTimestamp(item.getExpiration() == null ? FINAL_LOCKITEMTIME
				: item.getExpiration().getTime()));
		mplew.writeInt(-1);// 新增
		if (item.getType() == IItem.EQUIP) {
			...
			mplew.write(equip.getIdentify());// 1 未鉴定 0 C级 0x11 B级 0x12 A级 0x13
												// S级
			...
			mplew.writeLong(DateUtil
					.getFileTimestamp(equip.getUnlockTime() == null ? FINAL_TIME
							: equip.getUnlockTime().getTime()));
			mplew.writeInt(-1);
			/**
			 * V101新数据。
			 */
			...
			if (ii.isThrowingStar(item.getItemId())
				...
				mplew.write(HexTool
						.getByteArrayFromHexString("02 00 00 00 54 00 00 34"));
			}
		} else {
			...
			mplew.writeLong(DateUtil
					.getFileTimestamp(item.getExpiration() == null ? FINAL_TIME
							: item.getExpiration().getTime()));
			mplew.writeShort(0);// ??
	...
	public static MaplePacket serverNotice(int type, int channel,
			String message, boolean smegaEar) {
		return serverMessage(type, channel, message, false, smegaEar, null);
	}

	public static MaplePacket serverNotice(PhoneType type, int channel,
			String message, boolean smegaEar) {
		return serverMessage(type.getValue(), channel, message, false,
				smegaEar, null);
	}
	 ...
	 *
	 * @param type
	 *            类型 - [0 - 弹窗] [1 - 蓝色条] [2 - 红喇叭] [3 - 高质量喇叭] [5 - 系统公告] [8-9
	 *            - 道具喇叭] [0xf - 蛋糕喇叭] [0x0a - 缤纷喇叭] [0x10 - 馅饼喇叭] [0x11 -
	 *            心脏高级喇叭] [0x12 - 白骨高级喇叭] [0x13 - 红色抽奖公告(迷你蛋)] [0x14 - 绿色抽奖公告]
	 *            [0x16 - 服务器公告]
	 * @param channel
	 * @param message
	 * @param servermessage
	 ...
	 * @return
	 */
	public static MaplePacket serverMessage(int type, int channel,
			String message, boolean servermessage, boolean megaEar,
			Item showItem) {
	 ...
	 * @return
	 */
	public static MaplePacket getItemMega(int channel, String message,
			IItem item) {
	...
	public static MaplePacket getItemMegas(int channel, String message,
			IItem item, boolean megaEar) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	 ...
	 * @return
	 */
	public static MaplePacket getItemMegaI(int channel, String message,
			IItem item) {
	 ...
	 * @return
	 */
	public static MaplePacket getMegaphone(PhoneType type, int channel,
			String message, IItem item, boolean showEar) {
	 ...
	 * @return
	 */
	public static MaplePacket getMultiMega(int channel, List<String> message,
			boolean megaEar) {
	 ...
	 * @return
	 */
	public static MaplePacket getAvatarMega(MapleCharacter chr, int channel,
			int itemId, List<String> message, boolean ear) {
	 ...
	 */
	public static MaplePacket controlMonster(MapleMonster life,
			boolean newSpawn, boolean aggro) {
	 ...
	 */
	private static MaplePacket spawnMonsterInternal(MapleMonster life,
			boolean requestController, boolean newSpawn, boolean aggro,
			int effect, boolean makeInvis) {
		// if(show)//log.debug("调用的函数："+new Throwable().getStackTrace()[0]);
		// //显示调用的类 函数名 函数所在行
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		if (makeInvis) {
		...
		mplew.write(HexTool
				.getByteArrayFromHexString("E0 13 08 00 00 00 00 88"));// 97
		mplew.writeZero(40);
	...
	public static MaplePacket spawnFakeMonster(MapleMonster life, int effect) {
		...
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		// log.info((new
		// StringBuilder()).append("SPAWN_MONSTER_CONTROL 假的ID:").append(life.getObjectId()).toString());
		mplew.writeShort(SendPacketOpcode.SPAWN_MONSTER_CONTROL.getValue());
		...
		// mplew.write(-1);
		/*
		 * mplew.writeShort(-1);//97 mplew.write(0xF); mplew.writeZero(7);
		 * mplew.write(-1);//97
		 */
	 ...
	 */
	public static MaplePacket moveMonsterResponse(int objectid, short moveid,
	...
	public static MaplePacket moveMonsterResponse(int objectid, short moveid,
		...
		// 4F 01 [16 38 0C 00] [01 00] [00] [0A 00] [00] [00] 00 00 00 00
		// //093新增int
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	 ...
	 *
	 * @param gain
	 *            The amount of EXP gained.获取经验的数量
	 * @param inChat
	 *            In the chat box?
	 * @param white
	 *            White text or yellow?白色或者蓝色
	 * @return The exp gained packet.
	 */
	public static MaplePacket getShowExpGain(int gain, boolean inChat,
			boolean white) {

		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.SHOW_STATUS_INFO.getValue());
		mplew.write(3); // 0 = 显示未能找到掉在地上的部分金额 1 2=没效果 4 = 获得SP 3 = 经验, 5 = 人气,
						// 6 = 金币, 7 = 家族CP
	 ...
	 * @param animation
	 *            0 = disapear, 1 = fade out, 2+ = special
	 ...
	 */
	public static MaplePacket dropMesoFromMapObject(int amount, int itemoid,
			int dropperoid, int ownerid, Point dropfrom, Point dropto, byte mod) {
		return dropItemFromMapObjectInternal(amount, itemoid, dropperoid,
				ownerid, dropfrom, dropto, mod, true);
	}
	 ...
	 * @return The drop mesos packet.
	 */
	public static MaplePacket dropItemFromMapObject(int itemid, int itemoid,
			int dropperoid, int ownerid, Point dropfrom, Point dropto, byte mod) {
		return dropItemFromMapObjectInternal(itemid, itemoid, dropperoid,
				ownerid, dropfrom, dropto, mod, false);
	}
	 ...
	 */
	public static MaplePacket dropItemFromMapObjectInternal(int itemid,
			int itemoid, int dropperoid, int ownerid, Point dropfrom,
			Point dropto, byte mod, boolean mesos) {

		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		// 44 01 [01] [0F 00 00 00] [00] [8C 4A 0F 00] [E5 B3 29 00] [00] [DC
		// 00] [A2 00] [00 00 00 00] [DC 00] [A2 00] [00] [00] [00] [80] [05 BB
		// 46 E6 17 02 00 00]
		/*
		...
		mplew.writeZero(3);
		if (!mesos) {
			mplew.writeLong(DateUtil.getFileTimestamp(System
					.currentTimeMillis()));
		}
		if (mod != 2) {
	...
	public static MaplePacket spawnPlayerMapobject(final MapleCharacter chr) {
		...
		MapleGuildSummary gs = (chr.getGuildid() <= 0) ? null : chr.getClient()
				.getChannelServer().getGuildSummary(chr.getGuildid());

		String format = "[No.:%d]%s%s%s%s";
		format = String.format(format, chr.getId(), 会员,
				gs != null ? gs.getName() : "暂无家族", 转身, 飞升);
		if (gs != null) {
		...
		mplew.writeZero(3);
		mplew.writeInt((chr.getUsingMount() != null) ? chr.getUsingMount()
				.getUsingItemId() : 0);
		mplew.writeInt((chr.getUsingMount() != null) ? chr.getUsingMount()
				.getUsingSkillId() : 0);
		...
		mplew.writeInt(Math.min(chr.getInventory(MapleInventoryType.CASH)
				.countById(5110000), 100));
		...
		mplew.writeInt(chr.getMount() != null ? chr.getMount().getTiredness()
				: 0);
		mplew.write(0);
	...
	private static void addKuangLongStats(final MapleCharacter chr,
			MaplePacketLittleEndianWriter mplew) {
		boolean _hasSkill = chr.getBuffManager().hasBuff(61120007);
		if (_hasSkill) {
			mplew.write();
			mplew.writeShort(chr.getBuffManager().getBuffedValue(
					MapleBuffStat.KUANGLONG_JIANBI));
			mplew.writeInt(chr.getBuffManager().getBuffSource(
					MapleBuffStat.KUANGLONG_JIANBI));
			mplew.writeZero(3);
			mplew.writeInt(2);
			mplew.writeInt(5);
	...
	private static void AddPlayerStats(final MapleCharacter chr,
			MaplePacketLittleEndianWriter mplew) {
	...
	private static void addRings(MaplePacketLittleEndianWriter mplew,
			MapleCharacter chr, boolean isUpdateCharlook) {
		MapleInventory iv = chr.getInventory(MapleInventoryType.EQUIPPED);
		Collection<IItem> equippedC = iv.list();
		boolean 结婚戒指 = false;
		List<IEquip> 恋人戒指 = new ArrayList<IEquip>();
		...
		if (恋人戒指.size() > 0) {
			...
			for (IEquip ring : 恋人戒指) {
				...
				mplew.writeLong(ring.getPartnerUniqueId()); // 对方戒指的uniqueid
															// 对应Ring表里的partnerRingId
	...
	// 人物表情
	public static MaplePacket facialExpression(MapleCharacter from,
			int expression) {
	...
	public static MaplePacket movePlayer(int cid,
			List<LifeMovementFragment> moves) {
	...
	public static MaplePacket moveSummon(int cid, int oid, Point startPos,
			List<LifeMovementFragment> moves) {
	...
	public static MaplePacket moveDragon(int cid, Point startPos,
			List<LifeMovementFragment> moves) {
	...
	public static MaplePacket moveMonster(boolean useskill, int skill,
			int skill_1, int skill_2, int skill_3, int skill_4, int oid,
			Point startPos, Point nextPos, List<LifeMovementFragment> moves) {
	...
	private static void serializeMovementList2(LittleEndianWriter lew,
			List<LifeMovementFragment> moves) {
		lew.write(moves.size()); // 移动的次数
		for (LifeMovementFragment move : moves) {
			move.serialize(lew);
	...
	public static MaplePacket summonAttack(MapleCharacter chr, int summonOid,
			int newStance, List<SummonAttackEntry> allDamage,
			int numAttackMonsturAndCount) {

		// 45 01 [21 DF 49 00] [C2 7C 05 00] A6 84 01 43 19 A3 00 07 0A 3C 00 00
		// 00
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	// 召唤兽打怪
	public static MaplePacket damageSummon(int cid, int summonOid, int damage,
			int monsterIdFrom, int unkByte) {
	...
	public static MaplePacket closeRangeAttack(
			DamageParseHandler.AttackInfo attackInfo, MapleCharacter chr) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.CLOSE_RANGE_ATTACK.getValue());
		if (attackInfo.skill == 4211006) { // 金钱炸弹
			addMesoExplosion(mplew, chr.getId(), attackInfo.skill,
					attackInfo.stance, attackInfo.numAttackedAndDamage, 0,
					attackInfo.allDamage, attackInfo.speed, attackInfo.pos);
		} else {
	...
	public static MaplePacket rangedAttack(
			DamageParseHandler.AttackInfo attackInfo, MapleCharacter chr, int p) {
	...
	private static void addAttackBody(LittleEndianWriter lew,
			MapleCharacter chr, DamageParseHandler.AttackInfo attackInfo,
			int projectile) {
		...
		for (Pair<Integer, List<Integer>> oned : attackInfo.allDamage) {
			if (oned.getRight() != null) {
				...
				for (Integer eachd : oned.getRight()) {
					lew.writeInt(attackInfo.skill == 3221007 ? eachd.intValue() + 0x80000000
							: eachd.intValue());
				}
			}
		}
	...
	private static void addMesoExplosion(LittleEndianWriter lew, int cid,
			int skill, int stance, int numAttackedAndDamage, int projectile,
			List<Pair<Integer, List<Integer>>> damage, int speed, int pos) {

		// BC 00 90 E5 2F 00 00 5A 1A 3E 41 40 00 00 3F 00 03 0A 00 00 00 00
		// //078
		lew.writeInt(cid);
	...
	// NPC商店
	public static MaplePacket getNPCShop(MapleClient c, int sid,
		...
		for (MapleShopItem item : items) {
			...
			if (!ii.isThrowingStar(item.getItemId())
					&& !ii.isBullet(item.getItemId())) { // 非海星标和子弹
				mplew.writeZero(5);
	 ...
	 */
	public static MaplePacket addInventorySlot(MapleInventoryType type,
			IItem item) {
	...
	// 更新背包位置
	public static MaplePacket updateInventorySlot(MapleInventoryType type,
			IItem item) {
	...
	// 移动背包物品
	public static MaplePacket moveInventoryItem(MapleInventoryType type,
			short src, short dst) {
		return moveInventoryItem(type, src, dst, (byte) -1);
	}

	public static MaplePacket moveInventoryItem(MapleInventoryType type,
			short src, short dst, byte equipIndicator) {
	...
	public static MaplePacket moveAndMergeWithRestInventoryItem(
			MapleInventoryType type, short src, short dst, short srcQ,
			short dstQ) {
	...
	// 清除背包物品
	public static MaplePacket clearInventoryItem(MapleInventoryType type,
			short slot, boolean fromDrop) {
	...
	public static MaplePacket InventoryActions(boolean enable,
			List<InventoryActionsInfo> actions) {
	...
	public static MaplePacket scrolledItem(IItem scroll, IItem item,
			boolean destroyed) {
	...
	public static MaplePacket getScrollEffect(int chr,
			ScrollResult scrollSuccess, boolean legendarySpirit, int itemid,
			int equipid) {
	 ...
	 * @return
	 */
	public static MaplePacket removeItemFromMap(int oid, int animation,
			int cid, boolean pet, int slot) {
	...
	// 丢下道具
	public static MaplePacket dropInventoryItem(MapleInventoryType type,
			short src) {
	...
	// 更新掉落装备
	public static MaplePacket dropInventoryItemUpdate(MapleInventoryType type,
			IItem item) {
	...
	public static MaplePacket damagePlayer(int skill, int monsteridfrom,
			int cid, int damage) {
		return damagePlayer(skill, monsteridfrom, cid, damage, 0, 0, false, 0,
				false, 0, 0, 0);
	}

	// 玩家的伤害
	public static MaplePacket damagePlayer(int skill, int monsteridfrom,
			int cid, int damage, int fake, int direction, boolean pgmr,
			int pgmr_1, boolean is_pg, int oid, int pos_x, int pos_y) {
		// D9 00 [60 B5 6C 00] FF [01 00 00 00] [05 87 01 00] 01 00 00 01 00 00
		// 00
		// F5 00 [9F 1E 44 00] FF [30 04 00 00] [00] [17 7C 92 00] [00 00 00 00]
		// [30 04 00 00]
		// 01 01 [4B DF 22 00] FF 01 00 00 00 00 04 87 01 00 00 00 00 00 00 00
	...
	public static MaplePacket charInfo(MapleCharacter chr) {
		...
		String guildName = "-";
		String allianceName = "";
		MapleGuildSummary gs = chr.getClient().getChannelServer()
				.getGuildSummary(chr.getGuildid());
		if (chr.getGuildid() > 0 && gs != null) {
			guildName = gs.getName();
			try {
				MapleAlliance alliance = chr.getClient().getChannelServer()
						.getWorldInterface().getAlliance(gs.getAllianceId());
				if (alliance != null) {
		...
		for (MaplePet pet : pets) {
			...
			if (chr.getInventory(MapleInventoryType.EQUIPPED).getItem(
					(byte) -114) != null) {
				mplew.writeInt(chr.getInventory(MapleInventoryType.EQUIPPED)
						.getItem((byte) -114).getItemId());
			} else {
				mplew.writeInt(0);
			}
			mplew.writeInt(-1);
		...
		if (chr.getMount() != null
				...
				&& chr.getInventory(MapleInventoryType.EQUIPPED)
						.getItem((byte) -18).getItemId() == chr.getMount()
						.getItemId()) {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("SELECT * FROM wishlist WHERE charid = ?");
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT * FROM wishlist WHERE charid = ? ORDER BY sn DESC");
			ps.setInt(1, chr.getId());
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
		...
		IItem 正在带的勋章 = chr.getInventory(MapleInventoryType.EQUIPPED).getItem(
				(byte) -26);
		if (正在带的勋章 != null) {
		...
		// mplew.writeShort(x);//questid 对应上面的size 发送的是勋章任务的id
		// 去QuestInf.img.xml里找
		mplew.write(0); // 领袖气质
		mplew.write(0); // 洞察力
		mplew.write(0); // 意志
	 ...
	 * @return
	 */
	public static MaplePacket updateQuestInfo(MapleCharacter c, short quest,
			int npc, byte progress) {
	...
	// 更新完成任务
	public static MaplePacket updateQuestFinish(short quest, int npc,
			short nextquest) {
	...
	public static <E extends Buffstat> void maskBuffStats(List<E> buffstats,
			MaplePacketLittleEndianWriter mplew, boolean foreign, boolean give) {
		maskBuffStats(buffstats, mplew, foreign, give, null);
	}

	public static <E extends Buffstat> void maskBuffStats(List<E> buffstats,
			MaplePacketLittleEndianWriter mplew, boolean foreign, boolean give,
			MapleForeignBuffSkill foreignBuffSkill) {
		for (int i = GameConstants.MAX_STATUS; i > 0; i--) {
			int value = 0;
			for (E buffstat : buffstats) {
				if (buffstat.getPosition() == i
						&& (foreignBuffSkill == null || foreignBuffSkill
								.hasStat(buffstat))) {
	...
	public static void maskBuffStats(MaplePacketLittleEndianWriter mplew,
			List<Pair<MapleBuffStat, Integer>> buffstats, boolean foreign,
			boolean give) {
	...
	public static void maskBuffStats(MaplePacketLittleEndianWriter mplew,
			List<Pair<MapleBuffStat, Integer>> buffstats, boolean foreign,
			boolean give, MapleForeignBuffSkill foreignBuffSkill) {
		for (int i = GameConstants.MAX_STATUS; i > 0; i--) {
			int value = 0;
			for (Pair<MapleBuffStat, Integer> pair : buffstats) {
				if (pair.left.getPosition() == i
						&& (foreignBuffSkill == null || foreignBuffSkill
								.hasStat(pair.left))) {
	...
	public static void appledBuffStats(List<MapleBuffStat> buffstats,
			MaplePacketLittleEndianWriter mplew, int skillid) {
		...
		for (int i = 0; i < buffstats.size(); i++) {
			...
			case MAPLE_WARRIOR:
				...
				break;
			/*
			 * case 火焰咆哮_攻击力: i = buffstats.size(); mplew.writeInt(0); break;
			 * case 月光祝福_命中率: case 幸运的保护_HP: i = buffstats.size();
			 * mplew.writeLong(0); break;
			 */
			}
	...
	public static void appledBuffStats(MaplePacketLittleEndianWriter mplew,
			List<Pair<MapleBuffStat, Integer>> buffstats) {
	...
	public static MaplePacket giveBuff(int buffid, int bufflength,
			List<Pair<MapleBuffStat, Integer>> statups, MapleCharacter chr) {
	...
	public static MaplePacket cancelBuff(List<MapleBuffStat> statups,
		...
		maskBuffStats(statups, mplew, false, false);
	...
	public static MaplePacket giveForeignBuff(MapleCharacter c,
			List<Pair<MapleBuffStat, Integer>> statups, MapleStatEffect effect) {

		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		// [C5 00] [12 01 2B 00] [00 00 00 00 00 00 00 00] [00 00 00 00 80 00 00
		// 00] [0A 00] [00 00] [00] //077
		// [C5 00] [1F 01 2B 00] [00 00 00 00 00 00 00 00] [00 00 00 00 80 00 00
		// 00] [0A 00] [00 00] [00]
		// [C9 00] [1D CE 4A 00] [00 00 00 00 00 00 00 00] [02 00 00 00 80 00 00
		// 00] [28 E8] [03 00] [00 00] [00]
		mplew.writeShort(SendPacketOpcode.GIVE_FOREIGN_BUFF.getValue());
		mplew.writeInt(c.getId());
		maskBuffStats(mplew, statups, true, true, effect.getForeign());
		for (Pair<MapleBuffStat, Integer> statup : statups) {
			/*
			 * if (!statup.left.isFoot()) { if (effect.getSourceId() == 2003516
			 * && statup.getLeft().equals(MapleBuffStat.SPEED)) {
			 * mplew.write(((Integer) statup.getRight()));
			 * mplew.writeShort(statups.get(2).getRight());
			 * mplew.writeInt((effect.isSkill() ? effect.getSourceId() :
			 * -effect.getSourceId()));//技能ID break; } short a = ((Integer)
			 * statup.getRight()).shortValue(); if (effect.isShadowPartner()) {
			 * a = (short) effect.getX(); } mplew.writeShort(a == 1002 ? 1002 :
			 * a); mplew.writeInt((effect.isSkill() ? effect.getSourceId() :
			 * -effect.getSourceId()));//技能ID
			 * mplew.writeInt(effect.getDuration());//持续时间 }
			 */
			if (effect.getForeign().hasStat(statup.left)) {
				effect.getForeign().applyStat(mplew, statup.getLeft(),
						statup.getRight());
			}
		}
		if (effect.getForeign().hasStats()) {
			mplew.writeInt((effect.isSkill() ? effect.getSourceId() : -effect
					.getSourceId()));// 技能ID
		}
	...
	private static void addForeignData(MapleCharacter c,
			List<Pair<MapleBuffStat, Integer>> statups, MapleStatEffect effect,
			MaplePacketLittleEndianWriter mplew) {
	...
	private static void addForeignStats(MapleCharacter c,
			List<Pair<MapleBuffStat, Integer>> statups, MapleStatEffect effect,
			MaplePacketLittleEndianWriter mplew) {
	...
	public static MaplePacket cancelForeignBuff(int cid,
			List<MapleBuffStat> statups, MapleCharacter chr,
	...
	private static void appdCancelForeignBuff(int skillid,
			MaplePacketLittleEndianWriter mplew) {
		int a = cancelBuff(skillid);
		if (MapleStatEffect.isMonsterRiding(skillid)) {
			mplew.write(1);
	...
	// 减益buff,怪给角色
	public static MaplePacket giveDebuff(long mask,
	...
	public static MaplePacket giveForeignDebuff(int cid, long mask,
			List<Pair<MapleDisease, Integer>> statups, MobSkill skill) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.GIVE_FOREIGN_BUFF.getValue());
		mplew.writeInt(cid);
	...
	public static MaplePacket showGuildRanks3(int npcid, ResultSet rs)
			throws SQLException {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.GUILD_OPERATION.getValue());
		mplew.write(0x50);
	...
	public static MaplePacket showGuildRanks8(int npcid, ResultSet rs)
			throws SQLException {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.GUILD_OPERATION.getValue());
		mplew.write(0x50);
	...
	public static MaplePacket showGuildRanks9(int npcid, ResultSet rs)
			throws SQLException {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.GUILD_OPERATION.getValue());
		mplew.write(0x50);
	...
	public static MaplePacket showGuildRanks2(int npcid, ResultSet rs)
			throws SQLException {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.GUILD_OPERATION.getValue());
		mplew.write(0x50);
	...
	public static MaplePacket showGuildRanks5(int npcid, ResultSet rs)
			throws SQLException {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.GUILD_OPERATION.getValue());
		mplew.write(0x50);
	...
	// 玩家商店聊天
	public static MaplePacket getPlayerShopChat(MapleCharacter c, String chat,
			byte slot) {
	 ...
	 * @return
	 */
	public static MaplePacket getPlayerShop(MapleClient c,
			MaplePlayerShop shop, boolean owner) {
	...
	// 交易
	public static MaplePacket getTradeStart(MapleClient c,
			MapleInteractionRoom trade, byte number, boolean Istrade) {

		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

		mplew.writeShort(SendPacketOpcode.PLAYER_INTERACTION.getValue());
		mplew.writeSome(0xB, Istrade ? 0x4 : 0x3, 0x2);
	...
	public static MaplePacket getNPCTalk(int npc, byte msgType, String talk,
			String endBytes) {
	...
	public static MaplePacket getNPCTalkStyle(int npc, String talk,
			int styles[], int card) {
	...
	public static MaplePacket getNPCTalkNum(int npc, String talk, int def,
			int min, int max) {
	...
	public static MaplePacket showForeignEffect(int cid, int effect) {
		...
		mplew.write(effect); // 0 = Level up, 8 = ?(RESIST), 0x0c = job change,
								// 0x0d = Quest Complete
		return mplew.getPacket();
	}
	...
	public static MaplePacket 综合技能状态(int cid, int skillid, int effectid,
			int skillie) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.SHOW_FOREIGN_EFFECT.getValue());
		mplew.writeInt(cid); // ?
	...
	public static MaplePacket showBuffeffect(int cid, int skillid,
			int effectid, byte direction, int skilllevel) {
	...
	public static MaplePacket showBuffeffect(int cid, int skillid,
			int effectid, byte direction, boolean morph) {
	...
	public static MaplePacket 玩家的龙之魂(int cid, int skillid, int effectid,
			int skillid_Level) {
		if (show) {
	...
	public static MaplePacket showOwnBuffEffect(int skillid, int effectid,
		...
		mplew.write(skilllevel); // probably buff level but we don't know it and
									// it doesn't really matter
		return mplew.getPacket();
	}

	public static MaplePacket showOwnBuffEffect_(int skillid, int effectid,
			int skilllevel) {
	...
	public static MaplePacket showBerserk(int cid, int skilllevel,
			boolean Berserk) { // 恶龙附身？
	 ...
	 * @return
	 */
	public static MaplePacket updateSkill(int skillid, int level,
			int masterlevel, Timestamp expiration) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static MaplePacket updateSkill(int[] skillid, int level[],
			int[] masterlevel, Timestamp[] expiration) {
		if (skillid != null && level != null && masterlevel != null
				&& expiration != null && (skillid.length == level.length)) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.UPDATE_SKILLS.getValue());
			mplew.writeShort(1);
	...
	public static MaplePacket updateQuestMobKills(MapleQuestStatus status) {
		...
		for (int kills : status.getMobKills().values()) {
			killStr += StringUtil.getLeftPaddedStr(String.valueOf(kills), '0',
					3);
		}
		mplew.writeMapleAsciiString(killStr);
		mplew.writeInt(0);
	...
	// 仓库
	public static MaplePacket getStorage(int npcId, byte slots, long meso,
			MapleStorage storage) {
		...
		List<IItem> current = storage.AllEnumMap()
				.get(MapleInventoryType.EQUIP);
	...
	// 存入仓库
	public static MaplePacket storeStorage(byte slots, MapleInventoryType type,
	...
	// 仓库取出
	public static MaplePacket takeOutStorage(byte slots,
			MapleInventoryType type, Collection<IItem> items) {
	...
	// 显示BOSS HP
	public static MaplePacket showBossHP(int oid, int currHP, int maxHP,
	...
	// 人气反馈
	public static MaplePacket giveFameResponse(int mode, String charname,
			int newfame) {
	...
	public static MaplePacket partyCreated(MapleParty party) {
		...
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		// 3D 00 [08] [DC 40] [01 00] [FF C9 9A 3B] [FF C9 9A 3B] [00 00 00 00]
		// 6D 1D 01 25
	...
	public static MaplePacket partyInvite(MapleCharacter from) {
		...
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		// 3C 00 [04] [96 4B 00 00] [0A 00 CB A7 C6 F8 B5 C4 BF F1 CF E8] 78 00
		// 00 00 B2 01 00 00 00
		mplew.writeShort(SendPacketOpcode.PARTY_OPERATION.getValue());
		mplew.write(4);
	...
	public static MaplePacket updateParty(int forChannel, MapleParty party,
	...
	// 组队端口
	public static MaplePacket partyPortal(int townId, int targetId,
			Point position) {
	 ...
	 * 编写头MArK位数据,更高效的写入
	 */
	public static void markMonsterStat(MaplePacketLittleEndianWriter mplew,
			Map<MapleMonsterStat, Integer> stats) {
		// 编辑mark到数组
	...
	// 应用怪物状态 添加怪物状态
	public static MaplePacket applyMonsterStatus(MapleMonster mob,
			MapleCharacter chr, int oid, Map<MapleMonsterStat, Integer> stats,
			int skill, int delay, MobSkill mobskill) {
	...
	// 取消怪物状态
	public static MaplePacket cancelMonsterStatus(int cid, int oid,
			Map<MapleMonsterStat, Integer> stats, int skillid) {
	...
	// 召唤LOVE
	public static MaplePacket spawnLove(int oid, int itemid, String name,
			String msg, Point pos, int ft) {
	...
	public static MaplePacket spawnMist(MapleMist mist) {
		...
		mplew.writeInt(mist.isMobMist() ? mist.getMobOwner().getId() : mist
				.getOwner().getId());
		mplew.writeInt(mist.isItemMist() ? mist.getSourceId() : mist
				.isMobMist() ? mist.getMobSkill().getSkillId() : mist
				.getSourceSkill().getId());
		mplew.write(mist.isItemMist() ? 0 : mist.isMobMist() ? mist
				.getMobSkill().getSkillLevel() : mist.getOwner().getSkillLevel(
				mist.getSourceSkill()));
		...
		mplew.writeZero(16);
		// //log.debug("烟雾值1: "+(mist.isMobMist() ? 0 : mist.isPoison() ? 1 :
		// mist.isItemMist() ? 3 : 2));
		// ystem.out.println("itemid: "+mist.getSourceId());
		// //log.debug("烟雾值2: "+(mist.isMobMist() ? 0 : mist.isItemMist() ? 3 :
		// 8));
	...
	public static MaplePacket removeMist(MapleMist mist) {
		...
		mplew.writeInt(mist.getObjectId());
		/*
		 * if (!mist.isMobMist() && mist.getSourceSkill().getId() == 2111003)
		 * {// 致命毒雾 mplew.write(0); }
		 */
	...
	public static MaplePacket requestBuddylistAdd(int cidFrom, String nameFrom,
			int charlevel, int channel, int jobid) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	// 开始地图效果
	public static MaplePacket startMapEffect(String msg, int itemid,
			boolean active) {
	...
	public static void addThread(MaplePacketLittleEndianWriter mplew,
	...
	public static MaplePacket showThread(int localthreadid, ResultSet threadRS,
		...
		if (repliesRS != null) {
			...
			for (i = 0; i < replyCount && repliesRS.next(); i++) {
				...
				mplew.writeLong(DateUtil.getFileTimestamp(repliesRS
						.getLong("timestamp")));
	...
	// 技能效果
	public static MaplePacket skillEffect(MapleCharacter from, int skillId,
			int level, byte flags, int speed, int op) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static MaplePacket addMessengerPlayer(String from,
			MapleCharacter chr, int position, int channel) {// 招待添加玩家
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static MaplePacket updateMessengerPlayer(String from,
			MapleCharacter chr, int position, int channel) {// 更新招待玩家
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		...
		// mplew.writeMapleAsciiString(from);
		/*
		 * mplew.write(channel); mplew.write(0x00);
		 */
	...
	public static MaplePacket getcashinfo(MapleClient c, boolean MTS) {
		...
		mplew.writeShort(MTS ? SendPacketOpcode.MTS_OPEN.getValue()
				: SendPacketOpcode.CS_OPEN.getValue());
	...
	public static MaplePacket getshopitemsinfo(MapleClient c, boolean MTS) {
		...
		if (MTS) {
			mplew.writeInt(0);
			mplew.write(HexTool
					.getByteArrayFromHexString("77 75 68 61 6F 88 13 00 00 07 00 00 00 2C 01 00 00 18 00 00 00 A8 00 00 00 C0 E8 EE 37 C0 E8 CB 01"));
		} else {
			mplew.write(Setting.getcashpacket());
		}
	...
	// 现金装备
	public static void toCashItem(MaplePacketLittleEndianWriter mplew, int sn,
			int type1, int type2) {
	...
	// 购买商城物品
	public static MaplePacket showBoughtCSItem(MapleClient c,
			MapleCSInventoryItem item) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		...
		mplew.writeLong(item.getExpire() == null ? DateUtil
				.getFileTimestamp(FINAL_TIME) : DateUtil.getFileTimestamp(item
				.getExpire().getTime()));
		/*
	...
	// 商城送礼
	public static MaplePacket getGiftFinish(String name, int itemid,
			short amount) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.CS_OPERATION.getValue());
		mplew.write(0x6A);
	...
	public static MaplePacket showCannotToMe() { // 显示 不能向自己的帐号赠送
		// 23 01 3E A7 C0 62 00 00 00 00 00 01 FA 3C 00 00 00 00 00 CA 4A 0F 00
		// 2B 2D 31 01 01 00 00 00 60 4B 40 00 F4 AC 77 00 00 00 00 00 CD 77 3C
		// AB 25 CA 01 00 00 00 00 00 00 00 00
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.CS_OPERATION.getValue());
		mplew.write(0x6F);// 5E
	...
	public static MaplePacket sendWishList(int characterid) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		// 7E 01 48 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
		// 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
		// 9C 01 49 01 00 FE 9A 6F 01 00 00 00 00 DC F5 19 00 00 00 00 00 A7 A6
		// 4F 00 03 CF 7B 05 01 00 C3 B0 CF D5 B5 BA D4 CB D3 AA D4 B1 00 00 80
		// 05 BB 46 E6 17 02 00 00
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT sn FROM wishlist WHERE charid = ? LIMIT 10");
	...
	public static MaplePacket updateWishList(int characterid) {
		// 9C 01 4D 46 9F 98 00 58 9F 98 00 57 9F 98 00 59 9F 98 00 20 4A CB 01
		// 40 9F 98 00 CF 1F 9A 00 00 00 00 00 00 00 00 00 00 00 00 00
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	// 宠物移动
	public static MaplePacket movePet(int cid, Point startPos, int slot,
			List<LifeMovementFragment> moves, int xb, int yb) {
		// if(show)//log.debug("调用的函数："+new Throwable().getStackTrace()[0]);
		// //显示调用的类 函数名 函数所在行
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static MaplePacket petChat(int cid, int slot, int act, String text) {
		...
		// E0 00 [37 3A 0A 00] [01 00 00 00] [01 00] 0F 00 CE D8 E0 E0 21 20 CE
		// D8 E0 E0 E0 E0 E0 E0 21
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	// 宠物指令
	public static MaplePacket commandResponse(int cid, int slot, int animation,
			boolean success) {
	...
	// 召唤兽技能
	public static MaplePacket summonSkill(int cid, int summonSkillId,
			int newStance) {
	...
	public static MaplePacket getPlayerNPC(int id) {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("SELECT * FROM playernpcs WHERE id = ?");
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
		...
		try {
			PreparedStatement ps = con
					.prepareStatement("SELECT * FROM playernpcs_equip WHERE npcid = ? AND type = 0");
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
		...
		try {
			PreparedStatement ps = con
					.prepareStatement("SELECT * FROM playernpcs_equip WHERE npcid = ? AND type = 1");
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				mplew.writeInt(rs.getInt("equipid"));
	...
	public static MaplePacket showNotes(ResultSet notes, int count)
		...
		for (int i = 0; i < count; i++) {
			...
			mplew.writeLong(DateUtil.getFileTimestamp(notes
					.getLong("timestamp")));
			mplew.write(0);
			notes.next();
		}
	...
	public static MaplePacket updateAriantPQRanking(String name, int score,
			boolean empty) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	// 显示所有的角色信息
	public static MaplePacket showAllCharacterInfo(int worldid,
	...
	// 送货员
	public static MaplePacket sendDuey(byte operation,
			List<MapleDueyActions> packages) {
		...
		if (operation == 9) {
			...
			for (MapleDueyActions dp : packages) {
				...
				mplew.writeLong(DateUtil.getFileTimestamp(dp
						.sentTimeInMilliseconds()));
				mplew.writeLong(0);
				for (int i = 0; i < 48; i++) { // message is supposed to be
												// here...
	...
	public static MaplePacket sendMTS(List<MTSItemInfo> items, int tab,
			int type, int page, int pages) {
	...
	public static MaplePacket startMonsterCarnival(int team) {
		...
		mplew.write(team);
		mplew.write(HexTool
				.getByteArrayFromHexString("00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00"));
		return mplew.getPacket();
	}

	// 玩家死亡信息
	public static MaplePacket playerDiedMessage(String name, int lostCP,
			int team) { // CPQ
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static MaplePacket CPUpdate(boolean party, int curCP, int totalCP,
			int team) { // CPQ
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		if (!party) {
	...
	// 缩地石
	public static MaplePacket TrockRefreshMapList(MapleCharacter chr,
			byte rocktype) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	// return mplew.getPacket();
	// }
	// public static MaplePacket giveForeignEnergyCharge(int cid, int
	// barammount) {
	//
	// MaplePacketLittleEndianWriter mplew = new
	// MaplePacketLittleEndianWriter();
	...
	public static MaplePacket sendInteractionBox(MapleCharacter c) {
		...
		// BA 00 [46 08 01 00] [00 01 01] [45 4B 4C 00] [08 00 B0 D7 C9 AB CD C3
		// D7 D3] [8E 80] [01] 00 00 00 00 00 E1 00 22 00 04 58 00 00 00
		// BA 00 [91 C2 00 00] [00 01 01] [64 4B 4C 00] [08 00 D8 BC D1 C5 C3 EF
		// B5 FB] [81 01] [00] 00 00 00 00 00 ED 01 04 00 04 42 00 00 00
		// BA 00 [25 81 00 00] [00 01 01] [78 4B 4C 00] [06 00 BE DE D7 EC C4
		// F1] [95 3E] [00] 00 00 00 00 00 2E 02 04 00 16 3D 00 00 00
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.UPDATE_CHAR_BOX.getValue());
		mplew.writeInt(c.getId());
	...
	public static MaplePacket getInteraction(MapleCharacter chr,
			boolean firstTime) {
		...
		if (type == 1) {// 雇佣商店 的type是1 是程序内设的type
			...
			mplew.write(HexTool.getByteArrayFromHexString("0A 04 04"));// 05 04
																		// 04
		} else if (type == 3) {
			mplew.write(HexTool.getByteArrayFromHexString("0A 02 02"));// 05 02
																		// 02
		} else if (type == 4) {
			mplew.write(HexTool.getByteArrayFromHexString("0A 01 02"));// 05 01
																		// 02
		}
		...
		if (type == 1) {
			...
			mplew.writeMapleAsciiString(ips.getOwnerName());
			if (ips.isOwner(chr)) {
				mplew.writeInt(((HiredMerchant) ips).getTimeLeft()); // contains
																		// timing,
																		// suck
																		// my
																		// dick
	...
	public static MaplePacket getMiniBoxFull() { // 满了？
		...
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.PLAYER_INTERACTION.getValue());
		mplew.writeShort(PlayerInteractionHandler.Action.ENTER_RESPOSE
				.getCode());
		mplew.write(2);
		return mplew.getPacket();
	}
	...
	public static MaplePacket sendMaplePolice(int reason, String reasoning,
			int duration) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static MaplePacket updateMount(int charid, MapleMount mount,
			boolean levelup) {
		return updateMount(charid, mount.getLevel(), mount.getExp(),
				mount.getTiredness(), levelup);
	}

	public static MaplePacket updateMount(int charid, int newlevel, int newexp,
			int tiredness, boolean levelup) {
	...
	public static MaplePacket getCSInventory(MapleCharacter chr) {
		...
		for (MapleCSInventoryItem citem : allitems) {
			...
			mplew.writeLong(citem.getExpire() == null ? DateUtil
					.getFileTimestamp(FINAL_TIME) : DateUtil
					.getFileTimestamp(citem.getExpire().getTime()));
			mplew.writeLong(0);
	...
	public static MaplePacket getCSGifts(MapleCharacter chr) {
		...
		mplew.writeShort(SendPacketOpcode.CS_OPERATION.getValue());
		mplew.write(0x5F);// 4f
		Collection<MapleCSInventoryItem> inv = chr.getCSInventory()
				.getCSGifts().values();
	 ...
	 *                 对不起，正在准备冒险岛ONLINE商城 0x04 (4) - 因为有地气阻挡，无法接近。 0x05 (5) -
	 *                 无法进行瞬间移动的地区。 0x06 (6) - 因为有地气阻挡，无法接近。 0x07 (7) -
	 *                 你因不当行为，而遭游戏管理员禁止攻击
	 *                 ，禁止获取经验值和金币，禁止交易，禁止丢弃道具，禁止开启个人商店与精灵商人，禁止组队
	 *                 ，禁止使用拍卖系统，因此无法使用改功能。
	 * @返回数据包后通知。
	 *
	 * @param type
	...
	public static MaplePacket loadFamily(MapleCharacter player) {
		...
		String[] title = { "直接移动到学院成员身边", "直接召唤学院成员", "我的爆率 1.5倍(15分钟)",
				"我的经验值 1.5倍(15分钟)", "学院成员的团结(30分钟)", "我的爆率 2倍(15分钟)",
				"我的经验值 2倍(15分钟)", "我的爆率 2倍(30分钟)", "我的经验值 2倍(30分钟)",
				"我的组队爆率 2倍(30分钟)", "我的组队经验值 2倍(30分钟)" };
		String[] description = {
				"[对象] 我\n[效果] 直接可以移动到指定的学院成员身边。",
	...
	public static MaplePacket getFamilyInfo(MapleCharacter chr) {
		...
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.OPEN_FAMILY.getValue()); // who cares
																	// what
																	// header is
	...
	public static MaplePacket sendEngagementRequest(String name) {
		...
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.FAMILY_ACTION.getValue()); // <name>
																		// has
																		// requested
																		// engagement.
																		// Will
	...
	public static MaplePacket sendFamilyJoinResponse(boolean accepted,
			String added) {
	...
	public static MaplePacket sendTV(MapleCharacter chr, List<String> messages,
			int type, MapleCharacter partner, int delay) {
	...
	public static MaplePacket shenlong(int i) {
		...
		mplew.write(HexTool
				.getByteArrayFromHexString("DC 05 00 00 90 5F 01 00 DC 05 00 00 9B 00 00 00"));
		return mplew.getPacket();
	}
	...
	public static MaplePacket shenlong2(int i) {
		...
		mplew.writeShort(i); // 274/275 暗/亮 124/125 龙珠9个齐全,开始召唤神龙
		mplew.write(HexTool
				.getByteArrayFromHexString("02 CB 06 00 00 FB 44 00 00"));
		return mplew.getPacket();
	}
	...
	public static MaplePacket 机械技能特效(int type, int value, int skillid,
			int skilllevel, int unknow, int charlevel) {
	...
	// 给别人看的
	public static MaplePacket 机械技能特效(int cid, int type, int value, int skillid,
			int skilllevel, int unknow, int charlevel) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.SHOW_ITEM_GAIN_INCHAT.getValue());
		mplew.writeInt(cid);
	 ...
	 * @return
	 */
	public static MaplePacket 传送门的效果(int cid, Point pos, boolean first) { // 这里是假的
																			// 一个显示出来效果而已
	...
	public static MaplePacket 怪物炸弹效果(MapleCharacter chr, int x, int y,
	...
	public static MaplePacket getNPCTalkText(int npc, String talk, int Least,
			int max) {
	...
	public static MaplePacket spawnAndroid(MapleCharacter cid,
		...
		for (short i = -1200; i > -1207; i--) {
			final IItem item = cid.getInventory(MapleInventoryType.EQUIPPED)
					.getItem(i);
	...
	public static MaplePacket moveAndroid(int cid, Point pos,
		...
		mplew.writeInt(cid);
		mplew.writePos(pos);
		mplew.writeInt(Integer.MAX_VALUE); // time left in milliseconds? this
											// appears to go down...slowly
											// 1377440900
	...
	public static MaplePacket showAndroidEmotion(int cid, int animation) {
		...
		mplew.writeInt(cid);
		mplew.write(0);
		mplew.write(animation); // 1234567 = default smiles, 8 = 呕吐, 11 = kiss,
								// 14 = googly eyes, 17 = wink...//42
		return mplew.getPacket();
	}
	...
	public static MaplePacket getAttackEffect(int chrId, int skillId,
			List<Integer> monsters, int type) {
