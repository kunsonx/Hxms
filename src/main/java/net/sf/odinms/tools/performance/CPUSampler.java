...
public class CPUSampler {
	...
	public void save(Writer writer, int minInvocations, int topMethods)
		...
		for (int i = 0; i < topMethods
				&& i < topConsumers.getTopConsumers().size(); i++) {
			builder.append(topConsumers.getTopConsumers().get(i)
	...
	private void consumeStackTraces(Map<Thread, StackTraceElement[]> traces) {
		for (Entry<Thread, StackTraceElement[]> trace : traces.entrySet()) {
			...
			if (relevant != -1) {
				StackTrace st = new StackTrace(trace.getValue(), relevant,
	...
	private static class StackTrace {
		...
		public StackTrace(StackTraceElement[] trace, int startAt, State state) {
			...
			if (startAt == 0) {
				...
				System.arraycopy(trace, startAt, this.trace, 0,
						this.trace.length);
			}
		}
	...
	public static class StacktraceWithCount implements
			Comparable<StacktraceWithCount> {
		...
		public String toString(int totalInvoations, int traceLength) {
			return count + "/" + totalInvoations + " Sampled Invocations ("
	...
	public static class SampledStacktraces {
		...
		public SampledStacktraces(List<StacktraceWithCount> topConsumers,
				int totalInvocations) {
			super();
		...
		public String toString(int minInvocation) {
			...
			for (StacktraceWithCount swc : topConsumers) {
				if (swc.getCount() >= minInvocation) {
					ret.append(swc
							.toString(totalInvocations, Integer.MAX_VALUE));
					ret.append("\n");
				}
			}
			return ret.toString();
