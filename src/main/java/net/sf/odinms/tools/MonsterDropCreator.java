...
public class MonsterDropCreator {
	...
	private static boolean includeQuestDrops; // 用来寻找服务器的东西 不踩它
	protected static String monsterQueryData = "monsterdrops"; // 在你的数据源修改这个
	...
	public static void main(String args[]) throws FileNotFoundException,
		...
		MapleData data = MapleDataProviderFactory.getDataProvider(
				new File(System.getProperty("net.sf.odinms.wzpath")
						+ "/String.wz")).getData("MonsterBook.img");
				...
				for (MapleData drop : dataz.getChildByPath("reward")) {
					...
					for (Pair<Integer, MobInfo> Pair : mobCache) {
						if (Pair.getLeft() == monsterId) {
							if (Pair.getRight().getBoss() > 0) { // 是否是BOSS.
								if (rate > 10) {
					...
					sb.append("\r\n");
					/*
					 * sb.append("-- Name : "); for (Pair<Integer, String> Pair
					 * : itemNameCache) { if (Pair.getLeft() == itemid) {
					 * sb.append(Pair.getRight()); break; } }
					 */
					// sb.append("\r\n");
				}
		...
		/*
		 * for (Pair<Integer, String> Pair : itemNameCache) { if (Pair.getLeft()
		 * >= 2380000 && Pair.getLeft() <= 2388054) {
		 * bookName.append(Pair.getRight()); //bookName.delete(bookName.length()
		 * - 5, bookName.length()); // Get rid of the " Card" string
		 * 
		 * for (Pair<Integer, MobInfo> Pair_ : mobCache) { if
		 * (Pair_.getRight().getName().equalsIgnoreCase(bookName.toString())) {
		 * int rate = 7000; if (Pair_.getRight().getBoss() > 0) { rate /= 10; }
		 * SQL.append("INSERT INTO " + monsterQueryData + " VALUES (DEFAULT, " +
		 * Pair_.getLeft() + ", " + Pair.getLeft() + ", " + rate+",0"); if
		 * (includeQuestDrops) { SQL.append(", DEFAULT);"); } else {
		 * SQL.append(");"); } SQL.append("\r\n"); break; } } bookName.delete(0,
		 * 2147483647); // ;P wild guess LOL } }
		 */
	...
	public static void getAllMobs() {
		...
		MapleDataProvider data = MapleDataProviderFactory
				.getDataProvider(new File(System
						.getProperty("net.sf.odinms.wzpath") + "/String.wz"));
		MapleDataProvider mobData = MapleDataProviderFactory
				.getDataProvider(new File(System
