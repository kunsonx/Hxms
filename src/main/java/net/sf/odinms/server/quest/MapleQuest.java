...
public class MapleQuest {
	...
	private final static MapleDataProvider questData = MapleDataProviderFactory
			.getDataProvider(new File(System
					.getProperty("net.sf.odinms.wzpath") + "/Quest.wz"));
	...
	private MapleQuest(int id) {
		...
		MapleData startReqData = requirements
				.getChildByPath(String.valueOf(id)).getChildByPath("0");
		startReqs = new LinkedList<MapleQuestRequirement>();
		if (startReqData != null) {
			for (MapleData startReq : startReqData.getChildren()) {
				MapleQuestRequirementType type = MapleQuestRequirementType
				...
				if (req.getType().equals(MapleQuestRequirementType.MOB)) {
					for (MapleData mob : startReq.getChildren()) {
						relevantMobs.add(MapleDataTool.getInt(mob
								.getChildByPath("id")));
					}
				}
				startReqs.add(req);
		...
		MapleData completeReqData = requirements.getChildByPath(
		...
		if (completeReqData != null) {
			for (MapleData completeReq : completeReqData.getChildren()) {
				MapleQuestRequirement req = new MapleQuestRequirement(this,
						MapleQuestRequirementType.getByWZName(completeReq
								.getName()), completeReq);
				if (req.getType().equals(MapleQuestRequirementType.MOB)) {
		...
		// read acts
		MapleData startActData = actions.getChildByPath(String.valueOf(id))
				.getChildByPath("0");
		startActs = new LinkedList<MapleQuestAction>();
		if (startActData != null) {
			for (MapleData startAct : startActData.getChildren()) {
				MapleQuestActionType questActionType = MapleQuestActionType
						.getByWZName(startAct.getName());
				startActs.add(new MapleQuestAction(questActionType, startAct,
						this));
			}
		}
		MapleData completeActData = actions.getChildByPath(String.valueOf(id))
				.getChildByPath("1");
		...
		name = MapleDataTool.getString("name", questInfo, "");
	}
	...
	public boolean canComplete(MapleCharacter c, Integer npcid) {
		try {
			...
			int ids = c.getQuest(this).getQuest().getId();
			if (ids == 10284 || ids == 10480 || ids == 8248 || ids == 10286) {
				return true;
			}
	...
	public void start(MapleCharacter c, int npc, boolean force) {
		try {
			...
			if (arg1 || (arg2 && arg3)) {
				...
				MapleQuestStatus newStatus = new MapleQuestStatus(this,
	...
	public void complete(MapleCharacter c, int npc, Integer selection,
		...
		try {
			...
			if (force || (arg1 && canComplete)) {
				...
				newStatus.setForfeited(oldStatus.getForfeited());
				c.updateQuest(newStatus);
				// c.getClient().getSession().write(MaplePacketCreator.showOwnBuffEffect(0,
				// 10));
				// c.getMap().broadcastMessage(c,
				// MaplePacketCreator.showBuffeffect(c.getId(), 0, 10, (byte)
				// 3), false);
			} else {
