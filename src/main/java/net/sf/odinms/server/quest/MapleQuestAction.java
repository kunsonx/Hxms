...
public class MapleQuestAction {
	...
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(MapleQuestAction.class);
	...
	public void run(MapleCharacter c, Integer extSelection) {
		...
		case ITEM:
			...
			for (MapleData iEntry : data.getChildren()) {
				...
				if (MapleDataTool.getInt(iEntry.getChildByPath("count"), 0) < 0) { // remove
					...
					int itemId = MapleDataTool.getInt(iEntry
							.getChildByPath("id"));
					...
					try {
						MapleInventoryManipulator.removeById(c.getClient(),
								iType, itemId, quantity, true, false);
					...
					c.getClient()
							.getSession()
							.write(MaplePacketCreator.getShowItemGain(
									itemId,
					...
					MapleInventoryManipulator.addById(c.getClient(), itemId,
							quantity, logInfo.toString(), null);
					c.getClient()
							.getSession()
							.write(MaplePacketCreator.getShowItemGain(itemId,
									quantity, true));
				}
			}
			break;
		case NEXTQUEST:
			...
			c.getClient()
					...
					.write(MaplePacketCreator.updateQuestFinish(
							(short) quest.getId(), status.getNpc(),
							(short) nextQuest));
		...
		case SKILL:
			for (MapleData sEntry : data) {
				...
				int skillLevel = MapleDataTool.getInt(sEntry
				...
				for (MapleData applicableJob : applicableJobs) {
					MapleJob job = MapleJob.getById(MapleDataTool
				...
				if (shouldLearn) {
					...
					snmcmc.dropMessage("你已获得 "
							+ SkillFactory.getSkillName(skillid) + " 当前等级 "
							+ skillLevel + " 最高等级 " + masterLevel);
				}
			}
			break;
		case FAME:
		...
		case PETSKILL:
			...
			// c.getPet(0).setFlag((byte) (c.getPet(0).getFlag() |
			// InventoryConstants.Items.Flags.getFlagByInt(flag)));
			break;
		default:
		}
	}
