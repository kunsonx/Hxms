...
public class MapleQuestRequirement {
	...
	boolean check(MapleCharacter c, Integer npcid) {
		...
		case QUEST:
			for (MapleData questEntry : getData().getChildren()) {
				int qid = MapleDataTool.getInt(questEntry.getChildByPath("id"),
						-1);
				if (qid == -1) {
					return true;
				}
				...
				if (q == null
						&& MapleQuestStatus.Status.getById(
								MapleDataTool.getInt(
										questEntry.getChildByPath("state"), 0))
		...
		case ITEM:
			...
			for (MapleData itemEntry : getData().getChildren()) {
				int itemId = MapleDataTool.getInt(
						itemEntry.getChildByPath("id"), -1);
				if (itemId == -1) {
					return true;
				}
				short quantity = 0;
				...
				if (quantity < MapleDataTool.getInt(
						...
						|| MapleDataTool.getInt(
								itemEntry.getChildByPath("count"), 0) <= 0
						&& quantity > 0) {
					return false;
				}
			}
		...
		case MOB:
			for (MapleData mobEntry : getData().getChildren()) {
				int mobId = MapleDataTool.getInt(mobEntry.getChildByPath("id"),
						-1);
				int killReq = MapleDataTool.getInt(
						mobEntry.getChildByPath("count"), 1);
				if (mobId == -1) {
		...
		case FIELD_ENTER:
			...
			return false;
		case INTERVAL:
			return !c.getQuest(quest).getStatus()
					.equals(MapleQuestStatus.Status.COMPLETED)
					|| c.getQuest(quest).getCompletionTime() <= System
							.currentTimeMillis()
	...
	public String getDebug(MapleCharacter c) {
		try {
			...
			case QUEST:
				for (MapleData questEntry : getData().getChildren()) {
					int qid = MapleDataTool.getInt(
							questEntry.getChildByPath("id"), -1);
					if (qid == -1) {
						sb.append("QID_IS_NEGATIVE_1_PASS ");
						continue;
					}
					int rqs = MapleDataTool.getInt(
							questEntry.getChildByPath("state"), 0);
					MapleQuestStatus q = c
							.getQuest(MapleQuest.getInstance(qid));
					if (q == null
							&& MapleQuestStatus.Status.getById(rqs).equals(
					...
					if (q == null
							|| !q.getStatus().equals(
									MapleQuestStatus.Status.getById(rqs))) {
						sb.append("QID_");
						sb.append(qid);
						sb.append("_");
			...
			case ITEM:
				...
				for (MapleData itemEntry : getData().getChildren()) {
					...
					if (quantity < MapleDataTool.getInt(
						...
						sb.append(itemId);
						sb.append("_NEED");
						sb.append(MapleDataTool.getInt(
								itemEntry.getChildByPath("count"), -1));
						sb.append("_HAVE");
						sb.append(quantity);
						break outSwitch;
					}
			...
			case MOB:
				for (MapleData mobEntry : getData().getChildren()) {
					int mobId = MapleDataTool.getInt(
							mobEntry.getChildByPath("id"), -1);
					int killReq = MapleDataTool.getInt(
							mobEntry.getChildByPath("count"), 1);
					if (mobId == -1) {
			...
			// case INTERVAL:
			// return
			// !c.getQuest(quest).getStatus().equals(MapleQuestStatus.Status.COMPLETED)
			// || c.getQuest(quest).getCompletionTime() <=
			// System.currentTimeMillis() - MapleDataTool.getInt(getData()) * 60
			// * 1000;
			// case PET:
			// case MIN_PET_TAMENESS:
			default:
