...
public class MapleDoor2 extends AbstractMapleMapObject {
	...
	public MapleDoor2(MapleCharacter owner, Point targetPosition,
			boolean isFirst) {
		super();
		this.owner = owner;
	...
	public void sendSpawnData(MapleClient client) {
		if (target.getId() == client.getPlayer().getMapId()
			...
			client.getSession().write(
					MaplePacketCreator.传送门的效果(owner.getId(), targetPosition,
							isFirst));
			client.getSession().write(
					MaplePacketCreator.传送门的传送点(targetPosition));
