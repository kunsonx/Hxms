...
public class MapleMapInfo {
	...
	private int mapid, returnMapId, timeMobId, bossid, partyBonusRate, decHP,
			HPDecProtect, forcedReturnMap, fl, timeLimit, fieldType;
	private float monsterRate;
	private String FirstUserEnter, UserEnter, timeMobMessage, msg;
	...
	public MapleMap getMap(ChannelDescriptor channel) {
		...
		for (MapleMapCreateLifeInfo lifeInfo : getLifeInfos()) {
			...
			if (myLife instanceof MapleMonster) {
				...
				if (mobTime == -1) { // 不是不召唤 而是-1的时候只召唤一次
					...
					herbRocks.add(map.addMonsterSpawn(mob, mobTime)
							.getPosition());
				}
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("SELECT * FROM spawns WHERE mid = ?");
			ps.setInt(1, mapid);
		...
		if (herbRocks.size() > 0 && highestLevel >= 30
			...
			if (highestLevel >= 100) {
				...
				for (int i = (lowestLevel % 10 > highestLevel % 10 ? 0
						: (lowestLevel % 10)); i < (highestLevel % 10); i++) {
					for (int x = 0; x < 4; x++) { // to make heartstones rare
						allowedSpawn.add(100000 + i);
						allowedSpawn.add(200000 + i);
					}
			...
			final int numSpawn = Randomizer.getInstance().nextInt(
					allowedSpawn.size()) / 6; // 0-7
			for (int i = 0; i < numSpawn && !herbRocks.isEmpty(); i++) {
				final int idd = allowedSpawn.get(Randomizer.getInstance()
						.nextInt(allowedSpawn.size()));
				final int theSpawn = Randomizer.getInstance().nextInt(
						herbRocks.size());
				final MapleReactor myReactor = new MapleReactor(
						MapleReactorFactory.getReactor(idd), idd);
