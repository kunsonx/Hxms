...
public final class MapleSummon extends AbstractAnimatedMapleMapObject {
	...
	public MapleSummon(MapleCharacter owner, int skill, Point pos,
		...
		if (isPuppet() || isStationary() || isRobot() || skill == 3310100
				|| skill == 5321003) { // 3310100 地雷
			this.status = 0;
		} else if (isRobot2()) {
			this.status = 0;
			this.removeStatus = 5;
	...
	public void sendSpawnData(MapleClient client) {
		client.getSession().write(
				MaplePacketCreator.spawnSpecialMapObject(this, skillLevel));
	}

	@Override
	public void sendDestroyData(MapleClient client) {
		client.getSession().write(
				MaplePacketCreator.removeSpecialMapObject(this, true));
	}
