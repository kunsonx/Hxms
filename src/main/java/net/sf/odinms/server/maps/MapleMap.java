...
public class MapleMap {
	...
	private static final List<MapleMapObjectType> rangedMapobjectTypes = Arrays
			.asList(MapleMapObjectType.ITEM, MapleMapObjectType.MONSTER,
					...
					MapleMapObjectType.REACTOR, MapleMapObjectType.机械传送门);
	...
	private int mapid, partyBonusRate = 0, returnMapId,
			forcedReturnMap = 999999999, timeLimit, dropLife = 30000 // 以毫秒为单位掉落后消失的时间
			, decHP = 0, protectItem = 0, fieldType, timeMobId;
	private float monsterRate, origMobRate;
	private boolean dropsDisabled = false, clock, boat, docked,
			everlast = false, town, allowShops, hasEvent, lootable = true,
			canEnter = true, canExit = true, cannotInvincible = false,
			canVipRock = true, muted;
	private String mapName, streetName, UserEnter, FirstUserEnter,
			timeMobMessage = "";
	...
	public MapleMap(int mapid, ChannelDescriptor channel, int returnMapId,
			float monsterRate) {
		// this.mapid = mapid;
		...
		// if (monsterRate > 0) {
		// spawnWorker = TimerManager.getInstance().register(new
		// RespawnWorker(), 7000);//刷怪时间 以毫秒
		// }
	...
	public void respawn() {
		...
		int numShouldSpawn = (this.monsterSpawn.size() - this.spawnedMonstersOnMap
				.get()) * Math.round(this.monsterRate);
		if (numShouldSpawn > 0) {
	 ...
	 *            点卷倍率
	 */
	public void AutoGain(final int jsexp, final boolean isnx,
			final int cspointrate) {
		for (MapleCharacter chr : core.GetMapPlayers()) {
			int givexp = 0;
			givexp = ((jsexp * chr.getLevel()) + chr.getClient()
					.getChannelServer().getExpRate())
					* (chr.getVip() + 2);
			givexp *= 2;
			...
			chr.getClient()
					.getSession()
					.write(MaplePacketCreator.serverNotice(5, "[系统奖励] 获得 ["
							+ givexp + "] 点经验.[" + (chr.getVip() + 2)
							* cspointrate + "点]修为点[会员将加成]"));
			/*
			 * if (GameConstants.秋秋冒险岛 && chr.getVip() > 2) {
			 * chr.GainMoney((chr.getVip() - 2));
			 * chr.setDojoPoints(chr.getDojoPoints() + (chr.getVip() - 2));
			 * chr.getClient
			 * ().getSession().write(MaplePacketCreator.serverNotice(5,
			 * "[系统奖励] 获得 [" + (chr.getVip() - 2) + "] 点元宝," + "会员额外修为获得[" +
			 * (chr.getVip() - 2) + "]点修为.")); }
			 */
		}
	}
	...
	public MapleMap getReturnMap() {
		return ChannelServer.getInstance(channel).getMapFactory()
				.getMap(returnMapId);
	}
	...
	private void broadcastGMMessage(final MapleCharacter source,
			final MaplePacket packet, final double rangeSq,
	...
	public void spawnMesoDrop(final int meso, Point position,
		...
		spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
			...
			public void sendPackets(MapleClient c) {
				c.getSession().write(
						MaplePacketCreator.dropMesoFromMapObject(meso,
								mdrop.getObjectId(), dropper.getObjectId(),
	...
	private void dropFromMonster(MapleCharacter dropOwner, MapleMonster monster) {
		...
		while (shiftDirection < 3 && shiftCount < 1000) {
			...
			for (int i = 0; i < toDrop.size(); i++) {
				...
				if (drop == -1) {
					final int mesoRate = dropOwner.getClient()
							.getChannelServer().getMesoRate();
					double mesoDecrease = Math.pow(0.93,
							monster.getExp() / 300.0);
					if (mesoDecrease > 1.0) {
						mesoDecrease = 1.0;
					}
					int tempmeso = Math.min(30000,
							(int) (mesoDecrease * (monster.getExp())
									* (1.0 + Math.random() * 20) / 10.0));
					if (dropOwner.getBuffedValue(MapleBuffStat.MESOUP) != null) {
						tempmeso = (int) (tempmeso
								* dropOwner
					...
					if (meso > 0) {
						...
						TimerManager.getInstance().schedule(new Runnable() {
							public void run() {
								spawnMesoDrop(meso * mesoRate, dropPos,
										dropMonster, dropChar, explosive);
							}
					...
					if (type.equals(MapleInventoryType.EQUIP)) {
						Equip nEquip = ii.randomizeStats((Equip) ii
								.getEquipById(drop));
						idrop = nEquip;
					} else {
						idrop = new Item(drop, (byte) 0, (short) 1);
						if (ii.isArrowForBow(drop)
								|| ii.isArrowForCrossBow(drop)) { // Randomize
																	// quantity
																	// for
																	// certain
																	// items
					...
					final MapleMapItem mdrop = new MapleMapItem(idrop, dropPos,
							monster, dropOwner);
	...
	public boolean damageMonster(MapleCharacter chr, MapleMonster monster,
		...
		if (monster.getId() == 8500000 || monster.getId() == 8800000) {
			SpeedRankings.setStartTime(monster.getId() == 8500000 ? 1 : 0,
					monster.getId(), System.currentTimeMillis());
		}
			...
			synchronized (monster) {
				...
				if (damage > 0) {
					...
					if (!monster.isAlive()) { // 怪物已经死亡
						...
						} else if ((monster.getId() >= 8820002 && monster
								.getId() <= 8820006)
								|| (monster.getId() >= 8820015 && monster
										.getId() <= 8820018)) {
							for (MapleMapObject object : chr.getMap()
									.getMapObjects()) {
								MapleMonster mons = chr.getMap()
					...
					} else if ((monster.getId() >= 8820002 && monster.getId() <= 8820006)
							|| (monster.getId() >= 8820015 && monster.getId() <= 8820018)) {
						for (MapleMapObject object : chr.getMap()
								.getMapObjects()) {
							MapleMonster mons = chr.getMap().getMonsterByOid(
									object.getObjectId());
	...
	public void killMonster(int monsId) {
		List<MapleMapObject> lmmo = new ArrayList<MapleMapObject>(
				getMapObjects());
		for (MapleMapObject mmo : lmmo) {
			if (mmo instanceof MapleMonster) {
				if (((MapleMonster) mmo).getId() == monsId) {
					this.killMonster((MapleMonster) mmo,
							(MapleCharacter) getAllPlayers().get(0), false);
				}
			}
		}
	}
	...
	public void killMonster(final MapleMonster monster,
		...
		if (monster.getBuffToGive() > -1) {
			broadcastMessage(MaplePacketCreator.showOwnBuffEffect(
					monster.getBuffToGive(), 11));
			MapleItemInformationProvider mii = MapleItemInformationProvider
					.getInstance();
			final MapleStatEffect statEffect = mii.getItemEffect(monster
					.getBuffToGive());

			for (MapleCharacter character : core.GetMapPlayers()) {
				if (character.isAlive()) {
					statEffect.applyTo(character);
					broadcastMessage(MaplePacketCreator.showBuffeffect(
							character.getId(), monster.getBuffToGive(), 11,
							(byte) 3, 1));
				}
			}
	...
	// 吞噬用
	public void killMonster2(final MapleMonster monster,
			final MapleCharacter chr) {
		// chr.increaseEquipExp(monster.getExp()); //给予道具经验
		spawnedMonstersOnMap.decrementAndGet();
		monster.setHp(0);
		broadcastMessage(MaplePacketCreator.SpecialKillMonster(
				monster.getObjectId(), chr.getId()), monster.getPosition());
		removeMapObject(monster);
	}
	...
	public void updateMonsterController(MapleMonster monster) {
		...
		synchronized (monster) {
			...
			if (monster.getController() != null) {
				// monster has a controller already, check if he's still on this
				// map
				if (monster.getController().getMap() != this) {
					log.warn("Monstercontroller wasn't on same map");
					monster.getController().stopControllingMonster(monster);
				} else {
					// controller is on the map, monster has an controller,
					// everything is fine
					return;
				}
			}
	...
	public MapleMonster getMonsterById(int id) {
		for (MapleMapObject obj : core
				.GetMapObjects(MapleMapObjectType.MONSTER)) {
	...
	public MapleReactor getReactorByName(String name) {
		for (MapleMapObject obj : core
				.GetMapObjects(MapleMapObjectType.REACTOR)) {
	...
	public void spawnRevives(final MapleMonster monster) {
		...
		spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
			...
			public void sendPackets(MapleClient c) {
				c.getSession().write(
						MaplePacketCreator.spawnMonster(monster, false));
			}
		}, null);
	...
	public synchronized void spawnMonster(final MapleMonster monster) {
			...
			TimerManager.getInstance().schedule(new Runnable() {
				...
				public void run() {
					if (getAllPlayers().size() > 0) {
						killMonster(monster, (MapleCharacter) getAllPlayers()
								.get(0), false, false, 3);
					}
				}
			}, removeAfter);
		...
		spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
			public void sendPackets(MapleClient c) {
				c.getSession().write(
						MaplePacketCreator.spawnMonster(monster, true));
			}
		}, null);
		updateMonsterController(monster);
		spawnedMonstersOnMap.incrementAndGet();
	...
	public void spawnDojoMonster(final MapleMonster monster) {
		...
		spawnMonsterWithEffect(monster, 15, pts[Randomizer.getInstance()
				.nextInt(3)]);
	}
	...
	public synchronized void spawnFakeMonster(final MapleMonster monster) {
		...
		spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
			...
			public void sendPackets(MapleClient c) {
				c.getSession().write(
						MaplePacketCreator.spawnFakeMonster(monster, 0));
			}
		}, null);
		spawnedMonstersOnMap.incrementAndGet();
	}
	...
	public void spawnDoor(final MapleDoor door) {
		spawnAndAddRangedMapObject(door, new DelayedPacketCreation() {
			public void sendPackets(MapleClient c) {
				c.getSession().write(
						MaplePacketCreator.spawnDoor(door.getOwner().getId(),
				...
				if (door.getOwner().getParty() != null
					...
					c.getSession().write(
							MaplePacketCreator.partyPortal(door.getTown()
									.getId(), door.getTarget().getId(), door
									.getTargetPosition()));
				}
				c.getSession().write(
						MaplePacketCreator.spawnPortal(door.getTown().getId(),
	...
	public void spawnDoor2(final MapleDoor2 door) {
		spawnAndAddRangedMapObject(door, new DelayedPacketCreation() {
			public void sendPackets(final MapleClient c) {
				if (door.getTarget().getId() == c.getPlayer().getMapId()
						|| door.getOwner() == c.getPlayer()) {
					broadcastMessage(MaplePacketCreator.传送门的效果(door.getOwner()
							.getId(), door.getTargetPosition(), door.isFirst()));
					broadcastMessage(MaplePacketCreator.传送门的传送点(door
							.getTargetPosition()));
					/*
	...
	public void spawnSummon(final MapleSummon summon) {
		spawnAndAddRangedMapObject(summon, new DelayedPacketCreation() {
			...
			public void sendPackets(MapleClient c) {
				// log.debug("召唤Summon");
				int skillLevel = summon.getOwner().getSkillLevel(
						SkillFactory.getSkill(summon.getSkill()));
				c.getSession().write(
		...
		Runnable poisonTask = new Runnable() {
			...
			public void run() {
				if (mist.isPoison()) {
					...
					// SkillFactory.getSkill(mist.getSourceId()).getEffect(mist.getOwner().getSkillLevel(mist.getSourceId())).getX()
					// / 100;
					// for (MapleMapObject mo :
					// getMapObjectsInBox(mist.getBox(),
	...
	public void disappearingItemDrop(final MapleMapObject dropper,
		...
		broadcastMessage(MaplePacketCreator.dropItemFromMapObject(
				item.getItemId(), drop.getObjectId(), 0, 0,
	...
	public void spawnItemDrop(final MapleMapObject dropper,
			...
			public void sendPackets(MapleClient c) {
				c.getSession().write(
						MaplePacketCreator.dropItemFromMapObject(
								item.getItemId(), drop.getObjectId(), 0,
		...
		broadcastMessage(MaplePacketCreator.dropItemFromMapObject(
				item.getItemId(), drop.getObjectId(), 0,
				ffaDrop ? 0 : owner.getId(), dropper.getPosition(), droppos,
				(byte) 0), drop.getPosition());
	...
	private class TimerDestroyWorker implements Runnable {
		...
		public void run() {
			if (mapTimer != null) {
				...
				mapTimer = null;
				if (warpMap != -1) {
					MapleMap map2wa2 = ChannelServer.getInstance(channel)
							.getMapFactory().getMap(warpMap);
					String warpmsg = "你即将被传送到 " + map2wa2.getStreetName()
							+ " : " + map2wa2.getMapName();
					broadcastMessage(MaplePacketCreator
							.serverNotice(6, warpmsg));
					Collection<MapleCharacter> cmc = new LinkedHashSet<MapleCharacter>(
							getCharacters());
					for (MapleCharacter chr : cmc) {
						try {
							if (chr.getLevel() >= minWarp
	...
	public void addMapTimer(int duration) {
		ScheduledFuture<?> sf0f = TimerManager.getInstance().schedule(
				new TimerDestroyWorker(), duration * 1000);
	...
	public void addMapTimer(int duration, int mapToWarpTo, int minLevelToWarp) {
		ScheduledFuture<?> sf0f = TimerManager.getInstance().schedule(
				new TimerDestroyWorker(), duration * 1000);
		mapTimer = new MapleMapTimer(sf0f, duration, mapToWarpTo,
				minLevelToWarp, 256);
		broadcastMessage(mapTimer.makeSpawnData());
	}
	...
	private void activateItemReactors(MapleMapItem drop) {
		...
		final TimerManager tMan = TimerManager.getInstance(); // check for
																// reactors on
																// map that
																// might use
																// this item
		for (MapleMapObject o : core.values()) {
			if (o.getType() == MapleMapObjectType.REACTOR) {
				if (((MapleReactor) o).getStats() != null
					...
					if (((MapleReactor) o).getReactItem().getLeft() == item
							.getItemId()
							&& ((MapleReactor) o).getReactItem().getRight() <= item
									.getQuantity()) {
						...
						if (area.contains(drop.getPosition())) {
							...
							if (!reactor.isTimerActive()) {
								tMan.schedule(new ActivateItemReactor(drop,
										reactor, ownerClient), 5000);
	...
	public void AriantPQStart() {
		...
		for (MapleCharacter chars2 : this.getCharacters()) {
			broadcastMessage(MaplePacketCreator.updateAriantPQRanking(
					chars2.getName(), 0, false));
			broadcastMessage(MaplePacketCreator.serverNotice(
					0,
					MaplePacketCreator.updateAriantPQRanking(chars2.getName(),
							0, false).toString()));
			if (this.getCharacters().size() > i) {
				broadcastMessage(MaplePacketCreator.updateAriantPQRanking(null,
						0, true));
				broadcastMessage(MaplePacketCreator.serverNotice(
						0,
						MaplePacketCreator.updateAriantPQRanking(
								chars2.getName(), 0, true).toString()));
			}
			i++;
		}
	}
	...
	public void addPlayer(MapleCharacter chr) {
		...
		for (MaplePet pet : chr.getPets()) {
			if (pet != null) {
				chr.getClient()
						.getSession()
						.write(MaplePacketCreator.showPet(chr, pet, false,
								false));
			}
		}
		if (hasForcedEquip()) {
			chr.getClient().getSession()
					.write(MaplePacketCreator.showForcedEquip());
		}
		if (chr.getMapId() >= 140090100 && chr.getMapId() <= 140090500
				|| chr.getJob().getId() == 1000 && chr.getMapId() != 130030000) {
		...
		if (chr.getChalkboard() != null) {
			chr.getClient().getSession()
					.write(MaplePacketCreator.useChalkboard(chr, false));
		}
		/*
		...
		if (mapTimer != null) {
			chr.getClient().getSession()
					.write(MaplePacketCreator.getClock(mapTimer.getTimeLeft()));
		}
		if (chr.getEventInstance() != null
				&& chr.getEventInstance().isTimerStarted()) {
			chr.getClient()
					.getSession()
					.write(MaplePacketCreator.getClock((int) (chr
							.getEventInstance().getTimeLeft() / 1000)));
		}
		if (hasClock()) {
		...
		if (hasBoat() == 2) {
			chr.getClient().getSession()
					.write((MaplePacketCreator.boatPacket(true)));
		} else if (hasBoat() == 1
				&& (chr.getMapId() != 200090000 || chr.getMapId() != 200090010)) {
			chr.getClient().getSession()
					.write(MaplePacketCreator.boatPacket(false));
		}
		if (chr.getAndroid() != null) {
			chr.getAndroid().setPos(chr.getPosition());
			broadcastMessage(chr,
					MaplePacketCreator.spawnAndroid(chr, chr.getAndroid()));
		}

		chr.receivePartyMemberHP();// 检测组队Hp。
		chr.getClient()
				.getSession()
				.write(MaplePacketCreator.showMapEffect("maplemap/enter/"
						+ this.mapid));// 显示地图名
	}
	...
	public void removePlayer(MapleCharacter chr) {
			...
			for (MapleSummon summon : summons) {
				if (summon.isStationary()) {
					...
					if (!summon.isBuffSummon()) {
						...
						broadcastMessage(MaplePacketCreator
								.removeSpecialMapObject(summon, true));
		...
		for (MapleDoor2 door2 : chr.getDoors2()) {
			broadcastMessage(MaplePacketCreator.取消传送门(chr.getId(),
					door2.isFirst()));
	...
	public void broadcastMessage(final MapleCharacter source,
	...
	public void broadcastMessage(MapleCharacter source, MaplePacket packet,
		...
		broadcastMessage(repeatToSource ? null : source, packet,
				ranged ? MapleCharacter.MAX_VIEW_RANGE_SQ
						: Double.POSITIVE_INFINITY, source.getPosition());
	}
	...
	public void broadcastMessage(MaplePacket packet, Point rangedFrom) {
		broadcastMessage(null, packet, MapleCharacter.MAX_VIEW_RANGE_SQ,
				rangedFrom);
	}
	...
	public void broadcastMessage(MapleCharacter source, MaplePacket packet,
			Point rangedFrom) {
		broadcastMessage(source, packet, MapleCharacter.MAX_VIEW_RANGE_SQ,
				rangedFrom);
	}

	private void broadcastMessage(final MapleCharacter source,
			final MaplePacket packet, final double rangeSq,
			final Point rangedFrom) {
		for (MapleCharacter chr : core.GetMapPlayers()) {
			if (chr != source) {
	...
	private void sendObjectPlacement(MapleClient mapleClient) {
		...
		if (chr != null) {
			for (MapleMapObject o : getMapObjectsInRange(chr.getPosition(),
					MapleCharacter.MAX_VIEW_RANGE_SQ, rangedMapobjectTypes)) {
				if (o.getType() == MapleMapObjectType.REACTOR) {
	...
	public void movePlayer(final MapleCharacter player, Point newPosition) {
			...
			for (MapleMapObject mo : player.getVisibleMapObjects()) {
				if (mo != null) {
					if (core.get(mo.getObjectId()) == mo) {
						if (!player.isMapObjectVisible(mo)) { // monster entered
																// view range
							if (mo.getType() == MapleMapObjectType.SUMMON
									|| mo.getPosition().distanceSq(
											player.getPosition()) <= MapleCharacter.MAX_VIEW_RANGE_SQ) {
		...
		if (getId() == 240040611) { // 九灵巢穴
			if (getMapObjectsInRange(player.getPosition(), 25000,
					Arrays.asList(MapleMapObjectType.REACTOR)).size() > 0) {
	...
	public void resetSpawn() {
		...
		if (this.monsterRate > 0) {
			spawnWorker = TimerManager.getInstance().register(
					new RespawnWorker(), 10000);
		}
	}

	public void spawnDebug(MessageCallback mc) {
		mc.dropMessage("Spawndebug...");
		mc.dropMessage("Mapobjects in map: " + core.size()
				+ " \"spawnedMonstersOnMap\": " + spawnedMonstersOnMap
				+ " spawnpoints: " + monsterSpawn.size() + " maxRegularSpawn: "
				+ getMaxRegularSpawn() + " spawnRate: " + this.monsterRate
				+ " original spawnRate: " + this.origMobRate);
		int numMonsters = 0;
	...
	public void spawnPB() {
		killAllMonsters();
		spawnMonsterwithpos(MapleLifeFactory.getMonster(8820009), new Point(7,
				-42));
	}
		...
		public void run() {
			if (mapitem != null
				...
				synchronized (mapitem) {
					...
					reactor.setTimerActive(false);
					if (reactor.getDelay() > 0) { // This shit is negative..
													// Fix?
						tMan.schedule(new Runnable() {
							@Override
							public void run() {
	...
	private class RespawnWorker implements Runnable {
		...
		public void run() {
			...
			int numShouldSpawn = (int) Math
					.round(Math.random()
							* ((2 + playersOnMap / 1.1 + (getMaxRegularSpawn() - ispawnedMonstersOnMap) / 4.0)));
			...
			// k find that many monsters that need respawning and respawn them
			// O.o
			List<SpawnPoint> randomSpawn = new ArrayList<SpawnPoint>(
					monsterSpawn);
			Collections.shuffle(randomSpawn);
	...
	public boolean isPQMap() { // Does NOT include CPQ maps
		...
		if ((tmapid > 922010000 && tmapid < 922011100)
				|| (tmapid >= 103000800 && tmapid < 103000890)) { // kpq + lpq
																	// only atm
			return true;
		}
		return false;
		...
		for (MapleMapObject monstermo : monsters) {
			...
			if (monster.getId() == 3230300 || monster.getId() == 3230301
				...
				broadcastMessage(MaplePacketCreator.killMonster(
						monster.getObjectId(), true), monster.getPosition());
				removeMapObject(monster);
			}
		}
	...
	public int playerCount() {
		List players = getMapObjectsInRange(
				new java.awt.Point(0, 0),
				(1.0D / 0.0D),
	...
	public int gethiredmerchant() {
		return getMapObjectsInRange(
				new java.awt.Point(0, 0),
				(1.0D / 0.0D),
				Arrays.asList(new MapleMapObjectType[] { MapleMapObjectType.HIRED_MERCHANT }))
				.size();
	}
	...
	public final void killAllMonsters(final boolean animate) {
		for (final MapleMapObject monstermo : getAllMonster()) {
			...
			monster.setHp(0);
			broadcastMessage(MaplePacketCreator.killMonster(
					monster.getObjectId(), animate ? 1 : 0));
			removeMapObject(monster);
		}
	}

	public Collection<MapleCharacter> getNearestPvpChar(Point attacker,
			double maxRange, double maxHeight, Collection<MapleCharacter> chr) {
		Collection character = new LinkedList();
		for (MapleCharacter a : this.core.GetMapPlayers()) {
			if (chr.contains(a.getClient().getPlayer())) {
				Point attackedPlayer = a.getPosition();
				MaplePortal Port = a.getMap().findClosestSpawnpoint(
						a.getPosition());
				Point nearestPort = Port.getPosition();
				double safeDis = attackedPlayer.distance(nearestPort);
				double distanceX = attacker.distance(attackedPlayer.getX(),
						attackedPlayer.getY());
				if (MaplePvp.isLeft && attacker.x > attackedPlayer.x
						&& distanceX < maxRange && distanceX > 2.0
				...
				if (MaplePvp.isRight && attacker.x < attackedPlayer.x
						&& distanceX < maxRange && distanceX > 2.0
						&& attackedPlayer.y >= attacker.y - maxHeight
						&& attackedPlayer.y <= attacker.y + maxHeight
						&& safeDis > 2.0) {
