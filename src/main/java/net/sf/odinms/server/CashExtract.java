...
public class CashExtract {
	...
	public static void main(String args[]) throws FileNotFoundException,
		...
		MapleDataProvider IntegerProvider = MapleDataProviderFactory
				.getDataProvider(IntegerFile);
		MapleData cash = IntegerProvider.getData("Commodity.img");
		String output = args[0];
		File outputDir = new File(output);
		File cashTxt = new File((new StringBuilder()).append(output)
				.append("\\Commodity.txt").toString());
		outputDir.mkdir();
		...
		int priorityx;
		int snx;
		for (Iterator i$ = cash.getChildren().iterator(); i$.hasNext(); writer
				.println((new StringBuilder()).append("(")
						.append(child.getName()).append(",").append(snx)
						.append(",").append(itemx).append(",").append(countx)
						.append(",").append(pricex).append(",").append(periodx)
						.append(",").append(priorityx).append(",")
						.append(genderx).append(",").append(onsalex)
