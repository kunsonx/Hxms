...
public class MapleShop {
	...
	public void sendShop(MapleClient c) {
		...
		c.getSession().write(
				MaplePacketCreator.getNPCShop(c, getNpcId(), items));
	}

	public void buy(MapleClient c, int itemId, short quantity) {
		if (quantity <= 0) {
			log.warn(c.getPlayer().getName() + " is buying an invalid amount: "
					+ quantity + " of itemid: " + itemId);
			c.disconnect();
			return;
		}
		...
		if (item != null && item.getPrice() > 0
			...
			if (MapleInventoryManipulator.checkSpace(c, itemId, quantity, "")) {
				if (itemId >= 5000000 && itemId <= 5000100) {
					...
					MapleInventoryManipulator.addById(c, itemId, rechquantity,
							"Rechargable item purchased.", null);
				} else {
					MapleInventoryManipulator.addById(c, itemId, quantity, c
							.getPlayer().getName()
							+ " bought "
							+ quantity
							+ " for "
				...
				long value = -((long) item.getPrice() * quantity);
				c.getPlayer().gainMeso(value, false);
				c.getSession().write(
						MaplePacketCreator.confirmShopTransaction((byte) 0));
			} else {
				c.getSession().write(
						MaplePacketCreator.confirmShopTransaction((byte) 3));
			}
		}
	}

	public void sell(MapleClient c, MapleInventoryType type, byte slot,
			short quantity) {
		...
		if (quantity < 0) {
			AutobanManager.getInstance().addPoints(
					...
					1000,
					0,
					"Selling " + quantity + " " + item.getItemId() + " ("
							+ type.name() + "/" + slot + ")");
			return;
		}
		...
		if (quantity <= iQuant && iQuant > 0) {
			MapleInventoryManipulator.removeFromSlot(c, type, slot, quantity,
					false);
			double price;
			if (ii.isThrowingStar(item.getItemId())) {
			...
			c.getSession().write(
					MaplePacketCreator.confirmShopTransaction((byte) 0xB));
		}
	}
	...
	public void recharge(MapleClient c, byte slot) {
		MapleItemInformationProvider ii = MapleItemInformationProvider
				.getInstance();
		IItem item = c.getPlayer().getInventory(MapleInventoryType.USE)
				.getItem(slot);
		if (item == null
				|| (!ii.isThrowingStar(item.getItemId()) && !ii.isBullet(item
						.getItemId()))) {
			if (item != null
					&& (!ii.isThrowingStar(item.getItemId()) || !ii
							.isBullet(item.getItemId()))) {
				log.warn(c.getPlayer().getName() + " is trying to recharge "
						+ item.getItemId());
			}
			return;
		}
		...
		if (item.getQuantity() < 0) {
			log.warn(c.getPlayer().getName() + " is trying to recharge "
					+ item.getItemId() + " with quantity " + item.getQuantity());
		}
		if (item.getQuantity() < slotMax) {
			// calc price ;_;
			int price = (int) Math.round(ii.getPrice(item.getItemId())
					* (slotMax - item.getQuantity()));
			if (c.getPlayer().getMeso() >= price) {
				item.setQuantity(slotMax);
				c.getSession().write(
						MaplePacketCreator.updateInventorySlot(
								MapleInventoryType.USE, (Item) item));
		...
		try {
			...
			while (rs.next()) {
				if (ii.isThrowingStar(rs.getInt("itemid"))
					...
					MapleShopItem starItem = new MapleShopItem((short) 1,
