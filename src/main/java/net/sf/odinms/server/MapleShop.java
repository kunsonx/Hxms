...
public class MapleShop {
        ...
        if (item != null && item.getPrice() > 0 && c.getPlayer().getMeso() >= item.getPrice() * quantity) {
            if (MapleInventoryManipulator.checkSpace(c, itemId, quantity, "")) {
                if (itemId >= 5000000 && itemId <= 5000100) {
                    ...
                    MapleInventoryManipulator.addFromDrop(c, MaplePet.createPet(itemId));
                } else if (ii.isRechargable(itemId)) {
                    short rechquantity = ii.getSlotMax(c, item.getItemId());
                    MapleInventoryManipulator.addById(c, itemId, rechquantity, "Rechargable item purchased.", null);
                } else {
                    MapleInventoryManipulator.addById(c, itemId, quantity, c.getPlayer().getName() + " bought " + quantity + " for " + item.getPrice() * quantity + " from shop " + id, null);
                }
    ...
    public void sell(MapleClient c, MapleInventoryType type, byte slot, short quantity) {
        ...
        if (quantity <= iQuant && iQuant > 0) {
            ...
            c.getSession().write(MaplePacketCreator.confirmShopTransaction((byte) 0xB));
        }
    }
