...
public class MaplePortal implements Serializable {
	...
	public void enterPortal(MapleClient c) {
		...
		if (getScriptName() != null) {
			...
			if (!FourthJobQuestsPortalHandler.handlePortal(getScriptName(),
					c.getPlayer())) {
				changed = PortalScriptManager.getInstance()
			...
			if (to != null) {
				...
				if (pto == null) { // fallback for missing portals - no real
									// life case anymore - intresting for not
									// implemented areas
					pto = to.getPortal(0);
				}
				if (getTargetMapId() >= 910000019
						&& getTargetMapId() <= 910000022) {
				...
				player.getClient().getSession()
						.write(MaplePacketCreator.serverNotice(5, "无法进入下个地图！"));
			}
		}
		if (!changed) {
			c.getSession().write(MaplePacketCreator.enableActions());
	...
	public String toString() {
		return "地图门{" + "name=" + name + ", scriptName=" + scriptName
