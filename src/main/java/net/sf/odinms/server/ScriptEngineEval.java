...
public class ScriptEngineEval {
    ...
    private final static Compilable compEngine = (Compilable) sem.getEngineByName("javascript");
    private static String basescript = "var x = %d;"
            + "function u(i){"
            + "return Math.ceil(i);"
            + "} "
            + ""
    ...
    public static int Eval(int x, String exp, int defvalue, int sourceid) {
        ...
        try {
            CompiledScript script = compEngine.compile(String.format(basescript, x, exp.replace("y", "0")));//.replace("=", "")
            return ((Double) Double.parseDouble(script.eval().toString())).intValue();
