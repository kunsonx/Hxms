...
public class CashItemFactory {
    ...
    private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(CashItemFactory.class);
    ...
    public static CashItemInfo getItemInSql(int sn) {
        ...
        if (stats == null) {
            ...
            //            "from CashItemInfo as cs where cs.SN =:sn").setParameter("sn", sn).setMaxResults(1).uniqueResult();
                CashItemInfo cs  = (CashItemInfo) session.get(CashItemInfo.class, sn);
                session.close();
                if (cs != null) {
    ...
    public static List<CashItemInfo> getPackageItems(int itemId) {
        ...
        //            "from CashPackageList as cs where cs.packageid =:id").setParameter("id", itemId).setMaxResults(1).uniqueResult();
        CashPackageList list = (CashPackageList) session.get(CashPackageList.class, itemId);
        session.close();
        if (list != null) {
    ...
    public static int getSnFromId(int id) {
        if (!idLookup.containsKey(id)) {
            try {
                ...
                Session session = DatabaseConnection.getSession();
                CashItemInfo cs = (CashItemInfo) session.createQuery(
                        "from CashItemInfo as cs where cs.itemId =:id").setParameter("id", id).setMaxResults(1).uniqueResult();
                session.close();
                if (cs != null) {
