...
public class MapleStorage implements MapleItemsNameSpace {
    ...
    private EnumMap<MapleInventoryType, List<IItem>> typeItems = new EnumMap<MapleInventoryType, List<IItem>>(MapleInventoryType.class);
    private static Logger log = Logger.getLogger(MapleStorage.class);

    public MapleStorage(int id, Connection connection) {
        try {
            this.accountid = id;
            PreparedStatement ps = connection.prepareStatement("SELECT * FROM storages WHERE accountid = ?");
            ps.setInt(1, id);
            ...
            if (rs.next()) {
                ...
                ps = connection.prepareStatement("INSERT INTO storages (accountid, meso) VALUES (?, ?)");
                ps.setInt(1, id);
                ps.setLong(2, 0);
                ps.executeUpdate();
            }
    ...
    /* public static MapleStorage loadOrCreateFromDB(int id) {
     ...
     int storeId;
     try {
     DatabaseConnection.GetLock(id).lock();
     Connection con = DatabaseConnection.getConnection();
     ...
     equip.setPvpWatk((short) rs.getInt("pvpWatk"));
     ret.items.add(equip);
     } else {
     ...
     DatabaseConnection.GetLock(id).unlock();
     }
     return ret;
     }*/
    public int getSlots() {
    ...
    public void saveToDB(Connection con) throws SQLException {
        PreparedStatement ps = con.prepareStatement("UPDATE storages SET slots = ?, meso = ? WHERE accountid = ?");
        ps.setInt(1, slots);
        ps.setLong(2, meso);
    ...
    /*  public void saveToDB(Connection con) {
     ...
     //潜能
     pse.setInt(23, equip.getPotential_1());
     pse.setInt(24, equip.getPotential_2());
     pse.setInt(25, equip.getPotential_3());
     pse.setInt(26, equip.getIdentify());
     pse.setInt(27, equip.getStarlevel());
     pse.setInt(28, equip.getIdentified());
     //道具等级 道具经验值 耐久度
     pse.setInt(29, equip.getItemLevel());
     pse.setInt(30, equip.getItemExp());
     pse.setInt(31, equip.getItemSkill());
     pse.setInt(32, equip.getDurability());
     pse.setInt(33, equip.getPvpWatk());
     pse.executeUpdate();
     }
     }
     ps.close();
    ...
    public void sendStorage(MapleClient c, int npcId) {
        ...
        c.getSession().write(MaplePacketCreator.getStorage(npcId, slots, meso, this));
    }
