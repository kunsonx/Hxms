...
public class MapleMonsterCarnival {
	...
	public MapleMonsterCarnival(MapleParty p1, MapleParty p2, int mapid) {
		this.p1 = p1;
		this.p2 = p2;
		ChannelDescriptor chnl = p1.getLeader().getChannelDescriptor();
		ChannelDescriptor chnl1 = p2.getLeader().getChannelDescriptor();
		if (chnl.equals(chnl1)) {
			throw new RuntimeException(
					"ERROR: CPQ leaders are on different channels.");
		}
	...
	public void playerDisconnected(int charid) {
		if (leader1.getId() == charid || leader2.getId() == charid) {
			...
			map.broadcastMessage(MaplePacketCreator.serverNotice(5, "Maple "
					+ teamS + " has quitted the Monster Carnival."));
			return;
		} else {
			// map.broadcastMessage(MaplePacketCreator.serverNotice(5,
			// ChannelServer.getInstance(1).getPlayerStorage().getCharacterById(charid).getName()
	...
	public void finish(int winningTeam) {
		ChannelDescriptor chnl = leader1.getClient().getChannelDescriptor();
		ChannelDescriptor chnl1 = leader2.getClient().getChannelDescriptor();
		if (chnl.equals(chnl1)) {
			throw new RuntimeException(
		...
		if (winningTeam == 0) {
			for (MaplePartyCharacter mpc : leader1.getParty().getMembers()) {
				...
				if (mc != null) {
					mc.setCPQRanking(getRankByCP(this.redTotalCP));
					mc.changeMap(cs.getMapFactory().getMap(map.getId() + 2),
			...
			for (MaplePartyCharacter mpc : leader2.getParty().getMembers()) {
				...
				if (mc != null) {
					mc.setCPQRanking(getRankByCP(this.blueTotalCP));
					mc.changeMap(cs.getMapFactory().getMap(map.getId() + 3),
				...
				if (redWin) {
					mc.getClient()
							...
							.write(MaplePacketCreator
									.showEffect("quest/carnival/win"));
					mc.getClient()
							.getSession()
							.write(MaplePacketCreator
									.playSound("MobCarnival/Win"));
				} else {
					mc.getClient()
							.getSession()
							.write(MaplePacketCreator
									.showEffect("quest/carnival/lose"));
					mc.getClient()
							.getSession()
							.write(MaplePacketCreator
