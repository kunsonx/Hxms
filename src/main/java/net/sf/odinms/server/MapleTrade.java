...
public class MapleTrade implements MapleInteractionRoom {
    ...
    private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(MapleTrade.class);
...
//	}
    ...
    public static void visitTrade(MapleCharacter c1, MapleCharacter c2) {
        ...
        if (c1.getTrade() != null && c2.getTrade() == null) {
            ...
            c1.getClient().getSession().write(MaplePacketCreator.getTradePartnerAdd(c2));
            c2.getClient().getSession().write(MaplePacketCreator.getTradeStart(c2.getClient(), c2.getTrade(), (byte) 1, true));
        } else {
            cancelTrade(c1);
    ...
    public static void declineTrade(MapleCharacter dstCharacter, MapleCharacter src) {
        if (src != null) {
            src.getClient().getSession().write(
                    MaplePacketCreator.serverNotice(5, dstCharacter.getName() + " 拒绝了你的交易邀请"));
        }
        cancelTrade(src);
        /*   MapleTrade trade = dstCharacter.getTrade();
         if (trade != null) {
