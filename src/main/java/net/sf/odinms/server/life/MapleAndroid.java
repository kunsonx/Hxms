...
public class MapleAndroid implements Serializable {
    ...
    public static MapleAndroid loadFromDb(final int itemid, final long uid) {
        ...
        MapleAndroid android = (MapleAndroid) session.get(MapleAndroid.class, uid);
                //session.createQuery("from MapleAndroid as an where an.uniqueid =:uniqueid").setParameter("uniqueid", uid).uniqueResult();
        session.close();
        if (android != null) {
    ...
    public static MapleAndroid create(final int itemid, final long uniqueid) {
        ...
        MapleAndroidInfo aInfo = MapleItemInformationProvider.getInstance().getAndroidInfo(zxc);
        if (aInfo == null) {
            return null;
        }
        return create(itemid,
                uniqueid,
                0,
                aInfo.getHairs().get(Randomizer.getInstance().nextInt(aInfo.getHairs().size())).getValue(),
                aInfo.getFaces().get(Randomizer.getInstance().nextInt(aInfo.getFaces().size())).getValue());
    }

    public static MapleAndroid create(int itemid, long uniqueid, int skin, int hair, int face) {
