...
public class MapleMonster extends AbstractLoadedMapleLife {
	...
	private Collection<AttackerEntry> attackers = Collections
			.synchronizedList(new ArrayList<AttackerEntry>());
	private EventInstanceManager eventInstance = null;
	private Collection<MonsterListener> listeners = Collections
			.synchronizedList(new ArrayList<MonsterListener>(1));
	private MapleCharacter highestDamageChar;
	private final Map<MapleMonsterStat, MonsterStatusEffect> stati = Collections
			.synchronizedMap(new EnumMap<MapleMonsterStat, MonsterStatusEffect>(
					MapleMonsterStat.class));
	private final List<MonsterStatusEffect> activeEffects = Collections
			.synchronizedList(new ArrayList<MonsterStatusEffect>());
	private MapleMap map;
	private int VenomMultiplier = 0;
	private boolean fake = false;
	private boolean dropsDisabled = false;
	private List<Pair<Integer, Integer>> usedSkills = Collections
			.synchronizedList(new ArrayList<Pair<Integer, Integer>>());
	private final Map<Pair<Integer, Integer>, Integer> skillsUsed = Collections
			.synchronizedMap(new HashMap<Pair<Integer, Integer>, Integer>());
	private List<MapleMonsterStat> monsterBuffs = new ArrayList<MapleMonsterStat>();
	...
	public List<MapleMonsterStat> getMonsterBuffs() {
		return monsterBuffs;
	}
	...
	public int getDrop(MapleCharacter killer) {
		MapleMonsterInformationProvider mi = MapleMonsterInformationProvider
				.getInstance();
		int lastAssigned = -1;
		int minChance = 1;
		...
		for (DropEntry d : dl) {
			...
			d.assignedRangeLength = (int) Math
					.ceil(((double) 1 / (double) d.chance) * minChance);
			lastAssigned += d.assignedRangeLength;
		}
		int c = (int) (Math.random() * minChance);
		for (DropEntry d : dl) {
			int itemid = d.itemid;
			if ((c >= d.assignedRangeStart)
					&& (c < (d.assignedRangeStart + d.assignedRangeLength))) {
				if (d.questid != 0) {
					if (killer.getQuest(MapleQuest.getInstance(d.questid))
							.getStatus() == MapleQuestStatus.Status.STARTED) {
						return itemid;
					}
				} else {
					return itemid;
	...
	public int getMaxDrops(MapleCharacter chr) {
		...
		int maxDrops;
		if (isPQMonster()) {
			maxDrops = 1; // 组队任务只掉一个物品(通行证) - 我认为是这样 - = 嘉年华怪物不知道
		} else if (isExplosive()) {
			maxDrops = 1 * cserv.getBossDropRate();
		} else if (isBoss() && !isExplosive()) {
			maxDrops = 1 * cserv.getBossDropRate();
		} else {
			maxDrops = 1 * cserv.getDropRate();
			if (stati.containsKey(MapleMonsterStat.TAUNT)) {
				int alterDrops = stati.get(MapleMonsterStat.TAUNT).getStati()
						.get(MapleMonsterStat.TAUNT);
	...
	public boolean isPQMonster() {
		int id = getId();
		if ((id >= 9300000 && id <= 9300003)
				|| (id >= 9300005 && id <= 9300010)
				|| (id >= 9300012 && id <= 9300017)
				|| (id >= 9300169 && id <= 9300171)) {
			return true;
		} else {
			return false;
		}
	...
	public void damage(MapleCharacter from, int damage, boolean updateAttackTime) {
		...
		if (hasBossHPBar()) {
			from.getMap()
					.broadcastMessage(makeBossHPBarPacket(), getPosition());
		} else if (!isBoss()) {
			for (AttackerEntry mattacker : attackers) {
				for (AttackingMapleCharacter cattacker : mattacker
						.getAttackers()) {
					...
					if (cattacker.getAttacker().getMap() == from.getMap()) {
						if (cattacker.getLastAttackTime() >= okTime) {
							cattacker
									...
									.getClient()
									.getSession()
									.write(MaplePacketCreator.showMonsterHP(
											getObjectId(), remhppercentage));
						}
					}
				}
			}
	...
	public void heal(int hp, int mp) {
		...
		getMap().broadcastMessage(
				MaplePacketCreator.healMonster(getObjectId(), hp));
	}
	...
	private void giveExpToCharacter(MapleCharacter attacker, int exp,
		...
		if (attacker.getHp() > 0) {
			...
			if (exp > 0) {
				...
				if (stati.containsKey(MapleMonsterStat.TAUNT)) {
					int alterExp = stati.get(MapleMonsterStat.TAUNT).getStati()
							.get(MapleMonsterStat.TAUNT);
					personalExp *= 1.0 + (alterExp / 100.0);
				}
				Integer holySymbol = attacker
			...
			personalExp /= attacker.getDiseases().contains(MapleDisease.CURSE) ? 2
					: 1;
	...
	public MapleCharacter killBy(MapleCharacter killer) {
		...
		if (killer.inTutorialMap()) {
			totalBaseExpL = this.getExp()
					* killer.getClient().getChannelServer().getExpRate()
		...
		for (AttackerEntry attackEntry : attackers) {
			int baseExp = (int) Math.ceil(totalBaseExp
					* ((double) attackEntry.getDamage() / getMaxHp()));
			attackEntry.killedMob(killer.getMap(), baseExp,
					attackEntry == highest);
		}
		if (this.getController() != null) { // this can/should only happen when
			...
			getController()
					.getClient()
					.getSession()
					.write(MaplePacketCreator.stopControllingMonster(this
							.getObjectId()));
		...
		for (MonsterListener listener : listeners
				.toArray(new MonsterListener[listeners.size()])) {
		...
		if (controllers != null) {
			...
			controllers
					...
					.write(MaplePacketCreator
							.stopControllingMonster(getObjectId()));
		}
	...
	public MaplePacket makeBossHPBarPacket() {
		return MaplePacketCreator.showBossHP(getId(), getHp(), getMaxHp(),
				getTagColor(), getTagBgColor());
	}
	...
	public void sendSpawnData(MapleClient client) {
		...
		if (isFake()) {
			client.getSession().write(
					MaplePacketCreator.spawnFakeMonster(this, 0));
		} else {
			client.getSession().write(
					MaplePacketCreator.spawnMonster(this, false));
		}
		if (stati.size() > 0) {
			for (MonsterStatusEffect mse : activeEffects) {
				MaplePacket packet = MaplePacketCreator.applyMonsterStatus(
						this, client.getPlayer(), getObjectId(),
						mse.getStati(), mse.getSkill().getId(), 0, null);
	...
	public void sendDestroyData(MapleClient client) {
		client.getSession().write(
				MaplePacketCreator.killMonster(getObjectId(), false));
	}

	@Override
	public String toString() {
		return getName() + "(" + getId() + ") at " + getPosition().x + "/"
				+ getPosition().y + " with " + getHp() + "/" + getMaxHp()
				+ "hp, " + getMp() + "/" + getMaxMp() + " mp (alive: "
				+ isAlive() + " oid: " + getObjectId() + " hplocked: " + hpLock
				+ ")";
	}
	...
	public ElementalEffectiveness getEffectiveness(Element e) {
		if (activeEffects.size() > 0
				&& stati.get(MapleMonsterStat.DOOM) != null) {
			// 不处于巫毒术状态
	...
	public boolean applyStatus(MapleCharacter from,
		...
		case WEAK:
			break;
		default:
			throw new RuntimeException("不知道的属性: "
		...
		if (isBoss()
				&& !(status.getStati().containsKey(MapleMonsterStat.SPEED))) {
			return false;
		}

		for (MapleMonsterStat stat : status.getStati().keySet()) {
			MonsterStatusEffect oldEffect = stati.get(stat);
			if (oldEffect != null) {
				oldEffect.removeActiveStatus(stat);
				if (oldEffect.getStati().isEmpty()) {
					oldEffect.CancelCancelTask();
			...
			public void run() {
				if (isAlive()) {
					MaplePacket packet = MaplePacketCreator
							.cancelMonsterStatus(1, getObjectId(), status
									.getStati(), status.getSkill().getId());
					if (getController() != null) {
						packet = MaplePacketCreator.cancelMonsterStatus(
								getController().getId(), getObjectId(),
								status.getStati(), status.getSkill().getId());
					}
					map.broadcastMessage(packet, getPosition());
					if (getController() != null
							&& !getController().isMapObjectVisible(
									MapleMonster.this)) {
		...
		if (!this.getMap().hasEvent()) {
			if (poison) {
				...
				int poisonDamage = Math.min(Short.MAX_VALUE, (int) (getMaxHp()
						/ (70.0 - poisonLevel) + 0.999));
				if (status.getStati().containsKey(MapleMonsterStat.忍者伏击)) // 忍者伏击也有持续伤血的效果
				{
					status.setValue(MapleMonsterStat.忍者伏击,
							Integer.valueOf(poisonDamage));
				} else {
					status.setValue(MapleMonsterStat.POISON,
							Integer.valueOf(poisonDamage));
				}
				status.setPoisonSchedule(timerManager.register(new PoisonTask(
						poisonDamage, from, status, cancelTask, false), 1000,
						1000));
			} else if (venom) {
				int poisonLevel = 0;
				int matk = 0;
				int skillid = 0;
				if (from.getJob().getId() == 412) {
					skillid = 4120005;
				} else if (from.getJob().getId() == 412) {
					skillid = 4220005;
				} else if (from.getJob().getId() == 434) {
					skillid = 4340001;
				} else if (from.getJob().getId() == 1411) {
					skillid = 14110004;
				} else {
					return false;
				}
				...
				matk = SkillFactory.getSkill(skillid).getEffect(poisonLevel)
						.getMatk();
				Random r = new Random();
				int luk = from.getLuk();
				int maxDmg = (int) Math.ceil(Math.min(Short.MAX_VALUE, 0.2
						* luk * matk));
				int minDmg = (int) Math.ceil(Math.min(Short.MAX_VALUE, 0.1
						* luk * matk));
				...
				for (int i = 0; i < getVenomMulti(); i++) {
					poisonDamage = poisonDamage + (r.nextInt(gap) + minDmg); // 伤害叠加
				}
				...
				status.setPoisonSchedule(timerManager.register(new PoisonTask(
						poisonDamage, from, status, cancelTask, false), 1000,
						1000));
			} else if (status.getSkill().getId() == 4111003) { // 网影
				int webDamage = (int) (getMaxHp() / 50.0 + 0.999);
				status.setPoisonSchedule(timerManager.schedule(new PoisonTask(
						webDamage, from, status, cancelTask, true), 3500));
			}
			for (MapleMonsterStat stat : status.getStati().keySet()) {
			...
			MaplePacket packet = MaplePacketCreator.applyMonsterStatus(this,
					from, getObjectId(), status.getStati(), status.getSkill()
							.getId(), 0, null);
			map.broadcastMessage(packet, getPosition());
			if (getController() != null
					&& !getController().isMapObjectVisible(this)) {
				getController().getClient().getSession().write(packet);
			}
			ScheduledFuture<?> schedule = timerManager.schedule(cancelTask,
	...
	public void applyMonsterBuff(final MapleMonsterStat status, final int x,
			final int skillId, long duration, final MobSkill skill) {
		TimerManager timerManager = TimerManager.getInstance();
		final Runnable cancelTask = new Runnable() {
			@Override
			public void run() {
				if (isAlive()) {
					// MaplePacket packet =
					// MaplePacketCreator.cancelMonsterStatus(getController(),
					// getObjectId(), Collections.singletonMap(status,
					// Integer.valueOf(x)), 0);
					MaplePacket packet = MaplePacketCreator
							.cancelMonsterStatus(
									1,
					...
					if (getController() != null) {
						MaplePacketCreator.cancelMonsterStatus(getController()
								.getId(), getObjectId(), Collections
								.singletonMap(status, Integer.valueOf(x)), 0);
					}
					map.broadcastMessage(packet, getPosition());
					if (getController() != null
							&& !getController().isMapObjectVisible(
		...
		MaplePacket packet = MaplePacketCreator.applyMonsterStatus(this,
				getController(), getObjectId(),
				Collections.singletonMap(status, x), skillId, 0, skill);
		map.broadcastMessage(packet, getPosition());
		if (getController() != null
				&& !getController().isMapObjectVisible(this)) {
			getController().getClient().getSession().write(packet);
	...
	public void addMonsterBuff(MapleMonsterStat status) {
		this.monsterBuffs.add(status);
	}

	public void removeMonsterBuff(MapleMonsterStat status) {
		this.monsterBuffs.remove(status);
	}

	public boolean isBuffed(MapleMonsterStat status) {
	...
	public boolean canUseSkill(MobSkill toUse) {
		...
		if (toUse.getLimit() > 0) {
			...
			if (this.skillsUsed.containsKey(new Pair<Integer, Integer>(toUse
					.getSkillId(), toUse.getSkillLevel()))) {
				int times = this.skillsUsed.get(new Pair<Integer, Integer>(
	...
	public void usedSkill(final int skillId, final int level, long cooltime) {
		...
		if (this.skillsUsed.containsKey(new Pair<Integer, Integer>(skillId,
				level))) {
			int times = this.skillsUsed.get(new Pair<Integer, Integer>(skillId,
					level)) + 1;
			this.skillsUsed.remove(new Pair<Integer, Integer>(skillId, level));
			this.skillsUsed.put(new Pair<Integer, Integer>(skillId, level),
					times);
		} else {
	...
	public boolean containsStatus(MapleMonsterStat status) {
	...
	private final class PoisonTask implements Runnable {
		...
		private PoisonTask(int poisonDamage, MapleCharacter chr,
				MonsterStatusEffect status, Runnable cancelTask,
				boolean shadowWeb) {
		...
		public void run() {
			...
			if (damage >= hp) {
				...
				if (!shadowWeb) {
					cancelTask.run();
					status.CancelCancelTask();
				}
			}
			if (hp > 1 && damage > 0) {
				damage(chr, damage, false);
				if (shadowWeb) {
					map.broadcastMessage(MaplePacketCreator.damageMonster(
	...
	private class PartyAttackerEntry implements AttackerEntry {
		...
		public void killedMob(MapleMap map, int baseExp, boolean mostDamage) {
			...
			Map<MapleCharacter, Integer> expMap = new HashMap<MapleCharacter, Integer>(
					6);
			for (Entry<MapleCharacter, OnePartyAttacker> attacker : attackers_
					.entrySet()) {
				...
				for (MaplePartyCharacter partychar : party.getMembers()) {
					if (attacker.getKey().getLevel() - partychar.getLevel() <= 5
						...
						MapleCharacter pchr = cserv.getPlayerStorage()
				...
				double expFraction = (innerBaseExp * expBonus)
				...
				for (MapleCharacter expReceiver : expApplicable) {
					...
					double expWeight = (expReceiver == attacker.getKey() ? 2.0
							: 1.0);
			...
			for (Entry<MapleCharacter, Integer> expReceiver : expMap.entrySet()) {
				boolean white = mostDamage ? expReceiver.getKey() == highest
						: false;
				giveExpToCharacter(expReceiver.getKey(),
						expReceiver.getValue(), white, expMap.size());
			}
		}
