...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(MapleMonster.class);
    ...
    private Collection<AttackerEntry> attackers = Collections.synchronizedList(new ArrayList<AttackerEntry>());
    private EventInstanceManager eventInstance = null;
    private Collection<MonsterListener> listeners = Collections.synchronizedList(new ArrayList<MonsterListener>(1));
    private MapleCharacter highestDamageChar;
    private final Map<MapleMonsterStat, MonsterStatusEffect> stati = Collections.synchronizedMap(new EnumMap<MapleMonsterStat, MonsterStatusEffect>(MapleMonsterStat.class));
    private final List<MonsterStatusEffect> activeEffects = Collections.synchronizedList(new ArrayList<MonsterStatusEffect>());
    private MapleMap map;
    private int VenomMultiplier = 0;
    private boolean fake = false;
    private boolean dropsDisabled = false;
    private List<Pair<Integer, Integer>> usedSkills = Collections.synchronizedList(new ArrayList<Pair<Integer, Integer>>());
    private final Map<Pair<Integer, Integer>, Integer> skillsUsed = Collections.synchronizedMap(new HashMap<Pair<Integer, Integer>, Integer>());
    ...
    public int getMaxDrops(MapleCharacter chr) {
        ...
        if (isPQMonster()) {
            ...
            if (stati.containsKey(MapleMonsterStat.TAUNT)) {
                int alterDrops = stati.get(MapleMonsterStat.TAUNT).getStati().get(MapleMonsterStat.TAUNT);
    ...
    private void giveExpToCharacter(MapleCharacter attacker, int exp, boolean highestDamage, int numExpSharers) {
        ...
        if (attacker.getHp() > 0) {
            ...
            if (exp > 0) {
                ...
                if (stati.containsKey(MapleMonsterStat.TAUNT)) {
                    int alterExp = stati.get(MapleMonsterStat.TAUNT).getStati().get(MapleMonsterStat.TAUNT);
    ...
    public MapleCharacter killBy(MapleCharacter killer) {
        ...
        if (killer.inTutorialMap()) {
            totalBaseExpL = this.getExp() * killer.getClient().getChannelServer().getExpRate() * killer.getClient().getPlayer().hasEXPCard();
        } else {
            totalBaseExpL = this.getExp() * killer.getClient().getChannelServer().getExpRate() * killer.getClient().getPlayer().hasEXPCard();
        }
    ...
    public void sendSpawnData(MapleClient client) {
        ...
        if (stati.size() > 0) {
            for (MonsterStatusEffect mse : activeEffects) {
                MaplePacket packet = MaplePacketCreator.applyMonsterStatus(this, client.getPlayer(), getObjectId(), mse.getStati(), mse.getSkill().getId(), 0, null);
    ...
    public ElementalEffectiveness getEffectiveness(Element e) {
        if (activeEffects.size() > 0 && stati.get(MapleMonsterStat.DOOM) != null) {
            //不处于巫毒术状态
    ...
    public boolean applyStatus(MapleCharacter from, final MonsterStatusEffect status, boolean poison, long duration, boolean venom) {
        ...
        switch (stats.getEffectiveness(status.getSkill().getElement())) {
            ...
            case WEAK:
                break;
            default:
                throw new RuntimeException("不知道的属性: " + stats.getEffectiveness(status.getSkill().getElement()));
        }
        ...
        if (isBoss() && !(status.getStati().containsKey(MapleMonsterStat.SPEED))) {
            return false;
        }
        ...
        final Runnable cancelTask = new Runnable() {
            ...
            public void run() {
                if (isAlive()) {
                    MaplePacket packet = MaplePacketCreator.cancelMonsterStatus(1, getObjectId(), status.getStati(), status.getSkill().getId());
                    if (getController() != null) {
                        packet = MaplePacketCreator.cancelMonsterStatus(getController().getId(), getObjectId(), status.getStati(), status.getSkill().getId());
                    }
        ...
        if (!this.getMap().hasEvent()) {
            if (poison) {
                ...
                {
                    status.setValue(MapleMonsterStat.忍者伏击, Integer.valueOf(poisonDamage));
                } else {
                    status.setValue(MapleMonsterStat.POISON, Integer.valueOf(poisonDamage));
                }
            ...
            MaplePacket packet = MaplePacketCreator.applyMonsterStatus(this, from, getObjectId(), status.getStati(), status.getSkill().getId(), 0, null);
            map.broadcastMessage(packet, getPosition());
    ...
    public void applyMonsterBuff(final MapleMonsterStat status, final int x, final int skillId, long duration, final MobSkill skill) {
        TimerManager timerManager = TimerManager.getInstance();
        final Runnable cancelTask = new Runnable() {
            @Override
            public void run() {
                if (isAlive()) {
                    //MaplePacket packet = MaplePacketCreator.cancelMonsterStatus(getController(), getObjectId(), Collections.singletonMap(status, Integer.valueOf(x)), 0);   
                    MaplePacket packet = MaplePacketCreator.cancelMonsterStatus(1, getObjectId(), Collections.singletonMap(status, Integer.valueOf(x)), 0);
                    if (getController() != null) {
                        MaplePacketCreator.cancelMonsterStatus(getController().getId(), getObjectId(), Collections.singletonMap(status, Integer.valueOf(x)), 0);
                    }
                    map.broadcastMessage(packet, getPosition());
                    if (getController() != null && !getController().isMapObjectVisible(MapleMonster.this)) {
                        getController().getClient().getSession().write(packet);
        ...
        MaplePacket packet = MaplePacketCreator.applyMonsterStatus(this, getController(), getObjectId(), Collections.singletonMap(status, x), skillId, 0, skill);
        map.broadcastMessage(packet, getPosition());
        if (getController() != null && !getController().isMapObjectVisible(this)) {
            getController().getClient().getSession().write(packet);
        }
    ...
    private class PartyAttackerEntry implements AttackerEntry {
        ...
        public void killedMob(MapleMap map, int baseExp, boolean mostDamage) {
            ...
            Map<MapleCharacter, Integer> expMap = new HashMap<MapleCharacter, Integer>(6);
