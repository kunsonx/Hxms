...
public class MobSkill {
	...
	private Logger log = Logger.getLogger(MobSkill.class);
	private int skillId;
	...
	public void applyEffect(MapleCharacter player, MapleMonster monster,
		...
		case 100:
		case 110:
			monStat = MapleMonsterStat.WEAPON_ATTACK_UP;
			break;
		case 101:
		case 111:
			monStat = MapleMonsterStat.MAGIC_ATTACK_UP;
			break;
		case 102:
		case 112:
			monStat = MapleMonsterStat.WEAPON_DEFENSE_UP;
			break;
		case 103:
		case 113:
			monStat = MapleMonsterStat.MAGIC_DEFENSE_UP;
			break;
		case 114: // Heal
			heal = true;
			break;
		...
		case 127:
			...
			break;
		case 128: // Seduce
			if (makeChanceResult()) { // HT is 100%, yet some others mobs aren't
										// - we can handle those later
				/*
				 * MapleSquad htSquad =
				 * player.getClient().getChannelServer().getMapleSquad
				 * (MapleSquadType.HORNTAIL); if (htSquad != null &&
				 * htSquad.containsMember(player)) {
				 */
				int i = 0;
				for (MapleCharacter htMember : player.getMap().getCharacters()) {
					if (htMember.isAlive()
							&& htMember.getMap() == player.getMap()) {
						if (i < getCount()) {
		...
		case 129: // 驱逐
			...
			break;
		case 131: // 毒雾 乌贼怪 扎昆 驮狼雪人
			monster.getMap().spawnMist(
					new MapleMist(calculateBoundingBox(monster.getPosition(),
							true), monster, this), x * 10, false);
			break;
		case 140:
			if (makeChanceResult()
					&& !monster.isBuffed(MapleMonsterStat.MAGIC_IMMUNITY)) {
				monStat = MapleMonsterStat.WEAPON_IMMUNITY;
			}
			break;
		case 141:
			if (makeChanceResult()
					&& !monster.isBuffed(MapleMonsterStat.WEAPON_IMMUNITY)) {
				monStat = MapleMonsterStat.MAGIC_IMMUNITY;
			}
			break;
		case 200:
			...
			if (player.getEventInstance() != null) {
				if (player.getEventInstance().getName().indexOf("FoJ", 0) != -1) {
					canSpawn = false;
				} else if (player.getEventInstance().getName()
						.indexOf("BossQuest", 0) != -1
			...
			if (monster.getMap().getSpawnedMonstersOnMap() < 50 && canSpawn) {
				for (Integer mobId : getSummons()) {
					...
					case 8500004: // Pap bomb
						// Spawn between -500 and 500 from the monsters X
						// position
						xpos = (int) (monster.getPosition().getX()
								+ Math.ceil(Math.random() * 1000.0) - 500);
						if (ypos != -590) {
							ypos = (int) monster.getPosition().getY();
						}
						break;
					...
					// get map left and right wall. Any suggestions? PM TheRamon
					...
					monster.getMap().spawnMonsterWithEffect(toSpawn,
							getSpawnEffect(), toSpawn.getPosition());
				}
			}
			break;
		default:
			if (log.isDebugEnabled()) {
				player.dropMessage("未处理的怪物技能 skillid :" + skillId);
			}
			break;
		}
		if (monStat != null || heal) {
			if (lt != null && rb != null && skill) {
				List<MapleMapObject> objects = getObjectsInRange(monster,
				...
				if (heal) {
					...
					for (MapleMapObject mons : objects) {
						if (!monster.isBuffed(monStat)) {
							((MapleMonster) mons).applyMonsterBuff(monStat,
									getX(), getSkillId(), getDuration(), this);
						}
					}
				}
			} else {
				if (heal) {
					...
					if (!monster.isBuffed(monStat)) {
						monster.applyMonsterBuff(monStat, getX(), getSkillId(),
								getDuration(), this);
					}
				}
			}
		...
		if (disease != null || dispel || banish) {
			if (skill && lt != null && rb != null) {
				List<MapleCharacter> characters = getPlayersInRange(monster,
						player);
				for (MapleCharacter character : characters) {
					if (dispel) {
						...
						if (player.getEventInstance() == null) {
							...
							MaplePortal pto = to
									.getPortal((short) (0 + 10 * Math.random()));
	...
	private List<MapleCharacter> getPlayersInRange(MapleMonster monster,
		...
		Rectangle bounds = calculateBoundingBox(monster.getPosition(),
