...
public class MapleItemInformationProvider {
    ...
    static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(MapleItemInformationProvider.class);
    ...
    public final void loaddefData() {
        ...
        //    List<MapleItemInventryType> itlist = session.createQuery("from MapleItemInventryType").list();
        List<MapleItemInventryType> itlist = session.getNamedQuery("alliit").list();
        for (MapleItemInventryType mapleItemInventryType : itlist) {
            inventoryTypeCache.put(mapleItemInventryType.getId(), mapleItemInventryType.getType());
        }

        List<MapleScriptedItemNpc> list = session.getNamedQuery("allsin").list();
        for (MapleScriptedItemNpc mapleScriptedItemNpc : list) {
            scriptedItemCache.put(mapleScriptedItemNpc.getId(), mapleScriptedItemNpc.getNpc());
        }


        List<MapleExpCardListInfo> e_list = session.createQuery("from MapleExpCardListInfo").list();
        for (MapleExpCardListInfo mapleExpCardListInfo : e_list) {
            getExpCardTimes.put(mapleExpCardListInfo.getItemid(), mapleExpCardListInfo);
        }
        ...
        List<MapleItemWholePrice> w_list = session.getNamedQuery("alliwp").list();
        ...
        for (MapleItemPrice mapleItemPrice : ip_list) {
            priceCache.put(mapleItemPrice.getItemid(), mapleItemPrice.getPrice());
        }


        List<MapleEquipStats> eslist = session.getNamedQuery("alles").list();
        for (MapleEquipStats mapleEquipStats : eslist) {
            equipStatsCache.put(mapleEquipStats.getItemid(), mapleEquipStats.getBaseStats());
        }
    ...
    public MapleInventoryType getInventoryType(int itemId) {
        if (inventoryTypeCache.containsKey(itemId)) {
            ...
            /*Session session = DatabaseConnection.getSession();
             MapleItemInventryType type = (MapleItemInventryType) session.createQuery("from MapleItemInventryType as cs where cs.id = ?").setMaxResults(1).setInteger(0, itemId).uniqueResult();
             if (type != null) {
             stats = type.getType();
             }
             session.close();*/
    ...
    protected Map<String, Integer> getEquipStats(int itemId) {
        if (equipStatsCache.containsKey(itemId)) {
            ...
            Session session = DatabaseConnection.getSession();
            // MapleEquipStats stats = (MapleEquipStats) session.createQuery("from MapleEquipStats as cs where cs.itemid =:id").setParameter("id", itemId).setMaxResults(1).uniqueResult();
            MapleEquipStats stats = (MapleEquipStats) session.get(MapleEquipStats.class, itemId);
            session.close();
            equipStatsCache.put(itemId, stats != null ? stats.getBaseStats() : null);
    ...
    public String getName(int itemId) {
        ...
        Session session = DatabaseConnection.getSession();
        MapleItemName name = (MapleItemName) session.get(MapleItemName.class, itemId);
        session.close();
        if (name == null) {
            return null;
        }
    ...
    public boolean isDropRestricted(IItem item) {
        return isDropRestricted(item.getItemId()) | item.HasFlag(InventoryConstants.Items.Flags.UNTRADEABLE);
    }
        ...
        try {
            ...
            for (MapleData b : a.getChildren()) { //getChildren 子项名 0 1 2 4 8 16
                ...
                for (MapleData c : b.getChildren()) { //itemid 01032062
                    ...
                    if (Integer.parseInt(c.getName()) == 1072362) //这货 0_item 节点有问题 用HR打开会显示[未将对象引用设置到对象的实例]
                    {
                        continue;
                    }
    ...
    public IEquip MagniflerEquipWithId(IItem equip) {
        ...
        if (oldPotentialLevel == 0x11) { //B级
            ...
            for (int i = 1; i <= (int) (2 + Math.random() * (2 - 1 + 1)); i++) {
                nEquip.setPotential(i, randomPotentiaStat_1(1, nEquip.getItemId(), nEquip.getLevel()));
            }
        }
        if (!isFirst) {
            //不是第一次的话就重设潜能
            ...
            for (int i = 1; i <= count; i++) {
                nEquip.setPotential(i, randomPotentiaStat_1(nEquip.getIdentify(), nEquip.getItemId(), nEquip.getLevel()));
            }
            nEquip.setIdentify(nEquip.getIdentified());
        }
        return nEquip;
        ...
        switch (oldPotentiaLevel) {
            ...
            case 6: {
                ...
                if (rnd == 0) {
                    ...
                    int OptionId[] = {10051, 10052, 10053, 10054, 10055, 20070, 20086, 20181, 20201, 20206, 20291, 20351, 20352, 20353, 20366, 20376, 20396, 20401, 20406, 20501, 20511, 20650, 20656};
                    rndOne = OptionId[(int) Math.floor(Math.random() * OptionId.length)];
                } else if (rnd == 3) {
                    ...
                    int OptionId[] = {20041, 20042, 20043, 20044, 20045, 20046, 20047, 20048, 20051, 20052, 20053, 20054, 20055, 20070, 20086, 20181, 20201, 20206, 20291, 20351, 20352, 20353, 20366, 20376, 20396, 20401, 20406, 20501, 20511, 20650, 20656, 20041, 20042, 20043, 20044, 20045, 20046, 20047, 20048, 20051, 20052, 20053, 20054, 20055, 20070, 20086, 20181, 20201, 20206, 20291, 20351, 20352, 20353, 20366, 20376, 20396, 20401, 20406, 20501, 20511, 20650, 20656};
                    rndOne = OptionId[(int) Math.floor(Math.random() * OptionId.length)];
                }
                break;
            }
            case 7: {
                ...
                if (rnd == 0) {
                    ...
                    int OptionId[] = {20070, 20086, 20181, 20201, 20206, 20291, 20351, 20352, 20353, 20366, 20376, 20396, 20401, 20406, 20501, 20511, 20650, 20656, 30070, 30086, 30106, 30107, 30291, 30356, 30357, 30366, 30371, 30376, 30501, 30511, 30551, 30601, 30602, 30650, 30656, 30701, 30702, 31001, 31002, 31003, 31004};
                    rndOne = OptionId[(int) Math.floor(Math.random() * OptionId.length)];
                } else if (rnd == 3) {
    ...
    private int getItemType(int itemId) {
        if (itemId >= 1010000 && itemId < 1040000 || itemId >= 1122000 && itemId < 1143000) { //挂坠 耳环
            return 20;
            //cat = "Accessory";
    ...
    private int randomPotentiaStat_1(int oldPotentiaLevel, int itemId, byte itemLevel) {
        int rndOne = 0;
        int rnd = Randomizer.getInstance().nextInt(5);
    ...
    public IItem starScrollEquipWithId_2(IItem equip, int scrollId, boolean isGM) {
        if (equip instanceof Equip) {
            ...
            if (success) {
                ...
                //nEquip.setUpgradeLevel((byte) (nEquip.getUpgradeLevel() + 1));
                if (nEquip.getStr() > 0) {
