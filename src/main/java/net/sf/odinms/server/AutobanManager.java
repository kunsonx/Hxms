...
public class AutobanManager implements Runnable {
		...
		try {
			c.getChannelServer()
					...
					.broadcastMessage(null,
							MaplePacketCreator.serverNotice(0, s).getBytes());
		} catch (RemoteException e) {
			ServerExceptionHandler.HandlerRemoteException(e);
			c.getChannelServer().reconnectWorld();
		}
	...
	public synchronized void addPoints(MapleClient c, int points,
			long expiration, String reason) {
		if (c.getPlayer().isGM()) {
			return;
		}
		...
		if (this.points.get(acc) >= AUTOBAN_POINTS) {
			...
			StringBuilder banReason = new StringBuilder(
					"Autoban for Character ");
			banReason.append(name);
			banReason.append(" (IP ");
			banReason.append(c.getRemoteAddress());
			banReason.append("): ");
