...
public class MapleRPSGame implements MapleInteractionRoom {
	...
	public static void inviteRPSGame(MapleCharacter src, MapleCharacter dst) {
		/*
		 * if (src == null || src.getRPSGame() == null) { return; }
		 */
		if (dst == null) {// 假人代码处理
			cancelRPSGame(src);
		...
		if (!(src.getFame() >= 0 && dst.getFame() >= 0)) {// 整个表达式取反.
			src.getClient()
					...
					.write(MaplePacketCreator
							.serverNotice(5, "你或者对方没人气了.不能玩哦."));
			cancelRPSGame(src);
			return;
		}
		if (dst.getRPSGame() == null && dst.getTrade() == null) {

			dst.getClient().getSession()
					.write(MaplePacketCreator.getTradeInvite(src, false));
		} else {
			src.getClient().getSession()
	...
	public static void visitRPSGame(MapleCharacter src, MapleCharacter dst) {
		...
		if (src.getRPSGame() != null && dst.getRPSGame() == null) {
			...
			src.getClient().getSession()
					.write(MaplePacketCreator.getTradePartnerAdd(dst));
			dst.getClient()
					.getSession()
					.write(MaplePacketCreator.getTradeStart(dst.getClient(),
							dst.getRPSGame(), (byte) 1, false));
			dst.SendPacket(MaplePacketCreator.getRPSGameStart());
		}
	}

	public static void declineRPSGame(MapleCharacter dstCharacter,
			MapleCharacter src) {
		if (src != null) {
			src.getClient()
					.getSession()
					.write(MaplePacketCreator.serverNotice(5,
							dstCharacter.getName() + " 拒绝了你的挑战邀请"));
		}
		cancelRPSGame(src);
	}
	...
	public void doCreateResult() {
		...
		getPartner().chr.SendPacket(MaplePacketCreator.getRPSGameResult(
				getPartner().win, select));
	}
	...
	public void cancel() {
		chr.getClient().getSession()
				.write(MaplePacketCreator.getTradeCancel((byte) number));
	}
}
