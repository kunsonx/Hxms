...
public class MapleStatEffect implements Serializable {
	...
	public static final List<Pair<MapleBuffStat, Integer>> 恶魔复仇者 = Collections
			.singletonList(Pair.Create(MapleBuffStat.恶魔复仇者, 3));
	public static final List<Pair<MapleBuffStat, Integer>> 侠盗本能 = Collections
			.singletonList(Pair.Create(MapleBuffStat.侠盗本能_击杀点, 0));
	// public static List<MapleBuffStat> 骑宠BUFF =
	// Arrays.asList(MapleBuffStat.坐骑状态);
	...
	public static MapleStatEffect loadSkillEffectFromData(MapleData source,
			int sourceid, int level, boolean beginner, boolean overTime) {
		...
		ret.watk = (short) ret.技能解析(
				MapleDataTool.getString("pad", source, null), level, 0);
		ret.matk = (short) ret.技能解析(
				MapleDataTool.getString("mad", source, null), level, 0);
		ret.wdef = (short) ret.技能解析(
				MapleDataTool.getString("pdd", source, null), level, 0);
		ret.mdef = (short) ret.技能解析(
				MapleDataTool.getString("mdd", source, null), level, 0);
		ret.duration = ret.技能解析(MapleDataTool.getString("time", source, null),
				level, -1);
		ret.epad = ret.技能解析(MapleDataTool.getString("epad", source, null),
				level, 0);
		ret.epdd = ret.技能解析(MapleDataTool.getString("epdd", source, null),
				level, 0);
		ret.emdd = ret.技能解析(MapleDataTool.getString("emdd", source, null),
				level, 0);
		ret.acc = (short) ret.技能解析(
				MapleDataTool.getString("acc", source, null), level, 0);
		ret.avoid = (short) ret.技能解析(
				MapleDataTool.getString("eva", source, null), level, 0);
		ret.speed = (short) ret.技能解析(
				MapleDataTool.getString("speed", source, null), level, 0);
		ret.jump = (short) ret.技能解析(
				MapleDataTool.getString("jump", source, null), level, 0);
		ret.morphId = ret.技能解析(MapleDataTool.getString("morph", source, null),
				level, 0);
		ret.emhp = ret.技能解析(MapleDataTool.getString("emhp", source, null),
				level, 0);
		ret.emmp = ret.技能解析(MapleDataTool.getString("emmp", source, null),
				level, 0);
		ret.hp = (short) ret.技能解析(MapleDataTool.getString("hp", source, null),
				level, 0);
		ret.mp = (short) ret.技能解析(MapleDataTool.getString("mp", source, null),
				level, 0);
		ret.mpR = (ret.技能解析(MapleDataTool.getString("mpR", source, null),
				level, 0) / 100.0);
		ret.hpR = (ret.技能解析(MapleDataTool.getString("hpR", source, null),
				level, 0) / 100.0);
		ret.mpCon = (short) ret.技能解析(
				MapleDataTool.getString("mpCon", source, null), level, 0);
		ret.hpCon = (short) ret.技能解析(
				MapleDataTool.getString("hpCon", source, null), level, 0);
		...
		ret.powerCon = (byte) ret.技能解析(
				MapleDataTool.getString("powerCon", source, "0"), level, 0);
		ret.damage = ret.技能解析(MapleDataTool.getString("damage", source, null),
				level, 100);
		int iprop = ret.技能解析(MapleDataTool.getString("prop", source, null),
				level, 100);
		ret.prop = iprop / 100.0;
		// 技能提升Hp Mp上限
		ret.mhpR = (ret.技能解析(MapleDataTool.getString("mhpR", source, null),
				level, 0) / 100.0);
		ret.mmpR = (ret.技能解析(MapleDataTool.getString("mmpR", source, null),
				level, 0) / 100.0);
		// 常用取值结束

		ret.range = ret.技能解析(MapleDataTool.getString("range", source, null),
				level, 0);
		ret.dex = ret.技能解析(MapleDataTool.getString("dex", source, null), level,
				0);
		ret.mobCount = ret.技能解析(
				MapleDataTool.getString("mobCount", source, null), level, 1);
		ret.mastery = ret.技能解析(
				MapleDataTool.getString("mastery", source, null), level, 0);
		ret.attackCount = ret.技能解析(
				MapleDataTool.getString("attackCount", source, null), level, 1);
		ret.mobCount = ret.技能解析(
				MapleDataTool.getString("mobCount", source, null), level, 1);
		ret.cooldown = ret.技能解析(
				MapleDataTool.getString("cooltime", source, null), level, 0);
		ret.bulletCount = ret.技能解析(
				MapleDataTool.getString("bulletCount", source, null), level, 1);
		ret.itemCon = ret.技能解析(
				MapleDataTool.getString("itemCon", source, null), level, 0);
		ret.itemConNo = ret.技能解析(
				MapleDataTool.getString("itemConNo", source, null), level, 0);
		ret.moneyCon = ret.技能解析(
				MapleDataTool.getString("moneyCon", source, null), level, 0);
		ret.bulletConsume = ret.技能解析(
				MapleDataTool.getString("bulletConsume", source, null), level,
				0);
		ret.MDamageOver = ret.技能解析(
				MapleDataTool.getString("MDamageOver", source, null), level,
				999999);
		...
		// Map<MonsterStatus, Integer> monsterStatus = new
		// ArrayMap<MonsterStatus, Integer>();
		if (ret.skill) { // hack because we can't get from the
							// datafile...无法从数据文件获取的技能列表
			switch (sourceid) {
			case 2001002: // 魔法盾
			case 12001001:// 魔法盾
			case 22111001:// 魔法盾
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.MAGIC_GUARD, Integer.valueOf(ret.x)));
				break;
			case 2301003: // 神之保护
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.INVINCIBLE, Integer.valueOf(ret.x)));
				break;
			case 9001004: // 隐藏术
			...
			case 夜行者.隐身术:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.DARKSIGHT, Integer.valueOf(ret.x)));
				break;
			/*
			...
			case 4111001: // 聚财术
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.MESOUP, Integer.valueOf(ret.x)));
				break;
			case 4111002: // 影分身
			case 4211008:// 影分身
			case 14111000:// 影分身
			case 4331002: // 镜像分身
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.SHADOWPARTNER, Integer.valueOf(ret.x)));
				IsIntValue.add(sourceid);
				ret.initForeign(new MapleForeignBuffShortStat(
			...
			case 33101003: // 弩骑 无形箭：弩
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.SOULARROW, Integer.valueOf(ret.x)));
				break;
			case 1211004: // 火焰冲击
			case 1211006: // 寒冰冲击
			case 1211008: // 雷鸣冲击
			...
			case 魂骑士.灵魂属性:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.WK_CHARGE, Integer.valueOf(ret.x)));
				ret.initForeign(new MapleForeignBuffShortStat(
			...
			case 36101004:// 尖兵加速
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.BOOSTER, Integer.valueOf(ret.x)));
				break;
			case 5121009:// 极速领域
			case 奇袭者.极速领域:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.极速领域, Integer.valueOf(ret.x)));
				break;
			case 1101006: // 愤怒之火
			case 11101003: // 愤怒之火
				// 以上2个技能093取消防御减少
			case 1121010: // 葵花宝典 以上技能都更换了mask
				...
				// Integer>(MapleBuffStat.WATK, Integer.valueOf(ret.watk)));
				break;
			case 1301006: // 极限防御
			case 2001003: // 魔法铠甲
			case 炎术士.魔法铠甲:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.WDEF, Integer.valueOf(ret.wdef)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.MDEF, Integer.valueOf(ret.mdef)));
				break;
			case 1001003: // 圣甲术
			case 11001001:
				statups.add(new Pair<MapleBuffStat, Integer>(
			...
			case 2201001: // 精神力
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.MATK, Integer.valueOf(ret.matk)));
				break;
			case 4101004: // 轻功
			case 4201003: // 轻功
			case 9001001: // gm 轻功
			...
			case 4301003:// 暗影轻功
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.SPEED, Integer.valueOf(ret.speed)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.JUMP, Integer.valueOf(ret.jump)));
				break;
			case 2301004: // 祝福
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.祝福,
						Integer.valueOf(level)));
				break;
			case 3001003: // 集中术
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.ACC,
						Integer.valueOf(ret.acc)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.AVOID, Integer.valueOf(ret.avoid)));
				break;
			...
			case 13111001:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.EWatk, Integer.valueOf(ret.epad)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.CONCENTRATE, Integer.valueOf(ret.x)));
				break;
			case 5001005: // 疾驰
			case 4321000: // 龙卷风
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.疾驰_龙卷风, Integer.valueOf(ret.x)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.疾驰_龙卷风, Integer.valueOf(ret.y)));
				break;
			case 1101007: // 伤害反击
			case 1201007:// 伤害反击
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.POWERGUARD, Integer.valueOf(ret.x)));
				break;
			case 1301007:// 神圣之火
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.HYPERBODYHP, Integer.valueOf(ret.x)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.HYPERBODYMP, Integer.valueOf(ret.y)));
				break;
			case 1120003:
			case 1111002: // 斗气集中
			case 11111001:// 斗气集中
				ret.initForeign(new MapleForeignBuffNoSkill(MapleBuffStat.COMBO) {
					...
					public void writePacket(
							MaplePacketLittleEndianWriter mplew, int value) {
				...
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.COMBO, Integer.valueOf(1)));
				break;
			case 1011:// 勇士的意志
			case 20001011:// 勇士的意志
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.BERSERK_FURY, Integer.valueOf(1)));
				break;
			case 1004: // 骑兽技能
			case 10001004:// 骑兽技能
			case 20001004:// 骑兽技能
			...
			case 20011004:// 骑兽技能
			case 33001001:// 美洲豹骑士
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.骑宠1,
						Integer.valueOf(0)));
				break;
			case 1311006: // 龙咆哮
			case 21111009:
				...
				break;
			case 1311008: // 龙之力
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.龙之力,
						Integer.valueOf(ret.技能解析(
								MapleDataTool.getString("str", source, null),
								level, 0))));
				break;
			case 1121000: // 冒险岛勇士
			case 1221000: // 冒险岛勇士
			...
			case 36121008:// 尖兵勇士
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.MAPLE_WARRIOR, Integer.valueOf(ret.x)));
				break;
			case 3121002: // 火眼晶晶
			case 3221002: // 火眼晶晶
			case 33121004:// 弩骑火眼
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.SHARP_EYES, Integer.valueOf(ret.x << 8
								| ret.y)));
				break;
			case 1321007: // 灵魂助力
			...
			case 12111004:// 火魔兽
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.SUMMON, Integer.valueOf(1)));
				break;
			case 2311003: // 神圣祈祷
			case 9001002: // GM 神圣祈祷
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.HOLY_SYMBOL, Integer.valueOf(ret.x)));
				break;
			case 4111009: // 暗器伤人
			case 14111007:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.SHADOW_CLAW, Integer.valueOf(level)));
				break;
			case 2121004:// 终极无限
			case 2221004:// 终极无限
			case 2321004: // 终极无限
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.STANCE, Integer
								.valueOf((int) (ret.prop * 100))));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.INFINITY, Integer.valueOf(ret.x)));
			...
			case 32121005: // 战法 稳如泰山
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.STANCE, Integer.valueOf(ret.x))); // ret.x定值为1
				break;
			case 1005: // 英雄之回声
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.ECHO_OF_HERO, Integer.valueOf(ret.x)));
				break;
			/*
			 * case 2121002: // 魔法反击 case 2221002: // 魔法反击 case 2321002: // 魔法反击
			 * statups.add(new Pair<MapleBuffStat,
			 * Integer>(MapleBuffStat.MANA_REFLECTION, Integer.valueOf(1)));
			 * break;
			 */
			case 2321005: // (现 进阶祝福)圣灵之盾
				ret.indieMhp = ret.技能解析(
						MapleDataTool.getString("indieMhp", source, null),
						level, 0);
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.进阶祝福, Integer.valueOf(level)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.EMaxHp, Integer.valueOf(ret.indieMhp)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.EMaxMp, Integer.valueOf(ret.indieMhp)));
				break;
			case 3111002: // 射手 - 替身术
			case 3211002: // 游侠 - 替身术
			...
			case 33111003: // 野性陷阱
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.PUPPET, Integer.valueOf(1)));
				break;
			/*
			 * case 4341006: // 暗影双刀 - 傀儡召唤 statups.add(new Pair<MapleBuffStat,
			 * Integer>(MapleBuffStat.傀儡召唤, Integer.valueOf(1))); break;
			 */
			...
			case 32111010:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.快速移动精通, Integer.valueOf(ret.x)));
				ret.duration = 999999999;
				break;
			case 32121003: // 飓风
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.飓风,
						Integer.valueOf(1)));
				break;
			case 32111004: // 转化
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.转化,
			...
			case 战法.进阶蓝色灵气:
			case 战法.进阶黄色灵气:
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.灵气,
						Integer.valueOf(ret.x)));
				ret.duration = 999999999;
				break;
			case 21111001: // 灵巧击退
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.灵巧击退, Integer.valueOf(ret.x)));
				break;
			case 32101004: // 伤害吸收
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.伤害吸收, Integer.valueOf(ret.x)));
				break;
			case 21101003: // 抗压
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.抗压,
						Integer.valueOf(ret.x)));
				break;
			case 22131001: // 魔法屏障
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.魔法屏障, Integer.valueOf(ret.x)));
				break;
			case 4341007: // 荆棘 093更改 变为2个buff叠加 原有buff取消
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.荆棘, Integer.valueOf(ret.x)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.EWatk, Integer.valueOf(ret.epad)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.STANCE, Integer.valueOf(ret.x)));
				break;
			case 龙神.缓速术: // 缓速术
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.缓速术,
						Integer.valueOf(ret.x)));
				break;
			case 32111005: // 霸体
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.霸体,
						Integer.valueOf(ret.x)));
				break;
			case 35121003:
			...
			case 机械师.金属机甲_重机枪_4转:
				ret.duration = 2100000000;
				ret.overTime = true;
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.机械师,
						Integer.valueOf(level)));
				break;
			case 21100005: // 连环吸血
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.连环吸血, Integer.valueOf(ret.x)));
				break;
			case 32111006: // 重生
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.重生,
						Integer.valueOf(ret.x)));
				break;
			case 33121006: // 暴走状态
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.暴走HP, Integer.valueOf(ret.x)));
				// 这个决定是否出现翅膀
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.暴走攻击, Integer.valueOf(ret.y)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.SPEED, Integer.valueOf(ret.z)));
				break;
			case 33111004:// 弩骑 致盲
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.BLIND, Integer.valueOf(ret.x)));
				break;
			case 3221006: // 幻影步
			case 3121007: // 幻影步
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.幻影步_敏捷, Integer.valueOf(ret.dex)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.幻影步_回避几率, Integer.valueOf(ret.x)));
				break;
			/*
			 * case 33101006: // 吞噬 消化技能 statups.add(new Pair<MapleBuffStat,
			 * Integer>(MapleBuffStat.吞噬1, Integer.valueOf(ret.x)));
			 * statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.吞噬2,
			 * Integer.valueOf(ret.x))); statups.add(new Pair<MapleBuffStat,
			 * Integer>(MapleBuffStat.吞噬3, Integer.valueOf(ret.x)));
			 * statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.吞噬4,
			 * Integer.valueOf(ret.x))); statups.add(new Pair<MapleBuffStat,
			 * Integer>(MapleBuffStat.吞噬5, Integer.valueOf(ret.x))); break;
			 */
			case 33101004: // 地雷
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.地雷,
						Integer.valueOf(ret.x)));
				break;
			case 5111005: // 超人变形
			case 5121003: // 超级变身
			...
			case 15111002:// 奇袭者 - 超级变身
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.EWatk, Integer.valueOf(ret.epad)));
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.EWdef, Integer.valueOf(ret.epdd)));
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.EMdef, Integer.valueOf(ret.emdd)));
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.SPEED, Integer.valueOf(ret.speed)));
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.JUMP, Integer.valueOf(ret.jump)));
				// 根据封包发送顺序 这个值要放在最后发送
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.MORPH, Integer.valueOf(ret.morphId)));
				// 2002是冰骑士 换着玩
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.MORPH, Integer.valueOf(ret.morphId)));
				break;
			case 4341002: // 终极斩
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.终极斩,
						Integer.valueOf(ret.y)));
				break;
			case 4331003: // 死亡猫头鹰 奖励次数 5+u(x/4) 次
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.死亡猫头鹰, Integer.valueOf(ret.y)));
				ret.duration = 2100000000;
				break;
			case 21120007: // 战神之盾
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.战神之盾, Integer.valueOf(ret.x)));
				break;
			case 21000000: // 矛连击强化
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.矛连击强化, Integer.valueOf(level * 10)));
				ret.duration = 2100000000;
				break;
			case 22181003: // 灵魂之石
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.灵魂之石, Integer.valueOf(ret.x)));
				break;
			case 22151003: // 抗魔领域
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.抗魔领域, Integer.valueOf(ret.x)));
				break;
			case 22181000: // 玛瑙的祝福
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.WDEF, Integer.valueOf(ret.wdef)));
			...
			case 1211011: // 战斗命令
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.战斗命令, Integer.valueOf(ret.x)));
				break;
			case 1220013: // 祝福护甲
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.祝福护甲_防御次数, Integer.valueOf(ret.x))); // 防御次数
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.祝福护甲_物理攻击力, Integer.valueOf(ret.epad))); // 增加的攻击力
				break;
			case 5110001: // 能量获得
			case 15100004:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.能量获得, Integer.valueOf(10000))); // 满能量时才能发动
				break;
			case 2211008: // 自然力重置
			case 2111008:
			case 12101005:
			case 22121001:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.自然力重置, Integer.valueOf(ret.x)));
				break;
			case 13101006: // 风影漫步
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.SPEED, Integer.valueOf(ret.speed)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.风影漫步, Integer.valueOf(1))); // 也可以写ret.x
																	// 不过x节点定值为1
				break;
			case 机械师.完美机甲:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.完美机甲, Integer.valueOf(ret.x)));
				ret.duration = 2100000000;
				break;
			case 机械师.卫星防护:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.卫星防护, Integer.valueOf(ret.x)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.完美机甲, Integer.valueOf(ret.y)));
				ret.duration = 2100000000;
				break;
			...
			case 魂骑士.终极剑:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.骑士团主动终极, Integer.valueOf(1))); // 也可以写ret.x
																		// 不过x节点定值为1
				break;
			case 枪手.幸运骰子:
			case 拳手.幸运骰子:
			...
			case 5320007:
				...
				ret.skill = true;
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.幸运骰子, Integer.valueOf(1)));
				break;
			case 奇袭者.闪光击:
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.闪光击, Integer.valueOf(1))); //也可以写ret.x
				// 不过x节点定值为1
				break;
			case 龙神.玛瑙的保佑:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.玛瑙的保佑, Integer.valueOf(ret.x)));
				break;
			case 龙神.玛瑙的意志:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.玛瑙的意志, Integer.valueOf(ret.x)));
				break;

			// 给予怪物状态
			case 战神.战神之审判:
				monsterStatus.put(MapleMonsterStat.FREEZE, Integer.valueOf(1));
				monsterStatus.put(MapleMonsterStat.POISON,
						Integer.valueOf(ret.damage));
				break;

			case 机械师.加速器:
				monsterStatus
						.put(MapleMonsterStat.WDEF, Integer.valueOf(ret.y));
				monsterStatus.put(MapleMonsterStat.SPEED,
						Integer.valueOf(ret.x));
				break;
			case 1111007: // 魔击无效
			case 1211009:
			...
			case 22161002: // 鬼刻符
				...
				break;
			case 4341003: // 怪物炸弹
				monsterStatus.put(MapleMonsterStat.怪物炸弹,
						Integer.valueOf(ret.damage));
				break;
			case 4001002: // 诅咒术
				monsterStatus
						.put(MapleMonsterStat.WATK, Integer.valueOf(ret.x));
				monsterStatus
						.put(MapleMonsterStat.WDEF, Integer.valueOf(ret.y));
				break;
			case 1201006: // 压制术
			...
			case 4121003:// 挑衅
			case 4221003:// 挑衅
				monsterStatus.put(MapleMonsterStat.TAUNT,
						Integer.valueOf(ret.x));
				monsterStatus
						.put(MapleMonsterStat.MDEF, Integer.valueOf(ret.x));
				monsterStatus
						.put(MapleMonsterStat.WDEF, Integer.valueOf(ret.x));
				break;
			case 4121004: // 忍者伏击
			case 4221004: // 忍者伏击
				monsterStatus.put(MapleMonsterStat.忍者伏击,
						Integer.valueOf(ret.damage));
				break;
			case 2201004: // 冰冻术
			case 2211002: // 冰咆哮
			...
			case 3120006: {// 火冰凤凰。
				...
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.射手_精神连接, level));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.射手_精神连接, 3111005));
				ret.overTime = true;
				ret.skill = true;
				break;
			}
			...
			case 2321010: {// 魔力精通
				...
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.魔力精通, 1));
				break;
			}
			case 2311009: {// 神圣魔法盾
				ret.hpR = (ret.技能解析(MapleDataTool.getString("z", source, null),
						level, 0) / 100.0);
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.神圣魔法盾, ret.x));
				break;
			}
			case 2120010:
			case 2220010:
			case 2320011: {// 神秘瞄准术
				...
				ret.duration = 5000;
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.神秘瞄准术, ret.x));
				break;
			}
			case 5301003: {// 猴子魔法
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.EMaxHp, ret.技能解析(MapleDataTool.getString(
								"indieMhp", source, null), level, 0)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.EMaxMp, ret.技能解析(MapleDataTool.getString(
								"indieMmp", source, null), level, 0)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.EAcc, ret.技能解析(MapleDataTool.getString(
								"indieAcc", source, null), level, 0)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.EAvoid, ret.技能解析(MapleDataTool.getString(
								"indieEva", source, null), level, 0)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.EJump, ret.技能解析(MapleDataTool.getString(
								"indieJump", source, null), level, 0)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.ESpeed, ret.技能解析(MapleDataTool.getString(
								"indieSpeed", source, null), level, 0)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.EAllStat, ret.技能解析(MapleDataTool
								.getString("indieAllStat", source, null),
								level, 0)));
				break;
			}
			case 5320008: {// 超级猴子魔法
			...
			case 23101003:// 精神注入
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.精神注入_伤害, Integer.valueOf(ret.damage)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.精神注入_暴击, Integer.valueOf(ret.x)));
				IsIntValue.add(ret.getSourceId());
				break;
			case 23111004:// 火焰咆哮　
			{
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.火焰咆哮_攻击力, ret.技能解析(MapleDataTool
								.getString("indiePad", source, null), level, 0)));
				break;
			}
			case 23111005:// 水盾
			{
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.水盾_ASR, ret.技能解析(
								MapleDataTool.getString("asrR", source, null),
								level, 0)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.水盾_TER, ret.技能解析(
								MapleDataTool.getString("terR", source, null),
								level, 0)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.水盾_伤害减少, ret.x));
				break;
			}
			case 23121004: {// 古老意志
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.古老意志_体力, ret.emhp));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.古老意志_攻击, ret.技能解析(
								MapleDataTool.getString("damR", source, null),
								level, 0)));
				break;
			}
			case 31101003: {// 黑暗复仇
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.POWERGUARD, ret.y));
				break;
			}
			case 31111004: {// 黑暗忍耐
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.水盾_ASR, ret.y));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.水盾_TER, ret.y));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.黑暗忍耐_防御力, ret.x));
				break;
			}
			case 31121005: {// 黑暗变形
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.黑暗变形_攻击力, ret.技能解析(
								MapleDataTool.getString("damR", source, null),
								level, 0)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.古老意志_攻击, level));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.黑暗变形_HP增加,
						ret.技能解析(MapleDataTool.getString("indieMhpR", source,
								null), level, 0)));
				ret.overTime = true;
				ret.skill = true;
				break;
			...
			case 31121002: {// 吸血鬼之触
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.吸血鬼之触_伤害转化, ret.x));
				ret.overTime = true;
				ret.skill = true;
				break;
			}
			...
			{
				statups.add(Pair
						.Create(MapleBuffStat.ACC, new Integer(ret.acc)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.火焰咆哮_攻击力, ret.技能解析(MapleDataTool
								.getString("indiePad", source, null), level, 0)));
				break;
			...
			case 5721009:// 海盗精神
			{
				statups.add(Pair.Create(MapleBuffStat.AVOID, ret.技能解析(
						MapleDataTool.getString("eva", source, null), level, 0)));
				statups.add(Pair.Create(MapleBuffStat.STANCE, ret.x));//
				statups.add(Pair.Create(MapleBuffStat.古老意志_攻击,
						ret.技能解析(MapleDataTool.getString("damR", source, null),
								level, 0)));
			...
			case 4201011: {// 金钱护盾
				statups.add(Pair.Create(MapleBuffStat.MESOGUARD,
						Integer.valueOf(ret.x)));
				break;
			}
			/*
			 * case 4211008: //刀飞的影分身 statups.add(new Pair<MapleBuffStat,
			 * Integer>(MapleBuffStat.SHADOWPARTNER, Integer.valueOf(ret.x)));
			 * break;
			 */
			case 4211003: // 敛财术
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.PICKPOCKET, Integer.valueOf(ret.x)));
				ret.duration = 999999999;
				break;
			case 4221013: {// 侠盗本能
			...
			case 24111003: {// 幸运的保护
				...
				statups.add(Pair.Create(MapleBuffStat.幸运的保护_HP, ret.技能解析(
						MapleDataTool.getString("indieMhpR", source, null),
						level, 0)));
				statups.add(Pair.Create(MapleBuffStat.幸运的保护_MP, ret.技能解析(
						MapleDataTool.getString("indieMmpR", source, null),
						level, 0)));
				break;
			}
			case 24111005: {// 月光祝福
				statups.add(Pair.Create(MapleBuffStat.火焰咆哮_攻击力, ret.技能解析(
						MapleDataTool.getString("indieAcc", source, null),
						level, 0)));
				statups.add(Pair.Create(MapleBuffStat.月光祝福_命中率, ret.技能解析(
						MapleDataTool.getString("indiePad", source, null),
						level, 0)));
				break;
			}
			case 24121004: {// 圣歌祈祷
			...
			case 51111003: {// 闪耀之光
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.闪耀之光, ret.技能解析(MapleDataTool.getString(
								"indieDamR", source, null), level, 0)));
				break;
			}
			case 51111004: {// 灵魂恢复术FS
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.水盾_ASR, ret.x));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.水盾_TER, ret.y));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.黑暗忍耐_防御力, ret.z));
				break;
			}
			case 51121006: {// 灵魂之怒
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.灵魂之怒_伤害增加, ret.x * 100 + 1));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.灵魂之怒_最大暴击, ret.z));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.灵魂之怒_最小暴击, ret.y));
				break;
			}
			case 61120008: {// 终极变身
				statups.add(Pair.Create(MapleBuffStat.SPEED, new Integer(
						ret.speed)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.MORPH, Integer.valueOf(ret.getMorph())));
				statups.add(Pair.Create(MapleBuffStat.KUANGLONG_WS,
						ret.技能解析(MapleDataTool.getString("prop", source, null),
								level, 0)));
				statups.add(Pair.Create(MapleBuffStat.KUANGLONG_BL, ret.技能解析(
						MapleDataTool.getString("cr", source, null), level, 0)));
				statups.add(Pair.Create(MapleBuffStat.KUANGLONG_GJSD, ret.技能解析(
						MapleDataTool.getString("indieBooster", source, null),
						level, 0)));
				statups.add(Pair.Create(MapleBuffStat.KUANGLONG_GJL, ret.技能解析(
						MapleDataTool.getString("indieDamR", source, null),
						level, 0)));
				ret.overTime = true;
				ret.skill = true;
				ret.initForeign(new MapleForeignBuffByteStat(
						MapleBuffStat.SPEED), new MapleForeignBuffShortStat(
						MapleBuffStat.MORPH));
				break;
			}
			case 61120007: {
				statups.add(Pair.Create(MapleBuffStat.KUANGLONG_JIANBI, ret
						.技能解析(MapleDataTool.getString("padX", source, null),
								level, 0)));
				ret.onoffSkill = true;
				ret.overTime = true;
				ret.skill = true;
				...
				break;
			}// ret.技能解析(MapleDataTool.getString("damR", source, null), level,
				// 0)
			case 31201003: {// 深渊之怒
				statups.add(Pair.Create(MapleBuffStat.火焰咆哮_攻击力, ret.技能解析(
						MapleDataTool.getString("indiePad", source, null),
			...
			case 36111006: {// 全息投影
				...
				ret.onoffSkill = true;
				ret.duration = -89577;
				ret.initForeign(new MapleForeignBuffFixedShort(
						MapleBuffStat.SHADOWPARTNER, (short) -12406));
				break;
			}
			case 36121004: {// 攻击矩形
				statups.add(Pair.Create(MapleBuffStat.STANCE, ret.x));
			...
			case 36121003: {// 神秘代码
				...
				statups.add(Pair.Create(MapleBuffStat.神秘代码_总伤害增加, ret.技能解析(
						MapleDataTool.getString("indieDamR", source, null),
						level, 0)));
				break;
			}
	...
	private static MapleStatEffect loadFromData(MapleData source, int sourceid,
		...
		ret.cureDebuffs = cure;
		if ((ret.overTime) && (ret.getSummonMovementType() == null)
				&& !GameConstants.isAngelRingBuffer(sourceid)) {
			addBuffStatPairToListIfNotZero(statups, MapleBuffStat.WATK,
					Integer.valueOf(ret.watk));
			addBuffStatPairToListIfNotZero(statups, MapleBuffStat.WDEF,
					Integer.valueOf(ret.wdef));
		...
		if (GameConstants.isAngelRingBuffer(sourceid)) {
			...
			statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.E_Watk,
					Integer.valueOf(ret.watk)));
			statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.E_Matk,
					Integer.valueOf(ret.matk)));
		}
		MapleData ltd = source.getChildByPath("lt");
		if (ltd != null) {
		...
		ret.bulletConsume = MapleDataTool.getIntConvert("bulletConsume",
				source, 0);
		...
		if (skill) {
			...
			case 20011001:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.RECOVERY, Integer.valueOf(ret.x)));
				break;
			case 1002: // 新手 - 疾风步
			case 10001002:
			case 20001002:
			case 20011002:
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.SPEED, Integer.valueOf(ret.x)));
				break;
			case 1004: // 新手 - 骑兽技能
			case 10001004:
			case 20001004:
			...
			case 20011010:
				// statups.add(new Pair<MapleBuffStat,
				// Integer>(MapleBuffStat.DIVINE_BODY, 1));
				break;
			case 1011: // 新手 - 狂暴战魂
			case 10001011:
			case 20001011:
			...
			case 9001003: // 管理员 - 封印祝福
				...
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.AVOID, Integer.valueOf(ret.avoid)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.WATK, Integer.valueOf(ret.watk)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.MATK, Integer.valueOf(ret.matk)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.WDEF, Integer.valueOf(ret.wdef)));
			...
			// 潜入
			case 30001001:
				statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.潜入,
						Integer.valueOf(ret.x)));
				statups.add(new Pair<MapleBuffStat, Integer>(
						MapleBuffStat.SPEED, Integer.valueOf(ret.speed)));
				break;
			...
			case 2003516: {
				statups.add(Pair.Create(MapleBuffStat.巨人药水,
			...
			case 2300000:
				...
				if (!mob.isBoss()) {
					...
					if (absorbMp > 0) {
						...
						// applyto.getClient().getSession().write(MaplePacketCreator.showOwnBuffEffect(sourceid,
						// 1));
						// applyto.getMap().broadcastMessage(applyto,
						// MaplePacketCreator.showBuffeffect(applyto.getId(),
						// sourceid, 1, (byte) 3, 1), false);
					}
				}
				break;
			}
	...
	private boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto,
		...
		// log.debug("效果为玩家增加 HP ：" + hpchange + " ：倍率：" + hpR + " ：源ID：" +
		// sourceid);
		if (expinc > 0) {
		...
		if (primary) {
			if (itemConNo != 0) {
				MapleInventoryType type = MapleItemInformationProvider
						.getInstance().getInventoryType(itemCon);
				MapleInventoryManipulator.removeById(applyto.getClient(), type,
		...
		List<Pair<MapleStat, Number>> hpmpupdate = new ArrayList<Pair<MapleStat, Number>>(
				2);
		...
		if (hpchange != 0) {
			...
			applyto.setHp(newHp);
			hpmpupdate.add(new Pair<MapleStat, Number>(MapleStat.HP, Integer
					.valueOf(applyto.getHp())));
		}
		if (mpchange != 0) {
			...
			hpmpupdate.add(new Pair<MapleStat, Number>(MapleStat.MP, Integer
					.valueOf(applyto.getMp())));
		}
		applyto.getClient()
				.getSession()
		...
		if (moveTo != -1 && !skill) {
			...
			if (moveTo == 999999999) {
				...
				target = applyto.getClient().getChannelServer().getMapFactory()
						.getMap(moveTo);
			...
			if (target == applyto.getMap() || nearest
					&& applyto.getMap().isTown()) {
				return false;
			}
		}
		...
		if (summonMovementType != null && pos != null) {
			...
			if (!(summon == null || summon.getSkill() == 机械师.磁场
					&& applyfrom.getSummonAmount(机械师.磁场) < 3
					|| summon.getSkill() == 弩骑.地雷_自爆 || summon.getSkill() == 机械师.机器人工厂_机器人)) {
				// //log.debug("在召唤召唤兽的时候检测到已经存在此召唤兽 先进行取消");
				applyfrom.getMap()
						.broadcastMessage(
								MaplePacketCreator.removeSpecialMapObject(
										summon, true));
			...
			if (applyfrom.getMainsumons() != null
					&& summonMovementType.ISFOLLOW()
					&& (!GameConstants.isAngelRingSkill(sourceid))) {
				applyfrom.getMap().broadcastMessage(
						MaplePacketCreator.removeSpecialMapObject(
								applyfrom.getMainsumons(), true));
				applyfrom.getMap().removeMapObject(applyfrom.getMainsumons());
				applyfrom.removeVisibleMapObject(applyfrom.getMainsumons());
				applyfrom.getSummons().remove(
						applyfrom.getMainsumons().getSkill());
				applyfrom.setMainsumons(null);
			}
			if ((GameConstants.isAngelRingSkill(sourceid))
					&& applyfrom.getRingsumons() != null) {// 戒指BB。
				applyfrom.getMap().broadcastMessage(
						MaplePacketCreator.removeSpecialMapObject(
								applyfrom.getRingsumons(), true));
				applyfrom.getMap().removeMapObject(applyfrom.getRingsumons());
				applyfrom.removeVisibleMapObject(applyfrom.getRingsumons());
				applyfrom.getSummons().remove(
						applyfrom.getRingsumons().getSkill());
				applyfrom.setRingsumons(null);
			}
			MapleSummon tosummon = new MapleSummon(applyfrom, sourceid, pos,
					summonMovementType);
			...
			if (summonMovementType.ISFOLLOW()
					&& (!GameConstants.isAngelRingSkill(sourceid))) {// 跟随类型。
				...
				for (MapleDoor2 olddoor2 : applyto.getDoors2()) {
					if (olddoor2.isFirst() == firstDoor) {
						...
						applyto.getMap().broadcastMessage(
								MaplePacketCreator.取消传送门(applyto.getId(),
										olddoor2.isFirst()));
			...
			MapleDoor2 door2 = new MapleDoor2(applyto, applyto.getPosition(),
					firstDoor);
		...
		if (isHide()) {
			if (applyto.isHidden()) {
				...
				// false);
				for (MapleCharacter mapleCharacter : applyto.getMap()
						.getCharacters()) {
	...
	private List<MapleMapObject> getMonsterFormBouningBox(
			MapleCharacter applyfrom) {
		Rectangle bounds = calculateBoundingBox(applyfrom.getPosition(),
				applyfrom.isFacingLeft());
		return applyfrom.getMap().getMapObjectsInRect(bounds,
	...
	private void applyBuff(MapleCharacter applyfrom) {
		if (isPartyBuff() && (applyfrom.getParty() != null || isGMBuff())) {
			...
			List<MapleMapObject> affecteds = applyfrom.getMap()
					.getMapObjectsInRect(bounds, MapleMapObjectType.PLAYER);
			List<MapleCharacter> affectedp = new ArrayList<MapleCharacter>(
					affecteds.size());
			for (MapleMapObject affectedmo : affecteds) {
				MapleCharacter affected = (MapleCharacter) affectedmo;
				// this is new and weird...
				if (affected != null && isHeal() && affected != applyfrom
						&& affected.getParty() == applyfrom.getParty()
						&& affected.isAlive()) {
					int expadd = (int) ((calcHPChange(null, applyfrom, true) / 10)
							* (applyfrom.getClient().getChannelServer()
									.getExpRate() + ((Math.random() * 10) + 30)) * (Math
							.floor(Math.random()
									* (applyfrom.getSkillLevel(SkillFactory
											.getSkill(2301002))) / 100) * (applyfrom
							.getLevel() / 30)));
					if (affected.getHp() < affected.getMaxhp()
							- affected.getMaxhp() / 20) {
						applyfrom.gainExp(expadd, true, false, false);
					}
				}
				if (affected != applyfrom
						&& (isGMBuff() || applyfrom.getParty().equals(
								affected.getParty()))) {
					boolean isRessurection = isResurrection();
					if ((isRessurection && !affected.isAlive())
							|| (!isRessurection && affected.isAlive())) {
						affectedp.add(affected);
					}
					if (isTimeLeap()) {
						for (PlayerCoolDownValueHolder i : affected
								.getAllCooldowns()) {
			...
			for (MapleCharacter affected : affectedp) {
				// TODO actually heal (and others) shouldn't recalculate
				// everything
				// for heal this is an actual bug since heal hp is decreased
				// with the number
				// of affected players
	...
	private void applyMonsterBuff(MapleCharacter applyfrom) {
		Rectangle bounds = calculateBoundingBox(applyfrom.getPosition(),
				applyfrom.isFacingLeft());
		List<MapleMapObject> affected = applyfrom.getMap().getMapObjectsInRect(
				bounds, Arrays.asList(MapleMapObjectType.MONSTER));
		ISkill skill_ = SkillFactory.getSkill(sourceid);
		int i = 0;
		for (MapleMapObject mo : affected) {
			MapleMonster monster = (MapleMonster) mo;
			if (makeChanceResult()) {
				monster.applyStatus(applyfrom, new MonsterStatusEffect(
						getMonsterStati(), skill_, false), isPoison(),
						getDuration());
			}
			i++;
			if (i >= mobCount) {
	...
	public void silentApplyBuff(MapleCharacter chr, long starttime,
		 ...
		 */
		registryToPlayer(chr,
				((starttime + localDuration) - System.currentTimeMillis()),
				localstat);

		SummonMovementType summonMovementType = getSummonMovementType();
		if (summonMovementType != null) {
			final MapleSummon tosummon = new MapleSummon(chr, sourceid,
	...
	public static boolean isNormalMonsterRiding(int sourceid) {
		return sourceid == 1004 || sourceid == 10001004 || sourceid == 20001004
				|| sourceid == 20011004 || sourceid == 30001004;
	}
	...
	public boolean isGiveForeignBuff() {
		return isDs() || isCombo() || isShadowPartner() || isSoulArrow()
				|| isMorph() || isPirateMorph() // || isLingQi()
		;
	}
	...
	private void applyBuffEffect(final MapleCharacter applyfrom,
			final MapleCharacter applyto, boolean primary) {
		if (!MapleStatEffect.isMonsterRiding(sourceid)
				&& !isNotCancelBuffFirst(sourceid)
				&& !(sourceid >= 2022125 && sourceid <= 2022129)) {
		...
		MapleSkillEffectEvent.EffectEventData effectEventData = new MapleSkillEffectEvent.EffectEventData(
				localstatups, localDuration);
		MapleSkillEffectEvent.OnApplyBuffEffect(this, applyfrom, applyto,
				primary, effectEventData);
		...
		if (GameConstants.isAngelRingSkill(sourceid)) {
			...
			final MapleStatEffect.DelayAngelRingEffectAction dr = new MapleStatEffect.DelayAngelRingEffectAction(
					applyto, sourceid);
			dr.SetScheduledFuture(TimerManager.getInstance().register(
					new Runnable() {
						@Override
						public void run() {
							dr.run();
		...
		if (isMonsterRiding(sourceid)) {
			...
			IItem mount = applyfrom.getInventory(MapleInventoryType.EQUIPPED)
					.getItem((byte) -18);
			...
			localstatups = Collections.singletonList(Pair.Create(
					MapleBuffStat.坐骑状态, itemid));
		} else if (is骰子()) {
		...
		{
			if ((isDash() || isTornado())
					&& applyfrom.getBuffedValue(MapleBuffStat.疾驰_龙卷风) == null) {
				// //log.debug("龙卷风/疾驰"+localstatups);
				// applyto.getClient().getSession().write(MaplePacketCreator.giveDashOrTornado(localstatups,
				// sourceid, localDuration / 1000));
			} else if (isInfusion()) {
				// log.debug("极速领域");
				// applyto.getClient().getSession().write(MaplePacketCreator.giveInfusion(sourceid,
				// localDuration / 1000, x));
			} /*
			 * else if (isMonsterRiding(sourceid)) { //log.debug("骑宠");
			 * applyto.getClient
			 * ().getSession().write(MaplePacketCreator.giveMonsterRidingBuff
			 * (applyto, itemid, skillid, localstatups, localDuration)); }
			 */else if (isSkillBianShen()) {
				// log.debug("变身技能");
				// applyto.getClient().getSession().write(MaplePacketCreator.giveSkillBianShen(applyto,
				// localsourceid, localDuration, localstatups));
			} else if (is能量获得()) {
				// log.debug("能量获得技能");
				// applyto.getClient().getSession().write(MaplePacketCreator.givePirateBuff(0,
				// localDuration, localstatups));
			} else if (is骰子()) {
				// //log.debug("幸运骰子技能 点数："+localstatups.get(0).getRight());
				// applyto.getClient().getSession().write(MaplePacketCreator.骰子(localsourceid,
				// localstatups, localDuration, localstatups.get(0).getRight(),
				// applyto.getSkillLevel(5320007), applyto));
			} else {
				// //log.debug("普通技能");
				// if (!skill || isMonsterRiding(sourceid)) {
				applyto.getClient()
						.getSession()
						.write(MaplePacketCreator.giveBuff(
								(skill ? localsourceid : -localsourceid),
								localDuration, localstatups, applyto));
				// }
			}
		}
		...
		if (isGiveForeignBuff()) { // 总判断 记得要在下面出现的判断记得先在这里加
			...
			if (isDs()) {
				...
				stat.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.SPEED,
						Integer.valueOf(speed)));
				stat.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.MORPH,
						Integer.valueOf(morphId)));
			}
			if (stat != null) // log.debug("stat == null");
			{
				// applyto.getMap().broadcastMessage(applyto,
				// MaplePacketCreator.giveForeignBuff(applyto, stat, this),
				// false);
			}
			// localstatups = stat;
		} else if (isEnrage()) {
			...
			// applyto.getClient().getSession().write(MaplePacketCreator.giveBuff(localsourceid,
			// localDuration, Collections.singletonList(new Pair<MapleBuffStat,
			// Integer>(stat2, x))));
		}

		if ((localstatups != null && localstatups.size() > 0)
				&& (!isInfusion())) // 广播效果
		{
			// applyto.getMap().broadcastMessage(applyto,
			// MaplePacketCreator.giveForeignBuff(applyto, localstatups, this),
			// false);
		}
		if ((localstatups != null && localstatups.size() > 0) && refreshstyle) {
			...
			applyto.getMap().broadcastMessage(
					applyto,
					MaplePacketCreator.giveForeignBuff(applyto, localstatups,
							this), false);
			// }
		}

		if (((localstatups.size() > 0 || getSummonMovementType() != null) // &&
		...
		if (primary && !isHide()) {
			if (isDash()) {
				// applyto.getMap().broadcastMessage(applyto,
				// MaplePacketCreator.showDashEffecttoOthers(applyto.getId(),
				// localstatups, localDuration / 1000), false);
			}/*
			 ...
			 */else if (is骰子()) {
				// applyto.getClient().getSession().write(MaplePacketCreator.机械技能特效(3,
				// localstatups.get(0).getRight(), sourceid,
				// applyfrom.getSkillLevel(sourceid), 0, -1));
				// applyto.getMap().broadcastMessage(applyto,
				// MaplePacketCreator.机械技能特效(applyto.getId(), 3,
				// localstatups.get(0).getRight(), sourceid,
				// applyto.getSkillLevel(sourceid), 0, -1));
			} else {
				applyto.getMap().broadcastMessage(
						applyto,
						MaplePacketCreator.showBuffeffect(applyto.getId(),
	...
	private void registryToPlayer(MapleCharacter applyto, long localDuration,
			List<Pair<MapleBuffStat, Integer>> localstatups) {
		long starttime = System.currentTimeMillis();
		ScheduledFuture<?> schedule = null;
		if (!onoffSkill) {
			CancelEffectAction cancelAction = new CancelEffectAction(applyto,
					this, starttime);
			schedule = TimerManager.getInstance().schedule(cancelAction,
					localDuration);
		}
		applyto.registerEffect(this, starttime, schedule, localstatups);
	}

	private int calcHPChange(MapleCharacter applyto, MapleCharacter applyfrom,
			boolean primary) {
		int hpchange = 0;
		if (hp != 0) {
			if (!skill) {
				...
				hpchange += makeHealHP(hp / 100.0, applyfrom.getTotalMagic(),
						3, 5);
			}
		}
		if (hpR != 0) {
		...
		if (applyto != null && applyfrom.hasBufferStat(MapleBuffStat.神圣魔法盾)
				&& !primary) {
		...
		if (isChakra()) {
			hpchange += makeHealHP(getY() / 100.0, applyfrom.getTotalLuk(),
					2.3, 3.5);
		}
	...
	private int calcMPChange(MapleCharacter applyto, MapleCharacter applyfrom,
			boolean primary) {
		int mpchange = 0;
		if (mp != 0) {
			if (primary) {
		...
		if (isPirateMpRecovery()) {
			mpchange += (((getY() * getX()) / 10000.0) * applyfrom
					.getCurrentMaxHp());
		}
		return mpchange;
	}
	...
	public boolean isShadowPartner() {
		return skill && sourceid == 4111002 || sourceid == 夜行者.影分身
				|| sourceid == 双刀.镜像分身;
	}
	...
	public boolean isPoison() {
		return skill
				&& (sourceid == 2111003 || sourceid == 2101005
						|| sourceid == 2111006 || sourceid == 14111006
						|| sourceid == 炎术士.火牢术屏障 || sourceid == 战神.战神之审判);
	}

	private boolean isMist() {
		return skill
				&& (sourceid == 2111003 || sourceid == 4221006
						|| sourceid == 32121006 // 避难所
						|| sourceid == 龙神.极光恢复 || sourceid == 机械师.放大器_AF_11 || sourceid == 炎术士.火牢术屏障);
	}

	private boolean isSoulArrow() {
		return skill
				&& (sourceid == 3101004 || sourceid == 3201004 || sourceid == 13101003); // bow
																							// and
																							// crossbow
	}
	...
	public boolean isComboMove() {
		return skill
				&& (sourceid == 21100004 || sourceid == 21100005
						|| sourceid == 21110003 || sourceid == 21120006 || sourceid == 21120007);
	}
	...
	private boolean isHeroWill() {
		return skill
				&& (sourceid == 1121011 || sourceid == 1221012
						|| sourceid == 1321010 || sourceid == 2121008
						|| sourceid == 2221008 || sourceid == 2321009
						|| sourceid == 3121009 || sourceid == 3221008
						|| sourceid == 4121009 || sourceid == 4221008
						|| sourceid == 5121008 || sourceid == 5221010);
	}
	...
	public SummonMovementType getSummonMovementType() {
		...
		case 5321004:
		case 5711001:// 滑膛炮
		case 36121002:// 36121002 全息力场 ：穿透
						// 制造力场后，生成能量球。能量球每次碰到怪物时，给该怪物造成伤害。能量球不会受到敌人的攻击反射技能的伤害。可以通过多模式链接切换模式。
	...
	public static class DelayEffectAction implements Runnable {
		...
		public void run() {
			...
			if (realTarget != null) {
				realTarget.addMPHP((int) (realTarget.getMaxhp() * rate),
						(int) (realTarget.getMaxmp() * rate));
	...
	public boolean isLingQi() {
		return skill
				&& (sourceid == 32001003 || sourceid == 32101002
						|| sourceid == 32101003 || sourceid == 战法.进阶黑暗灵气
						|| sourceid == 战法.进阶蓝色灵气 || sourceid == 战法.进阶黄色灵气);
	}

	public boolean isSkillBianShen() {
		return skill
				&& (sourceid == 5111005 || sourceid == 5121003
						|| sourceid == 13111005 || sourceid == 15111002);
	}
	...
	public boolean is骰子() {
		return skill
				&& (sourceid == 枪手.幸运骰子 || sourceid == 拳手.幸运骰子
						|| sourceid == 机械师.幸运骰子 || sourceid == 5311005 || sourceid == 5320007);
	}
	...
	public MapleForeignBuffSkill getForeign() {
		/*
		 * if (refreshstyle && foreign == null) { foreign =
		 * MapleForeignBuffHelp.getForeignBuffSkill(this); }
		 */
		return foreign;
	}
