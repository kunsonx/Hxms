...
public class MakerItemFactory {
	...
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(MakerItemFactory.class);
		...
		if (createCache.get(toCreate) != null) {
			...
			try {
				...
				PreparedStatement ps = con
						.prepareStatement("SELECT * FROM makercreatedata WHERE itemid = ?");
				...
				MakerItemCreateEntry ret = new MakerItemCreateEntry(cost,
				...
				while (rs.next()) {
					ret.addReqItem(rs.getInt("req_item"), rs.getInt("count"));// 添加进集合里
																				// 这里是需要的魔法粉末
																				// 怪物结晶
				}
				rs.close();
