...
public class HiredMerchant extends PlayerInteractionManager {
		...
		try {
			...
			if (pItem.getBundles() > 0) {
				synchronized (items) {
					...
					newItem.setQuantity((short) (quantity * newItem
							.getQuantity()));
					if (c.getPlayer().getMeso() >= pItem.getPrice() * quantity) {
						if (quantity > 0 && pItem.getBundles() >= quantity
								&& pItem.getBundles() > 0) {
							if (newItem
									.HasFlag(InventoryConstants.Items.Flags.KARMA)) {
								newItem.CanceFlag(InventoryConstants.Items.Flags.KARMA);
							}
							if (MapleInventoryManipulator.checkSpace(c,
									newItem.getItemId(), newItem.getQuantity(),
									newItem.getOwner())
								...
								try {
									...
									PreparedStatement ps = con
											.prepareStatement("UPDATE characters SET MerchantMesos = MerchantMesos + ? WHERE id = ?");
	...
	public void sendDestroyData(MapleClient client) {
		client.getSession().write(
				MaplePacketCreator.destroyHiredMerchant(getOwnerId()));
	}
