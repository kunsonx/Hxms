...
public abstract class PlayerInteractionManager extends AbstractMapleMapObject implements IPlayerInteractionManager {
    ...
    /*
     ...
     ps = DatabaseConnection.getConnection().prepareStatement("INSERT INTO hiredmerchant (ownerid, itemid, quantity, upgradeslots, level, str, dex, `int`, luk, hp, mp, watk, matk, wdef, mdef, acc, avoid, hands, speed, jump, flag, vicious, itemexp, itemlevel, Identify, Starlevel,Potential_1,Potential_2,Potential_3, owner, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        try {
            ...
            if (overwrite) {
                for (MaplePlayerShopItem pItems : items) {
                    if (pItems.getBundles() > 0) {
                        if (pItems.getItem().getType() == 1) {
                            ps = DatabaseConnection.getConnection().prepareStatement("INSERT INTO hiredmerchanttemp (ownerid, itemid, quantity, upgradeslots, level, str, dex, `int`, luk, hp, mp, watk, matk, wdef, mdef, acc, avoid, hands, speed, jump, flag, vicious, itemexp, itemlevel, Identify, Starlevel,Potential_1,Potential_2,Potential_3, owner, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?, ?, ?, ?, ?, 1)");
                            Equip eq = (Equip) pItems.getItem();
                            ps.setInt(2, eq.getItemId());
                            ps.setInt(3, 1);
                            ps.setInt(4, eq.getUpgradeSlots());
                            ...
                            ps = DatabaseConnection.getConnection().prepareStatement("INSERT INTO hiredmerchanttemp (ownerid, itemid, quantity, owner, type) VALUES (?, ?, ?, ?, ?)");
                            ps.setInt(2, pItems.getItem().getItemId());
