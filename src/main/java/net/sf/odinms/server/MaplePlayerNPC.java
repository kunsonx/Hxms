...
public class MaplePlayerNPC {
	...
	public String toString() {
		return "Hello, I am #b" + getPlayerNPCName()
				+ "#k, and I am #rLEVEL 200!#k";
	}
	...
	private String getPlayerNPCName() {
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("SELECT name FROM playernpcs WHERE charid = ?");
	...
	public int getNextNpcID() {
		...
		if (getNPCIdByCharId() == -1) {
			try {
				Connection con = DatabaseConnection.getConnection();
				PreparedStatement ps = con
						.prepareStatement("SELECT COUNT(*) AS c FROM playernpcs WHERE town = ?");
	...
	public void createPlayerNPC(MapleCharacter player, int map) {
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("INSERT INTO playernpcs (id, name, hair, face, skin, dir, map, town, charid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public void insertEquips(int equipid, byte equippos, int type) {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("INSERT INTO playernpcs_equip (npcid, equipid, equippos, type, charid) VALUES (?, ?, ?, ?, ?)");
			ps.setInt(1, getNextNpcID());
			ps.setInt(2, equipid);
	...
	public void updateEquips(int equipid, byte equippos) {
		...
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("update playernpcs_equip set equipid = ? where equippos = ?");
			ps.setInt(1, equipid);
	...
	public void removeAllEquips() {
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("delete from playernpcs_equip where npcid = ?");
	...
	public int getItem(int type) {
		if (skip < player.getInventory(MapleInventoryType.EQUIPPED).countItem()) {
			try {
				player.getDatabaseLock().lock();
				Connection con = DatabaseConnection.getConnection();
				PreparedStatement ps = con
						.prepareStatement("SELECT * FROM inventoryitems WHERE characterid = ? AND inventorytype = -1 ORDER BY itemid LIMIT ?, 1");
				ps.setInt(1, getCharId());
	...
	public int getNPCIdByName(String name) {
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT * FROM playernpcs WHERE name = ?");
			ps.setString(1, name);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				return rs.getInt("id");
	...
	public int countPlayerNPCEquips() {
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT COUNT(*) AS c FROM playernpcs_equip WHERE charid = ?");
			ps.setInt(1, getCharId());
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
	...
	public int getNPCIdByCharId() {
		try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con
					.prepareStatement("SELECT * FROM playernpcs WHERE charid = ?");
			ps.setInt(1, getCharId());
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				return rs.getInt("id");
	...
	public void updatePNE(List<MapleCharacter> players) {
		for (MapleCharacter thisplayer : players) {
			if (thisplayer.getInventory(MapleInventoryType.EQUIPPED)
					.countItem() != countPlayerNPCEquips()) {
				removeAllEquips();
				createPNE();
				return;
			} else {
