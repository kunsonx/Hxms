...
public class MapleInventoryManipulator {
    ...
    private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(MapleInventoryManipulator.class);
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!ii.isThrowingStar(item.getItemId()) && !ii.isBullet(item.getItemId())) {
                ...
                while (quantity > 0) {
                    short newQ = (short) Math.min(quantity, slotMax);
                    quantity -= newQ;
                    Item nItem = (GameConstants.isPet(item.getItemId()) ? (Item) item : new Item(item.getItemId(), (byte) 0, newQ));
                    nItem.setOwner(item.getOwner());
    ...
    public static void equip(MapleClient c, short src, short dst) {
        ...
        if (source.getItemId() / 10000 == 166) {
            if (source.getAndroid() == null) {
                ...
                flag |= InventoryConstants.Items.Flags.ANDROID_ACTIVATED.GetValue();
                source.setFlag(flag);
            ...
            if (dst <= -1300) {
                ...
                c.getPlayer().updateAndroid(1 << ((dst * -1) - 1200), source.getItemId());
            }
        }
    ...
    public static void unequip(MapleClient c, short src, short dst) {
        ...
        if (target != null && src <= 0) { //要是准备移动到的位置有装备 而且来源的id小于0[就是说此物品是装备在身上的]
            c.getSession().write(MaplePacketCreator.getInventoryFull());
            return;
        }
    ...
    public static void drop(MapleClient c, MapleInventoryType type, short src, short quantity) {
        ...
        if (quantity < source.getQuantity() && !ii.isThrowingStar(source.getItemId()) && !ii.isBullet(source.getItemId())) {
            ...
            if (weddingRing) {
                ...
                if (!c.getChannelServer().allowUndroppablesDrop() && (ii.isDropRestricted(target))) {
                    c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos);
                } else {
                    if (LiRing) {
                        c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos);
            ...
            if (c.getPlayer().getMap().getEverlast()) {
                if (!c.getChannelServer().allowUndroppablesDrop() && ii.isDropRestricted(source)) {
    ...
    public static void RefreshItem(MapleClient c, MapleInventoryType type, Item item) {
        c.getSession().write(MaplePacketCreator.InventoryActions(false, Arrays.asList(new InventoryActionsInfo(item, type, InventoryActionsInfo.InventoryActions.DELETE),
                new InventoryActionsInfo(item, type, InventoryActionsInfo.InventoryActions.ADD))));
    }
    ...
    public static void CleanInventory(MapleClient c, MapleInventoryType type) {
        ...
        for (IItem iItem : c.getPlayer().getInventory(type).AllItems()) {
            infos.add(new InventoryActionsInfo((Item) iItem, type, InventoryActionsInfo.InventoryActions.DELETE));
        }
