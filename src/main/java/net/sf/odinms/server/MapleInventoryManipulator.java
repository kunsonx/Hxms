...
public class MapleInventoryManipulator {
	...
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(MapleInventoryManipulator.class);
	...
	public static boolean addFromDrop(MapleClient c, IItem item, boolean show,
		...
		if (!type.equals(MapleInventoryType.EQUIP)) {
			...
			List<IItem> existing = c.getPlayer().getInventory(type)
			...
			if (!ii.isThrowingStar(item.getItemId())
				...
				while (quantity > 0 || ii.isThrowingStar(item.getItemId())
					...
					short newSlot = c.getPlayer().getInventory(type)
							.addItem(nItem);
					if (newSlot == -1) {
						c.getSession().write(
								MaplePacketCreator.getInventoryFull());
						c.getSession().write(
					...
					c.getSession().write(
							MaplePacketCreator.addInventorySlot(type, nItem,
									true));
				}
			} else {
			...
			if (quantity == 1) {
				...
				c.getSession().write(
						MaplePacketCreator.addInventorySlot(type, item, true));
			} else {
		...
		if (show) {
			c.getSession().write(
					MaplePacketCreator.getShowItemGain(item.getItemId(),
							item.getQuantity()));
		}
		return true;
	}

	public static void removeAllById(MapleClient c, int itemId,
	...
	public static boolean addById(MapleClient c, int itemId, short quantity,
		...
		if (!type.equals(MapleInventoryType.EQUIP)) {
			...
			if (!ii.isThrowingStar(itemId) && !ii.isBullet(itemId)) {
					...
					while (quantity > 0) {
						if (i.hasNext()) {
							...
							if (oldQ < slotMax
								...
								quantity -= (newQ - oldQ);
								eItem.setQuantity(newQ);
								eItem.log("Added " + (newQ - oldQ)
										+ " items to stack, new quantity is "
										+ newQ + " (" + logInfo + " )", false);
								c.getSession().write(
										MaplePacketCreator.updateInventorySlot(
												type, eItem));
				...
				while (quantity > 0) { // add new slots if there is still
										// something left
					short newQ = (short) Math.min(quantity, slotMax);
					if (newQ != 0) {
						quantity -= newQ;
						Item nItem = new Item(itemId, (byte) 0, newQ);
						nItem.log("Created while adding by id. Quantity: "
								+ newQ + " (" + logInfo + ")", false);
						short newSlot = c.getPlayer().getInventory(type)
								.addItem(nItem);
						if (newSlot == -1) {
						...
						if (owner != null) {
							...
							nItem.AddFlag(Flags.签名是制作人);
						}
						c.getSession().write(
								MaplePacketCreator
										.addInventorySlot(type, nItem));
						if ((ii.isThrowingStar(itemId) || ii.isBullet(itemId))
								&& quantity == 0) {
							break;
						}
			...
			} else { // Throwing Stars and Bullets - Add all into one slot
						// regardless of quantity.
				Item nItem = new Item(itemId, (byte) 0, quantity);
				nItem.log("Created while adding by id. Quantity: " + quantity
						+ " (" + logInfo + " )", false);
				short newSlot = c.getPlayer().getInventory(type).addItem(nItem);
				if (newSlot == -1) {
					c.getSession().write(MaplePacketCreator.getInventoryFull());
			...
			if (quantity == 1) {
				...
				nEquip.log("Created while adding by id. (" + logInfo + " )",
						false);
				if (owner != null) {
				...
				c.getSession().write(
						MaplePacketCreator.addInventorySlot(type, nEquip));
			} else {
		...
		if (!type.equals(MapleInventoryType.EQUIP)) {
			...
			if (!ii.isThrowingStar(item.getItemId())
				...
				while (quantity > 0) {
					short newQ = (short) Math.min(quantity, slotMax);
					quantity -= newQ;
					Item nItem = (GameConstants.isPet(item.getItemId()) ? (Item) item
							: new Item(item.getItemId(), (byte) 0, newQ));
					nItem.setOwner(item.getOwner());
					nItem.log("Created while adding from drop. Quantity: "
							+ newQ + " (" + logInfo + " )", false);
					short newSlot = c.getPlayer().getInventory(type)
							.addItem(nItem);
					if (newSlot == -1) {
	...
	public static void removeFromSlot(MapleClient c, MapleInventoryType type,
			short slot, short quantity, boolean fromDrop) {
		removeFromSlot(c, type, slot, quantity, fromDrop, false);
	}

	public static void removeFromSlot(MapleClient c, MapleInventoryType type,
			short slot, short quantity, boolean fromDrop, boolean consume) {
		if (quantity < 0) {
			return;
		}
		...
		MapleItemInformationProvider ii = MapleItemInformationProvider
				.getInstance();
		boolean allowZero = consume
				&& (ii.isThrowingStar(item.getItemId()) || ii.isBullet(item
						.getItemId()));
		c.getPlayer().getInventory(type).removeItem(slot, quantity, allowZero);
		if (item.getQuantity() == 0 && !allowZero) {
			c.getSession().write(
					MaplePacketCreator.clearInventoryItem(type,
							item.getPosition(), fromDrop));
		} else {
			if (!consume) {
				item.log(c.getPlayer().getName() + " removed " + quantity
						+ ". " + item.getQuantity() + " left.", false);
			}
			c.getSession().write(
					MaplePacketCreator.updateInventorySlot(type, (Item) item,
							fromDrop));
		}
	}
	...
	public static void removeById(MapleClient c, MapleInventoryType type,
		...
		if (remremove > 0) {
			throw new InventoryException(
					"Not enough cheese available ( ItemID:" + itemId
							+ ", Remove Amount:" + (quantity - remremove)
							+ "| Current Amount:" + quantity + ")");
		}
	}

	public static void move(MapleClient c, MapleInventoryType type, short src,
			short dst) {
		if (src < 0 || dst < 0
				|| src > c.getPlayer().getInventory(type).getSlots()
				|| dst > c.getPlayer().getInventory(type).getSlots()) {
			return;
		}
		MapleItemInformationProvider ii = MapleItemInformationProvider
				.getInstance();
		...
		if (!type.equals(MapleInventoryType.EQUIP) && initialTarget != null
			...
			if ((olddstQ + oldsrcQ) > slotMax) {
				...
				c.getSession().write(
						MaplePacketCreator.moveAndMergeInventoryItem(type, src,
								dst, ((Item) c.getPlayer().getInventory(type)
										.getItem(dst)).getQuantity()));
			}
		} else {
			c.getSession().write(
					MaplePacketCreator.moveInventoryItem(type, src, dst));
		}
	}
	...
	public static void equip(MapleClient c, short src, short dst) {
		...
		Equip target = (Equip) c.getPlayer()
		...
		if (!ii.isCash(source.getItemId())) {
			...
			if ((type.equalsIgnoreCase("Cp") && dst != -1)
					...
					|| ((type.equalsIgnoreCase("Ma") || type
							.equalsIgnoreCase("MaPn")) && dst != -5)
					...
					|| ((type.equalsIgnoreCase("Wp") || type
							.equalsIgnoreCase("WpSi")) && dst != -11)
					|| (type.equalsIgnoreCase("Pe") && (dst != -17 && dst != -38))) {
				c.getSession().write(MaplePacketCreator.enableActions());
				return;
			}
		}
		c.getPlayer().recalcLocalStats();
		if ((ii.getName(source.getItemId()).contains("(Male)") && c.getPlayer()
				.getGender() != 0)
				|| (ii.getName(source.getItemId()).contains("(Female)") && c
						.getPlayer().getGender() != 1)
			...
			c.getPlayer().dropMessage("您的属性不足。不能装备该物品。");
			c.getSession().write(MaplePacketCreator.enableActions());
			return;
		}
		...
		if (dst == -6) { // unequip the overall
			IItem top = c.getPlayer().getInventory(MapleInventoryType.EQUIPPED)
					.getItem((byte) -5);
			...
			if (top != null && ii.isOverall(source.getItemId())) {
				if (c.getPlayer()
						...
						.isFull(bottom != null
								&& ii.isOverall(source.getItemId()) ? 1 : 0)) {
					c.getSession().write(MaplePacketCreator.getInventoryFull());
					c.getSession().write(
							MaplePacketCreator.getShowInventoryFull());
					return;
		...
		c.getSession().write(
				MaplePacketCreator.moveInventoryItem(MapleInventoryType.EQUIP,
						src, dst, (byte) 2));
		c.getPlayer().equipChanged();
	}
	...
	public static void unequip(MapleClient c, short src, short dst) {
		...
		MapleCharacter applyfrom = c.getPlayer();
		if (dst < 0) {
			log.warn("Unequipping to negative slot. ("
					+ c.getPlayer().getName() + ": " + src + "->" + dst + ")");
		}
		if (source == null) {
			return;
		}
		if (target != null && src <= 0) { // 要是准备移动到的位置有装备
											// 而且来源的id小于0[就是说此物品是装备在身上的]
			c.getSession().write(MaplePacketCreator.getInventoryFull());
			return;
		}
		...
		case 1112594:
		case 1112663: {
			if (applyfrom.getRingsumons() != null) {// 戒指BB。
			...
			if (applyfrom.hasBufferStat(MapleBuffStat.天使戒指)) {
				applyfrom.cancelBuffStats(MapleBuffStat.天使戒指);
			}
			if (applyfrom.getRingFuture() != null) {
				applyfrom.getRingFuture().cancel(true);
				applyfrom.setRingFuture(null);
			}
			break;
		}
		}

		if (source.getItemId() / 10000 == 166 || src == -35) {
		...
		if (c.getPlayer().getAndroid() != null) {
			if (src <= -1300) {
				...
				c.getPlayer().updateAndroid(1 << ((src * -1) - 1200), 0);
			}
		}
	...
	public static void drop(MapleClient c, MapleInventoryType type, short src,
			short quantity) {
		MapleItemInformationProvider ii = MapleItemInformationProvider
				.getInstance();
		if (src < 0) {
						...
						.broadcastGMMessage(
								...
								MaplePacketCreator
										.serverNotice(
												0,
												c.getPlayer().getName()
														+ " is dropping more than slotMax.")
										.getBytes());
			} catch (Throwable u) {
			}
		}
		if (quantity < 0 || source == null || quantity == 0
				&& !ii.isThrowingStar(source.getItemId())
				&& !ii.isBullet(source.getItemId())) {
			String message = "Dropping " + quantity + " "
					+ (source == null ? "?" : source.getItemId()) + " ("
					+ type.name() + "/" + src + ")";
			log.debug(" MapleInventoryManipulator 关闭客户端");
			log.info(MapleClient.getLogMessage(c, message));
			c.getSession().close(false); // disconnect the client as is
											// inventory is inconsistent with
											// the serverside inventory
			return;
		}
		...
		if (quantity < source.getQuantity()
			...
			target.log(c.getPlayer().getName() + " dropped part of a stack at "
					+ dropPos.toString() + " on map "
					+ c.getPlayer().getMapId()
					+ ". Quantity of this (new) instance is now " + quantity,
					false);
			source.setQuantity((short) (source.getQuantity() - quantity));
			source.log(
					c.getPlayer().getName() + " dropped part of a stack at "
							+ dropPos.toString() + " on map "
							+ c.getPlayer().getMapId()
							+ ". Quantity of this (leftover) instance is now "
							+ source.getQuantity(), false);
			c.getSession().write(
					MaplePacketCreator.dropInventoryItemUpdate(type, source));
			...
			if (weddingRing) {
				...
				if (!c.getChannelServer().allowUndroppablesDrop()
					...
					if (LiRing) {
						c.getPlayer()
								.getMap()
								.disappearingItemDrop(c.getPlayer(),
			...
			source.log(
					c.getPlayer().getName()
							+ " dropped this (with full quantity) at "
							+ dropPos.toString() + " on map "
							+ c.getPlayer().getMapId(), false);
	...
	public static void RefreshItem(MapleClient c, MapleInventoryType type,
			Item item) {
		c.getSession().write(
				MaplePacketCreator.InventoryActions(false, Arrays.asList(
						new InventoryActionsInfo(item, type,
								InventoryActionsInfo.InventoryActions.DELETE),
						new InventoryActionsInfo(item, type,
								InventoryActionsInfo.InventoryActions.ADD))));
	}

	public static void CleanInventory(MapleClient c, MapleInventoryType type) {
		ArrayList<InventoryActionsInfo> infos = new ArrayList<InventoryActionsInfo>();
		for (IItem iItem : c.getPlayer().getInventory(type).AllItems()) {
			infos.add(new InventoryActionsInfo((Item) iItem, type,
					InventoryActionsInfo.InventoryActions.DELETE));
		}
		c.getSession().write(MaplePacketCreator.InventoryActions(false, infos));
		c.getPlayer().getInventory(type).Claen();
	}
}
