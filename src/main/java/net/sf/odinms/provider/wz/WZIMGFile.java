...
public class WZIMGFile {
	...
	private void parse(WZIMGEntry entry, SeekableLittleEndianAccessor slea) {
		...
		case 1: {
			String name = WZTool.readDecodedStringAtOffsetAndReset(slea,
					file.getOffset() + slea.readInt());
			entry.setName(name);
			break;
		}
		default:
			log.error("Unknown Image identifier: " + marker + " at offset "
					+ (slea.getPosition() - file.getOffset()));
		}
		marker = slea.readByte();
		switch (marker) {
		case 0:
			entry.setType(MapleDataType.NULL);
			break;
		case 2:
	...
	private void parseExtended(WZIMGEntry entry,
		...
		if (type.equals("Property")) {
			...
			case 1:
				...
				break;
			default:
				log.error("Unknown UOL marker: " + uolmarker + " "
						+ entry.getName() + "");
			}
		} else {
