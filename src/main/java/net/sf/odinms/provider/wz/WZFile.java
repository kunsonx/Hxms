...
public class WZFile implements MapleDataProvider {
	...
	public WZFile(File wzfile, boolean provideImages) throws IOException {
		this.wzfile = wzfile;
		lea = new GenericLittleEndianAccessor(new InputStreamByteStream(
				new BufferedInputStream(new FileInputStream(wzfile))));
		RandomAccessFile raf = new RandomAccessFile(wzfile, "r");
		slea = new GenericSeekableLittleEndianAccessor(
	...
	private void parseDirectory(WZDirectoryEntry dir) {
		...
		for (int i = 0; i < entries; i++) {
			...
			switch (marker) {
			case 0x02:
				name = WZTool.readDecodedStringAtOffsetAndReset(slea,
	...
	public WZIMGFile getImgFile(String path) throws IOException {
		...
		WZFileEntry entry = (WZFileEntry) dir
				.getEntry(segments[segments.length - 1]);
		if (entry == null) {
			return null;
		}
		String fullPath = wzfile.getName()
				.substring(0, wzfile.getName().length() - 3).toLowerCase()
				+ "/" + path;
