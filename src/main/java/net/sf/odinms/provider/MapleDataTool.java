...
public class MapleDataTool {
	...
	public static int getIntFromBigBang(String path, MapleData data,
			int defalut, int level) {
		...
		for (int j = 0; j < temp.length(); j++)
			if (temp.substring(j, j + 1).toString().equals("+")
					|| temp.substring(j, j + 1).toString().equals("-")
					|| temp.substring(j, j + 1).toString().equals("*")
					|| temp.substring(j, j + 1).toString().equals("/"))
				num2++;
		if (temp.substring(0, 1).toString().equals("-"))
			for (int i = 0; i < temp.length(); i++)
				if (temp.substring(i, i + 1).toString().equals("+")
						|| temp.substring(i, i + 1).toString().equals("-")
						|| temp.substring(i, i + 1).toString().equals("*")
						|| temp.substring(i, i + 1).toString().equals("/"))
					num++;
		if (num == 1)
		...
		for (int i = 0; i < temp.length(); i++)
			if (temp.substring(i, i + 1).toString().equals("u"))
				temp = temp.substring(0, i) + "1*("
						+ temp.substring(i + 1, temp.length()) + "+1)";
		for (int i = 0; i < temp.length(); i++)
			if (temp.substring(i, i + 1).toString().equals("x"))
				temp = temp.substring(0, i) + level
						+ temp.substring(i + 1, temp.length());
		if (num2 == 0)
			return Integer.parseInt(temp);
...
/*
 * Location: C:\Documents and Settings\Administrator\桌面\AbomsDev.jar Qualified
 * Name: provider.MapleDataTool JD-Core Version: 0.5.4
 */